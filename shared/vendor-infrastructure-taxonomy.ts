/**
 * Vendor Infrastructure Taxonomy
 *
 * Comprehensive classification of third-party vendor infrastructure for:
 * - Asset ownership determination (customer-owned vs vendor-managed)
 * - Risk responsibility attribution (vendor, shared, customer)
 * - Supply chain risk assessment
 *
 * Categories:
 *   - email_provider: Email hosting/filtering (M365, Google Workspace, Proofpoint, etc.)
 *   - web_hosting: Website hosting platforms (Wix, Squarespace, WordPress.com, etc.)
 *   - cdn: Content Delivery Networks (Cloudflare, Akamai, Fastly, etc.)
 *   - dns_provider: DNS hosting services (Cloudflare DNS, Route53, NS1, etc.)
 *   - iaas: Infrastructure as a Service (AWS, Azure, GCP, DigitalOcean, etc.)
 *   - paas: Platform as a Service (Heroku, Vercel, Netlify, Railway, etc.)
 *   - saas: Software as a Service (Salesforce, HubSpot, Zendesk, etc.)
 *   - analytics: Analytics/tracking services (Google Analytics, Hotjar, etc.)
 *   - isp: Internet Service Providers / Hosting ISPs
 *   - registrar: Domain registrars (GoDaddy, Namecheap, etc.)
 *   - ci_cd: CI/CD platforms (GitHub Actions, GitLab CI, CircleCI, etc.)
 *   - identity_provider: Identity/SSO providers (Okta, Auth0, Azure AD, etc.)
 *   - payment_processor: Payment infrastructure (Stripe, PayPal, Square, etc.)
 *   - monitoring: APM/monitoring services (Datadog, New Relic, PagerDuty, etc.)
 *   - security_vendor: Security tools/services (CrowdStrike, Qualys, etc.)
 *
 * Responsibility Model:
 *   - vendor_responsibility: Vendor owns and patches the infrastructure. CVEs on these
 *     assets are NOT the customer's responsibility. Risk excluded from customer score.
 *   - shared_responsibility: Customer configures/uses vendor platform. Misconfigurations
 *     are customer's fault, but underlying infra CVEs are vendor's. Partial risk attribution.
 *   - customer_responsibility: Customer fully owns and operates. All risk attributed to customer.
 */
...
export type RiskResponsibility =
  | "vendor_responsibility"    // Vendor patches/manages — excluded from customer risk
  | "shared_responsibility"    // Customer configures, vendor manages infra — partial risk
  | "customer_responsibility"; // Customer fully owns — full risk attribution

export interface VendorDefinition {
  /** Display name of the vendor */
  name: string;
  /** Category of vendor infrastructure */
  category: VendorCategory;
  /** Who is responsible for vulnerabilities on this infrastructure */
  riskResponsibility: RiskResponsibility;
  /** Hostname regex patterns that identify this vendor's infrastructure */
  hostPatterns: RegExp[];
  /** CNAME patterns that indicate assets hosted on this vendor */
  cnamePatterns?: RegExp[];
  /** Known ASN numbers for this vendor (for IP-based detection) */
  asns?: number[];
  /** Brief description of what the vendor manages */
  description: string;
  /** What the customer is still responsible for even on this vendor's infra */
  customerResponsibilities?: string[];
}

export interface VendorClassification {
  vendor: VendorDefinition | null;
  category: VendorCategory | null;
  riskResponsibility: RiskResponsibility;
  matchMethod: "hostname" | "cname" | "asn" | "tag" | "none";
...
export const VENDOR_REGISTRY: VendorDefinition[] = [
  ...
  // ═══ SaaS (Software as a Service) ═════════════════════════════════════════
  {
    ...
    riskResponsibility: "vendor_responsibility",
    hostPatterns: [
      /salesforce\.com$/i, /force\.com$/i, /salesforceliveagent\.com$/i,
      /my\.salesforce\.com$/i, /lightning\.force\.com$/i, /visualforce\.com$/i,
    ],
...
/** Pre-built index for fast hostname matching */
let _hostnameIndex: { pattern: RegExp; vendor: VendorDefinition }[] | null = null;
let _cnameIndex: { pattern: RegExp; vendor: VendorDefinition }[] | null = null;
...
/**
 * Classify an asset's vendor ownership based on hostname, CNAME records, and tags.
 */
export function classifyVendor(asset: {
  hostname: string;
  cnames?: string[];
  tags?: string[];
  asn?: number;
  ...
  // 2. CNAME match (asset points to vendor infrastructure)
    ...
    for (const v of VENDOR_REGISTRY) {
      if (v.asns && v.asns.includes(asset.asn)) {
        return {
          ...
          matchMethod: "asn",
          confidence: 70, // Lower confidence — ASN match alone doesn't confirm vendor management
        };
      }
    }
  }

  // 4. Tag-based detection (e.g., tags from OSINT connectors)
  if (asset.tags) {
  ...
  // 5. No vendor match — customer-owned
  return {
    vendor: null,
    category: null,
...
/**
 * Batch classify multiple assets and partition by responsibility.
 */
export function partitionByResponsibility<T>(
  items: T[],
  getAssetInfo: (item: T) => { hostname: string; cnames?: string[]; tags?: string[]; asn?: number },
): {
  customerOwned: T[];
  vendorManaged: T[];
  sharedResponsibility: T[];
  classifications: Map<T, VendorClassification>;
} {
  const customerOwned: T[] = [];
  const vendorManaged: T[] = [];
  const sharedResponsibility: T[] = [];
  const classifications = new Map<T, VendorClassification>();
...
/**
 * Get a human-readable summary of risk responsibility for reporting.
 */
export function getRiskResponsibilityLabel(resp: RiskResponsibility): string {
  switch (resp) {
    case "vendor_responsibility":
 ...
 */
export function getCategoryLabel(cat: VendorCategory): string {
...
/**
 * Compute a vendor concentration risk score.
 * High concentration on a single vendor = higher supply chain risk.
 */
export function computeVendorConcentrationRisk(
  classifications: VendorClassification[]
): { score: number; band: string; topVendors: { name: string; count: number; percentage: number }[] } {
  ...
  let hhi = 0;
  const topVendors: { name: string; count: number; percentage: number }[] = [];
  ...
  // Normalize HHI to 0-100 score (HHI ranges from 1/n to 1)
  // Higher HHI = more concentrated = higher risk
  const concentrationScore = Math.round(hhi * 100);
  const band = concentrationScore >= 70 ? "CRITICAL"
    : concentrationScore >= 50 ? "HIGH"
    : concentrationScore >= 30 ? "MEDIUM"
    : concentrationScore >= 15 ? "LOW"
    : "MINIMAL";
