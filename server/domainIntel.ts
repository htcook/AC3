...
export async function runDomainIntelPipeline(
  ...
  try {
    passiveRecon = await runPassiveRecon(org.primaryDomain, {
      ...
      apiKeys: {
        ...
        github: ENV.GITHUB_PAT || ENV.GITHUB_CLASSIC_TOKEN || undefined,
        virustotal: ENV.VIRUSTOTAL_API_KEY || undefined,
        hibp: ENV.HIBP_API_KEY || undefined,
        whoisxml: ENV.WHOISXML_API_KEY || undefined,
        leakix: ENV.LEAKIX_API_KEY || undefined,
        fullhunt: ENV.FULLHUNT_API_KEY || undefined,
        netlas: ENV.NETLAS_API_KEY || undefined,
        hunter: ENV.HUNTER_API_KEY || undefined,
        passivetotal: ENV.PASSIVETOTAL_API_KEY || undefined,
        intelx: ENV.INTELX_API_KEY || undefined,
        hudson_rock: ENV.HUDSON_ROCK_API_KEY || undefined,
        leakcheck: ENV.LEAKCHECK_API_KEY || undefined,
      },
      timeout: 15000,
      maxConcurrent: 5,
  ...
  return {
    ...
    // Adapter: frontend reads pipeline.threatActorMatches with topMatches/matchSummary/totalCandidates
    threatActorMatches: threatMatchingResult ? {
      matchSummary: `${threatMatchingResult.summary.totalMatched} threat group(s) matched out of ${threatMatchingResult.summary.totalGroupsAnalyzed} analyzed. Top match: ${threatMatchingResult.summary.topGroupName || 'N/A'} (score: ${threatMatchingResult.summary.topGroupScore}).`,
      totalCandidates: threatMatchingResult.summary.totalGroupsAnalyzed,
      topMatches: threatMatchingResult.matchedGroups.map(g => ({
        actorId: g.groupId,
        name: g.groupName,
        aliases: g.aliases,
        type: g.groupType,
      ...
      attackPaths: threatMatchingResult.attackPaths,
      techniqueHeatmap: threatMatchingResult.techniqueHeatmap,
    } : undefined,
