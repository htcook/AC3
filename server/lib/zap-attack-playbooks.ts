 ...
 *
 * Technology names are validated against ZAP's canonical list before being
 * passed to the API to prevent "Bad Action" errors.
 *
 * Playbook categories:
...
export async function applyPlaybookToZap(
  ...
  if (playbook.contextTechnologies?.length) {
    try {
      const { filterValidZapTechnologies } = await import('./zap-technology-validator');
      const validTechs = filterValidZapTechnologies(playbook.contextTechnologies);
      if (validTechs.length > 0) {
        ...
        await zapRequest("/JSON/context/action/includeTechnologyList/", {
          contextName,
          technologyNames: validTechs.join(","),
        }, zapConfig);
      }
    } catch (err: any) {
