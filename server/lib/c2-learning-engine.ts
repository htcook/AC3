/**
 * C2 Learning Engine — Bidirectional Execution Feedback Loop
 *
 * Ingests execution results from all C2 frameworks (Caldera, Metasploit, Sliver)
 * and feeds them back into the TTP Knowledge Base to:
 *
 * 1. Update technique confidence scores based on real-world success/failure
 * 2. Refine environmental constraints (which OS/platform/privilege combos work)
 * 3. Enrich detection telemetry with actual artifacts observed
 * 4. Build execution history for technique reliability metrics
 * 5. Cross-pollinate learnings across C2 frameworks
 * 6. Generate "lessons learned" for future graph generation
 *
 * The engine operates on a simple feedback loop:
 *   Execute → Collect Result → Analyze → Update Knowledge → Improve Future Execution
 *
 * Author: Harrison Cook — AceofCloud
 */

import { getDb } from "../db";
import { ttpKnowledge } from "../../drizzle/schema";
import { eq } from "drizzle-orm";
import type { C2TaskResult, C2FrameworkType } from "./c2-abstraction";
import { invokeLLM } from "../_core/llm";

// ─── Types ──────────────────────────────────────────────────────────────────

export interface ExecutionFeedback {
  /** The technique ID (MITRE ATT&CK) that was executed */
  techniqueId: string;
  /** Which C2 framework performed the execution */
  framework: C2FrameworkType;
  /** The raw task result from the C2 adapter */
  taskResult: C2TaskResult;
  /** Target environment context */
  targetContext: TargetContext;
  /** Graph execution context (if part of an ability graph) */
...
export interface TargetContext {
  ...
  networkSegment?: string;   // internal, dmz, external
  defenses?: string[];       // EDR, AV, SIEM names detected
  osVersion?: string;        // e.g. "Windows Server 2019", "Ubuntu 22.04"
  domain?: string;           // AD domain if applicable
}
...
export interface GraphContext {
  ...
  totalNodes: number;
  predecessorResults: PredecessorResult[];
}
...
export interface LearningOutcome {
  techniqueId: string;
  framework: C2FrameworkType;
  /** Whether the technique succeeded */
  success: boolean;
  /** Confidence adjustment (-20 to +20) */
  confidenceAdjustment: number;
  /** New environmental constraint discoveries */
  newConstraints: EnvironmentalConstraint[];
  /** Observed telemetry signals */
  observedTelemetry: ObservedTelemetry[];
  /** Extracted artifacts (IOCs, file paths, registry keys) */
  extractedArtifacts: ExtractedArtifact[];
  /** Lessons learned for future execution */
  lessonsLearned: string[];
  /** Cross-framework applicability notes */
  crossFrameworkNotes: string[];
  /** Timestamp */
  analyzedAt: string;
}

export interface EnvironmentalConstraint {
  type: "required_os" | "required_privilege" | "required_software" | "network_access" | "defense_bypass" | "contraindication";
  value: string;
  confidence: number;
  source: "execution_result" | "llm_analysis" | "pattern_match";
}

export interface ObservedTelemetry {
  source: string;            // Sysmon, Windows Security, auditd, etc.
  eventId?: string;
  description: string;
  detectable: boolean;
  confidence: number;
  phase: string;             // execution, persistence, cleanup
}

export interface ExtractedArtifact {
  type: "file_path" | "registry_key" | "process" | "network_connection" | "dns_query" | "command_line" | "hash" | "mutex";
  value: string;
  context: string;
  framework: C2FrameworkType;
}

export interface TechniqueReliability {
  techniqueId: string;
  ...
  byFramework: Record<C2FrameworkType, { success: number; failure: number; rate: number }>;
  byPlatform: Record<string, { success: number; failure: number; rate: number }>;
  byPrivilege: Record<string, { success: number; failure: number; rate: number }>;
  commonFailureReasons: string[];
  bestFramework: C2FrameworkType | null;
...
/**
 * Analyze an execution result to extract learning outcomes.
 * Uses LLM for deep analysis of stdout/stderr output.
 */
async function analyzeExecution(feedback: ExecutionFeedback): Promise<LearningOutcome> {
  ...
  // Calculate confidence adjustment based on success/failure
  ...
  // Derive environmental constraints from the result
  outcome.newConstraints = deriveConstraints(feedback, success);

  // If we have meaningful output, use LLM for deeper analysis
  const outputLength = (taskResult.stdout + taskResult.stderr).length;
  if (outputLength > 50 && outputLength < 10000) {
    try {
      ...
      outcome.lessonsLearned.push(
        success
          ? `Technique ${techniqueId} succeeded on ${targetContext.platform} via ${framework}`
          : `Technique ${techniqueId} failed on ${targetContext.platform} via ${framework}: ${taskResult.stderr.slice(0, 200)}`
      );
    }
  }
  ...
  if (historicalData.length > 0) {
    const otherFrameworks = historicalData
      .filter(h => h.feedback.framework !== framework)
      .map(h => h.feedback.framework);
...
/**
 * Calculate confidence adjustment based on execution result.
 * Successful executions in harder environments give bigger boosts.
 */
...
/**
 * Extract artifacts (IOCs, file paths, etc.) from command output.
 */
function extractArtifactsFromOutput(output: string, framework: C2FrameworkType): ExtractedArtifact[] {
  ...
  if (ips) {
    for (const ip of [...new Set(ips)].filter(ip => !ip.startsWith("0.") && !ip.startsWith("255.")).slice(0, 10)) {
...
/**
 * Derive environmental constraints from execution feedback.
 */
function deriveConstraints(
  ...
  if (success) {
    // Record successful platform as a confirmed working environment
    constraints.push({
      type: "required_os",
    ...
    if (targetContext.privileges === "user") {
      // Worked without elevated privileges
      constraints.push({
...
/**
 * Use LLM to perform deep analysis of execution output.
 */
...
STDOUT (truncated):
${taskResult.stdout.slice(0, 3000)}

STDERR (truncated):
${taskResult.stderr.slice(0, 1000)}
...
Provide analysis in the following JSON format:
{
  ...
  try {
    ...
    return {
      telemetry: (parsed.telemetry || []).map((t: any) => ({
        source: t.source,
        eventId: t.eventId,
      ...
      additionalArtifacts: (parsed.additionalArtifacts || []).map((a: any) => ({
        type: a.type as ExtractedArtifact["type"],
        value: a.value,
        context: a.context,
        framework,
      })),
...
// ─── TTP Knowledge Update ───────────────────────────────────────────────────

/**
 * Update the TTP knowledge base with learning outcomes.
 */
async function updateTtpKnowledge(
  techniqueId: string,
  outcome: LearningOutcome
): Promise<void> {
  ...
  try {
    const existing = await db.select().from(ttpKnowledge)
      .where(eq(ttpKnowledge.techniqueId, techniqueId))
      .limit(1);

    if (existing.length === 0) return; // Only update existing entries
    ...
    // Merge environmental constraints
    const existingConstraints = (entry.environmentalConstraints as any) || {};
    ...
    // Merge artifacts into IOC patterns
    ...
    // Update purple team notes with lessons learned
    const existingNotes = entry.purpleTeamNotes || "";
    const newNotes = outcome.lessonsLearned.length > 0
    ...
    await db.update(ttpKnowledge)
      .set({
        ...
        purpleTeamNotes: newNotes.slice(0, 10000), // Prevent unbounded growth
...
function mergeConstraints(
  ...
  for (const c of newConstraints) {
    switch (c.type) {
      case "required_os": {
        const osList: string[] = merged.requiredOS || [];
...
function mergeTelemetry(
  ...
  return merged.slice(0, 50); // Cap at 50 entries
}

function deduplicateIocs(iocs: any[]): any[] {
...
/**
 * Get execution history for a specific technique.
 */
export function getHistoryForTechnique(techniqueId: string): ExecutionRecord[] {
  return executionHistory.filter(r => r.feedback.techniqueId === techniqueId);
}

/**
 * Get execution history for a specific framework.
 */
export function getHistoryForFramework(framework: C2FrameworkType): ExecutionRecord[] {
  return executionHistory.filter(r => r.feedback.framework === framework);
}

/**
 * Get all execution history (paginated).
 */
...
/**
 * Calculate technique reliability metrics across all executions.
 */
export function calculateTechniqueReliability(techniqueId: string): TechniqueReliability | null {
  const records = getHistoryForTechnique(techniqueId);
  if (records.length === 0) return null;

  const byFramework: Record<string, { success: number; failure: number }> = {};
  const byPlatform: Record<string, { success: number; failure: number }> = {};
  const byPrivilege: Record<string, { success: number; failure: number }> = {};
  ...
  const toRate = (stats: { success: number; failure: number }) => ({
    ...stats,
  ...
  ) as Record<C2FrameworkType, { success: number; failure: number; rate: number }>;
  ...
  // Find best framework and platform
  let bestFramework: C2FrameworkType | null = null;
  ...
  for (const [fw, stats] of Object.entries(frameworkRates)) {
    if (stats.rate > bestFrameworkRate && (stats.success + stats.failure) >= 2) {
      ...
      bestFramework = fw as C2FrameworkType;
    }
  }
  ...
  // Deduplicate and rank failure reasons
  const reasonCounts = new Map<string, number>();
  for (const r of failureReasons) {
    const key = r.toLowerCase().trim();
    reasonCounts.set(key, (reasonCounts.get(key) || 0) + 1);
  }
  const commonFailures = [...reasonCounts.entries()]
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5)
    .map(([reason]) => reason);
...
export function getLearningStats(): {
  ...
  byFramework: Record<string, { total: number; success: number; rate: number }>;
  topTechniques: Array<{ techniqueId: string; executions: number; successRate: number }>;
  recentLessons: string[];
  totalArtifactsExtracted: number;
  totalConstraintsLearned: number;
} {
  const byFramework: Record<string, { total: number; success: number }> = {};
  const byTechnique: Record<string, { total: number; success: number }> = {};
...
/**
 * Batch process multiple execution feedbacks (e.g., after a full graph execution).
 */
export async function batchProcessFeedback(
...
/**
 * Get the recommended C2 framework for a technique based on historical data.
 */
export function recommendFramework(
  techniqueId: string,
  targetPlatform: string
): { framework: C2FrameworkType; confidence: number; reason: string } | null {
  ...
  if (!platformData || (platformData.success + platformData.failure) < 2) {
    // Not enough platform-specific data, use overall best
  ...
  // Find best framework for this platform
  let bestFw: C2FrameworkType | null = null;
  let bestRate = 0;
  for (const [fw, stats] of Object.entries(reliability.byFramework)) {
    // Check if this framework has been used on this platform
    const fwPlatformRecords = executionHistory.filter(
      r => r.feedback.techniqueId === techniqueId &&
           r.feedback.framework === fw &&
           r.feedback.targetContext.platform === targetPlatform
    );
