...
const TECHNIQUE_GRAPH: TechniqueNode[] = [
  ...
  { id: "T1053.005-lateral", name: "Scheduled Task Lateral Movement (atexec)", tactic: "lateral-movement", tacticOrder: 9, description: "Move laterally via Windows Task Scheduler ATSVC pipe using Impacket atexec. Supports plaintext, Pass-the-Hash (NTLM), Pass-the-Ticket (ccache), and Pass-the-Key (AES) authentication. Lower detection profile than psexec (no service creation). Output written to ADMIN$ temp file.", platforms: ["windows"], prerequisites: ["credential-access", "ad-domain"], detectionRisk: "medium", tools: ["impacket-atexec", "atexec.py"], mitigations: ["Disable Task Scheduler RPC", "Monitor Event 4698/4699", "Block ATSVC named pipe", "Network segmentation"], stealthScore: 5, threatActors: ["apt29", "wizard_spider", "fin7", "conti"] },
