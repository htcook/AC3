 ...
 * ═══════════════════════════════════════════════════════════════════════
 * Evidence-grade detection of endpoint, host, and network-based security
 * monitoring across the target environment. Produces structured findings
 * with confidence scoring, MITRE ATT&CK mapping, and evasion playbooks.
 *
 * Domains:
 *   1. Endpoint — EDR/AV hooks, kernel callbacks, AMSI, ETW, process monitoring
 *   2. Host     — Sysmon, auditd, FIM, host firewall, registry monitoring
 *   3. Network  — SSL inspection, WAF, IDS/IPS, transparent proxy, DPI, CDN
 *
 * Each detection produces an InterceptionFinding with:
 *   - Vendor identification (product + version when possible)
 *   - Confidence score (0.0–1.0) with evidence chain
 *   - MITRE ATT&CK technique mapping (for evasion)
 *   - Evasion playbook (techniques to bypass the detection)
 *   - Evidence artifacts suitable for red/purple team reports
 */

// ─── Core Types ──────────────────────────────────────────────────────

export type InterceptionDomain = "endpoint" | "host" | "network";
...
export interface EvasionTechnique {
  ...
  opsecRisk: "low" | "medium" | "high" | "critical";
  /** Shell/PowerShell/Python snippets or tool references */
  implementation: string;
  /** Known to work against these specific products */
  testedAgainst: string[];
  /** MITRE ATT&CK technique for the evasion itself */
  mitre?: MitreMapping;
  /** Source: vendor docs, threat intel, user community, lab-tested */
  source: "vendor_docs" | "threat_intel" | "community" | "lab_tested" | "dfir_report";
}

export interface InterceptionFinding {
  id: string;
  ...
  mitre: MitreMapping[];
  evasionPlaybook: EvasionTechnique[];
  /** Host/IP where this was detected */
  detectedOn: string;
  ...
  operationalImpact: "blocks_exploitation" | "degrades_stealth" | "limits_persistence" | "monitors_c2" | "logs_activity" | "minimal";
  /** Recommended OPSEC adjustments */
  opsecRecommendations: string[];
}

export interface EvidenceArtifact {
  type: "process" | "service" | "driver" | "registry" | "file" | "network" | "certificate" | "header" | "behavior" | "config" | "api_hook" | "callback";
  name: string;
  value: string;
  /** Raw evidence data for report appendix */
  raw?: string;
}
...
export interface InterceptionReport {
  ...
  findings: InterceptionFinding[];
  summary: InterceptionSummary;
  /** Full evasion strategy recommendation */
  evasionStrategy: EvasionStrategy;
}

export interface InterceptionSummary {
  totalFindings: number;
  byDomain: Record<InterceptionDomain, number>;
  byConfidence: Record<ConfidenceLevel, number>;
  topVendors: Array<{ vendor: string; product: string; count: number }>;
  overallPosture: "hardened" | "moderate" | "weak" | "unknown";
  riskToRedTeam: "critical" | "high" | "medium" | "low";
}

export interface EvasionStrategy {
  /** Overall approach recommendation */
  approach: string;
  /** Ordered list of evasion priorities */
  priorities: Array<{
    finding: string;
    technique: string;
    rationale: string;
  ...
  /** C2 channel recommendations based on detected monitoring */
  c2Recommendations: string[];
  /** Payload recommendations based on detected EDR */
  payloadRecommendations: string[];
  /** Persistence recommendations based on detected host monitoring */
  persistenceRecommendations: string[];
}

// ─── Vendor Indicator Database ───────────────────────────────────────

export interface VendorIndicator {
  vendor: string;
  product: string;
  domain: InterceptionDomain;
  category: string;
  ...
  mitre: MitreMapping[];
  operationalImpact: InterceptionFinding["operationalImpact"];
}

export interface IndicatorPattern {
  type: "process_name" | "service_name" | "driver_name" | "registry_key" | "file_path" | "dll_name"
    | "api_hook" | "etw_provider" | "amsi_provider" | "kernel_callback" | "cert_issuer" | "http_header"
    | "response_pattern" | "dns_pattern" | "port_behavior" | "user_agent" | "sysmon_event"
    | "auditd_rule" | "fim_config" | "firewall_rule" | "minifilter_altitude";
  pattern: string | RegExp;
  confidence: number; // Base confidence when this indicator matches
  description: string;
}
...
export const VENDOR_DATABASE: VendorIndicator[] = [
  // ── CrowdStrike Falcon ──────────────────────────────────────────────
  {
    vendor: "CrowdStrike",
    product: "Falcon",
    ...
    indicators: [
      ...
      { type: "file_path", pattern: /WindowsSensor\.exe|CSFalconService\.exe|CSFalconContainer/i, confidence: 0.90, description: "CrowdStrike sensor binary" },
      { type: "registry_key", pattern: /CrowdStrike|CSFalcon/i, confidence: 0.90, description: "CrowdStrike registry entries" },
      ...
      { type: "kernel_callback", pattern: /csagent|csdevice/i, confidence: 0.90, description: "CrowdStrike kernel callbacks" },
      { type: "minifilter_altitude", pattern: /385200|385201/i, confidence: 0.80, description: "CrowdStrike minifilter altitude" },
      { type: "etw_provider", pattern: /CrowdStrike/i, confidence: 0.85, description: "CrowdStrike ETW provider" },
    ],
    evasionTechniques: [
      {
        id: "cs-ev-01", name: "Direct Syscalls (SysWhispers3)", difficulty: "moderate", opsecRisk: "medium",
      ...
      {
        id: "cs-ev-02", name: "Unhooking via Fresh NTDLL", difficulty: "easy", opsecRisk: "medium",
      ...
      {
        id: "cs-ev-03", name: "ETW Patching", difficulty: "easy", opsecRisk: "high",
      ...
      {
        id: "cs-ev-04", name: "Kernel Callback Removal (Vulnerable Driver)", difficulty: "expert", opsecRisk: "critical",
  ...
  // ── SentinelOne ─────────────────────────────────────────────────────
  {
    ...
    indicators: [
      ...
      { type: "driver_name", pattern: /sentinel|sentinelmonitor/i, confidence: 0.90, description: "SentinelOne kernel driver" },
      { type: "file_path", pattern: /SentinelAgent\.exe|SentinelOne/i, confidence: 0.90, description: "SentinelOne binary path" },
    ...
    evasionTechniques: [
      {
        id: "s1-ev-01", name: "Direct Syscalls + Sleep Obfuscation", difficulty: "hard", opsecRisk: "medium",
      ...
      {
        id: "s1-ev-02", name: "PPID Spoofing + Token Manipulation", difficulty: "moderate", opsecRisk: "medium",
  ...
  // ── Microsoft Defender for Endpoint ─────────────────────────────────
  {
    ...
    indicators: [
      ...
      { type: "service_name", pattern: /Sense|WinDefend|WdNisSvc|WdNisDrv/i, confidence: 0.90, description: "Defender service" },
      ...
      { type: "amsi_provider", pattern: /Windows Defender|MpOav/i, confidence: 0.90, description: "Defender AMSI provider" },
      { type: "etw_provider", pattern: /Microsoft-Windows-Windows Defender|Microsoft-Antimalware-Scan-Interface/i, confidence: 0.90, description: "Defender ETW providers" },
      { type: "minifilter_altitude", pattern: /328010/i, confidence: 0.85, description: "WdFilter minifilter altitude" },
    ],
    evasionTechniques: [
      {
        id: "mde-ev-01", name: "AMSI Bypass (AmsiScanBuffer Patch)", difficulty: "trivial", opsecRisk: "low",
      ...
      {
        id: "mde-ev-02", name: "Defender Exclusion Abuse", difficulty: "easy", opsecRisk: "low",
      ...
      {
        id: "mde-ev-03", name: "ETW Threat Intelligence Provider Blind", difficulty: "hard", opsecRisk: "high",
  ...
  // ── Carbon Black (VMware) ──────────────────────────────────────────
  {
    ...
    indicators: [
      ...
      { type: "driver_name", pattern: /carbonblack|cbk7|CbOsR/i, confidence: 0.90, description: "Carbon Black kernel driver" },
    ...
    evasionTechniques: [
      {
        id: "cb-ev-01", name: "Process Hollowing with Legitimate Parent", difficulty: "moderate", opsecRisk: "medium",
  ...
  // ── Sophos Intercept X ─────────────────────────────────────────────
  {
    ...
    indicators: [
      ...
      { type: "driver_name", pattern: /savonaccess|sophosed|hmpalert/i, confidence: 0.90, description: "Sophos kernel driver" },
      { type: "dll_name", pattern: /sophos.*\.dll|hmpalert\.dll/i, confidence: 0.85, description: "Sophos DLL" },
    ],
    evasionTechniques: [
      {
        id: "soph-ev-01", name: "Timestomping + Metadata Mimicry", difficulty: "easy", opsecRisk: "low",
  ...
  // ── Palo Alto Cortex XDR ───────────────────────────────────────────
  {
    ...
    indicators: [
      ...
      { type: "driver_name", pattern: /tdevice|cyverak|cyvrfsfd/i, confidence: 0.90, description: "Cortex XDR kernel driver" },
      { type: "file_path", pattern: /Cortex XDR|Palo Alto Networks\\Traps/i, confidence: 0.85, description: "Cortex XDR installation path" },
    ],
    evasionTechniques: [
      {
        id: "pa-ev-01", name: "Module Stomping (DLL Hollowing)", difficulty: "hard", opsecRisk: "medium",
  ...
  // ── Elastic EDR (Endpoint Security) ────────────────────────────────
  {
    ...
    category: "EDR",
    indicators: [
      { type: "process_name", pattern: /elastic-agent|elastic-endpoint|filebeat|winlogbeat/i, confidence: 0.90, description: "Elastic agent process" },
      { type: "service_name", pattern: /ElasticAgent|ElasticEndpoint/i, confidence: 0.90, description: "Elastic agent service" },
      { type: "file_path", pattern: /Elastic\\Agent|elastic-agent/i, confidence: 0.85, description: "Elastic agent path" },
    ],
    evasionTechniques: [
      {
        id: "el-ev-01", name: "Elastic Rule Evasion via Threshold Manipulation", difficulty: "moderate", opsecRisk: "low",
  ...
  // ── Sysmon (Host Monitoring) ───────────────────────────────────────
  {
    ...
    indicators: [
      ...
      { type: "sysmon_event", pattern: /EventID.*[1-9]|Sysmon.*operational/i, confidence: 0.80, description: "Sysmon event log entries" },
    ],
    evasionTechniques: [
      {
        id: "sm-ev-01", name: "Sysmon Config Extraction + Rule Gaps", difficulty: "easy", opsecRisk: "low",
      ...
      {
        id: "sm-ev-02", name: "Sysmon Driver Unload (Admin)", difficulty: "trivial", opsecRisk: "high",
  ...
  // ── Auditd (Linux Host Monitoring) ─────────────────────────────────
  {
    ...
    indicators: [
      ...
      { type: "file_path", pattern: /\/etc\/audit\/auditd\.conf|\/etc\/audit\/rules\.d/i, confidence: 0.90, description: "auditd config files" },
      { type: "auditd_rule", pattern: /auditctl|ausearch|aureport/i, confidence: 0.80, description: "auditd management commands" },
    ],
    evasionTechniques: [
      {
        id: "aud-ev-01", name: "Auditd Rule Gap Analysis", difficulty: "easy", opsecRisk: "low",
      ...
      {
        id: "aud-ev-02", name: "In-Memory Execution (memfd_create)", difficulty: "moderate", opsecRisk: "low",
  ...
  // ── OSSEC/Wazuh (FIM + HIDS) ──────────────────────────────────────
  {
    ...
    indicators: [
      ...
      { type: "service_name", pattern: /wazuh-agent|ossec/i, confidence: 0.90, description: "Wazuh service" },
      { type: "file_path", pattern: /\/var\/ossec|\/var\/wazuh|wazuh-agent/i, confidence: 0.90, description: "Wazuh installation path" },
      { type: "fim_config", pattern: /syscheck|realtime|check_all/i, confidence: 0.80, description: "Wazuh FIM configuration" },
    ],
    evasionTechniques: [
      {
        id: "waz-ev-01", name: "FIM Interval Exploitation", difficulty: "easy", opsecRisk: "low",
  ...
  // ── SSL/TLS Inspection (Network) ───────────────────────────────────
  {
    ...
    indicators: [
      ...
      { type: "http_header", pattern: /X-SSL-Inspection|X-Decrypted|X-BlueCoat|X-Zscaler/i, confidence: 0.90, description: "SSL inspection proxy headers" },
    ],
    evasionTechniques: [
      {
        id: "ssl-ev-01", name: "Certificate Pinning Bypass Detection", difficulty: "easy", opsecRisk: "low",
      ...
      {
        id: "ssl-ev-02", name: "Domain Fronting / CDN Tunneling", difficulty: "moderate", opsecRisk: "medium",
  ...
  // ── WAF (Network) ─────────────────────────────────────────────────
  {
    ...
    indicators: [
      ...
      { type: "http_header", pattern: /x-amz-cf-id|x-amz-cf-pop/i, confidence: 0.90, description: "AWS CloudFront/WAF" },
      { type: "http_header", pattern: /x-akamai|akamai/i, confidence: 0.90, description: "Akamai WAF" },
      { type: "http_header", pattern: /x-powered-by-plesk|modsecurity/i, confidence: 0.85, description: "ModSecurity WAF" },
      { type: "response_pattern", pattern: /access denied|request blocked|security policy|waf.*block/i, confidence: 0.80, description: "WAF block response" },
      { type: "response_pattern", pattern: /cloudflare.*ray|attention required.*cloudflare/i, confidence: 0.90, description: "Cloudflare challenge page" },
    ],
    evasionTechniques: [
      {
        id: "waf-ev-01", name: "WAF Rule Bypass via Encoding", difficulty: "moderate", opsecRisk: "low",
      ...
      {
        id: "waf-ev-02", name: "Origin IP Discovery", difficulty: "moderate", opsecRisk: "low",
  ...
  // ── IDS/IPS (Network) ──────────────────────────────────────────────
  {
    ...
    indicators: [
      ...
      { type: "port_behavior", pattern: /tcp_rst_injection|connection_reset/i, confidence: 0.80, description: "TCP RST injection (inline IPS)" },
      { type: "http_header", pattern: /x-ids|x-ips|x-nids/i, confidence: 0.75, description: "IDS/IPS proxy headers" },
    ],
    evasionTechniques: [
      {
        id: "ids-ev-01", name: "Protocol-Level Evasion (Fragmentation)", difficulty: "moderate", opsecRisk: "low",
      ...
      {
        id: "ids-ev-02", name: "Encrypted C2 Channels", difficulty: "easy", opsecRisk: "low",
  ...
  // ── Next-Gen Firewall (Network) ────────────────────────────────────
  {
    ...
    indicators: [
      ...
      { type: "response_pattern", pattern: /application.*blocked|category.*blocked|url.*filtering/i, confidence: 0.80, description: "NGFW URL filtering block" },
    ...
    evasionTechniques: [
      {
        id: "ngfw-ev-01", name: "Application Tunneling", difficulty: "moderate", opsecRisk: "medium",
...
/**
 * Analyze engagement/scan data for interception indicators.
 * Accepts heterogeneous evidence from multiple sources:
 *   - Process lists from agent beacons
 *   - Service enumerations
 *   - Network scan results (headers, certs, behaviors)
 *   - Host reconnaissance output
 */
export function fingerprintInterceptions(input: FingerprintInput): InterceptionReport {
  const findings: InterceptionFinding[] = [];
  const scanId = input.scanId || `ifp-${Date.now()}`;
  const target = input.target || "unknown";

  // Match each vendor's indicators against the input evidence
  for (const vendor of VENDOR_DATABASE) {
    const matchedIndicators: Array<{ indicator: IndicatorPattern; evidence: EvidenceArtifact }> = [];
    ...
    if (matchedIndicators.length > 0) {
      // Calculate confidence from matched indicators
...
export interface FingerprintInput {
  scanId?: string;
  target?: string;
  /** Process list from agent or enumeration */
  processes?: Array<{ name: string; pid?: number; path?: string; commandLine?: string; user?: string }>;
  /** Windows services */
  services?: Array<{ name: string; displayName?: string; status?: string; startType?: string; binaryPath?: string }>;
  /** Kernel drivers */
  drivers?: Array<{ name: string; path?: string; altitude?: string }>;
  /** Registry keys found */
  registryKeys?: Array<{ path: string; valueName?: string; valueData?: string }>;
  /** Files found on disk */
  files?: Array<{ path: string; name?: string; hash?: string }>;
  /** DLLs loaded in processes */
  dlls?: Array<{ name: string; path?: string; loadedIn?: string }>;
  /** HTTP response headers from web scanning */
  httpHeaders?: Array<{ name: string; value: string; url?: string }>;
  /** TLS certificate information */
  certificates?: Array<{ subject: string; issuer: string; serialNumber?: string; fingerprint?: string }>;
  /** Network behavior observations */
  networkBehaviors?: Array<{ type: string; description: string; evidence?: string }>;
  /** Raw text output from commands (e.g., auditctl -l, fltMC instances) */
  commandOutputs?: Array<{ command: string; output: string }>;
  /** Technologies detected by fingerprinting */
  technologies?: string[];
  /** Risk signals from passive recon */
  riskSignals?: Array<{ type: string; severity: string; rationale: string }>;
  /** WAF detection results */
  wafDetected?: string;
  /** Passive recon raw data */
  passiveReconData?: any;
}

// ─── Indicator Matching ──────────────────────────────────────────────

function matchIndicator(indicator: IndicatorPattern, input: FingerprintInput): EvidenceArtifact | null {
  ...
  switch (indicator.type) {
    ...
    case "dll_name":
      for (const d of input.dlls || []) {
        if (pattern.test(d.name) || pattern.test(d.path || "")) {
          return { type: "api_hook", name: d.name, value: d.path || d.name, raw: d.loadedIn ? `Loaded in: ${d.loadedIn}` : undefined };
        }
      }
      break;
    ...
    case "firewall_rule":
      ...
      // Also check registry for Windows-based indicators
      for (const r of input.registryKeys || []) {
    ...
    case "http_header":
      for (const h of input.httpHeaders || []) {
        if (pattern.test(h.name) || pattern.test(h.value)) {
          return { type: "header", name: h.name, value: h.value, raw: h.url ? `URL: ${h.url}` : undefined };
        }
      }
      break;
  ...
  // Check technologies array as a catch-all
...
function calculateConfidence(matches: Array<{ indicator: IndicatorPattern; evidence: EvidenceArtifact }>): ConfidenceScore {
  if (matches.length === 0) return { level: "unconfirmed", score: 0, rationale: "No indicators matched" };

  // Use the highest single indicator confidence as the base
  ...
  // Boost for diverse evidence types
  const evidenceTypes = new Set(matches.map(m => m.evidence.type));
  const diversityBoost = Math.min(0.03 * (evidenceTypes.size - 1), 0.10);

  const finalScore = Math.min(maxConfidence + corroborationBoost + diversityBoost, 1.0);

  let level: ConfidenceLevel;
  if (finalScore >= 0.95) level = "confirmed";
  else if (finalScore >= 0.80) level = "high";
  else if (finalScore >= 0.50) level = "medium";
  else if (finalScore >= 0.20) level = "low";
...
function generateOpsecRecommendations(vendor: VendorIndicator): string[] {
  const recs: string[] = [];
...
function buildSummary(findings: InterceptionFinding[]): InterceptionSummary {
  const byDomain: Record<InterceptionDomain, number> = { endpoint: 0, host: 0, network: 0 };
  const byConfidence: Record<ConfidenceLevel, number> = { confirmed: 0, high: 0, medium: 0, low: 0, unconfirmed: 0 };
  const vendorCounts = new Map<string, { vendor: string; product: string; count: number }>();
  ...
  let overallPosture: InterceptionSummary["overallPosture"] = "unknown";
  ...
  // Risk to red team
  let riskToRedTeam: InterceptionSummary["riskToRedTeam"] = "low";
...
// ─── Evasion Strategy Builder ────────────────────────────────────────

function buildEvasionStrategy(findings: InterceptionFinding[]): EvasionStrategy {
  const priorities: EvasionStrategy["priorities"] = [];
  const c2Recs: string[] = [];
  const payloadRecs: string[] = [];
  const persistenceRecs: string[] = [];

  // Sort findings by operational impact severity
  ...
  for (const finding of sorted) {
    // Pick the best evasion technique (lowest difficulty + lowest opsec risk)
    const bestEvasion = finding.evasionPlaybook
      .sort((a, b) => {
        const diffOrder: Record<string, number> = { trivial: 0, easy: 1, moderate: 2, hard: 3, expert: 4 };
  ...
  // C2 recommendations based on network findings
  ...
  // Payload recommendations based on EDR findings
  ...
  // Persistence recommendations based on host monitoring
  ...
  const approach = edrFindings.length > 0
    ? `Environment has ${edrFindings.length} EDR product(s) detected. Prioritize userland hook bypass and in-memory execution. ${hasSSLInspection ? "Network traffic is SSL-inspected — use domain fronting for C2." : ""} ${hostFindings.length > 0 ? `${hostFindings.length} host monitoring tool(s) detected — extract configs for gap analysis.` : ""}`
    : `No EDR detected on target. ${hasWAF ? "WAF present — focus on WAF bypass for web attacks." : "Standard exploitation approach should work."} ${hostFindings.length > 0 ? "Host monitoring present — review persistence strategy." : ""}`;
...
// ─── Convenience: Build FingerprintInput from engagement/DI scan data ──

/**
 * Convert engagement scan results (assets, findings, passive recon) into
 * a FingerprintInput suitable for the fingerprinting engine.
 */
export function buildFingerprintInputFromEngagement(data: {
  assets?: any[];
  findings?: any[];
  passiveRecon?: any;
  agentData?: any;
  ...
  // Extract from agent data (if Caldera/Ember agent has reported back)
  if (data.agentData) {
...
/**
 * Build FingerprintInput from DI scan results (discovered assets).
 */
export function buildFingerprintInputFromDIScan(data: {
  assets?: any[];
  scan?: any;
  target?: string;
}): FingerprintInput {
  const input: FingerprintInput = {
    scanId: data.scan?.id ? `di-${data.scan.id}` : undefined,
  ...
  for (const asset of data.assets || []) {
    // Technologies
    const techs = asset.technologies || asset.detectedTechnologies?.map((t: any) => t.name) || [];
    ...
    // Certificates from posture findings
