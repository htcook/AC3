/**
 * Affiliated Domain Discovery Enrichment
 * 
 * Discovers all domains owned by or affiliated with the target organization.
 * Combines multiple intelligence sources:
 * 
 * 1. SecurityTrails reverse WHOIS (registrant org/email → all domains)
 * 2. SecurityTrails associated domains endpoint
 * 3. crt.sh Certificate Transparency — org-name search (not just wildcard)
 * 4. DNS correlation (shared NS, MX, SPF includes)
 * 5. LLM knowledge for well-known affiliations (e.g., PBS → mypbs.org, pbskids.org)
 * 
 * This module runs as a DI pipeline enrichment stage and produces a structured
 * result that feeds into the scan detail UI and PDF report.
 */
...
export interface AffiliatedDomain {
  domain: string;
  relationship: "same_registrant" | "same_org" | "shared_certificate" | "dns_correlation" | "llm_knowledge" | "spf_include";
...
export interface AffiliatedDomainResult {
  targetDomain: string;
  searchedAt: number;
...
// ─── SecurityTrails: Reverse WHOIS + Associated Domains ─────────────────────

async function discoverViaSecurityTrails(
  domain: string
): Promise<{ domains: AffiliatedDomain[]; registrantOrg: string | null; registrantEmail: string | null }> {
  ...
  const results: AffiliatedDomain[] = [];
  let registrantOrg: string | null = null;
  let registrantEmail: string | null = null;

  try {
    // Step 1: Get WHOIS data for the target domain
    ...
    if (whoisResp.ok) {
      ...
      // Step 2: Reverse WHOIS by registrant org
      if (registrantOrg) {
        try {
      ...
      // Step 3: Reverse WHOIS by registrant email (skip privacy-protected)
    ...
    // Step 4: SecurityTrails associated domains endpoint
    try {
...
// ─── crt.sh: Organization Name Search ───────────────────────────────────────

async function discoverViaCrtShOrgName(orgName: string, targetDomain: string): Promise<AffiliatedDomain[]> {
  if (!orgName) return [];
  const results: AffiliatedDomain[] = [];
  const seenDomains = new Set<string>();

  try {
    // Search crt.sh by organization name (O= field in certificates)
    ...
    for (const cert of certs) {
      ...
      for (const name of names) {
        ...
        // Skip if it's the target domain or a subdomain of it
...
// ─── DNS Correlation: Shared NS/MX/SPF ─────────────────────────────────────

async function discoverViaDNSCorrelation(domain: string): Promise<AffiliatedDomain[]> {
  const results: AffiliatedDomain[] = [];

  try {
    // Get SPF record for include domains
...
// ─── LLM Knowledge: Well-Known Affiliations ─────────────────────────────────

async function discoverViaLLMKnowledge(domain: string, orgName: string | null): Promise<AffiliatedDomain[]> {
  const results: AffiliatedDomain[] = [];
    ...
    if (content) {
      ...
      for (const item of (parsed.affiliatedDomains || [])) {
        if (item.domain && item.domain !== domain) {
          const confMap: Record<string, number> = { high: 75, medium: 55, low: 35 };
          results.push({
...
export async function runAffiliatedDomainDiscovery(
  domain: string,
  orgName?: string | null
): Promise<AffiliatedDomainResult> {
  ...
  // Collect results
  const allDomains: AffiliatedDomain[] = [];
  let registrantOrg: string | null = null;
  let registrantEmail: string | null = null;
  ...
  // If we got an org name from WHOIS but didn't have one, re-run crt.sh
  ...
  const affiliatedDomains = Array.from(domainMap.values())
