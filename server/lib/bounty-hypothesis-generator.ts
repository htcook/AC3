/**
 * Bug Bounty Hypothesis Generator
 * 
 * Specialist module that takes reconnaissance data and produces ranked, testable
 * hypotheses about potential vulnerabilities. Each hypothesis includes:
 * - Specific vulnerability class and affected endpoint
 * - Confidence level with explicit reasoning
 * - Evidence that supports or would change the confidence
 * - Concrete verification steps for a human researcher
 * - Estimated effort and potential bounty value
 * 
 * Architecture follows the expert recommendation: "The hypothesis generator is
 * probably the highest-value first build because it does the work that's hardest
 * for humans to do manually (synthesizing across many signals into specific
 * testable hypotheses)."
 */
...
export interface ReconData {
  ...
  wafDetected?: string;
  cdnDetected?: string;
  historicalArtifacts?: HistoricalArtifact[];
  configAnomalies?: ConfigAnomaly[];
  passiveFindings?: PassiveFinding[];
}
...
export interface TechStackFingerprint {
  ...
  version?: string;
  confidence: number;
  source: 'header' | 'body' | 'script' | 'meta' | 'cookie' | 'url_pattern' | 'error_page' | 'wappalyzer';
}
...
export interface HistoricalArtifact {
  url: string;
  discoveredAt: string;
  type: 'endpoint' | 'api' | 'config' | 'backup' | 'admin' | 'debug';
  stillAccessible: boolean;
}

export interface ConfigAnomaly {
  category: 'cors' | 'csp' | 'headers' | 'tls' | 'dns' | 'api_gateway' | 'auth' | 'cache';
  description: string;
...
export type VulnClass =
  ...
  | 'sqli_classic' | 'sqli_blind' | 'sqli_time_based'
  | 'ssrf' | 'ssrf_blind'
  | 'idor' | 'bola'
  | 'auth_bypass' | 'broken_auth' | 'session_fixation'
...
export interface Hypothesis {
  ...
  evidenceThatWouldChangeConfidence: string[];
  verificationSteps: VerificationStep[];
  estimatedEffort: 'minutes' | 'hours' | 'days';
  potentialSeverity: 'none' | 'low' | 'medium' | 'high' | 'critical';
  potentialBountyRange?: { min: number; max: number; currency: string };
  chainPotential: ChainLink[];
...
export interface Evidence {
  type: 'tech_fingerprint' | 'config_anomaly' | 'historical' | 'behavioral' | 'passive_scan' | 'known_cve' | 'endpoint_pattern';
  description: string;
...
export interface ChainLink {
  fromVulnClass: VulnClass;
  toVulnClass: VulnClass;
...
export interface HypothesisGenerationResult {
  ...
  reconQuality: ReconQualityAssessment;
  generatedAt: number;
}
...
export interface HypothesisSummary {
  ...
  topChainOpportunities: ChainLink[];
  estimatedResearchHours: number;
  highValueTargets: string[];
}

export interface ReconQualityAssessment {
...
interface TechVulnMapping {
  techPattern: RegExp;
  vulnClasses: Array<{
    vulnClass: VulnClass;
    baseConfidence: number;
    versionCondition?: (version: string) => boolean;
    reasoning: string;
    verificationHint: string;
  }>;
}

const TECH_VULN_MAPPINGS: TechVulnMapping[] = [
  {
...
// ─── Configuration Anomaly → Hypothesis Mapping ──────────────────────────────

interface ConfigVulnMapping {
  category: ConfigAnomaly['category'];
  pattern: RegExp;
  vulnClass: VulnClass;
...
const CONFIG_VULN_MAPPINGS: ConfigVulnMapping[] = [
...
const CHAIN_PATTERNS: ChainLink[] = [
  ...
  { fromVulnClass: 'info_disclosure', toVulnClass: 'auth_bypass', chainDescription: 'Leaked credentials or tokens → authentication bypass', impactMultiplier: 3.0 },
  { fromVulnClass: 'ssrf', toVulnClass: 'rce', chainDescription: 'SSRF to internal service → remote code execution via internal API', impactMultiplier: 4.0 },
  ...
  { fromVulnClass: 'idor', toVulnClass: 'sensitive_data_exposure', chainDescription: 'IDOR on user objects → mass data exfiltration', impactMultiplier: 2.0 },
  { fromVulnClass: 'sqli_classic', toVulnClass: 'rce', chainDescription: 'SQL injection → file write → web shell → RCE', impactMultiplier: 4.0 },
  { fromVulnClass: 'open_redirect', toVulnClass: 'auth_bypass', chainDescription: 'Open redirect in OAuth flow → token theft → account takeover', impactMultiplier: 3.0 },
  ...
  { fromVulnClass: 'graphql_introspection', toVulnClass: 'idor', chainDescription: 'GraphQL schema leak → discover hidden mutations → IDOR on sensitive objects', impactMultiplier: 2.0 },
  ...
  { fromVulnClass: 'race_condition', toVulnClass: 'business_logic', chainDescription: 'Race condition on payment/transfer → double-spend or balance manipulation', impactMultiplier: 3.0 },
...
const VULN_SEVERITY_MAP: Record<VulnClass, string> = {
  xss_reflected: 'medium', xss_stored: 'high', xss_dom: 'medium',
  sqli_classic: 'critical', sqli_blind: 'high', sqli_time_based: 'high',
  ssrf: 'high', ssrf_blind: 'medium',
  idor: 'high', bola: 'high',
  auth_bypass: 'critical', broken_auth: 'high', session_fixation: 'medium',
  rce: 'critical', command_injection: 'critical', code_injection: 'critical',
  path_traversal: 'high', lfi: 'high', rfi: 'critical',
  open_redirect: 'low', csrf: 'medium',
  info_disclosure: 'low', sensitive_data_exposure: 'high',
  xxe: 'high', ssti: 'high', deserialization: 'critical',
  race_condition: 'medium', subdomain_takeover: 'medium',
  cors_misconfiguration: 'medium', csp_bypass: 'low',
  jwt_weakness: 'high', graphql_introspection: 'low', graphql_injection: 'high',
  api_mass_assignment: 'high', business_logic: 'medium',
  privilege_escalation: 'critical', cache_poisoning: 'high',
  http_request_smuggling: 'high', websocket_hijacking: 'medium',
};

// ─── Bounty Range Estimation ─────────────────────────────────────────────────

const SEVERITY_BOUNTY_RANGES: Record<string, { min: number; max: number }> = {
  critical: { min: 5000, max: 50000 },
  high: { min: 1000, max: 15000 },
  medium: { min: 250, max: 3000 },
  low: { min: 50, max: 500 },
  none: { min: 0, max: 0 },
};
...
function assessReconQuality(recon: ReconData): ReconQualityAssessment {
  const missing: string[] = [];
  const recommendations: string[] = [];
  let score = 0;

  if (recon.techStack.length > 0) score += 25;
  else { missing.push('technology stack fingerprinting'); recommendations.push('Run Wappalyzer or similar tech detection'); }

  if (recon.openPorts.length > 0) score += 15;
  else { missing.push('port scan results'); recommendations.push('Run Nmap service detection scan'); }

  if (recon.endpoints.length > 0) score += 20;
  else { missing.push('endpoint enumeration'); recommendations.push('Run Gobuster/ffuf directory enumeration'); }

  if (recon.subdomains.length > 0) score += 10;
  else { missing.push('subdomain enumeration'); recommendations.push('Run subfinder/amass for subdomain discovery'); }

  if (recon.headers && Object.keys(recon.headers).length > 0) score += 10;
  else { missing.push('HTTP response headers'); recommendations.push('Collect response headers from main endpoints'); }

  if (recon.certificates) score += 5;
  else { missing.push('TLS certificate analysis'); recommendations.push('Analyze TLS certificates for alt names and issuer'); }

  if (recon.historicalArtifacts && recon.historicalArtifacts.length > 0) score += 10;
  else { missing.push('historical reconnaissance (Wayback)'); recommendations.push('Check Wayback Machine for historical endpoints'); }

  if (recon.configAnomalies && recon.configAnomalies.length > 0) score += 5;
  else { missing.push('configuration analysis'); recommendations.push('Analyze CORS, CSP, and security headers'); }

  return { overallScore: score, missingData: missing, recommendations };
}

function generateTechStackHypotheses(recon: ReconData): Hypothesis[] {
  ...
  for (const tech of recon.techStack) {
    for (const mapping of TECH_VULN_MAPPINGS) {
      ...
      for (const vulnMapping of mapping.vulnClasses) {
        ...
        const affectedEndpoint = corroboratingEndpoints.length > 0
          ? corroboratingEndpoints[0].path
          : recon.targetDomain;
        ...
        hypotheses.push({
          ...
          supportingEvidence: [
            ...
            ...(corroboratingEndpoints.map(e => ({
              type: 'endpoint_pattern' as const,
          ...
          disconfirmingEvidence: [
            ...(recon.wafDetected ? [{ type: 'behavioral' as const, description: `WAF detected (${recon.wafDetected}) may block exploitation`, strength: 'moderate' as const, source: 'waf_detection' }] : []),
          ],
          ...
          potentialSeverity: severity as any,
...
function generateConfigAnomalyHypotheses(recon: ReconData): Hypothesis[] {
  const hypotheses: Hypothesis[] = [];
...
function generateHistoricalHypotheses(recon: ReconData): Hypothesis[] {
  const hypotheses: Hypothesis[] = [];
...
function generateSubdomainHypotheses(recon: ReconData): Hypothesis[] {
  const hypotheses: Hypothesis[] = [];

  // Check for potential subdomain takeover candidates
  ...
  // Check cert alt names for additional subdomains not in the enumerated list
  const certSubdomains = recon.certificates?.altNames || [];
  const allSubdomains = [...new Set([...recon.subdomains, ...certSubdomains])];
  ...
  if (allSubdomains.length > 20) {
    hypotheses.push({
      ...
      potentialBountyRange: { min: 250, max: 3000, currency: 'USD' },
...
function getToolForVulnClass(vulnClass: VulnClass): string {
  const toolMap: Partial<Record<VulnClass, string>> = {
...
function findChainOpportunities(vulnClass: VulnClass): ChainLink[] {
  return CHAIN_PATTERNS.filter(c => c.fromVulnClass === vulnClass || c.toVulnClass === vulnClass);
}

function estimateDuplicateLikelihood(vulnClass: VulnClass, technology: string): 'low' | 'medium' | 'high' {
  // Common vulns in popular tech = high duplicate likelihood
...
function rankHypotheses(hypotheses: Hypothesis[]): Hypothesis[] {
  return hypotheses.sort((a, b) => {
    // Primary: confidence score (higher first)
    const confDiff = b.confidenceScore - a.confidenceScore;
    if (Math.abs(confDiff) > 0.1) return confDiff;

    // Secondary: severity (higher first)
    const sevOrder: Record<string, number> = { critical: 5, high: 4, medium: 3, low: 2, none: 1 };
    const sevDiff = (sevOrder[b.potentialSeverity] || 0) - (sevOrder[a.potentialSeverity] || 0);
    if (sevDiff !== 0) return sevDiff;

    // Tertiary: lower duplicate likelihood preferred
    const dupOrder: Record<string, number> = { low: 3, medium: 2, high: 1 };
    const dupDiff = (dupOrder[b.duplicateLikelihood] || 0) - (dupOrder[a.duplicateLikelihood] || 0);
    if (dupDiff !== 0) return dupDiff;

    // Quaternary: less effort preferred
    const effortOrder: Record<string, number> = { minutes: 3, hours: 2, days: 1 };
...
export function generateHypotheses(recon: ReconData): HypothesisGenerationResult {
  ...
  // Build summary
  const byConfidence: Record<ConfidenceLevel, number> = { high: 0, medium: 0, low: 0, speculative: 0 };
  ...
  // Find top chain opportunities
  const allChains = ranked.flatMap(h => h.chainPotential);
  const uniqueChains = allChains.filter((c, i) => 
    allChains.findIndex(x => x.fromVulnClass === c.fromVulnClass && x.toVulnClass === c.toVulnClass) === i
  );
  const topChains = uniqueChains.sort((a, b) => b.impactMultiplier - a.impactMultiplier).slice(0, 5);

  // Estimate research hours
  const effortMap: Record<string, number> = { minutes: 0.25, hours: 2, days: 8 };
  const estimatedResearchHours = ranked.reduce((sum, h) => sum + (effortMap[h.estimatedEffort] || 2), 0);

  // Identify high-value targets (endpoints with multiple hypotheses)
  const endpointCounts = new Map<string, number>();
  for (const h of ranked) {
    endpointCounts.set(h.affectedEndpoint, (endpointCounts.get(h.affectedEndpoint) || 0) + 1);
  }
  const highValueTargets = Array.from(endpointCounts.entries())
...
/**
 * Generate hypotheses for a specific program using program-specific context
 */
export function generateProgramAwareHypotheses(
  recon: ReconData,
  programContext: {
    avgBounty?: number;
    maxBounty?: number;
  ...
  // Adjust bounty ranges based on program data
  if (programContext.avgBounty || programContext.maxBounty) {
    for (const h of result.hypotheses) {
      if (h.potentialBountyRange && programContext.maxBounty) {
        const severityMultiplier: Record<string, number> = { critical: 0.8, high: 0.5, medium: 0.2, low: 0.05 };
  ...
  // Boost confidence for vuln classes the program commonly resolves
  if (programContext.commonCWEs && programContext.commonCWEs.length > 0) {
    for (const h of result.hypotheses) {
      // If the program has a history of accepting this type of finding, boost confidence
...
function vulnClassToCWE(vulnClass: VulnClass): string | null {
  const mapping: Partial<Record<VulnClass, string>> = {
    xss_reflected: 'CWE-79', xss_stored: 'CWE-79', xss_dom: 'CWE-79',
    sqli_classic: 'CWE-89', sqli_blind: 'CWE-89', sqli_time_based: 'CWE-89',
    ssrf: 'CWE-918', ssrf_blind: 'CWE-918',
    idor: 'CWE-639', bola: 'CWE-639',
    auth_bypass: 'CWE-287', broken_auth: 'CWE-287',
    rce: 'CWE-94', command_injection: 'CWE-78', code_injection: 'CWE-94',
    path_traversal: 'CWE-22', lfi: 'CWE-98', rfi: 'CWE-98',
    open_redirect: 'CWE-601', csrf: 'CWE-352',
    info_disclosure: 'CWE-200', sensitive_data_exposure: 'CWE-200',
    xxe: 'CWE-611', ssti: 'CWE-1336', deserialization: 'CWE-502',
    subdomain_takeover: 'CWE-284', cors_misconfiguration: 'CWE-942',
    jwt_weakness: 'CWE-347', privilege_escalation: 'CWE-269',
  };
