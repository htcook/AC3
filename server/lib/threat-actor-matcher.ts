/**
 * Threat Actor Matcher
 * 
 * Analyzes Domain Intel scan results (discovered assets, tech stack, sector, 
 * hosting type, risk profile) and cross-references against the 401+ threat actors
 * in the database to surface the most likely adversaries targeting the organization.
 * 
 * Matching factors:
 * 1. Sector alignment (threat actor targetSectors vs org sector)
 * 2. Region alignment (threat actor targetRegions vs org region)
 * 3. Technology stack overlap (threat actor tools/techniques vs discovered tech)
 * 4. Client type relevance (MSP, SaaS, IaaS, etc.)
 * 5. Threat level and sophistication weighting
 * 6. Active recency (recently active actors score higher)
 * 7. LLM-powered contextual analysis for nuanced matching
 */
...
export interface MatchedThreatActor {
  ...
  matchReasons: string[];
  relevantTechniques: Array<{ id: string; name: string; tactic: string }>;
  recommendedActions: string[];
  confidence: number; // 0-100
}

export interface ThreatActorMatchResult {
  topMatches: MatchedThreatActor[];
  totalCandidates: number;
  matchSummary: string;
  sectorThreats: string[];
  techStackThreats: string[];
}

// Sector normalization mapping for fuzzy matching
...
// Tech stack keywords that map to specific threat actor tools/techniques
const TECH_THREAT_MAP: Record<string, string[]> = {
  "microsoft": ["T1059.001", "T1053.005", "T1021.001", "T1003.001"], // PowerShell, Scheduled Tasks, RDP, LSASS
  "active directory": ["T1003.001", "T1558.003", "T1021.002", "T1087.002"], // Kerberoasting, SMB, AD enum
  "exchange": ["T1190", "T1505.003", "T1114.002"], // ProxyLogon/ProxyShell, Web Shell, Email Collection
  "office365": ["T1078.004", "T1114.003", "T1528"], // Cloud accounts, Email forwarding, Steal tokens
  "azure": ["T1078.004", "T1580", "T1538"], // Cloud accounts, Cloud infra discovery
  "aws": ["T1078.004", "T1580", "T1530"], // Cloud accounts, S3 access
  "linux": ["T1059.004", "T1053.003", "T1021.004"], // Bash, Cron, SSH
  "docker": ["T1610", "T1613", "T1611"], // Container deploy, Container discovery, Escape
  "kubernetes": ["T1610", "T1613", "T1611"],
  "wordpress": ["T1190", "T1505.003"], // Exploit public app, Web shell
  "apache": ["T1190", "T1505.003"],
  "nginx": ["T1190"],
  "iis": ["T1190", "T1505.003"],
  "vpn": ["T1133", "T1078"], // External Remote Services, Valid Accounts
  "citrix": ["T1133", "T1190"],
  "vmware": ["T1190", "T1195.002"], // Exploit, Supply chain
  "fortinet": ["T1190", "T1133"],
  "palo alto": ["T1190", "T1133"],
  "cisco": ["T1190", "T1133", "T1557"],
  "sql": ["T1190", "T1505.001"], // SQL injection, Stored procedures
  "rdp": ["T1021.001", "T1563.002"], // Remote Desktop, RDP Hijacking
  "smb": ["T1021.002", "T1570"], // SMB/Windows Admin Shares, Lateral Tool Transfer
};

// Client type to threat actor type relevance
const CLIENT_TYPE_WEIGHTS: Record<string, Record<string, number>> = {
...
function sectorMatch(actorSectors: string[], orgSector: string): number {
  if (!actorSectors || actorSectors.length === 0) return 0.1; // Unknown targeting = low baseline
  const normalizedOrg = normalizeSector(orgSector);
  
  for (const s of actorSectors) {
    const normalizedActor = normalizeSector(s);
    if (normalizedActor === normalizedOrg) return 1.0;
...
function techOverlap(actorTechniques: Array<{ id: string }>, discoveredTech: string[]): { score: number; matchedTechniques: string[] } {
  ...
  const score = Math.min(1.0, matchCount / 3); // 3+ matches = full score
  return { score, matchedTechniques: Array.from(relevantTechIds) };
}

function recencyScore(lastActive: string | null): number {
  if (!lastActive) return 0.3; // Unknown = moderate baseline
  const year = parseInt(lastActive);
  if (isNaN(year)) return 0.3;
  const currentYear = new Date().getFullYear();
  const age = currentYear - year;
  if (age <= 0) return 1.0; // Active this year
  if (age <= 1) return 0.9;
  if (age <= 2) return 0.7;
  if (age <= 3) return 0.5;
  if (age <= 5) return 0.3;
...
function threatLevelWeight(level: string | null): number {
  switch (level) {
...
/** 
 * Score all threat actors against the org profile and scan results.
 * Returns scored and ranked matches.
 */
export async function matchThreatActors(params: {
  sector: string;
  clientType: string;
  discoveredTechnologies: string[];
  discoveredAssets: Array<{ hostname: string; assetType: string; technologies?: string[] }>;
  riskScore: number;
  region?: string;
  criticalFunctions?: string[];
}): Promise<ThreatActorMatchResult> {
  // Fetch all threat actors from DB
  const { actors } = await db.listThreatActors({ limit: 500 });
  
  // Collect all discovered technologies
  const allTech = new Set<string>(params.discoveredTechnologies);
  ...
  // Score each actor
  const scored: Array<MatchedThreatActor & { rawScore: number }> = [];
  ...
  for (const actor of actors) {
    ...
    // 3. Client type relevance (weight: 15)
    const typeWeights = CLIENT_TYPE_WEIGHTS[params.clientType] || {};
    const typeMultiplier = typeWeights[actor.type] || 1.0;
    const typeScore = Math.min(1.0, (typeMultiplier - 0.7) / 0.8); // Normalize 0.7-1.5 to 0-1
    ...
    // 5. Threat level (weight: 10)
    const threatWeight = threatLevelWeight(actor.threatLevel);
    const threatScore = Math.min(1.0, (threatWeight - 0.7) / 0.8);
    ...
    // Only include actors with meaningful match scores
    if (finalScore >= 15) {
      const relevantTechniques = techniques
        .filter(t => matchedTechniques.includes(t.id))
        .slice(0, 10);
  ...
  // Generate summary
  const sectorThreats = topMatches
    .filter(m => m.matchReasons.some(r => r.includes("sector")))
    .map(m => m.name);
  const techStackThreats = topMatches
    .filter(m => m.matchReasons.some(r => r.includes("tech stack")))
    .map(m => m.name);
...
function generateRecommendedActions(actor: any, params: any): string[] {
...
function generateMatchSummary(matches: MatchedThreatActor[], params: any): string {
...
/**
 * Enhanced matching with LLM contextual analysis.
 * Uses the LLM to provide nuanced threat actor recommendations
 * based on the full Domain Intel scan results.
 */
export async function matchThreatActorsWithLLM(params: {
  orgProfile: { customerName: string; sector: string; clientType: string; criticalFunctions: string[] };
  discoveredAssets: Array<{ hostname: string; assetType: string; technologies?: string[]; riskBand?: string }>;
  overallRiskScore: number;
  executiveSummary: string;
  campaignRecommendations: any[];
  topDatabaseMatches: MatchedThreatActor[];
}): Promise<{
  ...
  const topActorSummary = params.topDatabaseMatches.slice(0, 15).map(m => 
    `${m.name} (${m.type}, ${m.origin || 'unknown origin'}, score: ${m.matchScore}, reasons: ${m.matchReasons.join('; ')})`
  ).join("\n");
  
  const assetSummary = params.discoveredAssets.slice(0, 20).map(a =>
  ...
  try {
    ...
    return {
      enhancedMatches: params.topDatabaseMatches.slice(0, 10).map(m => ({
        ...
        priorityLevel: m.matchScore >= 70 ? "critical" as const : m.matchScore >= 50 ? "high" as const : "medium" as const,
      })),
