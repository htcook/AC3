 ...
 * 
 * Auto-generates Sigma, YARA, and Suricata detection rules from
 * threat actor TTPs, techniques, and behavioral indicators.
 */
...
export interface RuleGenerationResult {
  ...
  rulesByTactic: Record<string, number>;
  rules: GeneratedRule[];
  generatedAt: string;
}
...
interface RuleTemplate {
  sigma?: (tech: TechniqueInput) => string;
  yara?: (tech: TechniqueInput) => string;
  suricata?: (tech: TechniqueInput) => string;
}
...
function generateSid(): number {
  return 2000000 + Math.floor(Math.random() * 999999);
}
...
function getSeverityForTactic(tactic: string): "low" | "medium" | "high" | "critical" {
  const m: Record<string, "low" | "medium" | "high" | "critical"> = {
    "reconnaissance": "low", "resource-development": "low",
    "initial-access": "high", "execution": "high",
    "persistence": "medium", "privilege-escalation": "high",
    "defense-evasion": "high", "credential-access": "critical",
    "discovery": "medium", "lateral-movement": "high",
    "collection": "medium", "command-and-control": "high",
    "exfiltration": "critical", "impact": "critical",
  };
  return m[tactic] || "medium";
}

function getTacticFromId(techniqueId: string): string {
  const m: Record<string, string> = {
    "T1595": "reconnaissance", "T1592": "reconnaissance",
    "T1566": "initial-access", "T1190": "initial-access",
    "T1059": "execution", "T1053": "execution",
    "T1078": "persistence", "T1547": "persistence",
    ...
    "T1082": "discovery", "T1083": "discovery", "T1057": "discovery", "T1087": "discovery",
...
function generateGenericSigmaRule(tech: TechniqueInput): string {
...
export function generateRulesForActor(params: {
  actorName: string;
  ...
  for (const tech of techniques) {
    const techInput: TechniqueInput = { id: tech.id, name: tech.name, tactic: tech.tactic, actorName, tools, malware };
    ...
    if (templates) {
      if (templates.sigma) {
        ruleCounter++;
        rules.push({
          id: `rule-${ruleCounter}`, ruleType: "sigma",
          ruleName: `${actorName} - ${tech.name} (Sigma)`,
          techniqueId: tech.id, techniqueName: tech.name, tactic,
          ruleContent: templates.sigma(techInput),
          description: `Sigma detection rule for ${tech.name} as used by ${actorName}`,
          severity, confidence: 75, dataSource: "Windows Event Logs / Sysmon", platform: "Windows",
        });
      }
      if (templates.yara) {
        ruleCounter++;
        rules.push({
          id: `rule-${ruleCounter}`, ruleType: "yara",
          ...
          severity, confidence: 70, dataSource: "File System / Memory", platform: "Cross-platform",
        });
      }
      if (templates.suricata) {
        ruleCounter++;
        rules.push({
          id: `rule-${ruleCounter}`, ruleType: "suricata",
          ...
          severity, confidence: 65, dataSource: "Network Traffic", platform: "Network",
        });
      }
    } else {
      ruleCounter++;
      rules.push({
        ...
        severity, confidence: 40, dataSource: "Windows Event Logs", platform: "Windows",
      });
    }
  }

  const rulesByType: Record<string, number> = {};
  const rulesByTactic: Record<string, number> = {};
  for (const rule of rules) {
    rulesByType[rule.ruleType] = (rulesByType[rule.ruleType] || 0) + 1;
    rulesByTactic[rule.tactic] = (rulesByTactic[rule.tactic] || 0) + 1;
  }

  return { actorName, totalRules: rules.length, rulesByType, rulesByTactic, rules, generatedAt: new Date().toISOString() };
}

export async function generateRulesWithLLM(params: {
  actorName: string;
  techniques: Array<{ id: string; name: string; tactic: string }>;
  tools?: string[];
  malware?: string[];
...
Return JSON array of objects with: { techniqueId, sigmaRule }`;
    ...
    const response = await invokeLLM({ _caller: "rule-generator",
