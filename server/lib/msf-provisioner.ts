/**
 * Metasploit Droplet Provisioner
 *
 * Automates the lifecycle of Metasploit Framework instances on DigitalOcean:
 * 1. Provision a droplet with Docker + MSF container
...
export interface ProvisionRequest {
  name: string;
  region?: string;       // default: nyc1
  size?: string;         // default: s-2vcpu-4gb (4GB RAM minimum for MSF)
  rpcPort?: number;      // default: 55553
  rpcUser?: string;      // default: msf
  autoDestroy?: boolean; // destroy after engagement ends
  engagementId?: number;
}
...
function generateCloudInit(rpcPort: number, rpcUser: string, rpcPass: string): string {
  return `#!/bin/bash
set -euo pipefail
...
/**
 * Provision a new Metasploit Framework droplet on DigitalOcean.
 * Returns connection details once the droplet is created (IP may take a moment to assign).
 */
export async function provisionMsfDroplet(req: ProvisionRequest): Promise<ProvisionResult> {
  const region = req.region || "nyc1";
  const size = req.size || "s-2vcpu-4gb";
  const rpcPort = req.rpcPort || 55553;
  const rpcUser = req.rpcUser || "msf";
  const rpcPass = crypto.randomBytes(24).toString("base64url"); // Strong random password
...
/**
 * Get the IP address of a droplet (may need polling after creation).
 */
export async function getDropletIp(dropletId: string): Promise<string | null> {
  try {
...
/**
 * Get full droplet status.
 */
export async function getDropletStatus(dropletId: string): Promise<DropletStatus | null> {
  try {
    const data = await doFetch(`/droplets/${dropletId}`);
...
/**
 * Destroy a Metasploit droplet.
 */
export async function destroyMsfDroplet(dropletId: string): Promise<{ success: boolean; error?: string }> {
  try {
...
/**
 * Power cycle a droplet (reboot).
 */
export async function rebootDroplet(dropletId: string): Promise<{ success: boolean; error?: string }> {
  try {
...
/**
 * Get SSH keys on the account to inject into new droplets.
 */
async function getAccountSshKeys(): Promise<number[]> {
  try {
    const data = await doFetch("/account/keys?per_page=50");
    return (data.ssh_keys || []).map((k: any) => k.id);
  } catch {
    return [];
  }
}
...
/**
 * List all MSF-tagged droplets on the account.
 */
export async function listMsfDroplets(): Promise<DropletStatus[]> {
  try {
