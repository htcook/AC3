...
level: ${request.severity}`;
  ...
  condition:
    ...
    case "snort":
    case "suricata":
      return `alert tcp any any -> any ${request.targetPort || "any"} (msg:"AC3 - ${request.exploitModule} (${cveStr})"; flow:to_server,established; content:"${request.targetService}"; nocase; classtype:attempted-admin; sid:${1000000 + ruleCounter}; rev:1; metadata:created_at ${now.replace(/-/g, "_")};)`;
