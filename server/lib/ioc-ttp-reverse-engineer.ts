/**
 * IOC-to-TTP Reverse Engineering Module
 *
 * Decomposes Indicators of Compromise (IOCs) into the MITRE ATT&CK techniques
 * they imply. Every IOC is forensic evidence of how an actor operates:
 *   - A C2 domain → DNS tunneling (T1071.004) or DGA (T1568.002)
 *   - A malware hash → specific execution technique (T1059.x)
 *   - A registry key → persistence mechanism (T1547.001)
 *   - A file path → staging location (T1074.001)
 *   - A mutex name → process injection marker (T1055)
 *
 * Two derivation methods:
 *   1. Pattern matching — deterministic rules for well-known IOC patterns
 *   2. LLM analysis — for ambiguous IOCs that require contextual reasoning
 *
 * Results are stored in the ioc_ttp_mappings table and feed back into:
 *   - Exploit playbook selection (actor uses this technique → try this exploit)
 *   - Caldera ability generation (IOC implies this command → create ability)
 *   - Ember red team planning (actor's IOCs reveal their methodology)
 */
...
export interface IocInput {
  type: string;       // domain, ip, hash_md5, hash_sha256, url, email, file_path, registry_key, mutex, user_agent, certificate_hash, etc.
  value: string;
  description?: string;
  actorId?: string;
  actorName?: string;
...
export interface TtpMapping {
  ...
  reasoning: string;
  confidence: number;
  derivationMethod: "pattern_match" | "llm_analysis" | "malware_analysis" | "behavioral_analysis" | "infrastructure_analysis" | "manual";
  context?: Record<string, any>;
}

export interface IocTtpResult {
  ioc: IocInput;
  mappings: TtpMapping[];
  enriched: boolean;
}
...
const PATTERN_RULES: PatternRule[] = [
  ...
  },
  {
    iocType: "ip",
    valueMatcher: (v) => {
      // Known VPS/hosting ranges often used for C2
...
function matchPatterns(ioc: IocInput): TtpMapping[] {
  const mappings: TtpMapping[] = [];

  for (const rule of PATTERN_RULES) {
    // Check IOC type match
    const typeMatch = Array.isArray(rule.iocType)
      ? rule.iocType.includes(ioc.type)
      : rule.iocType === ioc.type;

    if (!typeMatch) continue;

    // Check value pattern/matcher
...
async function analyzeWithLLM(ioc: IocInput, existingMappings: TtpMapping[]): Promise<TtpMapping[]> {
...
async function persistMappings(ioc: IocInput, mappings: TtpMapping[]): Promise<void> {
...
/**
 * Reverse-engineer a single IOC into its implied TTPs.
 * Uses pattern matching first, then LLM for deeper analysis.
 */
export async function reverseEngineerIoc(ioc: IocInput, options?: {
  skipLLM?: boolean;
  persist?: boolean;
}): Promise<IocTtpResult> {
  ...
  // Step 2: LLM analysis (deeper, contextual)
  let llmMappings: TtpMapping[] = [];
  if (!skipLLM) {
    llmMappings = await analyzeWithLLM(ioc, patternMappings);
  }

  // Step 3: Deduplicate by technique ID, keeping highest confidence
  const allMappings = [...patternMappings, ...llmMappings];
  const deduped = new Map<string, TtpMapping>();
...
/**
 * Batch reverse-engineer multiple IOCs.
 * Processes pattern matching for all, then batches LLM calls.
 */
export async function batchReverseEngineerIocs(
  iocs: IocInput[],
  options?: { skipLLM?: boolean; persist?: boolean; concurrency?: number }
): Promise<IocTtpResult[]> {
  const { skipLLM = false, persist = true, concurrency = 3 } = options || {};
  const results: IocTtpResult[] = [];

  // Process in batches to respect LLM rate limits
...
/**
 * Reverse-engineer all IOCs for a specific threat actor from the database.
 * Pulls IOCs from threat_actor_iocs table and processes them.
 */
export async function reverseEngineerActorIocs(actorId: string, options?: {
  skipLLM?: boolean;
  limit?: number;
}): Promise<{ actorId: string; totalIocs: number; totalMappings: number; results: IocTtpResult[] }> {
  const db = await getDb();
  ...
  `);
  const actorName = (actorRows[0] as any[])?.[0]?.name || actorId;

  const iocs: IocInput[] = rows.map((r: any) => ({
...
/**
 * Query TTP mappings for a specific technique across all actors.
 * Useful for finding which actors use a specific technique.
 */
export async function getActorsByTechnique(techniqueId: string): Promise<Array<{
  actorId: string;
  actorName: string;
  confidence: number;
  reasoning: string;
...
/**
 * Build a TTP context block for the exploit pipeline.
 * Returns IOC-derived technique intelligence for a specific actor.
 */
export async function buildIocDerivedTtpContext(actorId: string): Promise<string> {
  const db = await getDb();
  if (!db) return "";
...
/**
 * Get summary statistics for IOC-to-TTP mappings.
 */
export async function getIocTtpStats(): Promise<{
  totalMappings: number;
  byDerivationMethod: Record<string, number>;
  byTactic: Record<string, number>;
  topTechniques: Array<{ id: string; name: string; count: number }>;
  actorsWithMappings: number;
}> {
  const db = await getDb();
  ...
  for (const r of (methodRows as any[]) || []) {
  ...
  for (const r of (tacticRows as any[]) || []) {
  ...
  `);
  const topTechniques = ((techRows as any[]) || []).map((r: any) => ({
  ...
  `);
  const actorsWithMappings = (actorRows as any[])?.[0]?.cnt || 0;
