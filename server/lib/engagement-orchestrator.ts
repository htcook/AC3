...
Return valid JSON per the response_format schema.`;
  ...
  try {
    ...
    const parseResult = parseLLMJson(content, { label: 'ScanPlan' });
    if (!parseResult.ok) throw new Error(parseResult.error);
    parsed = parseResult.data;
  } catch (parseErr: any) {
    addLog(state, { phase: state.phase, type: 'error', title: 'Scan Plan Parse Error', detail: `LLM returned invalid JSON for scan plan: ${parseErr.message}` });
...
Rules: pentest=test each asset systematically; red_team=find weakest entry,exploit,C2,pivot; WAF-aware scanning; correlate findings across tools; flag high-risk actions; stay in scope.${exploitPhaseInstructions}${bankingOpsCtx}`;
  ...
  try {
    ...
    const result = parseLLMJson<{ decision: string; reasoning: string; actions: Array<{ type: string; params: Record<string, any> }> }>(content, { label: 'OpsDecision' });
    if (!result.ok) throw new Error(`JSON parse failed: ${result.error}`);
    return result.data;
  } catch (e: any) {
