/**
 * Shared knowledge data loader — fetches JSON data from the ScanForge scan server's
 ...
 * Pattern: try local JSON file first (bundled in Docker image), then fetch from ScanForge API.
 *
 * Cache behaviour:
...
/** Internal: fetch from ScanForge API */
async function fetchFromScanForge<T>(filename: string): Promise<T | null> {
  try {
    ...
    console.warn(`[KnowledgeLoader] ScanForge fetch returned ${res.status} for ${filename}`);
  } catch (e: any) {
    console.warn(`[KnowledgeLoader] ScanForge fetch error for ${filename}:`, e.message);
  }
  return null;
}

/** Internal: load from local file or ScanForge API */
async function loadFresh<T>(filename: string): Promise<T | null> {
  // Try multiple local paths (dev vs production bundled layout)
  const searchPaths = [
    join(__esm_dirname, filename),                                    // Dev: same dir as source
    join(process.cwd(), "server", "lib", "knowledge", filename),     // Docker: relative to CWD
    join(__esm_dirname, "..", "..", "server", "lib", "knowledge", filename), // Bundled: dist/server/ → ../../server/lib/knowledge/
  ];
  ...
  for (const localPath of searchPaths) {
    if (existsSync(localPath)) {
      try {
        ...
        console.log(`[KnowledgeLoader] Loaded ${filename} from local file (${localPath})`);
        return data;
      } catch (e: any) {
  ...
  // Fetch from ScanForge scan service API
  const data = await fetchFromScanForge<T>(filename);
  if (data !== null) {
    console.log(`[KnowledgeLoader] Loaded ${filename} from ScanForge scan service`);
  }
  return data;
}
...
/**
 * Load a JSON knowledge file. Tries local file first, then ScanForge API.
