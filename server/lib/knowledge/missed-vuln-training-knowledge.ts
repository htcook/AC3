/**
 * Missed Vulnerability Training Knowledge
 * ────────────────────────────────────────
 * Comprehensive training data for vulnerability classes that the engagement
 * LLM consistently misses. Derived from accuracy analysis of Full Red Team
 * engagement 1770049 against OWASP Juice Shop and other lab apps.
 *
 * This module provides:
 * 1. Detailed detection signatures for each missed vuln class
 * 2. Specific exploitation payloads and techniques
 * 3. Tool-specific commands for automated detection
 * 4. Business logic testing patterns
 * 5. Context injection for the scan feedback loop and attack planner
 *
 * Integration points:
 * - llm-scan-feedback.ts: Injected into the analysis prompt so the LLM
 *   knows to request scans for these vuln classes
 * - engagement-orchestrator.ts: Injected into the scan plan enrichment
 * - llm-specialists/vuln-verifier.ts: Used for false positive triage
 */

// ─── Types ──────────────────────────────────────────────────────────────────

export interface MissedVulnPattern {
  id: string;
  name: string;
  category: MissedVulnCategory;
  cwe: string[];
  owasp: string[];
  severity: "critical" | "high" | "medium" | "low" | "info";
  /** Why automated scanners typically miss this */
  whyMissed: string;
  /** Indicators that this vuln class may be present */
  detectionSignals: string[];
  /** Specific HTTP requests/payloads to test */
  testPayloads: TestPayload[];
  /** Tools that can detect this and their specific arguments */
  toolCommands: ToolCommand[];
  /** Step-by-step exploitation guide */
  exploitationSteps: string[];
  /** Which lab apps contain this vuln */
...
export type MissedVulnCategory =
  | "business_logic"
  | "api_abuse"
  | "crypto_weakness"
  | "deserialization"
...
export const MISSED_VULN_PATTERNS: MissedVulnPattern[] = [
  // ═══ 1. NoSQL DoS ═══════════════════════════════════════════════════════════
  {
    id: "nosql-dos",
...
/**
 * Build a comprehensive context string for the LLM scan feedback loop.
 * Injects missed vuln detection patterns so the LLM knows what to look for.
 */
export function buildMissedVulnContext(opts?: {
  targetPreset?: string;
  categories?: MissedVulnCategory[];
  maxPatterns?: number;
}): string {
  ...
  const sections = patterns.map(p => {
    const payloadExamples = p.testPayloads.slice(0, 2).map(tp =>
      `  ${tp.method} ${tp.endpoint}: ${tp.description}`
    ).join('\n');
    const toolExamples = p.toolCommands.slice(0, 1).map(tc =>
...
/**
 * Build a compact context for the attack planner.
 */
export function buildMissedVulnAttackContext(targetPreset?: string): string {
  let patterns = MISSED_VULN_PATTERNS;
  if (targetPreset) {
  ...
  return patterns.map(p =>
    `- **${p.name}** (${p.severity}): ${p.exploitationSteps[0]} | ATT&CK: ${p.attackTechniques.join(', ')}`
  ).join('\n');
}

/**
 * Get specific test payloads for a target.
 */
export function getMissedVulnPayloads(targetPreset: string): TestPayload[] {
  return MISSED_VULN_PATTERNS
    .filter(p => p.applicableLabs.includes(targetPreset))
    .flatMap(p => p.testPayloads);
}

/**
 * Get tool commands for detecting missed vulns on a specific target.
 */
export function getMissedVulnToolCommands(targetPreset: string): ToolCommand[] {
  return MISSED_VULN_PATTERNS
    .filter(p => p.applicableLabs.includes(targetPreset))
    .flatMap(p => p.toolCommands);
}

/**
 * Get all missed vuln patterns for a specific category.
 */
export function getMissedVulnsByCategory(category: MissedVulnCategory): MissedVulnPattern[] {
  return MISSED_VULN_PATTERNS.filter(p => p.category === category);
}

/**
 * Get a summary of all missed vuln patterns for display.
 */
export function getMissedVulnSummary(): Array<{
  id: string;
  name: string;
  category: string;
  severity: string;
