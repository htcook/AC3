/**
 * Darkweb Data Feed Connectors — Access Brokers & Information Operations
 *
 * Sources:
 *   IABs:  Curated OSINT catalog of known Initial Access Brokers,
 *          cross-referenced with ransomware.live victim data.
 *   IO:    Curated catalog of known Information Operations campaigns
 *          from DFRLab FIAT, EUvsDisinfo, Stanford IO Archive, OSINT.
 */
import { getDb } from "../db";
import { accessBrokerListings, infoOpsCampaigns, threatActors } from "../../drizzle/schema";
...
// ─── Known Initial Access Brokers (OSINT-curated) ──────────────────────

interface IABSeedEntry {
  brokerId: string;
  brokerName: string;
  aliases: string[];
  description: string;
  listingType: "vpn_access" | "rdp_access" | "citrix_access" | "webshell" | "domain_admin" | "cloud_access" | "email_access" | "database_access" | "zero_day" | "exploit_kit" | "credential_dump" | "other";
  accessType: string;
  forumSource: string;
  activeForums: string[];
  brokerReputation: "established" | "rising" | "new" | "unknown";
  linkedRansomwareGroups: string[];
  mitreTechniques: string[];
  accessLevel: "domain_admin" | "local_admin" | "user" | "service_account" | "unknown";
  status: "active" | "sold" | "expired" | "removed" | "law_enforcement";
  firstSeen: string;
  lastActive: string;
  victimSector?: string;
...
// ─── Known Information Operations Campaigns (OSINT-curated) ────────────

interface IOSeedEntry {
  campaignId: string;
  campaignName: string;
  aliases: string[];
  attributedTo: string;
  ...
  operationType: "disinformation" | "influence" | "hack_and_leak" | "astroturfing" | "election_interference" | "propaganda" | "cyber_espionage_io" | "economic_coercion" | "diplomatic_pressure" | "other";
  status: "active" | "disrupted" | "dormant" | "attributed" | "ongoing";
...
const KNOWN_IO_CAMPAIGNS: IOSeedEntry[] = [
  {
...
/**
 * Seed / update the access broker listings catalog.
 * Uses upsert logic — inserts new entries, updates existing ones.
 */
export async function syncAccessBrokers(): Promise<{ inserted: number; updated: number; total: number }> {
  ...
  // Also ensure threat_actors has entries with type "access_broker" for each IAB
  for (const iab of KNOWN_IABS) {
...
/**
 * Seed / update the information operations campaigns catalog.
 * Uses upsert logic — inserts new entries, updates existing ones.
 */
export async function syncInfoOpsCampaigns(): Promise<{ inserted: number; updated: number; total: number }> {
  const db = await requireDb();
  let inserted = 0;
  let updated = 0;
  ...
  // Also ensure threat_actors has entries with type "influence_ops" for each IO operator group
  for (const io of KNOWN_IO_CAMPAIGNS) {
    ...
    if (existingActor.length === 0) {
      await db.insert(threatActors).values({
        ...
        lastActive: io.endDate || new Date().toISOString().slice(0, 7),
        ...
        techniques: io.linkedCyberOps.length > 0
          ? io.linkedCyberOps.map((op) => ({ id: op, name: op, tactic: "influence" }))
          : [],
        tools: io.techniques,
        dataSource: "osint_curated",
