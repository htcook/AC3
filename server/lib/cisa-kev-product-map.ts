/**
 * CISA KEV CVE-to-Product Mapping Engine
 * 
 * Provides comprehensive CVE-to-technology-family mappings sourced from
 * the CISA Known Exploited Vulnerabilities (KEV) catalog.
 * 
 * Architecture:
 * - Static TECH_FAMILY_KEYWORDS: maps technology families to detection keywords
 * - Static VENDOR_TO_FAMILIES: maps vendor+product strings to tech families
 * - Dynamic KEV loader: fetches CISA KEV JSON and builds CVE→family index
 * - Fallback: hardcoded map for the most critical/common CVEs when offline
 * 
 * The guardrails system calls lookupCVEProduct(cveId) to get the required
 * technology family and keywords, then validates against the target's
 * confirmed tech stack.
 */

// ─── Technology Family Definitions ───────────────────────────────────────────
// Each family defines the keywords that MUST be present in the target's
// confirmed technology stack for an exploit targeting that family to proceed.

export const TECH_FAMILY_KEYWORDS: Record<string, string[]> = {
  // Microsoft
...
// ─── Vendor+Product → Family Mapping ─────────────────────────────────────────
// Used by the dynamic KEV loader to classify each CVE entry

interface VendorProductRule {
  vendor: string;
  products: string[];
  family: string;
}

const VENDOR_PRODUCT_RULES: VendorProductRule[] = [
  // Microsoft
...
// In-memory index: CVE → product info
let kevIndex: Map<string, CVEProductInfo> = new Map();
let kevLoaded = false;
let kevLastRefresh = 0;
let lastNewKevCves: string[] = [];
const KEV_REFRESH_INTERVAL = 24 * 60 * 60 * 1000; // 24 hours
const KEV_URL = 'https://www.cisa.gov/sites/default/files/feeds/known_exploited_vulnerabilities.json';

/**
 * Auto-exploit generation trigger for newly detected KEV entries.
 * Runs asynchronously after KEV refresh detects new CVEs.
 * Rate-limited to 5 per refresh cycle to control LLM costs.
 */
async function triggerAutoExploitForNewKEVs(cveIds: string[]): Promise<void> {
  const MAX_PER_CYCLE = 5;
  const toProcess = cveIds.slice(0, MAX_PER_CYCLE);
  console.log(`[KEV→AutoExploit] Generating exploits for ${toProcess.length} new KEV CVEs...`);

  try {
    const { generateExploitFromCVE } = await import('./cve-exploit-gen-engine');
    let generated = 0;
    let failed = 0;

    for (const cveId of toProcess) {
      try {
        const result = await generateExploitFromCVE({
          cveId,
          deployToCaldera: true,
          priority: 'critical', // KEV = actively exploited = critical priority
        });
        if (result.success) {
          generated++;
          console.log(`[KEV→AutoExploit] ✓ ${cveId}: Nuclei template + Caldera ability deployed`);
        } else {
          failed++;
          console.warn(`[KEV→AutoExploit] ✗ ${cveId}: Generation failed`);
        }
      } catch (err: any) {
        failed++;
        console.warn(`[KEV→AutoExploit] ✗ ${cveId}: ${err.message}`);
      }
    }

    console.log(`[KEV→AutoExploit] Complete: ${generated} generated, ${failed} failed out of ${toProcess.length}`);
  } catch (err: any) {
    console.error(`[KEV→AutoExploit] Engine import failed: ${err.message}`);
  }
}

/**
 * Get the list of new KEV CVEs detected in the last refresh cycle.
 * Used by the daily scheduler to include in notifications.
 */
export function getNewKevCves(): string[] {
  return [...lastNewKevCves];
}

/**
 * Classify a KEV entry into a technology family using vendor+product rules
 */
function classifyKEVEntry(entry: KEVEntry): string | null {
  ...
  for (const rule of VENDOR_PRODUCT_RULES) {
    ...
    // If no product filter, vendor match is sufficient
    if (rule.products.length === 0) return rule.family;

    // Check product match
    const productMatch = rule.products.some(
      p => product.toLowerCase().includes(p.toLowerCase()) ||
           vulnName.toLowerCase().includes(p.toLowerCase())
    );
...
/**
 * Load and index the CISA KEV catalog
 * Returns the number of CVEs indexed
 */
export async function loadKEVCatalog(): Promise<number> {
  try {
    ...
    const newIndex = new Map<string, CVEProductInfo>();
    let classified = 0;
    ...
    // Delta detection: identify new KEV entries not in previous index
    const newKevCves: string[] = [];
    for (const cveId of newIndex.keys()) {
      if (!kevIndex.has(cveId)) {
        newKevCves.push(cveId);
      }
    }
    if (newKevCves.length > 0) {
      console.log(`[KEV] ⚠️ ${newKevCves.length} NEW KEV entries detected: ${newKevCves.slice(0, 5).join(', ')}${newKevCves.length > 5 ? '...' : ''}`);
      // Store for downstream auto-exploit generation
      lastNewKevCves = newKevCves;
      // Fire auto-exploit generation asynchronously (non-blocking)
      triggerAutoExploitForNewKEVs(newKevCves).catch(err => {
        console.warn(`[KEV] Auto-exploit gen failed (non-fatal): ${err.message}`);
      });
    } else {
      lastNewKevCves = [];
    }
...
/**
 * Ensure the KEV index is loaded, refresh if stale
 */
export async function ensureKEVLoaded(): Promise<void> {
...
// ─── Static Fallback Map ─────────────────────────────────────────────────────
// Used when the KEV catalog is unavailable (offline, timeout, etc.)
// Contains the most critical/commonly exploited CVEs

const STATIC_CVE_PRODUCT_MAP: Record<string, { family: string; keywords: string[] }> = {
  // Rejetto HFS
...
export interface CVEProductLookupResult {
  family: string;
  keywords: string[];
  vendor?: string;
  product?: string;
  ransomwareLinked?: boolean;
  source: 'kev_live' | 'static_fallback' | 'not_found';
}

/**
 * Look up the technology family and required keywords for a CVE.
 * Checks the live KEV index first, falls back to the static map.
 */
export function lookupCVEProduct(cveId: string): CVEProductLookupResult {
  // Try live KEV index first
...
/**
 * Check if a CVE's required technology family matches the target's confirmed tech stack.
 * Returns null if no mismatch detected, or a violation description if mismatched.
 */
export function validateCVEAgainstTarget(
  cveId: string,
  confirmedTechnologies: string[],
  serviceVersions: string[] = []
): { violation: string; family: string; keywords: string[] } | null {
  const lookup = lookupCVEProduct(cveId);
  if (lookup.source === 'not_found') return null; // Unknown CVE, can't validate
  ...
  // Check if ANY of the required keywords appear in the target's evidence
  const hasMatch = lookup.keywords.some(kw =>
    allEvidence.some(ev => ev.includes(kw.toLowerCase()))
  );

  if (hasMatch) return null; // Technology matches, no violation
...
/**
 * Get statistics about the current KEV index
 */
export function getKEVStats(): {
  loaded: boolean;
  totalCVEs: number;
  families: number;
  lastRefresh: number;
...
/**
 * Get all ransomware-linked CVEs from the KEV index
 */
export function getRansomwareLinkedCVEs(): string[] {
  return Array.from(kevIndex.entries())
    .filter(([_, info]) => info.ransomwareLinked)
    .map(([cve]) => cve);
}
