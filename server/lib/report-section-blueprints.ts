...
const APPENDIX: SectionBlueprint = {
  ...
  title: 'Appendix',
  required: false,
  promptGuidance: 'Supporting artifacts, evidence catalog, tool execution logs, and supplementary data. Include raw Nuclei verification output, web crawl security header analysis, and active scan tool execution logs.',
  dataSources: ['artifacts', 'tool_results', 'audit_log', 'nuclei_findings', 'web_crawl_results', 'active_scan_results'],
  subsections: [
...
const PENTEST_BLUEPRINT: ReportBlueprint = {
  ...
  sections: [
    ...
    },
    {
      ...
      dataSources: ['assets', 'recon_data', 'nuclei_findings', 'web_crawl_results'],
    },
    FINDINGS_SUMMARY,
    {
      ...
      required: true,
      promptGuidance: 'For each finding: title, CVSS v3.1 vector with per-metric justification, description, business impact, technical details with reproduction steps, evidence artifacts (labeled E-1 through E-N), and specific remediation. Reference supporting artifacts by label. Include raw tool output (Nuclei template matches, curl commands, response headers) as evidence blocks. Each finding MUST include: (1) scan timestamp (UTC and local) so the client can correlate against IDS/WAF/SIEM/SOC feeds, (2) source tool and version, (3) corroboration tier (confirmed/corroborated/unverified), (4) raw evidence in code blocks. For false positives: include the original alert, validation method, and rationale for dismissal.',
      dataSources: ['findings', 'evidence', 'artifacts', 'nuclei_findings', 'active_scan_results', 'scan_timestamps'],
    },
    {
...
const RED_TEAM_BLUEPRINT: ReportBlueprint = {
  ...
  sections: [
    ...
    FINDINGS_SUMMARY,
    {
      ...
      title: 'Detailed Findings',
      required: true,
      promptGuidance: 'For each finding: CVSS scoring with vector breakdown, attack narrative context, evidence artifacts with scan timestamps, business impact in terms of adversary objectives, and remediation. Findings should be ordered by attack chain position, not just severity. Each finding MUST include: (1) raw scanner/tool output as evidence, (2) scan timestamp (UTC and local), (3) source tool and version, (4) corroboration tier (confirmed/corroborated/unverified), (5) screenshot path if available. Format evidence in code blocks with timestamps so the client can cross-reference against their IDS, WAF, SIEM, and SOC feeds.',
      dataSources: ['findings', 'evidence', 'artifacts', 'scan_timestamps'],
      subsections: [
        { id: 'confirmed_findings', title: 'Confirmed Findings', promptGuidance: 'Findings validated through manual testing or multi-tool corroboration. Include full evidence chain with timestamps.' },
        { id: 'corroborated_findings', title: 'Corroborated Findings', promptGuidance: 'Findings detected by multiple sources but not manually validated. Include all source outputs.' },
        { id: 'unverified_findings', title: 'Unverified Findings', promptGuidance: 'Single-source findings requiring additional validation. Include raw output and recommended verification steps.' },
      ],
    },
    {
      id: 'false_positive_disposition',
      title: 'False Positive Disposition',
      required: true,
      promptGuidance: 'Document ALL scanner alerts that were determined to be false positives. For each false positive: (1) original alert title and severity, (2) source tool and scan timestamp, (3) validation method used to determine false positive status, (4) rationale for dismissal with supporting evidence, (5) any environmental factors that triggered the false alert. This section proves due diligence and helps the client tune their detection systems. If no Critical or High findings were confirmed, this section MUST explicitly state that all Critical/High alerts were reviewed and cleared as false positives with individual justification for each.',
      dataSources: ['findings', 'false_positive_findings', 'scan_results'],
      subsections: [
        { id: 'fp_critical_high', title: 'Critical/High Alerts Cleared', promptGuidance: 'Each Critical or High severity alert that was determined to be a false positive, with full validation evidence and rationale.' },
        { id: 'fp_medium_low', title: 'Medium/Low Alerts Cleared', promptGuidance: 'Medium and Low severity alerts cleared as false positives, grouped by category where possible.' },
      ],
    },
    {
      id: 'scan_evidence_summary',
      title: 'Scan Evidence and Timestamps',
      required: true,
      promptGuidance: 'Provide a consolidated table of ALL scan activities performed during the engagement. For each scan: tool name, target(s), start timestamp (UTC), end timestamp (UTC), duration, finding count, and raw output reference. This section enables the client to correlate red team activity against their IDS alerts, WAF logs, SIEM events, and SOC ticket timestamps. Include Burp Suite, NMAP, ZAP, Nuclei, and any other scanner results with their exact execution times.',
      dataSources: ['scan_results', 'tool_results', 'scan_timestamps', 'burp_results'],
      subsections: [
        { id: 'scan_timeline', title: 'Scan Execution Timeline', promptGuidance: 'Chronological table of all scans with start/end timestamps, tool, target, and outcome.' },
        { id: 'scan_raw_outputs', title: 'Raw Scan Outputs', promptGuidance: 'Key raw outputs from each scanner, formatted in code blocks with timestamps. Include Burp Suite findings, NMAP results, ZAP alerts, and Nuclei matches.' },
        { id: 'defensive_observations', title: 'Defensive Control Observations', promptGuidance: 'Document which scans were blocked, rate-limited, or detected by WAF/IDS. Include HTTP status codes, block messages, and timing of defensive responses.' },
      ],
    },
    {
      id: 'test_plan_coverage',
      title: 'Test Plan Coverage Matrix',
      required: true,
      promptGuidance: 'Map every item from the original test plan to its execution status and result. For each test plan item: (1) test plan reference ID, (2) test description, (3) execution status (completed/skipped/blocked), (4) result (finding confirmed/no issue identified/false positive/blocked by controls), (5) evidence reference, (6) scan timestamp. This ensures complete traceability between the agreed test plan and actual testing performed.',
      dataSources: ['test_plan', 'findings', 'scan_results', 'engagement_timeline'],
    },
    {
      id: 'credential_exposure',
      title: 'Credential Exposure Assessment',
    ...
    REMEDIATION_ROADMAP,
    {
      id: 'exploit_validation_audit',
      title: 'Exploit Validation Audit Trail',
      required: true,
      promptGuidance: 'Provide a complete audit trail of all exploit attempts during the engagement. Include: confirmed successes (count), total exploit attempts, attempts blocked (by WAF, IDS, network controls), re-run results, and the method used to validate each. This mirrors the Celereum report Appendix A format and provides the client with a verifiable record of offensive actions taken.',
      dataSources: ['exploit_attempts', 'waf_blocks', 'detection_events'],
      subsections: [
        { id: 'exploit_summary_table', title: 'Exploit Attempt Summary', promptGuidance: 'Table: Confirmed Successes, Total Attempts, Blocked, WAF Blocks, Network Blocks, Re-Run Results.' },
        { id: 'exploit_detail_log', title: 'Detailed Exploit Log', promptGuidance: 'Per-exploit: timestamp, technique, target, payload summary, result, blocking control (if any).' },
      ],
    },
    APPENDIX,
  ],
...
const VULN_ASSESSMENT_BLUEPRINT: ReportBlueprint = {
  ...
  sections: [
    ...
    },
    {
      ...
      dataSources: ['scan_reports', 'tool_results', 'nuclei_findings', 'web_crawl_results', 'active_scan_results'],
      subsections: [
        { id: 'scanner_summary', title: 'Scanner Summary', promptGuidance: 'Which scanners were used and their coverage.' },
        { id: 'nuclei_verification', title: 'Nuclei Verification Results', promptGuidance: 'Active verification results from Nuclei templates including matched templates, curl commands, and extracted evidence.' },
        { id: 'web_crawl_analysis', title: 'Web Crawl Security Analysis', promptGuidance: 'Security header grades, detected technologies, exposed paths, and form analysis from web crawling.' },
