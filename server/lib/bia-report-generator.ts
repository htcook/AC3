 ...
 * 
 * Generates NIST IR 8286D-aligned Business Impact Analysis reports
 * using FIPS 199 security categorization, mission function mapping,
 * and live data from the domain intel pipeline.
 * 
 * References:
 * - NIST SP 800-34 Rev. 1: Contingency Planning Guide
 * - NIST SP 800-60 Vol. 1: Guide for Mapping Types of Information
 * - NIST IR 8286D: Using Business Impact Analysis to Inform Risk Prioritization
 * - FIPS 199: Standards for Security Categorization
 */

import type { Fips199Category, CriticalityTier, CarverScores, ShockScores } from './scoring-engine';
import { CRITICALITY_TIERS, MISSION_FUNCTION_BASELINES, ESSENTIAL_SERVICE_BASELINES } from './scoring-engine';

// ─── Types ──────────────────────────────────────────────────────────────

export interface BiaAssetInput {
  id: number;
  hostname: string;
  assetType: string;
...
export interface BiaReportSection {
  id: string;
  title: string;
  content: string;
...
export interface BiaReportTable {
  caption: string;
  headers: string[];
  rows: string[][];
}
...
const FIPS_LEVEL_ORDER: Record<string, number> = { low: 1, moderate: 2, high: 3 };
const FIPS_LEVEL_NAMES: Record<number, string> = { 1: 'LOW', 2: 'MODERATE', 3: 'HIGH' };
...
function inferFips199(asset: BiaAssetInput): Fips199Category {
  if (asset.fips199Category) return asset.fips199Category;

  // Infer from mission function and business impact level
  const mf = asset.missionFunction;
  const bil = asset.businessImpactLevel;

  let conf: 'low' | 'moderate' | 'high' = 'low';
  let integ: 'low' | 'moderate' | 'high' = 'low';
  let avail: 'low' | 'moderate' | 'high' = 'low';

  // Mission function drives categorization
...
function fips199LevelLabel(level: string): string {
...
function inferRecoveryObjectives(asset: BiaAssetInput): RecoveryObjectives {
  const tier = asset.criticalityTier || inferCriticalityTier(asset);
  const tierInfo = CRITICALITY_TIERS[tier as CriticalityTier] || CRITICALITY_TIERS[3];

  const rtoMap: Record<number, string> = {
    1: '< 1 hour', 2: '1–4 hours', 3: '4–24 hours', 4: '24–72 hours', 5: '72+ hours'
  };
  const rpoMap: Record<number, string> = {
    1: '< 15 minutes', 2: '1 hour', 3: '4 hours', 4: '24 hours', 5: '72 hours'
  };
  const mtpdMap: Record<number, string> = {
    1: '4 hours', 2: '24 hours', 3: '72 hours', 4: '1 week', 5: '2 weeks'
  };
...
function inferCriticalityTier(asset: BiaAssetInput): number {
...
export function generateBiaReport(
  org: BiaOrgInput,
  assets: BiaAssetInput[],
  overallRiskScore: number,
  overallRiskBand: string,
): BiaReport {
  ...
  // Compute system-level FIPS 199 categorization (high-water mark across all assets)
...
function buildExecutiveOverview(
  org: BiaOrgInput,
  assets: BiaAssetInput[],
  systemCat: { confidentiality: string; integrity: string; availability: string; overall: string },
  overallRiskScore: number,
  overallRiskBand: string,
): BiaReportSection {
  ...
  const kevFindings = assets.reduce((s, a) => s + (a.postureFindings?.filter((f: any) => f.kevListed)?.length || 0), 0);
...
function buildFips199Section(
  org: BiaOrgInput,
  assets: BiaAssetInput[],
  allFips: Fips199Category[],
  systemCat: { confidentiality: string; integrity: string; availability: string; overall: string },
): BiaReportSection {
  const content = [
...
function buildMissionFunctionSection(assets: BiaAssetInput[]): BiaReportSection {
  // Group assets by mission function
  const byFunction = new Map<string, BiaAssetInput[]>();
...
function buildCriticalitySection(assets: BiaAssetInput[]): BiaReportSection {
...
function buildRecoveryObjectivesSection(assets: BiaAssetInput[]): BiaReportSection {
...
function buildDependencySection(assets: BiaAssetInput[]): BiaReportSection {
  const assetsWithDeps = assets.filter(a => a.missionDependencies &&
    (a.missionDependencies.upstreamAssets.length > 0 ||
     a.missionDependencies.downstreamAssets.length > 0 ||
     a.missionDependencies.sharedServices.length > 0));
...
function buildRiskDistributionSection(
  assets: BiaAssetInput[],
  critical: BiaAssetInput[],
  high: BiaAssetInput[],
  medium: BiaAssetInput[],
  low: BiaAssetInput[],
): BiaReportSection {
  // Top findings by severity
  const allFindings = assets.flatMap(a => (a.postureFindings || []).map((f: any) => ({ ...f, parentAsset: a.hostname })));
  const topFindings = allFindings
    .sort((a: any, b: any) => (b.severity || 0) - (a.severity || 0))
    .slice(0, 20);

  const findingRows = topFindings.map((f: any) => [
    f.parentAsset || '',
    f.category || '',
  ...
  const content = [
    ...
    `**KEV-Listed:** ${allFindings.filter((f: any) => f.kevListed).length}\n`,
    `**Confirmed Findings:** ${allFindings.filter((f: any) => f.corroborationTier === 'confirmed').length}\n`,
    `**Probable Findings:** ${allFindings.filter((f: any) => f.corroborationTier === 'probable').length}`,
  ].join('');
...
function buildRecommendationsSection(
  ...
  // KEV remediation
  const kevAssets = assets.filter(a => a.postureFindings?.some((f: any) => f.kevListed));
  if (kevAssets.length > 0) {
    recommendations.push(
      `**1. CISA KEV Remediation (IMMEDIATE):** ${kevAssets.length} assets have CISA Known Exploited Vulnerabilities. ` +
      `Per BOD 22-01, these must be remediated within the specified due dates. Priority assets: ` +
      kevAssets.slice(0, 5).map(a => a.hostname).join(', ') + '.'
    );
  }
  ...
  if (critical.length > 0) {
    recommendations.push(
      `**2. Critical Asset Hardening (24-48 HOURS):** ${critical.length} assets scored in the critical risk band. ` +
      `Implement compensating controls immediately: network segmentation, enhanced monitoring, and emergency patching. ` +
      `Priority: ` + critical.slice(0, 5).map(a => `${a.hostname} (${a.hybridRiskScore})`).join(', ') + '.'
    );
  }
  ...
  if (authAssets.length > 0) {
    recommendations.push(
      `**3. Authentication & C2 Infrastructure (HIGH PRIORITY):** ${authAssets.length} assets serve authentication or command-and-control functions. ` +
      `Compromise of these assets enables lateral movement across the entire environment. ` +
      `Ensure MFA enforcement, privileged access management, and continuous monitoring.`
    );
  }
  ...
  if (tier1Assets.length > 0) {
    recommendations.push(
      `**4. Recovery Planning (ONGOING):** ${tier1Assets.length} assets are classified as Tier 1-2 (Mission/Business Critical) ` +
      `with RTOs under 24 hours. Validate that current backup and disaster recovery capabilities can meet these objectives. ` +
      `Conduct tabletop exercises to test recovery procedures.`
    );
  }

  // Dependency risk
  const highDependencyAssets = assets.filter(a =>
    a.missionDependencies && a.missionDependencies.downstreamAssets.length >= 3
  );
  if (highDependencyAssets.length > 0) {
    recommendations.push(
      `**5. Dependency Risk Mitigation:** ${highDependencyAssets.length} assets have 3+ downstream dependents, ` +
      `creating single points of failure. Implement redundancy, failover mechanisms, and circuit breakers ` +
      `to limit cascading failure propagation.`
    );
  }

  // General
  recommendations.push(
    `**6. Continuous Monitoring:** Implement automated vulnerability scanning on a weekly cadence for critical assets ` +
    `and monthly for all others. Integrate CISA KEV feed monitoring to detect newly listed vulnerabilities affecting your environment.`
  );

  recommendations.push(
    `**7. Compliance Alignment:** ${org_compliance_note(assets[0])} ` +
    `Review control implementations against the ${assets[0]?.fips199Category ? 'FIPS 199 categorization' : 'identified risk levels'} ` +
    `and update the System Security Plan (SSP) accordingly.`
  );
...
function org_compliance_note(_asset: BiaAssetInput): string {
