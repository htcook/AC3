/**
 * CI/CD Cron Scheduler Engine
 * 
 * Manages scheduled CI/CD scans with cron expressions.
 * Runs a background interval that checks for due pipelines and triggers scans.
 */
 ...
 */
export function matchesCron(date: Date, cron: CronFields): boolean {
 ...
 */
export function getNextRunTime(after: Date, cronExpr: string): Date | null {
...
/**
 * Check all scheduled pipelines and trigger any that are due
 */
async function checkScheduledPipelines(): Promise<void> {
  if (isRunning) return; // Prevent concurrent checks
  isRunning = true;
  ...
  try {
    ...
    // Find all active pipelines with scheduling enabled
    ...
    const pipelines = ((rows as any).rows || rows) as any[];
    ...
    for (const pipeline of pipelines) {
      try {
        ...
        // Check if this pipeline is due to run
        let shouldRun = false;
        if (nextRun && now >= nextRun) {
          shouldRun = true;
        } else if (!nextRun) {
          // No next run calculated yet — check if cron matches now
          const cron = parseCronExpression(cronExpr);
        ...
        // Update pipeline with last/next run times
        ...
        // Kick off the scan asynchronously (same logic as triggerRun)
...
/**
 * Start the cron scheduler (checks every 60 seconds)
 */
export function startCronScheduler(): void {
  if (schedulerInterval) return;
  console.log("[CronScheduler] Starting CI/CD cron scheduler (60s interval)");
  schedulerInterval = setInterval(checkScheduledPipelines, 60_000);
  // Also run an initial check after 30s to catch any immediately-due jobs
  setTimeout(checkScheduledPipelines, 30_000);
}
