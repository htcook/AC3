/**
 * CI/CD Threat Report Generator
 * 
 * Generates HTML-based threat assessment reports for CI/CD runs
 * that can be converted to PDF on the client side or served as HTML.
 */
...
export function generateThreatReportHtml(data: ThreatReportData): string {
  ...
  const findingsRows = (tc?.enrichedFindings || sr.findings.map(f => ({
    ...
    attributedGroups: [] as any[],
    riskTags: [] as string[],
    killChainPhases: [] as string[],
  }))).slice(0, 50).map((f, i) => `
  ...
  <div class="container">
    ...
    <table style="margin-bottom: 24px;">
      ...
      <tr style="border-bottom: 1px solid #e5e7eb;">
        ...
        <td style="padding: 6px 8px; font-size: 12px;">${sr.duration ? `${(sr.duration / 1000).toFixed(1)}s` : "N/A"}</td>
      </tr>
    </table>
