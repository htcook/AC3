/**
 * ICS/SCADA Threat Intelligence Module
 *
 * Automated ingestion and enrichment pipeline for Industrial Control System
 * and SCADA-specific threat intelligence:
 *
 *   1. CISA ICS Advisories RSS — dedicated ICS advisory feed (ics-advisories.xml)
 *   2. CISA CSAF OT Repository — machine-readable JSON advisories from GitHub
 *   3. ICS-specific vendor advisory feeds (Siemens, Schneider, Rockwell, etc.)
 *   4. Open-source ICS/SCADA tool catalog — Conpot, GRFICSv2, Malcolm, etc.
 *   5. ICS malware family knowledge base — Stuxnet through ZionSiphon
 *   6. Auto-tagging: flags threat actors with ICS/SCADA capability when discovered
 *   7. MITRE ATT&CK for ICS technique cross-mapping
 *
 * This module integrates with the existing threat-intel-daily pipeline
 * and enriches the aptIcsMappings, icsExploits, and threatActors tables.
 */
...
// ─── ICS Keywords for Detection ─────────────────────────────────────────────

const ICS_KEYWORDS = [
  // Protocols
  'modbus', 'dnp3', 'iec 104', 'iec 61850', 'opc ua', 'opc-ua', 'opcua', 'opc da',
  'bacnet', 'profinet', 'ethercat', 'ethernet/ip', 'hart', 'foundation fieldbus',
  's7comm', 'fins', 'melsec', 'codesys', 'enip',
  // Device types
  'plc', 'rtu', 'hmi', 'dcs', 'scada', 'ics', 'safety instrumented',
  'sis', 'engineering workstation', 'historian', 'data diode',
  // Vendors
  'siemens', 'schneider electric', 'rockwell', 'allen-bradley', 'honeywell',
  'abb', 'ge digital', 'yokogawa', 'emerson', 'omron', 'mitsubishi electric',
  'beckhoff', 'wago', 'moxa', 'advantech', 'delta electronics', 'phoenix contact',
  'unitronics', 'codesys', 'inductive automation', 'ignition',
  // Sectors
  'power grid', 'water treatment', 'oil and gas', 'pipeline', 'manufacturing',
  'nuclear', 'chemical plant', 'electric utility', 'substation',
  // Malware families
  'stuxnet', 'industroyer', 'crashoverride', 'triton', 'trisis', 'hatman',
  'pipedream', 'incontroller', 'frostygoop', 'fuxnet', 'iocontrol',
  'blackenergy', 'havex', 'irongate', 'industroyer2', 'chaya_003',
  'dynowiper', 'zionsiphon', 'shamoon', 'night dragon',
  // General
  'operational technology', 'critical infrastructure', 'industrial control',
  'cyber-physical', 'purdue model', 'ot network', 'ot security',
];
...
export interface IcsMalwareFamily {
  name: string;
  aliases: string[];
  year: number;
  attribution: string;
  targetedVendors: string[];
  targetedProtocols: string[];
...
export const ICS_MALWARE_FAMILIES: IcsMalwareFamily[] = [
  {
    name: "Stuxnet",
    aliases: ["W32.Stuxnet"],
    year: 2010,
...
// ─── Open-Source ICS/SCADA Tool Catalog ─────────────────────────────────────

export interface IcsOpenSourceTool {
  name: string;
  category: 'honeypot' | 'assessment' | 'monitoring' | 'simulation' | 'framework' | 'protocol_analysis' | 'forensics';
  description: string;
  githubUrl: string;
  license: string;
  protocols: string[];
...
export const ICS_OPEN_SOURCE_TOOLS: IcsOpenSourceTool[] = [
  // Honeypots
  {
    name: "Conpot",
    category: "honeypot",
...
// ─── 1. CISA ICS Advisories RSS Feed ────────────────────────────────────────

/**
 * Ingests CISA ICS-specific advisories from the dedicated ICS advisory RSS feed.
 * These are separate from general CISA advisories and contain vendor, product,
 * CVSS, and sector information specific to ICS/SCADA systems.
 */
export async function ingestCisaIcsAdvisories(): Promise<IcsIngestResult> {
  const start = Date.now();
  const result: IcsIngestResult = { source: "CISA ICS Advisories", fetched: 0, newRecords: 0, errors: [], durationMs: 0 };

  try {
    const db = await requireDb();

    // CISA ICS-specific advisory feed
    ...
    for (const item of items.slice(0, 50)) {
      try {
        // Extract ICSA advisory ID from title or link
        const icsaMatch = item.title.match(/ICSA-\d{2}-\d{3}-\d{2}/i) ||
          item.link.match(/icsa-\d{2}-\d{3}-\d{2}/i);
        const advisoryId = icsaMatch ? icsaMatch[0].toUpperCase() : `CISA-ICS-${hashString(item.link)}`;

        // Extract vendor from title (usually "Vendor ProductName")
        ...
        await db.insert(icsExploits).values({
          ...
          iceDescription: item.description?.slice(0, 5000) || '',
          ...
          icePublishedDate: item.pubDate || new Date().toISOString(),
          iceSector: detectIcsSectors(text),
          iceReferences: [item.link],
        }).onDuplicateKeyUpdate({ set: { iceDescription: item.description?.slice(0, 5000) || '' } });

        // Also insert into incidentReports for cross-referencing
...
// ─── 2. CISA CSAF OT Repository (GitHub) ───────────────────────────────────

/**
 * Fetches machine-readable CSAF 2.0 JSON advisories from the CISA CSAF GitHub repo.
 * These contain structured vendor, product, CVE, and CVSS data for ICS/OT systems.
 */
export async function ingestCisaCsafOt(): Promise<IcsIngestResult> {
  const start = Date.now();
  const result: IcsIngestResult = { source: "CISA CSAF OT", fetched: 0, newRecords: 0, errors: [], durationMs: 0 };

  try {
    const db = await requireDb();

    // Use GitHub API to list recent CSAF files in the OT directory
    ...
    const jsonFiles = files.filter((f: any) => f.name.endsWith('.json')).slice(-30); // Last 30 advisories
    ...
    for (const file of jsonFiles) {
      try {
        ...
        const csaf = await fileRes.json() as any;
        ...
        const cves = vulns.map((v: any) => v.cve).filter(Boolean);

        // Extract vendor and product from product tree
        const branches = productTree.branches || [];
        const vendor = branches[0]?.name || 'Unknown';
        const products = branches.flatMap((b: any) =>
          (b.branches || []).map((pb: any) => pb.name)
        ).filter(Boolean);

        // Extract CVSS scores
        const cvssScores = vulns.flatMap((v: any) =>
          (v.scores || []).map((s: any) => s.cvss_v3?.baseScore || 0)
        );
        ...
        await db.insert(icsExploits).values({
          ...
          iceDescription: doc.notes?.find((n: any) => n.category === 'summary')?.text?.slice(0, 5000) || '',
          iceAffectedVendor: vendor,
          ...
          icePublishedDate: doc.tracking?.current_release_date || new Date().toISOString(),
          iceSector: [],
...
// ─── 3. Siemens ProductCERT CSAF Feed ───────────────────────────────────────

/**
 * Ingests Siemens ProductCERT advisories from their CSAF index.
 * Siemens is the largest ICS vendor and publishes the most advisories.
 */
export async function ingestSiemensProductCert(): Promise<IcsIngestResult> {
  const start = Date.now();
  const result: IcsIngestResult = { source: "Siemens ProductCERT", fetched: 0, newRecords: 0, errors: [], durationMs: 0 };

  try {
    const db = await requireDb();

    // Siemens publishes a CSAF index
    ...
    // Parse CSAF index and fetch recent advisories
    ...
    for (const entry of entries) {
      try {
        ...
        await db.insert(icsExploits).values({
          ...
          iceAffectedVendor: 'Siemens',
          icePublishedDate: new Date().toISOString(),
          iceReferences: [url],
...
// ─── 4. Auto-Tag Actors with ICS/SCADA Capability ──────────────────────────

/**
 * Scans threat actor descriptions, events, and associated malware to automatically
 * flag actors that have demonstrated ICS/SCADA targeting capability.
 * Updates the targetsIcsScada field on the threat_actors table.
 */
export async function autoTagIcsActors(): Promise<{ tagged: number; alreadyTagged: number; errors: string[] }> {
  const result = { tagged: 0, alreadyTagged: 0, errors: [] as string[] };

  try {
    const db = await requireDb();

    // Get all actors not yet tagged as ICS
    ...
    for (const actor of actors) {
      // Check if already tagged via description marker
      if (actor.description?.includes('[ICS/SCADA-CAPABLE]')) {
        result.alreadyTagged++;
        continue;
      }

      // Check if actor description mentions ICS keywords
      ...
      if (hasIcsKeyword) {
        await db.update(threatActors)
          .set({ description: sql`CONCAT(COALESCE(description,''), ' [ICS/SCADA-CAPABLE]')` })
          .where(eq(threatActors.actorId, actor.actorId));
        result.tagged++;
        continue;
      }
      // Check if actor has events mentioning ICS
      try {
        ...
        if (ICS_KEYWORDS.some(kw => eventText.includes(kw))) {
          // Note: targetsIcsScada is on accessBrokerListings, not threatActors
          // Tag actor description instead
          await db.update(threatActors)
            .set({ description: sql`CONCAT(COALESCE(description,''), ' [ICS/SCADA-CAPABLE]')` })
            .where(eq(threatActors.actorId, actor.actorId));
          result.tagged++;
    ...
    // Also check aptIcsMappings table and ensure those actors are tagged
    try {
...
// ─── 5. ICS Malware → Actor Cross-Mapping ───────────────────────────────────

/**
 * Cross-maps ICS malware families to threat actors in the catalog.
 * When a malware family is attributed to a group, ensures the actor
 * is tagged with ICS capability and the malware is linked.
 */
export async function crossMapIcsMalwareToActors(): Promise<{ mapped: number; errors: string[] }> {
  const result = { mapped: 0, errors: [] as string[] };
  ...
  try {
    ...
    for (const malware of ICS_MALWARE_FAMILIES) {
      ...
      // Try to find the attributed actor in our catalog
      ...
      for (const actor of actors) {
        ...
        // Check if the attribution matches this actor
        if (attributionLower.includes(actorLower) || actorLower.includes(attributionLower.split('(')[0].trim().toLowerCase())) {
          ...
          await db.insert(threatGroupEvents).values({
            ...
            tgeConfidence: 90,
            eventDate: new Date(`${malware.year}-01-01`).toISOString(),
...
/**
 * Run all ICS/SCADA intelligence ingestion and enrichment.
 * Called by the threat-intel-daily scheduler.
 */
export async function runIcsScadaIntelIngest(): Promise<{
  totalSources: number;
  successfulSources: number;
  totalNewRecords: number;
  actorsTagged: number;
  malwareMapped: number;
  results: IcsIngestResult[];
  durationMs: number;
}> {
  const start = Date.now();
  const results: IcsIngestResult[] = [];
  let actorsTagged = 0;
  let malwareMapped = 0;

  console.log("[ICS-SCADA-Intel] Starting ICS/SCADA intelligence ingestion...");

  // Phase 1: Ingest ICS advisories
  const sources = [
  ...
  // Phase 2: Auto-tag actors with ICS capability
  try {
  ...
  // Phase 3: Cross-map ICS malware to actors
  try {
...
function extractProductFromTitle(title: string, vendor: string): string {
  // Remove vendor name and common prefixes
  let product = title
    .replace(vendor, '')
    .replace(/^[\s:—-]+/, '')
    .replace(/ICSA-\d{2}-\d{3}-\d{2}\s*/i, '')
    .trim();
  return product.slice(0, 255) || 'Unknown';
}

function extractCvssFromText(text: string): number {
  const cvssMatch = text.match(/cvss[:\s]*(\d+\.?\d*)/i);
  if (cvssMatch) return parseFloat(cvssMatch[1]);
  return 0;
}

function detectIcsSectors(text: string): string[] {
...
// ─── Exports for API/Router use ─────────────────────────────────────────────

export function getIcsMalwareFamilies(): IcsMalwareFamily[] {
  return ICS_MALWARE_FAMILIES;
}

export function getIcsOpenSourceTools(): IcsOpenSourceTool[] {
  return ICS_OPEN_SOURCE_TOOLS;
}

export function getIcsVendors(): typeof ICS_VENDORS {
  return ICS_VENDORS;
}

export function getIcsKeywords(): string[] {
  return [...ICS_KEYWORDS];
}
