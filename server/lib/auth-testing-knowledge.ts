 ...
 * - CARVER+Shock scoring overlay for auth findings
 * - LLM reasoning framework for auth analysis
 * - OAuth/OIDC + SAML assessment checklist
 *
 * License compliance:
 * - OWASP WSTG: CC BY-SA 4.0 (attribution required)
 * - MITRE ATT&CK®: royalty-free with attribution
 * - Open-source tooling docs: varies by tool
 */

// ─── Auth Testing Methodology ───────────────────────────────────────────────

export interface AuthTestingPhase {
  phase: string;
  order: number;
  objectives: string[];
  artifacts: string[];
...
export const AUTH_TESTING_PHASES: AuthTestingPhase[] = [
  {
    ...
    artifacts: [
      "HTTP headers", "Cookies", "Redirect chains",
...
export interface AuthAttackClass {
  id: string;
  name: string;
  description: string;
  signals: string[];
  subtypes: AttackSubtype[];
  safeTestNotes: string[];
  mitreTechniques: string[];
  relatedPhases: string[];
}

export const AUTH_ATTACK_TAXONOMY: AuthAttackClass[] = [
  {
    id: "AUTH-ATTACK-001",
...
export interface AuthMitreMapping {
  findingType: string;
  tactic: string;
  techniqueId: string;
...
export const AUTH_MITRE_MAPPINGS: AuthMitreMapping[] = [
  {
...
// ─── OAuth/OIDC + SAML Assessment Checks ────────────────────────────────────

export interface SSOCheck {
  id: string;
  protocol: "oauth_oidc" | "saml";
  name: string;
  whatToVerify: string[];
...
export const SSO_ASSESSMENT_CHECKS: SSOCheck[] = [
  {
    id: "SSO-OAUTH-001",
    protocol: "oauth_oidc",
...
// ─── CARVER+Shock Auth Scoring Overlay ──────────────────────────────────────

export interface CarverAuthOverlay {
  defaultWeights: {
    criticality: number;
...
export const AUTH_CARVER_OVERLAY: CarverAuthOverlay = {
  defaultWeights: {
    criticality: 9,
    accessibility: 8,
    recuperability: 6,
...
/**
 * Calculate adjusted CARVER score for an auth finding.
 */
export function calculateAuthCarverScore(
  conditions: string[]
): { weights: Record<string, number>; totalScore: number; maxScore: number; percentage: number } {
    ...
    if (conditions.includes(adj.conditionKey)) {
      for (const [factor, delta] of Object.entries(adj.changes)) {
        if (factor in weights) {
          (weights as any)[factor] = Math.max(0, Math.min(10, (weights as any)[factor] + (delta as number)));
        }
      }
    }
  }
  ...
  const maxScore = 60; // 6 factors × 10 max each
...
/// ─── Federal Compliance Auth Controls (mapped to NIST 800-53 / FedRAMP baselines) ────

export interface FederalAuthControl {
  controlId: string;
  title: string;
  baseline: "moderate" | "high";
  authRelevance: string;
  testingNotes: string;
}

export const FEDERAL_AUTH_CONTROLS: FederalAuthControl[] = [
  // Moderate baseline
...
export const AUTH_REASONING_SYSTEM_PROMPT = `
...
/**
 * Build auth-specific knowledge context for LLM injection.
 * Designed to be appended to the STRIKE ADVISOR's existing context.
 */
export function buildAuthKnowledgeContext(): string {
  ...
  // Phase summaries
  const phaseSummary = AUTH_TESTING_PHASES.map(p =>
    `**Phase ${p.order}: ${p.phase}** — ${p.objectives[0]}. Tools: ${p.tools.join(", ")}.`
  ).join("\n");
  sections.push(`### Auth Testing Phases\n${phaseSummary}`);

  // Attack class summaries
  const attackSummary = AUTH_ATTACK_TAXONOMY.map(a =>
    `**${a.name}** [${a.mitreTechniques.join(",")}] — ${a.subtypes.length} subtypes. Signals: ${a.signals.slice(0, 2).join("; ")}.`
  ).join("\n");
...
// ─── Tooling Stack Knowledge ────────────────────────────────────────────────

export interface AuthToolEntry {
  name: string;
  category: string;
  license: string;
  useCases: string[];
...
export const AUTH_TOOLING_STACK: AuthToolEntry[] = [
  {
    name: "OWASP ZAP",
...
export interface AuthEvidenceEvent {
  timestamp: string;
  tool: string;
  target: string;
  eventType: string;
...
export interface AuthFinding {
  ...
  severity: "critical" | "high" | "medium" | "low" | "info";
  summary: string;
  mitre?: { tactic: string; techniqueId: string; techniqueName: string };
  fedrampControls?: string[];
  evidenceRefs: string[];
  remediation: string[];
  carverScore?: { weights: Record<string, number>; totalScore: number; percentage: number };
}

export interface AuthReportSection {
  sectionTitle: string;
...
// ─── Cross-Module Utility: Generic Pipeline Guardrails ──────────────────────
// This pattern is reusable across ALL platform engines (not just auth testing)
...
export const STRICT_MODE_GUARDRAILS: PipelineGuardrails = {
  maxRps: 0.1,
...
export const STANDARD_MODE_GUARDRAILS: PipelineGuardrails = {
  maxRps: 0.5,
...
/**
 * Validate an action against guardrails. Returns violations if any.
 * This is a CROSS-MODULE utility — usable by scan-webhooks, credential tools,
 * engagement workflow, exploitation bridge, and any future engine.
 */
export function validateAgainstGuardrails(
  action: { rps?: number; attemptsPerAccount?: number; target?: string; hasEvidence?: boolean },
  guardrails: PipelineGuardrails
): { valid: boolean; violations: string[] } {
  const violations: string[] = [];

  if (action.rps !== undefined && action.rps > guardrails.maxRps) {
    violations.push(`Rate ${action.rps} RPS exceeds limit of ${guardrails.maxRps} RPS`);
  }
  if (action.attemptsPerAccount !== undefined && action.attemptsPerAccount > guardrails.maxAttemptsPerAccount) {
  ...
  if (guardrails.requireScopeAllowlist && guardrails.scopeAllowlist && action.target) {
    const inScope = guardrails.scopeAllowlist.some(s =>
      action.target!.includes(s)
    );
    if (!inScope) {
