/**
 * External Credential Attack Tool Integration
 *
 * Integrates three industry-standard credential attack tools via subprocess invocation:
 *
 *   1. THC Hydra (AGPL-3.0) — The fastest parallelized network login cracker.
 *      Best for: High-speed brute force across 50+ protocols (SSH, FTP, HTTP, RDP,
 *      MySQL, PostgreSQL, MSSQL, VNC, SNMP, SMTP, POP3, IMAP, Telnet, LDAP, SMB,
 *      SIP, Redis, MongoDB, Memcached, etc.). Excels at raw throughput with up to
 *      64 parallel connections. Preferred when speed matters and the target has no
 *      aggressive rate limiting.
 *
 *   2. Medusa (GPLv2) — Speedy, parallel, and modular login brute-forcer.
 *      Best for: Stable, reliable service authentication testing with modular
 *      architecture. Supports SSH, FTP, HTTP, Telnet, MySQL, PostgreSQL, MSSQL,
 *      VNC, SNMP, SVN, POP3, IMAP, SMTP, RDP, SMB, NCP, AFP, CVS, and more.
 *      Preferred when stability and module-level control matter more than raw speed.
 *      Better error recovery than Hydra on flaky targets. Thread-per-host model
 *      gives finer control over per-target parallelism.
 *
 *   3. NetExec (BSD 2-Clause, successor to CrackMapExec) — Swiss army knife for
 *      pentesting Windows/Active Directory environments.
 *      Best for: SMB/WinRM/LDAP/MSSQL/RDP/SSH/FTP/WMI credential validation,
 *      pass-the-hash, pass-the-ticket, Kerberoasting, AS-REP roasting, domain
 *      enumeration, share enumeration, GPO abuse, SAM dump, LSA secrets, NTDS.dit
 *      extraction. The ONLY tool to use for Active Directory attacks. Supports
 *      NTLM hash authentication natively. Preferred for any Windows domain or
 *      post-exploitation credential validation scenario.
 *
 * Architecture: All tools are invoked as external binaries via child_process.
 * This means:
 *   - No copyleft obligations (AGPL/GPL safe via subprocess boundary)
 *   - Tools are optional dependencies (platform works without them)
 *   - Auto-detection of installed tools at startup
 *   - Graceful fallback to built-in engine when tools are unavailable
 *
 * LLM Integration: The module includes a comprehensive tool knowledge base that
 * the LLM uses to recommend the optimal tool for each attack scenario based on:
 *   - Target protocol and service
 *   - Attack mode (brute force, spray, stuffing, pass-the-hash)
 *   - Target environment (Linux, Windows, Active Directory)
 *   - Rate limiting / WAF presence
 *   - Required features (hash auth, Kerberos, domain enum)
 *
 * @module external-credential-tools
 */

import { spawn, execSync } from "child_process";
import type { AttackResult, AttackSuccess, ServiceProtocol, LockoutDetection } from "./credential-attack-engine";

// ─── Tool Definitions ─────────────────────────────────────────────────────

export type ExternalTool = "hydra" | "medusa" | "netexec" | "builtin";
...
export interface ToolCapability {
  ...
  license: string;
  /** Protocols this tool supports natively */
  protocols: string[];
  /** Attack modes this tool supports */
  attackModes: string[];
  /** Special capabilities beyond basic credential testing */
  specialCapabilities: string[];
  /** Performance characteristics */
  performance: {
    maxParallelConnections: number;
    supportsResume: boolean;
    supportsOutputParsing: boolean;
    relativeSpeed: "fast" | "moderate" | "slow";
  };
  /** Ideal use cases — used by LLM for tool selection */
  idealFor: string[];
...
export interface ExternalAttackConfig {
  tool: ExternalTool;
  target: {
    host: string;
    port: number;
  ...
  usernames: string[];
  passwords: string[];
  /** Or a file path to username list */
  usernameFile?: string;
  /** Or a file path to password list */
  passwordFile?: string;
  /** Parallelism */
  threads: number;
  ...
  extraFlags?: string[];
  /** Rate limiting delay (ms between attempts) */
  delayMs?: number;
  /** NetExec-specific: attack type */
  netexecModule?: "smb" | "winrm" | "ldap" | "mssql" | "rdp" | "ssh" | "ftp" | "wmi";
  /** NetExec-specific: post-auth actions */
  netexecPostAuth?: string[];
}

export interface ExternalAttackResult {
  tool: ExternalTool;
  sessionId: string;
  target: string;
  protocol: string;
  ...
  successfulLogins: AttackSuccess[];
  failedAttempts: number;
  errors: string[];
  status: "completed" | "stopped_timeout" | "stopped_manual" | "error" | "tool_not_found";
  rawOutput: string;
  /** Tool-specific metadata */
  metadata: Record<string, any>;
}

// ─── Tool Knowledge Base (LLM Training Data) ─────────────────────────────

/**
 * Comprehensive knowledge base for LLM tool selection.
 * This is the "training data" the LLM uses to understand each tool's
 * strengths, weaknesses, and optimal use cases.
 */
export const TOOL_KNOWLEDGE_BASE = {
  hydra: {
    fullName: "THC Hydra",
    license: "AGPL-3.0",
    ...
    protocols: {
      native: [
        ...
        "http-proxy", "http-proxy-urlenum", "socks5",
        "mysql", "postgres", "mssql", "oracle-listener", "oracle-sid",
        "rdp", "vnc", "smb", "smbnt",
  ...
  medusa: {
    ...
    protocols: {
      native: [
        ...
        "rdp", "vnc", "rlogin", "rexec", "rsh",
        "smb", "smbnt",
        "smtp", "smtp-vrfy", "pop3", "imap",
        "snmp",
        "svn", "cvs",
        "ncp", "nntp",
        "afp",
  ...
  netexec: {
    ...
    protocols: {
      native: [
        "smb", "winrm", "ldap", "mssql", "rdp", "ssh", "ftp", "wmi",
      ],
...
/**
 * LLM system prompt for tool selection decisions.
 * This prompt teaches the LLM how to evaluate attack scenarios and
 * recommend the optimal tool.
 */
export const TOOL_SELECTION_SYSTEM_PROMPT = `You are an expert penetration tester and credential attack specialist. Your role is to analyze attack scenarios and recommend the optimal tool from the following three options:
...
const toolDetectionCache = new Map<string, DetectedTool>();

function detectTool(name: string, versionFlag: string = "--version"): DetectedTool {
  const cached = toolDetectionCache.get(name);
  if (cached) return cached;

  try {
    const whichResult = execSync(`which ${name} 2>/dev/null`, { encoding: "utf-8", timeout: 5000 }).trim();
    if (!whichResult) {
    ...
    try {
      const versionOutput = execSync(`${name} ${versionFlag} 2>&1 || true`, { encoding: "utf-8", timeout: 5000 });
...
/** Clear the tool detection cache (useful after installing tools) */
export function clearToolDetectionCache(): void {
  toolDetectionCache.clear();
}

/** Detect all external credential attack tools */
export function detectAllTools(): Record<ExternalTool, DetectedTool> {
  return {
...
/** Get detailed capability info for all tools */
export function getToolCapabilities(): ToolCapability[] {
  ...
  return [
    ...
    },
    {
      ...
      attackModes: [
        "brute_force", "password_spray", "pass_the_hash",
        "kerberoasting", "asrep_roasting", "credential_dump",
      ],
      specialCapabilities: [
    ...
    },
    {
      ...
      license: "Proprietary",
      protocols: [
        "http_form", "http_basic", "http_digest", "http_json_api",
        "ssh", "ftp", "telnet", "redis",
      ],
...
/**
 * Use the LLM to recommend the optimal tool for a given attack scenario.
 * The LLM has been trained on each tool's strengths, weaknesses, and
 * protocol coverage via the TOOL_SELECTION_SYSTEM_PROMPT.
 */
export async function recommendTool(scenario: {
  targetHost: string;
  targetPort: number;
  protocol: string;
  attackMode: string;
  ...
  isMultiHost?: boolean;
  hasWaf?: boolean;
  targetOs?: "windows" | "linux" | "unknown";
  needsPostExploit?: boolean;
  connectionStability?: "stable" | "flaky" | "unknown";
  ...
  // Build the scenario description for the LLM
  ...
  try {
    ...
    const response = await invokeLLM({ _caller: "external-credential-tools.recommendTool", _priority: 'bulk',
      messages: [
    ...
    // Validate the recommendation — ensure the tool is actually available
    if (!availableTools.includes(parsed.recommended)) {
      // Fall back to first available tool or builtin
    ...
    return {
      recommended: parsed.recommended as ExternalTool,
    ...
    // Fallback: deterministic recommendation without LLM
    return deterministicRecommendation(scenario, availableTools);
  }
}

/**
 * Deterministic tool recommendation fallback (no LLM required).
 * Uses the decision framework from the knowledge base.
 */
function deterministicRecommendation(
  ...
  // Decision tree based on TOOL_KNOWLEDGE_BASE
  // Rule 1: Active Directory → NetExec
  ...
  // Rule 4: Flaky connections → Medusa
  ...
  // Rule 5: Multi-host scanning → Medusa preferred
  ...
  // Rule 6: WAF/rate limiting → Medusa (better delay control) or Hydra with -W
  if (scenario.hasWaf) {
    if (has("medusa")) {
      return {
        recommended: "medusa",
  ...
  // Rule 7: Default — Hydra for speed, Medusa for stability
  if (has("hydra")) {
    return {
      recommended: "hydra",
      ...
      confidence: 80,
      alternatives: [
        ...(has("medusa") ? [{ tool: "medusa" as ExternalTool, reason: "More stable but slower alternative" }] : []),
        ...(has("netexec") ? [{ tool: "netexec" as ExternalTool, reason: "Better for AD environments" }] : []),
      ],
...
function buildHydraCommand(config: ExternalAttackConfig): string[] {
  const args: string[] = [];
  ...
  if (config.usernameFile) {
    ...
    // Will need to write to temp file
  ...
  if (config.delayMs && config.delayMs > 0) {
    args.push("-W", String(Math.ceil(config.delayMs / 1000)));
  }
...
function mapProtocolToHydra(protocol: string, config: ExternalAttackConfig): string {
  const host = config.target.host;
  const port = config.target.port;

  switch (protocol) {
    case "ssh": return `ssh://${host}:${port}`;
    case "ftp": return `ftp://${host}:${port}`;
    case "telnet": return `telnet://${host}:${port}`;
    case "rdp": return `rdp://${host}:${port}`;
    case "vnc": return `vnc://${host}:${port}`;
    case "mysql": return `mysql://${host}:${port}`;
    case "postgres":
    case "postgresql": return `postgres://${host}:${port}`;
    case "mssql": return `mssql://${host}:${port}`;
    case "redis": return `redis://${host}:${port}`;
    case "mongodb": return `mongodb://${host}:${port}`;
    case "smtp": return `smtp://${host}:${port}`;
    case "pop3": return `pop3://${host}:${port}`;
    case "imap": return `imap://${host}:${port}`;
    case "snmp": return `snmp://${host}:${port}`;
    case "smb": return `smb://${host}:${port}`;
    case "ldap": return `ldap2://${host}:${port}`;
    case "http_basic": return `http-get://${host}:${port}`;
    case "http_form":
    case "http_json_api": {
      ...
      // Return just the target, form spec goes in extra args
      return `${host} http-post-form "${path}:${formParams}:F=${failStr}"`;
    }
    default: return `${protocol}://${host}:${port}`;
  }
}

function buildMedusaCommand(config: ExternalAttackConfig): string[] {
  const args: string[] = [];
...
function mapProtocolToMedusa(protocol: string): string {
...
function buildNetExecCommand(config: ExternalAttackConfig): string[] {
  const args: string[] = [];
...
function mapProtocolToNetExec(protocol: string): string {
  const mapping: Record<string, string> = {
    smb: "smb",
    winrm: "winrm",
    ldap: "ldap",
...
function parseHydraOutput(output: string): { successes: AttackSuccess[]; totalAttempts: number; errors: string[] } {
  const successes: AttackSuccess[] = [];
  const errors: string[] = [];
  let totalAttempts = 0;

  const lines = output.split("\n");
  for (const line of lines) {
    // Success line: [22][ssh] host: 192.168.1.1   login: admin   password: secret123
    ...
    // Attempt count: 1 of 1 target successfully completed, 1 valid password found
    const summaryMatch = line.match(/(\d+)\s+valid\s+password/i);
    if (summaryMatch) {
      // Already counted via successes
    }

    // Total attempts from verbose output
  ...
  // If we couldn't count attempts from verbose, estimate from output
  if (totalAttempts === 0) {
...
function parseMedusaOutput(output: string): { successes: AttackSuccess[]; totalAttempts: number; errors: string[] } {
  const successes: AttackSuccess[] = [];
  const errors: string[] = [];
  let totalAttempts = 0;
  ...
  const lines = output.split("\n");
  for (const line of lines) {
    // Success: ACCOUNT FOUND: [ssh] Host: 192.168.1.1 User: admin Password: secret123 [SUCCESS]
...
function parseNetExecOutput(output: string): { successes: AttackSuccess[]; totalAttempts: number; errors: string[]; metadata: Record<string, any> } {
  const successes: AttackSuccess[] = [];
  const errors: string[] = [];
  const metadata: Record<string, any> = { adminAccess: [], shares: [], domainInfo: null };
  let totalAttempts = 0;

  const lines = output.split("\n");
  for (const line of lines) {
    // Success: SMB  192.168.1.1  445  DC01  [+] DOMAIN\admin:Password1
    ...
    // Failure: SMB  192.168.1.1  445  DC01  [-] DOMAIN\admin:WrongPass
    if (line.includes("[-]")) {
      totalAttempts++;
    }

    // Success also counts as attempt
    if (line.includes("[+]")) {
      totalAttempts++;
    }
 ...
 * Handles temp file creation, subprocess management, output parsing,
 * and result normalization.
 */
export async function executeExternalAttack(config: ExternalAttackConfig): Promise<ExternalAttackResult> {
  ...
  // Write temp credential files if needed
  ...
  return new Promise((resolve) => {
    ...
    const proc = spawn(binary, args, {
      timeout: config.globalTimeout * 1000,
    ...
    proc.on("close", (code) => {
      ...
      let parsed: { successes: AttackSuccess[]; totalAttempts: number; errors: string[]; metadata?: Record<string, any> };
      ...
      resolve({
        ...
        rawOutput: fullOutput.substring(0, 10000),
        metadata: (parsed as any).metadata || { exitCode: code },
      });
    });
...
function getInstallCommand(tool: ExternalTool): string {
  switch (tool) {
    case "hydra": return "sudo apt-get install -y hydra";
    case "medusa": return "sudo apt-get install -y medusa";
    case "netexec": return "pip install netexec";
    default: return "N/A";
  }
}
...
/** Get the full tool knowledge base for display in the UI */
export function getToolKnowledgeBase(): typeof TOOL_KNOWLEDGE_BASE {
  return TOOL_KNOWLEDGE_BASE;
}

/** Get the LLM system prompt (for debugging/transparency) */
export function getToolSelectionPrompt(): string {
  return TOOL_SELECTION_SYSTEM_PROMPT;
}

/**
 * Quick protocol-to-tool recommendation without LLM.
 * Returns the best tool for a given protocol based on the knowledge base.
 */
export function quickToolRecommendation(protocol: string, isAD: boolean = false): ExternalTool {
  ...
  const hydraFirst = [
    "ssh", "ftp", "telnet", "http_form", "http_basic", "http_digest", "http_json_api",
    "mysql", "postgresql", "postgres", "mssql", "redis", "mongodb",
    "vnc", "smtp", "pop3", "imap", "snmp", "ldap", "rdp",
  ];
