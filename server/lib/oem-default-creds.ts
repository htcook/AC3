 ...
 * 
 * Data is loaded at runtime from the DO scan server's knowledge API
 * instead of being embedded inline (saves ~60KB from the deployment bundle).
 *
 * Consumed by:
...
import { loadKnowledgeData } from "./knowledge/knowledge-loader";
...
/** Type for a single credential entry */
type CredEntry = {
  vendor: string;
  product: string;
  version?: string;
...
/** Singleton: loaded once from DO, cached in memory */
let _credsLoaded = false;
let _creds: CredEntry[] = [];

async function ensureCreds(): Promise<CredEntry[]> {
  if (_credsLoaded) return _creds;
  _creds = await loadKnowledgeData<CredEntry[]>("oem_default_creds.json", []);
  _credsLoaded = true;
  return _creds;
}

/** 
 * Synchronous access to loaded credentials.
 * Returns the cached data if loaded, or empty array if not yet initialized.
 * Call initOemCreds() at server startup to ensure data is available.
 */
export function getBuiltinCreds(): CredEntry[] {
  return _creds;
}

/** Backwards-compatible export — uses Proxy for sync access after init */
export const BUILTIN_DEFAULT_CREDS: CredEntry[] = new Proxy([] as CredEntry[], {
...
/** Eagerly load credentials (call at server startup) */
export async function initOemCreds(): Promise<void> {
  await ensureCreds();
}
 ...
 */
export async function matchCredentialsForTechnology(tech: {
  name?: string;
  vendor?: string;
  version?: string;
  cpe?: string;
 ...
 */
export async function matchCredentialsForAsset(technologies: Array<{
  name?: string;
  vendor?: string;
  version?: string;
  cpe?: string;
...
/**
 * Search the DB for credentials matching a query.
 */
 ...
 */
export async function matchCredentialsForAssets(assets: Array<{
  hostname: string;
  technologies: string[];
...
/**
 * Persist matched credentials to the database.
 */
