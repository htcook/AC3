/**
 * FIPS 140-3 DigitalOcean Infrastructure Hardening
 *
 * Implements FIPS 140-3 compliance for all DigitalOcean infrastructure:
 *   - VPC-only networking (no public endpoints for internal services)
 *   - FIPS-approved TLS 1.2+ for all inter-service communication
 *   - mTLS enforcement between workers and Redis
 *   - Firewall rules blocking all inbound except from authorized sources
 *   - SSH hardening with FIPS-approved algorithms only
 *   - Audit logging for all infrastructure operations
 *   - Key rotation schedules for all credentials
 *
 * Network Architecture:
 *   ┌─────────────────────────────────────────────────────────┐
 *   │  DigitalOcean VPC (10.132.0.0/20) — Private Network    │
 *   │                                                         │
 *   │  ┌──────────┐  ┌──────────┐  ┌──────────┐             │
 *   │  │  Redis    │  │  Scan    │  │  OSINT   │             │
 *   │  │  Queue    │←→│  Worker  │  │  Worker  │             │
...
export interface FIPSInfraAuditEntry {
  timestamp: number;
  action: string;
  resource: string;
  resourceId: string;
...
export interface InfraComplianceReport {
  timestamp: number;
...
const auditLog: FIPSInfraAuditEntry[] = [];
const MAX_AUDIT_ENTRIES = 10_000;

function logAudit(entry: Omit<FIPSInfraAuditEntry, "timestamp">): void {
  const full: FIPSInfraAuditEntry = { ...entry, timestamp: Date.now() };
  auditLog.push(full);
...
export function getAuditLog(limit = 100): FIPSInfraAuditEntry[] {
...
/**
 * Generate the VPC configuration for Caldera infrastructure.
 * All services MUST be deployed within this VPC.
 */
...
/**
 * Create a VPC via DigitalOcean API.
 */
export async function createVPC(): Promise<{ id: string; ipRange: string }> {
...
/**
 * List existing VPCs to find the Caldera VPC.
 */
export async function findCalderaVPC(): Promise<{ id: string; ipRange: string } | null> {
  const token = ENV.DIGITALOCEAN_ACCESS_TOKEN;
  ...
  const data = await response.json();
  const vpc = (data.vpcs || []).find((v: any) => v.name === "caldera-fips-vpc");
...
/**
 * Generate FIPS-compliant firewall rules for each service type.
 * All rules enforce VPC-only access with no public inbound.
 */
export function getFirewallConfigs(): Record<string, FirewallRuleSet> {
  return {
...
/**
 * Create a firewall via DigitalOcean API.
 */
export async function createFirewall(config: FirewallRuleSet): Promise<string> {
  const token = ENV.DIGITALOCEAN_ACCESS_TOKEN;
  if (!token) throw new Error("DIGITALOCEAN_ACCESS_TOKEN not configured");
...
/**
 * Generate FIPS-compliant sshd_config for DO droplets.
 * Enforces FIPS-approved algorithms only.
 */
export function generateFIPSSSHDConfig(options: {
  allowedUsers?: string[];
  allowedIPs?: string[];
  port?: number;
} = {}): string {
...
/**
 * Generate SSH banner for compliance.
 */
export function generateSSHBanner(): string {
  return `
*******************************************************************************
*                                                                             *
...
// ─── Droplet Provisioning with FIPS Hardening ───────────────────────────────

/**
 * Generate cloud-init user data for a FIPS-hardened scan worker.
 * Includes: FIPS SSH config, firewall rules, no public services.
 */
export function generateFIPSWorkerUserData(options: {
  workerType: "scan" | "osint" | "c2" | "redis";
  vpcSubnet?: string;
  redisHost?: string;
  apiCallbackUrl?: string;
} = { workerType: "scan" }): string {
...
/**
 * Run a comprehensive FIPS 140-3 compliance check on the DO infrastructure.
 */
export async function runComplianceCheck(): Promise<InfraComplianceReport> {
  ...
  checks.push({
    ...
    details: vpc
      ? `VPC configured: ${vpc.id} (${vpc.ipRange})`
      : "No Caldera VPC found. Create one with createVPC().",
    remediation: vpc ? undefined : "Run createVPC() to create an isolated VPC for all Caldera infrastructure.",
  });
  ...
  checks.push({
    ...
    details: ENV.JWT_SECRET
      ? "JWT secret configured for HMAC operations."
      : "JWT_SECRET not configured — HMAC integrity checks disabled.",
    remediation: ENV.JWT_SECRET ? undefined : "Set JWT_SECRET environment variable.",
  });
  ...
  checks.push({
    ...
    details: allNoPublicInbound
      ? "All firewall configs block public inbound. Only VPC/tag-based access allowed."
      : "Some firewall configs allow public inbound access.",
    remediation: allNoPublicInbound ? undefined : "Review firewall configs and remove 0.0.0.0/0 from inbound rules.",
  });
  ...
  checks.push({
    ...
    remediation: auditLog.length > 0 ? undefined : "Audit trail is empty — infrastructure operations will be logged as they occur.",
  });

  // SC-23: Session Authenticity (mTLS)
  checks.push({
    id: "sc-23-mtls",
  ...
  checks.push({
    ...
    details: ENV.DIGITALOCEAN_ACCESS_TOKEN
      ? "DO API token configured. SSH keys managed via DO API."
      : "DO API token not configured — infrastructure provisioning unavailable.",
  });
  ...
  const report: InfraComplianceReport = {
    timestamp: Date.now(),
    overallCompliant,
    checks,
    recommendations,
...
const keyRotationSchedules: KeyRotationSchedule[] = [
  {
    ...
    autoRotate: false, // Requires manual rotation to avoid lockout
  },
  {
    ...
    lastRotated: Date.now(),
    nextRotation: Date.now() + 30 * 24 * 60 * 60 * 1000,
    autoRotate: true,
  },
  {
    ...
    lastRotated: Date.now(),
    nextRotation: Date.now() + 7 * 24 * 60 * 60 * 1000,
    autoRotate: true,
  },
  {
...
export function getKeyRotationSchedules(): KeyRotationSchedule[] {
  return keyRotationSchedules.map((s) => ({
    ...s,
    nextRotation: s.lastRotated + s.rotationIntervalDays * 24 * 60 * 60 * 1000,
  }));
}

export function getOverdueRotations(): KeyRotationSchedule[] {
  const now = Date.now();
...
export function getInfrastructureSummary(): {
  vpc: VPCConfig;
  firewalls: Record<string, { name: string; inboundRules: number; outboundRules: number; publicInbound: boolean }>;
  sshHardening: { fipsAlgorithms: boolean; passwordAuth: boolean; vpcOnly: boolean };
  keyRotation: { total: number; overdue: number };
  auditEntries: number;
  nistControls: string[];
} {
  const firewallConfigs = getFirewallConfigs();
  const firewalls: Record<string, any> = {};
  ...
  return {
    ...
    nistControls: [
      "AC-4", "AC-17", "AU-2", "AU-3", "CA-2", "CA-7",
      "IA-5", "SC-7", "SC-8", "SC-12", "SC-13", "SC-17", "SC-23",
    ],
  };
}
