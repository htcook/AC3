 ...
 */
export async function claimEngagement(engagementId: number): Promise<{
  claimed: boolean;
  ...
  try {
    ...
    if (currentOwner && currentOwner !== SERVER_INSTANCE_ID) {
      ...
      if (age < CLAIM_EXPIRY_MS) {
        // Another server has a fresh claim — deny
        return {
          claimed: false,
          currentOwner,
    ...
    // Step 2: Atomic CAS — claim only if still unclaimed or stale
