/**
 * NIST 800-115 Aligned Test Plan Generator
 *
 * Generates formal penetration test plans and red team exercise plans
 * aligned with NIST SP 800-115, PTES, OWASP, and MITRE ATT&CK standards.
 * Incorporates NIST SP 800-81r3 (March 2026) DNS security assessment
 * guidance for comprehensive infrastructure testing.
 *
 * The test plan is generated AFTER passive discovery and scoping (RoE)
 * but BEFORE active scanning begins. This is the optimal stage because:
 *   1. We have complete asset inventory from passive recon
 *   2. RoE defines authorized scope and constraints
 *   3. We can accurately plan methodology, tools, and timeline
 *   4. Customer reviews and approves before active testing
 *
 * Supports two plan types:
 *   - Penetration Test Plan
 *   - Red Team Exercise Plan (same standards + C2/persistence/exfil objectives)
 *
...
export type TestPlanType = "penetration_test" | "red_team_exercise";

/**
 * Assessment attack vectors — standard categories for comprehensive
 * security assessment coverage. These represent the primary threat
 * entry points that should be evaluated during any assessment.
 */
export type AssessmentAttackVector =
  | "av1_social_engineering"             // Social Engineering / Phishing
  | "av2_external_network"               // External Network Attack Surface
  | "av2_insider_threat"                 // Insider Threat Assessment
  | "av3_web_application"                // Web Application Attack Surface
  | "av4_tenant_isolation"               // Multi-Tenant Isolation Testing
  | "av5_mobile_application"             // Mobile Application Security
  | "av6_client_side_application"        // Client-Side Application/Agent Testing
  | "av7_dns_infrastructure"             // DNS Infrastructure Security (per NIST SP 800-81r3)
  | "av8_cloud_infrastructure"           // Cloud Infrastructure Misconfiguration
  | "av9_api_security";                  // API Security Testing
...
export interface TestPlanInput {
  ...
  /** Engagement type from the engagement record */
  ...
  roe: {
    ...
    escalationContacts?: Array<{ name: string; role: string; phone?: string; email?: string }>;
    emergencyProcedure?: string;
    dataHandling?: string;
    signedBy?: string;
    signedAt?: string;
  };
  /** Passive recon results per asset */
  passiveReconResults: Record<string, PassiveReconSummary>;
  /** Asset inventory from passive discovery */
  assets: Array<{
    hostname: string;
    ip?: string;
    type: string;
    ...
    wafDetected?: string;
    certificates?: Array<{ subject: string; issuer?: string; validTo?: string }>;
  }>;
  /** Applicable assessment attack vectors */
  applicableAttackVectors?: AssessmentAttackVector[];
  /** Compliance frameworks to assess against */
  complianceFrameworks?: string[];
  /** Scan profile preference */
  scanProfile?: "quick" | "standard" | "deep" | "stealth";
  /** DNS security assessment data (per NIST SP 800-81r3) */
  dnsAssessmentData?: {
    domains: string[];
    nameservers?: Array<{ hostname: string; ip: string; provider?: string }>;
    dnssecEnabled?: boolean;
    encryptedDnsSupport?: { dot: boolean; doh: boolean; doq: boolean };
    protectiveDnsDeployed?: boolean;
    danglingCnames?: string[];
    lameDelegations?: string[];
    zoneTransferExposed?: boolean;
  };
  /** Red team specific: C2 infrastructure plan */
  c2Plan?: {
    framework?: string;
    infrastructure?: string[];
    communicationChannels?: string[];
  };
  /** Red team specific: objectives */
...
export interface PassiveReconSummary {
  subdomains: string[];
  ipAddresses: string[];
  technologies: string[];
  services: Array<{ port: number; service: string; version?: string }>;
  ...
  emailSecurity?: { spf: boolean; dkim: boolean; dmarc: boolean };
  breachExposure?: { count: number; sources: string[] };
  /** DNS-specific recon data (per NIST SP 800-81r3) */
  dnsRecords?: {
    ns: string[];
    mx: string[];
    txt: string[];
    cname: Array<{ name: string; target: string }>;
    soa?: { primary: string; refresh: number; retry: number; expire: number; ttl: number };
...
export interface TestPlanSection {
  ...
  title: string;
  content: string;
  /** NIST 800-115 section reference */
  nistReference?: string;
  /** Additional standards reference */
...
export interface TestPlan {
  ...
  /** Structured data extracted from the plan */
  structuredData: {
    /** Total estimated duration */
    estimatedDuration: string;
    /** Attack vectors to be tested */
    attackVectors: Array<{
      id: AssessmentAttackVector | string;
      name: string;
      description: string;
      targets: string[];
      methodology: string;
  ...
  /** Approval tracking */
  approvalStatus: "pending" | "approved" | "denied" | "revision_requested";
...
export async function generateTestPlan(input: TestPlanInput): Promise<TestPlan> {
  const planId = `TP-${input.engagementId}-${Date.now()}`;
  const isRedTeam = input.planType === "red_team_exercise" ||
                    input.engagementType === "red_team" ||
                    input.engagementType === "purple_team";

  // Build comprehensive context for the LLM
  const context = buildPlanContext(input);

  // Generate the plan sections using LLM
  const sections = await generatePlanSections(input, context, isRedTeam);

  // Generate structured data (attack vectors, tools, schedule)
  ...
  const plan: TestPlan = {
    ...
    title: isRedTeam
      ? `Red Team Exercise Plan — ${input.organizationName}`
      : `Penetration Test Plan — ${input.organizationName}`,
    subtitle: input.systemName
      ? `${input.systemName} — Security Assessment`
      : `Security Assessment for ${input.engagementName}`,
...
function buildPlanContext(input: TestPlanInput): string {
  const lines: string[] = [];
  ...
  for (const [domain, recon] of Object.entries(input.passiveReconResults)) {
    ...
    // DNS-specific recon data (NIST SP 800-81r3 aligned)
    if (recon.dnsRecords) {
  ...
  // DNS Assessment Data (NIST SP 800-81r3)
...
async function generatePlanSections(
  input: TestPlanInput,
  context: string,
  isRedTeam: boolean,
): Promise<TestPlanSection[]> {
...
function getSectionDefinitions(input: TestPlanInput, isRedTeam: boolean): SectionDefinition[] {
...
${isRedTeam ? "- Red team specific objectives (data exfiltration targets, persistence goals, lateral movement objectives)" : ""}`,
      nistReference: "NIST SP 800-115 §3.2",
      standardsReference: "PTES §2 — Intelligence Gathering",
    },
    {
...
- Evidence handling and chain of custody requirements`,
      nistReference: "NIST SP 800-115 §3.3",
      standardsReference: "PTES §2.1 — Pre-engagement Interactions",
    },
    {
      id: "methodology",
...
- Assessment attack vector coverage:
  ...
  AV9: API Security Testing`,
      ...
      standardsReference: "PTES §3-7 — Full Methodology",
    },
    {
      id: "attack_vectors",
...
For each vector, note whether it is applicable based on the asset inventory and scope. If not applicable, explain why.`,
      nistReference: "NIST SP 800-115 §4.3",
      standardsReference: "PTES §4 — Vulnerability Analysis",
    },
  ];

  // DNS Security Assessment section (NIST SP 800-81r3)
  if (hasDnsData) {
    sections.push({
...
Use the DNS assessment data from the context to tailor the plan to the specific targets.`,
      nistReference: "NIST SP 800-115 §4.2",
...
- Cloud-specific tools if applicable`,
      nistReference: "NIST SP 800-115 §4.2",
      standardsReference: "PTES §4.4 — Active Vulnerability Scanning",
    },
    {
      id: "schedule",
...
Base the schedule on the asset count, complexity, and scope from the context.`,
      nistReference: "NIST SP 800-115 §3.4",
      standardsReference: "PTES §2.1 — Timeline",
    },
    {
      id: "risk_management",
...
- DNS-specific risks: potential for zone disruption during AXFR testing, DNSSEC validation impact`,
      nistReference: "NIST SP 800-115 §3.5",
      standardsReference: "PTES §2.1 — Risk Management",
    },
    {
...
- Final report delivery timeline and format`,
      nistReference: "NIST SP 800-115 §3.6",
    },
    {
      id: "deliverables",
...
For each deliverable, specify format, content outline, and delivery timeline.`,
      nistReference: "NIST SP 800-115 §5",
      standardsReference: "PTES §7 — Reporting",
    },
    {
...
- Background check status (if required)`,
      nistReference: "NIST SP 800-115 §3.7",
      standardsReference: "PTES §2.1 — Team Qualifications",
    },
  );

  // Red Team specific sections
  if (isRedTeam) {
    sections.push(
      {
...
async function generateStructuredData(
  ...
  context: string,
  isRedTeam: boolean,
): Promise<TestPlan["structuredData"]> {
...
function buildFallbackStructuredData(
  input: TestPlanInput,
  isRedTeam: boolean,
): TestPlan["structuredData"] {
  ...
  return {
    ...
    attackVectors: [
      {
        ...
        tools: ["subfinder", "amass", "httpx", "nmap", "Shodan API", "crt.sh"],
    ...
    toolInventory: [
      { tool: "nmap", purpose: "Port scanning and service enumeration", phase: "Active Discovery", license: "GPL-2.0" },
