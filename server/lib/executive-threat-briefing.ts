/**
 * Executive Threat Briefing — Dynamic Actor-to-Enterprise Matching Engine
 *
 * Correlates threat actors to a client's specific attack surface by cross-referencing:
 * - Domain intel scan data (sector, clientType, assets, findings)
 * - CARVER risk cards (criticality scores, threat likelihood, priority tier)
 * - Discovered assets (CARVER scores, risk bands, technologies)
 * - Threat actor catalog (targetSectors, techniques, tools, malware, IOCs)
 * - Threat group events (temporal trends, recent activity)
 *
 * Re-computes on every query from live data — no static snapshots.
 */
import { getDb } from "../db";
import { sql, eq, desc, and, gte, isNotNull, inArray } from "drizzle-orm";
import {
  domainIntelScans,
  carverRiskCards,
  discoveredAssets,
...
// Maps various sector labels to canonical forms for matching
...
function sectorsOverlap(actorSectors: string[], clientSector: string): { match: boolean; matchedSectors: string[] } {
...
interface RelevanceFactors {
  sectorMatch: number;       // 0-40 points
  threatLevelWeight: number; // 0-20 points
  carverAlignment: number;   // 0-20 points
  recentActivity: number;    // 0-10 points
  iocOverlap: number;        // 0-10 points
}

function computeRelevanceScore(factors: RelevanceFactors): number {
  return Math.min(100, Math.round(
    factors.sectorMatch +
    factors.threatLevelWeight +
    factors.carverAlignment +
...
export interface ThreatBriefingInput {
  scanId?: number;       // specific domain intel scan
  sector?: string;       // override sector
  limit?: number;        // max actors to return (default 15)
}
...
export interface MatchedActor {
  ...
  topTechniques: Array<{ id: string; name: string; tactic: string }>;
  topTools: string[];
  recentEvents: Array<{ title: string; severity: string; date: string | null }>;
  iocCount: number;
  recommendedActions: string[];
  attackVectors: string[];
}

export interface ThreatBriefingResult {
  scan: {
    id: number;
    domain: string;
    sector: string | null;
  ...
  trends: {
    eventsByMonth: Array<{ month: string; count: number; critical: number; high: number }>;
    actorActivityTrend: Array<{ actorId: string; name: string; eventsLast30d: number; eventsLast90d: number; trend: "rising" | "stable" | "declining" }>;
  };
  carverProfile: {
    ...
    avgEffect: number;
    avgRecuperability: number;
    avgRecognizability: number;
...
export async function computeExecutiveThreatBriefing(input: ThreatBriefingInput): Promise<ThreatBriefingResult> {
  ...
  if (input.scanId) {
    ...
    if (scan) {
      ...
      // Load discovered assets for this scan
  ...
  // If no scan selected, get the most recent completed scan
  if (!scanData) {
    const [latestScan] = await db.select().from(domainIntelScans)
      .where(eq(domainIntelScans.status, "completed"))
      .orderBy(desc(domainIntelScans.updatedAt))
      .limit(1);
    if (latestScan) {
      scanData = {
        id: latestScan.id,
  ...
  // ── Step 2: Load CARVER profile for the scan ───────────────────────────────
  let carverProfile: ThreatBriefingResult["carverProfile"] = null;
  if (scanData) {
    const carverCards = await db.select().from(carverRiskCards)
      .where(eq(carverRiskCards.domainIntelScanId, scanData.id))
      .limit(100);
    ...
    if (carverCards.length > 0) {
      ...
      const threatLikelihoodAgg: Record<string, number[]> = {};
      ...
      carverProfile = {
        ...
        priorityBreakdown,
        topThreatLikelihoods: Object.entries(threatLikelihoodAgg)
          .map(([threat, vals]) => ({
  ...
  // ── Step 3: Load all threat actors with parsed fields ──────────────────────
  const allActors = await db.select().from(threatActors).limit(500);

  // ── Step 4: Load IOC counts per actor ──────────────────────────────────────
  ...
  // ── Step 5: Load recent events for trend analysis ──────────────────────────
  const cutoff90d = new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).toISOString().slice(0, 19).replace("T", " ");
  const recentEvents = await db.select({
    ...
    .where(gte(threatGroupEvents.eventDate, cutoff90d))
    .orderBy(desc(threatGroupEvents.eventDate))
    .limit(2000);

  // Build per-actor event counts
  const actorEvents30d = new Map<string, number>();
  const actorEvents90d = new Map<string, number>();
  const cutoff30d = Date.now() - 30 * 24 * 60 * 60 * 1000;
  const monthlyEvents: Record<string, { count: number; critical: number; high: number }> = {};
  ...
  // ── Step 6: Score and rank actors ──────────────────────────────────────────
  const limit = input.limit || 15;
  const scoredActors: MatchedActor[] = [];

  // Extract top threat likelihoods from CARVER for alignment scoring
  ...
  for (const actor of allActors) {
    ...
    if (clientSector && targetSectors.length > 0) {
      ...
      if (match) {
        ...
        // Partial credit for adjacent sectors (e.g., defense actor targeting government client)
        const clientNorm = normalizeSector(clientSector);
      ...
      // Actors with no sector targeting are somewhat relevant to everyone
      sectorMatchScore = 10;
    }

    // ── Threat Level Weight (0-20 points) ──
    const threatLevelMap: Record<string, number> = { critical: 20, high: 15, medium: 10, low: 5 };
    const threatLevelWeight = threatLevelMap[actor.threatLevel || "medium"] || 10;

    // ── CARVER Alignment (0-20 points) ──
    ...
    if (carverProfile && topThreatTypes.length > 0) {
      ...
      // Check if actor type/motivation aligns with CARVER threat likelihoods
      for (const threatType of topThreatTypes) {
        if (
          (threatType.includes("ransomware") && (actorType === "ransomware" || actorType === "cybercrime")) ||
          (threatType.includes("apt") && actorType === "apt") ||
          (threatType.includes("espionage") && (motivation.includes("espionage") || actorType === "apt")) ||
          (threatType.includes("credential") && techniques.some((t: any) => (t.tactic || "").includes("credential"))) ||
          (threatType.includes("supply") && techniques.some((t: any) => (t.name || "").toLowerCase().includes("supply"))) ||
          (threatType.includes("api") && techniques.some((t: any) => (t.name || "").toLowerCase().includes("api")))
        ) {
          carverAlignment = Math.min(20, carverAlignment + 7);
        }
      }
      // Bonus for high CARVER criticality + high threat level actor
    ...
    // Only include actors with some relevance
    if (relevanceScore < 15) continue;

    // Build attack vectors based on techniques
    ...
    for (const t of techniques.slice(0, 20)) {
      ...
      if (tactic && !tacticSet.has(tactic)) {
        ...
        attackVectors.push(tactic.replace(/-/g, " ").replace(/\b\w/g, (c: string) => c.toUpperCase()));
      }
    }
    ...
    if (techniques.some((t: any) => (t.tactic || "").includes("initial-access"))) {
      recommendedActions.push("Review perimeter controls against initial access techniques");
    }

    // Get actor's recent events
    const actorRecentEvents = recentEvents
      .filter(e => e.actorId === actor.actorId)
      .slice(0, 3)
      .map(e => ({ title: e.title, severity: e.severity || "medium", date: e.date }));
    ...
    scoredActors.push({
      ...
      matchedSectors,
      topTechniques: techniques.slice(0, 5).map((t: any) => ({
        id: typeof t === "string" ? t : (t.id || ""),
        name: typeof t === "string" ? t : (t.name || ""),
        tactic: typeof t === "string" ? "" : (t.tactic || ""),
      })),
      topTools: tools.slice(0, 5).map((t: any) => typeof t === "string" ? t : (t.name || t)),
  ...
  const topAttackVectors = [...vectorCounts.entries()]
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5)
    .map(([v]) => v);

  const avgRelevance = topActors.length > 0
    ? Math.round(topActors.reduce((s, a) => s + a.relevanceScore, 0) / topActors.length)
    : 0;

  const sectorRiskLevel = criticalActors >= 3 ? "critical" :
    criticalActors >= 1 || highActors >= 3 ? "high" :
    highActors >= 1 ? "elevated" :
    topActors.length > 0 ? "moderate" : "low";

  // ── Step 8: Build trends ───────────────────────────────────────────────────
  const eventsByMonth = Object.entries(monthlyEvents)
    .filter(([m]) => m !== "unknown")
  ...
  const actorActivityTrend = topActors.slice(0, 10).map(a => {
    ...
    const trend: "rising" | "stable" | "declining" =
      e30 > e60to90 * 1.5 ? "rising" :
      e30 < e60to90 * 0.5 ? "declining" : "stable";
    return {
      actorId: a.actorId,
      name: a.name,
      eventsLast30d: e30,
...
// ─── Scan list for selector ──────────────────────────────────────────────────
export async function getRecentScansForBriefing() {
  const db = await getDb();
  if (!db) return [];
...
// ─── Helpers ─────────────────────────────────────────────────────────────────
function safeParseArr(val: any): any[] {
  if (!val) return [];
