/**
 * Rule Validation Against Evidence
 * 
 * Tests generated detection rules against the actual exploit evidence
 * captured during validation runs. This closes the loop:
 * 
 *   Exploit → Evidence → Rule Generation → Rule Validation Against Evidence
 * 
 * For each generated rule, this module:
 * 1. Parses the rule's detection logic (patterns, strings, conditions)
 * 2. Tests those patterns against captured evidence artifacts
 * 3. Reports whether the rule would have detected the attack
 * 4. Calculates detection confidence and false positive risk
 * 
 * @module rule-evidence-validator
 */

import type { GeneratedRule, RuleFormat } from "./llm-rule-generator";

// ─── Types ─────────────────────────────────────────────────────────

export interface EvidenceArtifact {
  id: string;
  type: "console_output" | "session_info" | "evidence_report" | "text_screenshot" | "network_capture" | "file_artifact";
...
export interface RuleValidationResult {
  ...
  coveragePercent: number;     // % of rule patterns that matched evidence
...
interface ExtractedPattern {
  pattern: string;
  type: "string" | "regex" | "keyword" | "port" | "protocol";
  required: boolean;
}

/**
 * Extract testable patterns from a detection rule.
 */
function extractPatternsFromRule(rule: GeneratedRule): ExtractedPattern[] {
...
function extractSigmaPatterns(content: string): ExtractedPattern[] {
  const patterns: ExtractedPattern[] = [];
  ...
  for (const line of lines) {
    ...
    if (inDetection) {
      // Extract field values from detection section
...
function extractYaraPatterns(content: string): ExtractedPattern[] {
  const patterns: ExtractedPattern[] = [];
  
  // Extract string definitions: $name = "value"
  ...
  // Extract hex patterns: $name = { XX XX XX }
...
function extractSnortPatterns(content: string): ExtractedPattern[] {
  const patterns: ExtractedPattern[] = [];
  
  // Extract content matches: content:"value"
...
function extractKqlPatterns(content: string): ExtractedPattern[] {
  const patterns: ExtractedPattern[] = [];
...
function extractSplPatterns(content: string): ExtractedPattern[] {
  const patterns: ExtractedPattern[] = [];
...
/**
 * Validate a single rule against a single evidence artifact.
 */
export function validateRuleAgainstEvidence(
  rule: GeneratedRule,
  ...
  const detected = requiredPatterns.length > 0
    ? requiredMatched >= Math.ceil(requiredPatterns.length * 0.5)
    : matchCount > 0;
  
  // Calculate confidence
  let detectionConfidence: RuleValidationResult["detectionConfidence"];
  if (!detected) {
  ...
  const falsePositiveRisk: RuleValidationResult["falsePositiveRisk"] =
    patterns.length <= 1 ? "high" :
    patterns.length <= 3 ? "medium" : "low";
...
/**
 * Validate multiple rules against multiple evidence artifacts.
 */
export function batchValidateRules(
  rules: GeneratedRule[],
  evidence: EvidenceArtifact[]
  ...
  for (const rule of rules) {
    // Test each rule against all evidence, take best result
    let bestResult: RuleValidationResult | null = null;
  ...
  const summary = `Validated ${results.length} rule(s) against ${evidence.length} evidence artifact(s). ` +
    `${rulesDetected} detected the attack (${Math.round((rulesDetected / Math.max(results.length, 1)) * 100)}% detection rate). ` +
    `Average pattern coverage: ${avgCoverage}%.`;
