/**
 * Bug Bounty Submission Optimizer
 * 
 * Formats findings for specific program requirements, generates clean
 * reproduction steps, crafts impact narratives for triage teams, and
 * ensures severity justification aligns with program taxonomy.
 * 
 * "Programs respond better to clean submissions; the LLM can ensure consistency."
 */
...
const VULN_CVSS_DEFAULTS: Record<string, CVSSEstimate> = {
  rce: {
...
const VULN_CWE_MAP: Record<string, { id: string; name: string }> = {
...
interface PlatformGuidelines {
  titleFormat: (title: string, severity: string) => string;
  severityScale: string[];
  requiresCWE: boolean;
  requiresCVSS: boolean;
...
function assessSubmissionQuality(submission: OptimizedSubmission, input: FindingInput): { score: number; issues: string[] } {
  let score = 100;
  const issues: string[] = [];

  // Title quality
  if (submission.title.length < 20) { score -= 10; issues.push('Title is too short — should clearly describe the vulnerability'); }
  if (submission.title.length > 150) { score -= 5; issues.push('Title is too long — consider shortening for readability'); }

  // Reproduction steps
  if (submission.reproductionSteps.length < 2) { score -= 20; issues.push('Too few reproduction steps — include at least 3 clear steps'); }
  if (submission.reproductionSteps.length < 3) { score -= 10; issues.push('Consider adding more detailed reproduction steps'); }

  // Evidence
  if (!input.reproductionEvidence || input.reproductionEvidence.length === 0) {
    score -= 15; issues.push('No reproduction evidence provided — add request/response pairs or screenshots');
  }

  // Impact statement
  if (submission.impactStatement.length < 50) { score -= 10; issues.push('Impact statement is too brief — explain the real-world consequences'); }

  // Severity justification
  if (submission.severityJustification.length < 30) { score -= 10; issues.push('Severity justification needs more detail'); }

  // CWE
  if (!submission.cweId || submission.cweId === 'CWE-0') { score -= 5; issues.push('Missing CWE classification'); }

  // Technical details
  if (submission.technicalDetails.length < 100) { score -= 10; issues.push('Technical details section needs more depth'); }

  // Remediation
  if (submission.remediation.length < 50) { score -= 5; issues.push('Remediation guidance could be more specific'); }

  return { score: Math.max(score, 0), issues };
}

// ─── Core Submission Optimization ────────────────────────────────────────────

function generateReproductionSteps(input: FindingInput): ReproductionStep[] {
  const steps: ReproductionStep[] = [];
  let stepNum = 1;

  // Step 1: Navigate to the affected endpoint
  steps.push({
    stepNumber: stepNum++,
  ...
  // Step 2: Prepare the attack payload (vuln-class specific)
...
function getPayloadStep(vulnClass: string, endpoint: string): Omit<ReproductionStep, 'stepNumber'> | null {
  const payloads: Record<string, Omit<ReproductionStep, 'stepNumber'>> = {
...
function getImpactDescription(vulnClass: string): string {
...
function generateImpactStatement(input: FindingInput): string {
  const impact = getImpactDescription(input.vulnClass);
  const severityContext = input.severity === 'critical' || input.severity === 'high'
    ? 'This vulnerability poses a significant risk to the application and its users.'
    : 'This vulnerability could be leveraged by an attacker to compromise user security.';
...
function generateSeverityJustification(input: FindingInput): string {
...
export function optimizeSubmission(input: FindingInput): OptimizedSubmission {
  const platform = input.platform || 'other';
  const guidelines = PLATFORM_GUIDELINES[platform] || PLATFORM_GUIDELINES.other;

  // Format title per platform
  const formattedTitle = guidelines.titleFormat(input.title, input.severity);
  const truncatedTitle = formattedTitle.length > guidelines.maxTitleLength
    ? formattedTitle.substring(0, guidelines.maxTitleLength - 3) + '...'
    : formattedTitle;
  ...
  const submission: OptimizedSubmission = {
...
/**
 * Batch optimize multiple submissions
 */
export function batchOptimizeSubmissions(inputs: FindingInput[]): OptimizedSubmission[] {
  return inputs.map(input => optimizeSubmission(input));
}

/**
 * Get submission quality statistics for a batch
 */
export function getSubmissionQualityStats(submissions: OptimizedSubmission[]): {
