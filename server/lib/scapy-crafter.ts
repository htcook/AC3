/**
 * Scapy Packet Crafter — Custom Packet Construction & Injection
 * ═══════════════════════════════════════════════════════════════
 * Provides packet crafting capabilities via Python/Scapy on the scan server.
 *
 * Architecture:
 *   Dashboard → generates Python/Scapy script → SSH → scan server → execute → parse results
 *
 * Capabilities:
 *   - Custom TCP/UDP/ICMP packet construction
 *   - Protocol-specific probes (SYN, ACK, FIN, XMAS, NULL scans)
 *   - OS fingerprinting via crafted packets
 *   - Firewall rule mapping via ACK/Window probes
 *   - ICMP tunneling detection probes
 *   - DNS amplification testing
 *   - ARP scanning and discovery
 *   - Traceroute with custom TTL manipulation
 *   - TCP ISN prediction analysis
 *   - IP ID sequence analysis
 *
 * @module scapy-crafter
 */
...
export type PacketProtocol = "tcp" | "udp" | "icmp" | "arp" | "dns" | "raw";
export type TcpFlagSet = "S" | "SA" | "A" | "F" | "FA" | "R" | "RA" | "P" | "PA" | "FPU" | "" | "SF";

export interface CraftedPacketConfig {
  /** Target IP or hostname */
  target: string;
  /** Target port(s) — single or range */
  ports?: number[];
  /** Protocol to craft */
  ...
  delay?: number;
  /** Timeout for response wait (seconds) */
  timeout?: number;
  /** Whether to capture responses */
  captureResponses?: boolean;
  /** Engagement ID for audit trail */
  engagementId?: number;
}

export interface ScapyProbeResult {
  ...
  durationMs: number;
  /** Detailed per-packet results */
  results: ScapyPacketResult[];
  /** Summary analysis */
  analysis: string;
  /** Raw Scapy output */
  rawOutput: string;
...
export interface ScapyPacketResult {
  /** Destination port */
  dstPort?: number;
  /** Response received? */
  responded: boolean;
  ...
  portState?: "open" | "closed" | "filtered" | "open|filtered" | "unfiltered";
  /** Response type (e.g., "SA", "R", "ICMP unreachable") */
  responseType?: string;
  /** OS hint from response */
  osHint?: string;
...
export interface ProbeConfig {
  ...
  target: string;
  ports?: number[];
  /** Additional options per template */
  options?: Record<string, any>;
  engagementId?: number;
}

// ═══════════════════════════════════════════════════════════════
// §2 — SCAPY SCRIPT GENERATORS
// ═══════════════════════════════════════════════════════════════

/**
 * Generate a Scapy Python script for a given probe template.
 * The script outputs JSON results to stdout for parsing.
 */
export function generateScapyScript(config: ProbeConfig): string {
...
function scriptPostamble(probeType: string): string {
  return `
...
function generateSynScanScript(target: string, ports: number[]): string {
...
function generateAckScanScript(target: string, ports: number[]): string {
  return `${scriptPreamble()}
target = "${target}"
ports = ${JSON.stringify(ports)}
for port in ports:
...
function generateFinScanScript(target: string, ports: number[]): string {
  return `${scriptPreamble()}
target = "${target}"
ports = ${JSON.stringify(ports)}
for port in ports:
...
function generateXmasScanScript(target: string, ports: number[]): string {
  return `${scriptPreamble()}
target = "${target}"
ports = ${JSON.stringify(ports)}
for port in ports:
...
function generateNullScanScript(target: string, ports: number[]): string {
  return `${scriptPreamble()}
target = "${target}"
ports = ${JSON.stringify(ports)}
for port in ports:
...
function generateWindowScanScript(target: string, ports: number[]): string {
  return `${scriptPreamble()}
target = "${target}"
ports = ${JSON.stringify(ports)}
for port in ports:
...
function generateOsFingerprintScript(target: string): string {
  return `${scriptPreamble()}
target = "${target}"
...
function generateFirewallMapScript(target: string, ports: number[]): string {
  return `${scriptPreamble()}
target = "${target}"
ports = ${JSON.stringify(ports)}
...
function generateTracerouteScript(target: string, maxHops: number): string {
  return `${scriptPreamble()}
target = "${target}"
max_hops = ${maxHops}
...
function generateArpDiscoverScript(target: string): string {
  return `${scriptPreamble()}
target = "${target}"
# ARP scan the subnet
...
function generateDnsAmplificationScript(target: string): string {
  return `${scriptPreamble()}
target = "${target}"
...
function generateIcmpTunnelProbeScript(target: string): string {
  return `${scriptPreamble()}
target = "${target}"
...
function generateTcpIsnScript(target: string, port: number): string {
  return `${scriptPreamble()}
target = "${target}"
port = ${port}
...
function generateIpIdScript(target: string, port: number): string {
  return `${scriptPreamble()}
target = "${target}"
port = ${port}
...
function generateIdleScanZombieScript(target: string): string {
  return `${scriptPreamble()}
target = "${target}"
...
/**
 * Generate a Scapy script for custom packet construction.
 */
export function generateCustomPacketScript(config: CraftedPacketConfig): string {
  const target = config.target;
  const count = config.count || 1;
  const timeout = config.timeout || 3;
  const delay = config.delay || 0;

  let packetConstruction: string;
...
// ═══════════════════════════════════════════════════════════════
// §4 — SCAN SERVER EXECUTION
// ═══════════════════════════════════════════════════════════════

/**
 * Execute a Scapy probe on the scan server.
 * Writes the Python script to a temp file, executes it, and parses JSON output.
 */
export async function executeScapyProbe(config: ProbeConfig): Promise<ScapyProbeResult> {
  ...
  // Execute with sudo (Scapy needs raw socket access)
  ...
  // Parse JSON output
  let probeResult: ScapyProbeResult;
  try {
    ...
    probeResult = {
      ...
      rawOutput: result.stdout.substring(0, 5000),
    };
  } catch {
    probeResult = {
...
/**
 * Execute a custom crafted packet on the scan server.
 */
export async function executeCustomPacket(config: CraftedPacketConfig): Promise<ScapyProbeResult> {
  const { executeTool } = await import("./scan-server-executor");
...
// ═══════════════════════════════════════════════════════════════
// §5 — ANALYSIS GENERATORS
// ═══════════════════════════════════════════════════════════════

function generateAnalysis(template: ProbeTemplate, results: ScapyPacketResult[]): string {
...
// ═══════════════════════════════════════════════════════════════
// §6 — SCAN SERVER PROVISIONING
// ═══════════════════════════════════════════════════════════════

/**
 * Install packet analysis tools on the scan server.
 * Idempotent — safe to run multiple times.
 */
export async function provisionPacketTools(): Promise<{
  success: boolean;
  installed: string[];
  failed: string[];
  output: string;
