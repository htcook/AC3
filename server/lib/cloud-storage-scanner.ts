 ...
 * 
 * Detects and assesses open S3 buckets, Azure Blob containers,
 * GCS buckets, Firebase databases, and other misconfigured cloud
 * storage/apps discovered during engagements.
 * 
 * Integration points:
 * - Post-discovery phase: auto-detect cloud-hosted assets from CNAME/headers
 * - Phase B tool deployment: run cloud_enum, s3scanner on discovered domains
 * - Vuln detection: classify and score cloud misconfigurations
 * - LLM training: structured knowledge base for cloud misconfig patterns
 */
...
export interface CloudAssetSignature {
  provider: "aws" | "azure" | "gcp" | "firebase" | "digitalocean" | "alibaba" | "oracle" | "unknown";
  service: string;
...
/**
 * CNAME patterns that indicate cloud-hosted storage/services.
 * Used during post-discovery to classify assets.
 */
const CLOUD_CNAME_PATTERNS: Array<{
  pattern: RegExp;
  provider: CloudAssetSignature["provider"];
  service: string;
}> = [
  // AWS
...
/**
 * HTTP response header patterns that indicate cloud services.
 */
const CLOUD_HEADER_PATTERNS: Array<{
  header: string;
  pattern: RegExp;
  provider: CloudAssetSignature["provider"];
  service: string;
...
/**
 * Error message patterns that reveal cloud storage misconfigurations.
 */
const CLOUD_ERROR_SIGNATURES: Array<{
  pattern: RegExp;
  provider: CloudAssetSignature["provider"];
  service: string;
...
export interface CloudDetectionResult {
  ...
  suggestedScans: CloudScanSuggestion[];
  storageEndpoints: string[];
}

export interface CloudScanSuggestion {
  tool: string;
  command: string;
  rationale: string;
  priority: number;
...
export function detectCloudAsset(asset: {
  ...
  technologies?: string[];
  cnames?: string[];
  toolResults?: Array<{ tool: string; outputPreview?: string; findings?: Array<{ title: string }> }>;
}): CloudDetectionResult {
  const signatures: CloudAssetSignature[] = [];
  const storageEndpoints: string[] = [];
  ...
  if (asset.dnsRecords) {
    ...
    // Extract CNAMEs from DNS records
  ...
  // 2. Check HTTP headers from httpx output
  if (asset.headers) {
    ...
    for (const line of headerLines) {
      ...
      for (const hp of CLOUD_HEADER_PATTERNS) {
        if (headerKey === hp.header.toLowerCase() && hp.pattern.test(headerValue)) {
          // Avoid duplicate provider+service
  ...
  // 3. Check httpx tool results for cloud indicators
        ...
        try {
          for (const line of tr.outputPreview.split("\n")) {
            ...
            try {
              ...
              // Check response headers in httpx JSON
              if (obj.header) {
  ...
  // 4. Check technologies detected by httpx
  ...
  // 5. Check hostname patterns directly
  for (const cp of CLOUD_CNAME_PATTERNS) {
...
function generateCloudScanSuggestions(
  ...
  // Extract base domain for keyword-based enumeration
  ...
  // 3. Azure Blob container scanning
  ...
  // 6. Nuclei cloud-specific templates
  if (providers.size > 0) {
  ...
  // 7. DigitalOcean Spaces scanning
...
// ─── Cloud Storage Scan Execution ───────────────────────────────────────────

export interface CloudScanFinding {
  provider: CloudAssetSignature["provider"];
  service: string;
  resource: string;
  misconfigType: string;
...
/**
 * Execute cloud storage scans on the scan server and parse results.
 */
export async function executeCloudStorageScan(
  hostname: string,
  suggestions: CloudScanSuggestion[],
  options: {
    maxScans?: number;
  ...
  findings: CloudScanFinding[];
  rawResults: Array<{ tool: string; command: string; stdout: string; stderr: string; exitCode: number; durationMs: number }>;
}> {
  const maxScans = options.maxScans ?? 10;
  const timeoutSeconds = options.timeoutSeconds ?? 120;
  const findings: CloudScanFinding[] = [];
  const rawResults: Array<{ tool: string; command: string; stdout: string; stderr: string; exitCode: number; durationMs: number }> = [];

  // Execute top-priority scans
...
/**
 * Parse cloud scan tool output into structured findings.
 */
export function parseCloudScanOutput(
  tool: string,
  stdout: string,
  stderr: string,
  hostname: string
  ...
  // Check against error signature patterns
  ...
  switch (tool) {
        ...
        try {
          ...
          if (obj.bucket_name) {
            ...
            findings.push({
              ...
              title: isPublic
                ? `[AWS] S3 bucket "${obj.bucket_name}" is publicly accessible`
                : `[AWS] S3 bucket "${obj.bucket_name}" exists (access denied)`,
              description: isPublic
                ? `S3 bucket allows public listing/access. Objects may contain sensitive data.`
                : `S3 bucket exists but public access is denied.`,
              evidence: JSON.stringify(obj, null, 2).slice(0, 500),
              remediationSteps: isPublic
                ? ["Enable S3 Block Public Access at account level", "Review and restrict bucket policy", "Enable S3 access logging", "Enable server-side encryption"]
                : [],
              complianceFrameworks: isPublic ? ["CIS AWS 2.1.5", "NIST 800-53 AC-3", "PCI DSS 7.1"] : [],
              cwe: isPublic ? "CWE-284" : undefined,
            });
          }
        } catch { /* not JSON */ }
      }
    ...
    case "cloud_enum": {
      // cloud_enum text output — each line is a discovered resource
      for (const line of stdout.split("\n")) {
        const trimmed = line.trim();
    ...
    case "nuclei": {
      // Nuclei JSONL output with cloud-specific templates
      for (const line of stdout.split("\n")) {
        const trimmed = line.trim();
        if (!trimmed) continue;
        try {
          ...
          if (obj.info?.name && obj.info?.severity) {
            ...
            const isCloud = /cloud|s3|bucket|azure|gcp|firebase|storage|misconfig/i.test(templateId) ||
                           /cloud|s3|bucket|azure|gcp|firebase|storage/i.test(obj.info.name);
            if (isCloud) {
              findings.push({
                ...
                severity: obj.info.severity as any,
                ...
                cwe: obj.info.classification?.cwe?.[0] ? `CWE-${obj.info.classification.cwe[0]}` : undefined,
              });
            }
          }
        } catch { /* not JSON */ }
  ...
  // Deduplicate by resource + misconfigType
...
function detectProviderFromNuclei(templateId: string, name: string): CloudAssetSignature["provider"] {
...
// ─── Remediation & Compliance Mappings ──────────────────────────────────────

function getRemediationSteps(provider: string, service: string, misconfigType: string): string[] {
...
function getComplianceFrameworks(misconfigType: string): string[] {
...
function getCWE(misconfigType: string): string | undefined {
...
// ─── LLM Knowledge Base for Cloud Misconfiguration Detection ────────────────

/**
 * Structured knowledge base that the LLM uses during scan planning
 * to recognize cloud assets and select appropriate tools.
 */
export const CLOUD_MISCONFIG_KNOWLEDGE_BASE = {
...
// ─── Exports for LLM Prompt Integration ─────────────────────────────────────

/**
 * Generate the cloud detection context string for the LLM scan planning prompt.
 * This is injected into the system prompt so the LLM knows how to handle cloud assets.
 */
export function getCloudDetectionPromptContext(): string {
  return `
