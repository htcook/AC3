/**
 * SSL/TLS Deep Scan Module
 *
 * Comprehensive TLS/SSL security analysis that checks:
 * - Protocol version support (SSLv2, SSLv3, TLS 1.0/1.1/1.2/1.3)
 * - Cipher suite enumeration with strength grading
 * - Certificate chain validation (expiry, self-signed, weak signature, SAN mismatch, chain completeness)
 * - OCSP stapling detection and certificate revocation
 * - Protocol downgrade attacks (POODLE, DROWN, Logjam, FREAK, ROBOT)
 * - Known TLS CVEs (Heartbleed, CCS Injection, Ticketbleed, CRIME/BREACH)
...
export interface TLSDeepScanConfig {
  ...
  /** Check for known CVEs (Heartbleed, etc.) */
  checkCVEs?: boolean;
  /** Enumerate all cipher suites */
  enumerateCiphers?: boolean;
  /** Check certificate chain */
  checkCertChain?: boolean;
  /** Check OCSP stapling */
  checkOCSP?: boolean;
  /** SNI hostname (defaults to host) */
  sniHostname?: string;
  /** STARTTLS protocol (smtp, ftp, imap, pop3, etc.) */
  starttls?: string;
}
...
export interface TLSProtocol {
  ...
  version: string;
  supported: boolean;
  grade: "good" | "acceptable" | "weak" | "critical" | "unknown";
  notes: string;
}
...
export interface TLSFinding {
  id: string;
  category: "protocol" | "cipher" | "certificate" | "vulnerability" | "configuration";
  severity: "critical" | "high" | "medium" | "low" | "info";
  title: string;
  description: string;
  recommendation: string;
...
export interface TLSDeepScanResult {
  scanId: number | null;
...
export const PROTOCOL_GRADES: Record<string, { grade: TLSProtocol["grade"]; notes: string }> = {
...
export const WEAK_CIPHERS: Record<string, { grade: TLSCipherSuite["grade"]; notes: string }> = {
...
export const STRONG_CIPHERS: Set<string> = new Set([
...
export const TLS_VULNERABILITIES: Array<{
  id: string;
  name: string;
  cve: string | null;
...
export function parseNmapSSLEnum(output: string): {
  ...
  const protocols: TLSProtocol[] = [];
  const cipherSuites: TLSCipherSuite[] = [];

  // Match protocol sections
  ...
  while ((match = protocolRegex.exec(output)) !== null) {
    ...
    const gradeInfo = PROTOCOL_GRADES[proto] || { grade: "unknown" as const, notes: "Unknown protocol version" };
    protocols.push({
      name: proto,
      version: proto,
      supported: true,
...
/**
 * Parse openssl s_client output for certificate and connection details.
 */
export function parseOpenSSLOutput(output: string): {
  protocol: string | null;
  cipher: string | null;
  certificate: Partial<TLSCertificate>;
  compression: boolean;
  secureRenegotiation: boolean;
  sessionTickets: boolean;
} {
  const protocol = (output.match(/Protocol\s*:\s*(TLSv[\d.]+|SSLv[\d.]+)/i)
    || output.match(/SSL-Session:[\s\S]*?Protocol\s*:\s*(TLSv[\d.]+)/i))?.[1] || null;
  ...
  if (notAfter) {
    try {
      ...
      expiresWithin30Days = !expired && expiryDate < new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000);
    } catch { /* ignore */ }
  }
  ...
  // Weak key check
  const weakKey = pubKeyBits !== null && (
    (pubKeyAlg?.includes("rsa") && pubKeyBits < 2048) ||
    (pubKeyAlg?.includes("ec") && pubKeyBits < 256)
  );
  ...
  const certificate: Partial<TLSCertificate> = {
    subject,
    issuer,
    serialNumber: serial,
    notBefore,
...
/**
 * Grade a cipher suite by name.
 */
export function gradeCipherSuite(name: string): TLSCipherSuite {
  ...
  // Default grading based on components
  ...
  let grade: TLSCipherSuite["grade"] = "acceptable";
  let notes = "";
...
function extractBits(name: string): number | null {
...
export function validateCertificate(cert: Partial<TLSCertificate>, hostname: string): TLSCertificate {
  const issues: string[] = [];
  ...
  for (const proto of protocols) {
    if (proto.supported && (proto.grade === "critical" || proto.grade === "weak")) {
      findings.push({
        ...
        recommendation: proto.grade === "critical"
          ? `Disable ${proto.name} immediately. It is fundamentally broken.`
          : `Disable ${proto.name}. Migrate to TLS 1.2+ only.`,
  ...
  if (certificate) {
    for (const issue of certificate.issues) {
      ...
      findings.push({
        ...
        severity: isExpiry && issue.includes("has expired") ? "critical"
          : isSelfSigned ? "high"
          : isMismatch ? "high"
          : isWeakKey ? "high"
          : "medium",
        ...
        recommendation: isExpiry ? "Renew the certificate immediately."
          : isSelfSigned ? "Replace with a certificate from a trusted CA (e.g., Let's Encrypt)."
          : isMismatch ? "Reissue the certificate with the correct Subject Alternative Names."
          : isWeakKey ? "Reissue the certificate with RSA 2048+ or ECDSA P-256+ and SHA-256+ signature."
          : "Review and fix the certificate configuration.",
        cve: null,
...
export function calculateGradeScore(
  ...
  for (const p of supportedProtos) {
    if (p.grade === "critical") score -= 20; // SSLv2/SSLv3
    if (p.grade === "weak") score -= 10; // TLS 1.0/1.1
  }
  if (!supportedProtos.some(p => p.version === "TLSv1.2" || p.version === "TLSv1.3")) {
    score -= 15; // No modern protocol
  }
  ...
  // Cap at F if critical issues exist
  if (certificate?.expired || insecureCiphers > 0 || critVulns > 0 ||
      supportedProtos.some(p => p.grade === "critical")) {
...
export async function startTLSDeepScan(config: TLSDeepScanConfig): Promise<TLSDeepScanResult> {
  ...
  let rawOutput = "";
  let protocols: TLSProtocol[] = [];
  let cipherSuites: TLSCipherSuite[] = [];
  let certificate: TLSCertificate | null = null;
  ...
  // ── Phase 2: openssl s_client for certificate and connection details ────
  ...
  // ── Phase 3: Certificate details with openssl x509 ─────────────────────
  ...
  if (config.checkCVEs !== false) {
    ...
    try {
      ...
      const sslv3Supported = !poodleResult.stdout.includes("alert handshake failure") &&
                              !poodleResult.stdout.includes("no protocols available") &&
                              !poodleResult.stdout.includes("wrong version number");
    ...
    try {
      ...
      const sslv2Supported = !drownResult.stdout.includes("unknown option") &&
                              !drownResult.stdout.includes("no protocols available") &&
                              !drownResult.stdout.includes("alert handshake failure");
    ...
    // Check for deprecated protocol vulnerabilities based on protocol scan
    ...
    // Check for Sweet32 based on cipher suites
