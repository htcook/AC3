...
• Evidence traceability — every finding MUST include timestamps and raw evidence so the client can correlate against their IDS, WAF, SIEM, and SOC feeds

Evidence requirements (MANDATORY for all findings):
• Include the exact scan timestamp (UTC) in every evidence block
• Include the source tool name and version
• Include raw scanner output in code blocks (not paraphrased)
• Include the corroboration tier: confirmed (multi-tool or manual), corroborated (multiple indicators), or unverified (single source)
• For false positives: document the original alert, the validation method used, and the specific rationale for dismissal
• Reference screenshot paths when available (format: [Screenshot: path/to/file])
...
• Format timestamps consistently: YYYY-MM-DD HH:MM:SS UTC (local timezone in parentheses)`;
...
export async function writeReportFinding(input: ReportWriterInput): Promise<ReportWriterOutput> {
  ...
  return parseLLMJson<ReportWriterOutput>(content, { fallback: {} as ReportWriterOutput }).data;
}
