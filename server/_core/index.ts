...
async function startServer() {
  ...
  app.post('/api/scheduled/threat-intel-daily', async (req, res) => {
    try {
      ...
      // Phase 9.5: Auto-Exploit Generation for new CVEs/KEVs
      // When new high-severity CVEs are detected, automatically generate
      // Nuclei templates + Caldera abilities and deploy to the arsenal.
      try {
        ...
        const newCveCount = cvePhase?.added || 0;
        const criticalCount = zeroDayPhase?.criticalCount || 0;

        if (newCveCount > 0 || criticalCount > 0) {
          const { generateExploitFromCVE } = await import("../lib/cve-exploit-gen-engine");
          const { getDynamicCves } = await import("../lib/nvd-cve-refresh");
          const { ensureKEVLoaded, lookupCVEProduct } = await import("../lib/cisa-kev-product-map");
          await ensureKEVLoaded();

          // Get newly added CVEs from the dynamic store
          const dynamicCves = getDynamicCves();
          // Filter to high/critical severity (CVSS >= 7.0) or KEV-listed
          const candidates = dynamicCves
            .filter(cve => {
              const cvssScore = cve.cvss || 0;
              const kevLookup = lookupCVEProduct(cve.cveId);
              return cvssScore >= 7.0 || kevLookup.found;
            })
            .slice(0, 10); // Rate limit: max 10 per run

          let generated = 0;
          let failed = 0;
          const generatedExploits: Array<{ cveId: string; status: string }> = [];
          ...
          for (const cve of candidates) {
            try {
              ...
              const result = await generateExploitFromCVE({
                ...
                priority: kevLookup.found ? 'critical' : 'high',
              });
              if (result.success) {
                generated++;
                generatedExploits.push({ cveId: cve.cveId, status: 'generated' });
              } else {
                failed++;
                generatedExploits.push({ cveId: cve.cveId, status: 'failed' });
              }
            } catch (genErr: any) {
              failed++;
              generatedExploits.push({ cveId: cve.cveId, status: `error: ${genErr.message}` });
            }
          }

          console.log(`[AutoExploitGen] Generated ${generated}/${candidates.length} exploits (${failed} failed)`);
          results.phases.push({
            phase: 'auto_exploit_generation',
            success: true,
            candidates: candidates.length,
            generated,
            failed,
            exploits: generatedExploits,
          });
        } else {
          results.phases.push({
            phase: 'auto_exploit_generation',
            success: true,
            skipped: true,
            reason: 'No new CVEs detected in this cycle',
          });
        }
      } catch (err: any) {
        console.error('[AutoExploitGen] Phase failed:', err.message);
        results.phases.push({ phase: 'auto_exploit_generation', success: false, error: err.message });
      }
      ...
      try {
        ...
        const autoExploitPhase = results.phases.find((p: any) => p.phase === 'auto_exploit_generation');
        if (autoExploitPhase?.success && !autoExploitPhase.skipped) {
          summaryLines.push(`\u26A1 Auto-Exploit Arsenal: ${autoExploitPhase.generated} new exploits generated from ${autoExploitPhase.candidates} CVE candidates (deployed to Caldera)`);
          if (autoExploitPhase.failed > 0) {
            summaryLines.push(`  • ${autoExploitPhase.failed} generation(s) failed`);
          }
        }
  ...
  server.listen(port, () => {
    ...
    setTimeout(() => {
      ...
      // Initialize Advisory Ingestion Scheduler (every 6 hours — CISA ICS-CERT, FBI, NSA)
      import("../lib/advisory-ingestion-scheduler").then(({ initAdvisoryIngestionScheduler }) => {
        initAdvisoryIngestionScheduler();
        console.log("[AdvisoryIngestion] Live advisory ingestion scheduler initialized (6h interval)");
      }).catch((err) => {
        console.warn("[AdvisoryIngestion] Failed to initialize advisory ingestion scheduler:", err);
      });
      // Initialize Actor Genome Training Pipeline (weekly on Sundays at 04:00 UTC)
      import("../lib/actor-genome-training-pipeline").then(({ initGenomeTrainingScheduler }) => {
        initGenomeTrainingScheduler();
        console.log("[ActorGenome] Training pipeline scheduler initialized (weekly)");
      }).catch((err) => {
        console.warn("[ActorGenome] Failed to initialize genome training scheduler:", err);
      });
