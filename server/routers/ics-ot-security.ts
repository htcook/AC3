...
import {
  ...
  captureBaseline,
  checkIntegrity,
  detectUnauthorizedConnection,
  correlateWithThreatActors,
  getMonitoringStatus,
  getAlerts,
  getDevices as getPlcDevices,
  getBaselineForDevice,
  acknowledgeAlert,
  resolveAlert,
  getFleetVulnerabilityAssessment,
  simulateAttackPattern,
  type PlcDevice,
  type IntegrityBaseline,
} from "../lib/plc-integrity-monitor";
...
export const icsOtSecurityRouter = router({
  ...
  // ─── PLC Integrity Monitoring ─────────────────────────────────────────────

  /** Initialize PLC integrity monitoring with device fleet */
  plcInitializeMonitor: protectedProcedure
    .input(z.object({
      devices: z.array(z.object({
        id: z.string(),
        name: z.string(),
        vendor: z.enum(["rockwell", "schneider", "siemens", "unitronics", "other"]),
        model: z.string(),
        firmwareVersion: z.string(),
        ipAddress: z.string(),
        protocol: z.enum(["ethernet_ip", "modbus", "s7comm", "pcom", "opc_ua"]),
        port: z.number(),
        facilityName: z.string(),
        facilityType: z.enum(["water_treatment", "wastewater", "pump_station", "lift_station", "water_tower", "power_substation", "gas_distribution", "other"]),
        sector: z.enum(["water", "energy", "government", "manufacturing", "other"]),
        connectionType: z.enum(["ethernet", "cellular", "serial", "vpn", "direct_internet"]),
        isInternetExposed: z.boolean(),
        lastKnownGoodHash: z.string(),
        lastCheckin: z.string().transform(s => new Date(s)),
        status: z.enum(["healthy", "degraded", "compromised", "offline", "unknown"]),
        engagementId: z.number().optional(),
      })),
    }))
    .mutation(({ input }) => {
      return initializeMonitor(input.devices as unknown as PlcDevice[]);
    }),

  /** Capture integrity baseline for a device */
  plcCaptureBaseline: protectedProcedure
    .input(z.object({
      deviceId: z.string(),
      baseline: z.object({
        deviceId: z.string(),
        capturedAt: z.string().transform(s => new Date(s)),
        projectFileHash: z.string(),
        ladderLogicHash: z.string(),
        aoiHashes: z.record(z.string()),
        ipConfig: z.object({
          address: z.string(),
          subnet: z.string(),
          gateway: z.string(),
        ...
        credentials: z.object({
          ...
          lastChanged: z.string().transform(s => new Date(s)),
        }),
        safetyLogic: z.object({
          shutdownEnabled: z.boolean(),
          alarmSystemEnabled: z.boolean(),
          safetyInterlocks: z.array(z.string()),
        }),
        runMode: z.enum(["run", "program", "remote", "test"]),
        firmwareVersion: z.string(),
        moduleInventory: z.array(z.string()),
      }),
    }))
    .mutation(({ input }) => {
      return captureBaseline(input.deviceId, input.baseline as unknown as IntegrityBaseline);
    }),

  /** Check device integrity against baseline */
  plcCheckIntegrity: protectedProcedure
    .input(z.object({
      deviceId: z.string(),
      currentState: z.object({
        projectFileHash: z.string().optional(),
        ladderLogicHash: z.string().optional(),
        aoiHashes: z.record(z.string()).optional(),
        ipConfig: z.object({
          address: z.string(),
          subnet: z.string(),
          gateway: z.string(),
        ...
        runMode: z.enum(["run", "program", "remote", "test"]).optional(),
      }),
    }))
    .mutation(({ input }) => {
      const alerts = checkIntegrity(input.deviceId, input.currentState as unknown as Partial<IntegrityBaseline>);
      return { alerts, count: alerts.length };
    }),

  /** Detect unauthorized engineering software connection */
  plcDetectUnauthorizedConnection: protectedProcedure
    .input(z.object({
      deviceId: z.string(),
      sourceIp: z.string(),
      sourcePort: z.number(),
      softwareName: z.string(),
      authorizedIps: z.array(z.string()),
    }))
    .mutation(({ input }) => {
      const alert = detectUnauthorizedConnection(
        input.deviceId, input.sourceIp, input.sourcePort,
        input.softwareName, input.authorizedIps
      );
      return { alert, detected: !!alert };
    }),

  /** Correlate alerts with known threat actor TTPs */
  plcCorrelateThreats: protectedProcedure
    .input(z.object({
      deviceId: z.string().optional(),
    }))
    .query(({ input }) => {
      const deviceAlerts = getAlerts({
        deviceId: input.deviceId,
        acknowledged: false,
      });
      if (deviceAlerts.length === 0) {
        return { primarySuspect: "None", confidence: "n/a", matchedTtps: [] as string[], cisaAdvisories: [] as string[], allMatches: [] as any[] };
      }
      return correlateWithThreatActors(deviceAlerts);
    }),

  /** Get PLC monitoring status dashboard */
  plcMonitoringStatus: protectedProcedure.query(() => {
    return getMonitoringStatus();
  }),

  /** Get PLC integrity alerts */
  plcGetAlerts: protectedProcedure
    .input(z.object({
    ...
    .query(({ input }) => {
      return getAlerts(input as any);
    }),

  /** Get all monitored PLC devices */
  plcGetDevices: protectedProcedure.query(() => {
    return getPlcDevices();
  }),

  /** Get device baseline */
  plcGetBaseline: protectedProcedure
    .input(z.object({ deviceId: z.string() }))
    .query(({ input }) => {
      return getBaselineForDevice(input.deviceId) || null;
    }),

  /** Acknowledge an alert */
  plcAcknowledgeAlert: protectedProcedure
  ...
  /** Resolve an alert */
  plcResolveAlert: protectedProcedure
    .input(z.object({ alertId: z.string() }))
    .mutation(({ input }) => {
      return { success: resolveAlert(input.alertId) };
    }),

  /** Get fleet vulnerability assessment */
  plcVulnerabilityAssessment: protectedProcedure.query(() => {
    return getFleetVulnerabilityAssessment();
  }),

  /** Simulate attack patterns for red team exercises */
  plcSimulateAttack: protectedProcedure
    .input(z.object({
      pattern: z.enum(["credential_lockout", "safety_logic_tamper", "full_chain"]),
    }))
    .query(({ input }) => {
      return simulateAttackPattern(input.pattern);
    }),

  // ─── Real-Time PLC Polling Engine ───────────────────────────────────────────

  /** Register a PLC endpoint for real-time polling */
  pollingRegisterEndpoint: protectedProcedure
    .input(z.object({
      id: z.string(),
      name: z.string(),
      host: z.string(),
      port: z.number().min(1).max(65535),
      protocol: z.enum(["modbus_tcp", "ethernet_ip", "s7comm", "dnp3"]),
      unitId: z.number().optional(),
      slot: z.number().optional(),
      pollIntervalMs: z.number().min(500).max(60000).default(5000),
      timeoutMs: z.number().min(100).max(30000).default(3000),
    ...
    .mutation(async ({ input }) => {
      const { getPollingEngine } = await import("../lib/plc-polling-engine");
      const engine = getPollingEngine();
      engine.registerEndpoint({ ...input, enabled: true });
      return { success: true, endpointId: input.id };
    }),

  /** Start polling a registered endpoint */
  pollingStart: protectedProcedure
    .input(z.object({ endpointId: z.string() }))
    .mutation(async ({ input }) => {
      const { getPollingEngine } = await import("../lib/plc-polling-engine");
      const engine = getPollingEngine();
      const success = await engine.startPolling(input.endpointId);
      return { success };
    }),

  /** Stop polling an endpoint */
  pollingStop: protectedProcedure
    .input(z.object({ endpointId: z.string() }))
    .mutation(async ({ input }) => {
      const { getPollingEngine } = await import("../lib/plc-polling-engine");
      const engine = getPollingEngine();
      engine.stopPolling(input.endpointId);
      return { success: true };
    }),

  /** Get connection health for all endpoints */
  pollingHealth: protectedProcedure.query(async () => {
    const { getPollingEngine } = await import("../lib/plc-polling-engine");
    const engine = getPollingEngine();
    return {
      endpoints: engine.getAllHealth(),
      stats: engine.getStats(),
    };
  }),

  /** Get drift alerts from polling engine */
  pollingAlerts: protectedProcedure
    .input(z.object({
      severity: z.enum(["critical", "high", "medium", "low", "info"]).optional(),
    ...
    .query(async ({ input }) => {
      const { getPollingEngine } = await import("../lib/plc-polling-engine");
      const engine = getPollingEngine();
      return engine.getAlerts(input);
    }),

  /** Acknowledge a polling drift alert */
  pollingAcknowledgeAlert: protectedProcedure
    .input(z.object({ alertId: z.string() }))
    .mutation(async ({ input }) => {
      const { getPollingEngine } = await import("../lib/plc-polling-engine");
      const engine = getPollingEngine();
      return { success: engine.acknowledgeAlert(input.alertId) };
    }),

  /** Capture current state as baseline */
  pollingCaptureBaseline: protectedProcedure
    .input(z.object({ endpointId: z.string() }))
    .mutation(async ({ input, ctx }) => {
      const { getPollingEngine } = await import("../lib/plc-polling-engine");
      const engine = getPollingEngine();
      const baseline = engine.captureBaseline(input.endpointId, ctx.user?.name || "operator");
      return { success: !!baseline, baseline };
    }),

  /** Get registered endpoints list */
  pollingEndpoints: protectedProcedure.query(async () => {
    const { getPollingEngine } = await import("../lib/plc-polling-engine");
    const engine = getPollingEngine();
  ...
  pollingRemoveEndpoint: protectedProcedure
    .input(z.object({ endpointId: z.string() }))
    .mutation(async ({ input }) => {
      const { getPollingEngine } = await import("../lib/plc-polling-engine");
      const engine = getPollingEngine();
      engine.removeEndpoint(input.endpointId);
      return { success: true };
    }),

  /** Get CyberAv3ngers detection signatures */
  pollingSignatures: protectedProcedure.query(async () => {
    const { CYBERAV3NGERS_SIGNATURES } = await import("../lib/plc-polling-engine");
    return CYBERAV3NGERS_SIGNATURES;
  }),
});
