/**
 * Prowler / ScoutSuite Cloud Security Posture Management Router
 *
 * Triggers Prowler or ScoutSuite scans on the ScanForge server using
 * client-provided cloud credentials. Parses structured JSON output
 * and stores findings in the engagement pipeline.
 */

import { z } from "zod";
import { protectedProcedure, router } from "../_core/trpc";
import { TRPCError } from "@trpc/server";
import { executeRawCommand, executeTool } from "../lib/scan-server-executor";
...
export interface ProwlerScanResult {
  provider: string;
  totalChecks: number;
  passed: number;
  failed: number;
...
function parseProwlerJsonOutput(stdout: string): ProwlerFinding[] {
  ...
  for (const line of lines) {
    try {
      ...
      // Skip non-JSON lines (progress output, banners, etc.)
    }
  }
...
function normalizeSeverity(s: string): ProwlerFinding["severity"] {
...
function normalizeStatus(s: string): ProwlerFinding["status"] {
...
function parseScoutSuiteOutput(stdout: string): ProwlerFinding[] {
  const findings: ProwlerFinding[] = [];
  try {
    // ScoutSuite outputs a JSON report file — we parse the summary
    ...
    if (report.services) {
      for (const [serviceName, serviceData] of Object.entries(report.services as Record<string, any>)) {
        ...
        for (const [findingId, finding] of Object.entries(findings_data as Record<string, any>)) {
...
export const prowlerIntegrationRouter = router({
  ...
  runProwlerScan: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (input.provider === "aws") {
        ...
        // GCP uses service account key file
      ...
      try {
        ...
        const scanResult: ProwlerScanResult = {
          ...
          warnings,
          findings,
          rawOutput: stdout.substring(0, 50000), // Cap raw output
  ...
  getComplianceFrameworks: protectedProcedure
    ...
    .query(({ input }) => {
      const services: Record<string, string[]> = {
        aws: [
          ...
          "cloudformation", "cloudfront", "cloudtrail", "cloudwatch", "codebuild",
          "config", "dax", "dms", "dynamodb", "ec2", "ecr", "ecs", "efs",
          "eks", "elasticache", "elb", "elbv2", "emr", "es", "firehose",
          "glacier", "glue", "guardduty", "iam", "kinesis", "kms", "lambda",
          "organizations", "rds", "redshift", "route53", "s3", "sagemaker",
          "secretsmanager", "securityhub", "ses", "shield", "sns", "sqs",
          "ssm", "trustedadvisor", "vpc", "waf", "wafv2",
        ],
        azure: [
          "aks", "appservice", "cosmosdb", "defender", "entra", "iam",
          "keyvault", "monitor", "mysql", "network", "postgresql",
          "sqlserver", "storage", "vm",
        ],
        gcp: [
          "bigquery", "cloudsql", "compute", "dns", "iam", "kms",
          "logging", "monitoring", "networking", "storage",
        ],
      };
