...
export const engagementOpsRouter = router({
    ...
    execute: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        // Auto-detect training lab from target domain — ONLY for DO-hosted test lab targets
        // Uses the domain-safety-whitelist to precisely identify approved lab infrastructure.
        // This ensures auto-approval ONLY applies to AC3/DO lab IPs and known vulnerable apps,
        // NOT to production/client targets that happen to be on aceofcloud.io subdomains.
        if (state.trainingLabMode === undefined) {
          const allTargets = [
            ...parseTargets(engagement.targetDomain || ''),
            ...parseTargets(engagement.targetIpRange || ''),
          ];
          if (allTargets.length > 0 && allTargets.every(t => isDomainWhitelisted(t))) {
            state.trainingLabMode = true;
            console.log(`[EngOps] Training lab auto-detected for #${input.engagementId} — all ${allTargets.length} target(s) are on the approved whitelist`);
          }
        }
