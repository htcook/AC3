...
Return ONLY a JSON object with vulnerabilities array.`;
  ...
  testCredentials: protectedProcedure
    ...
    /** Admin: Clear all errored engagements from memory and DB snapshots */
    clearErroredEngagements: protectedProcedure
      .input(z.object({
        engagementIds: z.array(z.number()).optional(),
        clearAll: z.boolean().optional(),
      }).optional())
      .mutation(async ({ input, ctx }) => {
        const { getHealthStatus, clearOpsState } = await import('../lib/engagement-orchestrator');
        const health = getHealthStatus();
        const errored = health.engagements.details.filter(
          (e: any) => e.phase === 'error'
        );
        
        const targetIds = input?.clearAll
          ? errored.map((e: any) => e.id)
          : (input?.engagementIds || errored.map((e: any) => e.id));
        
        const cleared: number[] = [];
        const failed: Array<{ id: number; error: string }> = [];
        
        for (const engId of targetIds) {
          try {
            await clearOpsState(engId);
            cleared.push(engId);
          } catch (err: any) {
            failed.push({ id: engId, error: err.message });
          }
        }
        
        await db.logActivity({
          userId: ctx.user.id,
          action: 'engagement_errored_cleanup',
          details: `Cleared ${cleared.length} errored engagement(s): [${cleared.join(', ')}]${failed.length ? ` | Failed: ${failed.map(f => `#${f.id}: ${f.error}`).join(', ')}` : ''}`,
        });
        ...
        return {
          ...
          remainingErrored: errored.length - cleared.length,
        };
      }),

    /** Admin: Clear degraded engagements (stuck at 100% but not completed) */
    clearDegradedEngagements: protectedProcedure
      .input(z.object({
        engagementIds: z.array(z.number()).optional(),
        action: z.enum(['clear', 'complete']).default('clear'),
      }).optional())
      .mutation(async ({ input, ctx }) => {
        const { getHealthStatus, clearOpsState, getOpsState, broadcastOpsUpdate, persistOpsStateNow } = await import('../lib/engagement-orchestrator');
        const health = getHealthStatus();
        const degraded = health.engagements.details.filter(
          (e: any) => e.phase === 'degraded'
        );
        
        const targetIds = input?.engagementIds || degraded.map((e: any) => e.id);
        ...
        for (const engId of targetIds) {
          try {
            if (action === 'clear') {
              ...
              // Mark as completed
              const state = getOpsState(engId);
              if (state) {
                state.phase = 'completed' as any;
                state.isRunning = false;
                state.error = undefined;
                broadcastOpsUpdate(engId, { type: 'phase_change', phase: 'completed' });
                await persistOpsStateNow(engId);
              } else {
                await clearOpsState(engId);
              }
            }
            processed.push(engId);
          } catch (err: any) {
            failed.push({ id: engId, error: err.message });
          }
        }
        ...
        await db.logActivity({
          userId: ctx.user.id,
          action: 'engagement_degraded_cleanup',
          details: `${action === 'clear' ? 'Cleared' : 'Completed'} ${processed.length} degraded engagement(s): [${processed.join(', ')}]`,
        });
