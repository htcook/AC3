...
export const riskRegisterRouter = router({
  ...
  exportPoamDocx: protectedProcedure
    .input(z.object({
      statusFilter: z.string().optional(),
      severityFilter: z.string().optional(),
      systemName: z.string().default("AC3 Platform"),
      preparedBy: z.string().default("AceofCloud Security Engineering"),
    ...
    .mutation(async ({ input }) => {
      ...
      let conditions: any[] = [];
      if (input.statusFilter) conditions.push(eq(riskRegisterEntries.status, input.statusFilter as any));
      if (input.severityFilter) conditions.push(eq(riskRegisterEntries.severity, input.severityFilter as any));

      const entries = await db.select().from(riskRegisterEntries)
        .where(conditions.length > 0 ? and(...conditions) : undefined)
        .orderBy(desc(sql`FIELD(severity,'critical','high','medium','low','info')`), asc(riskRegisterEntries.poamId));
      ...
      const sevColor = (sev: string) => {
        switch (sev) { case "critical": return "FF0000"; case "high": return "FF6600"; case "medium": return "FFB300"; case "low": return "2196F3"; default: return "9E9E9E"; }
      };

      // Build table rows
      const headerCells = ["POA&M ID", "Weakness", "Severity", "Status", "Affected Assets", "Control ID", "Detected", "Scheduled Completion", "Mitigation Plan", "Risk Decision"].map(h =>
        new docx.TableCell({
          children: [new docx.Paragraph({ children: [new docx.TextRun({ text: h, bold: true, color: "FFFFFF", size: 18, font: "Calibri" })], alignment: docx.AlignmentType.CENTER })],
          shading: { fill: "0D47A1", type: docx.ShadingType.SOLID },
          verticalAlign: docx.VerticalAlign.CENTER,
          width: { size: h === "Mitigation Plan" ? 2500 : h === "Weakness" ? 2000 : 1000, type: docx.WidthType.DXA },
        })
      );
      ...
      const dataRows = entries.map((e, i) =>
        new docx.TableRow({
          children: [
            e.poamId || "", e.weaknessName || "", e.severity || "", e.status || "",
            e.affectedAssets || "", e.controlId || "N/A",
            e.detectedDate ? new Date(e.detectedDate).toLocaleDateString("en-US") : "N/A",
            e.scheduledCompletionDate ? new Date(e.scheduledCompletionDate).toLocaleDateString("en-US") : "N/A",
            e.mitigationPlan || "Pending", e.riskDecision || "pending",
          ].map((val, ci) =>
            new docx.TableCell({
              children: [new docx.Paragraph({ children: [new docx.TextRun({ text: String(val), size: 16, font: "Calibri", color: ci === 2 ? sevColor(e.severity) : undefined, bold: ci === 2 })], alignment: ci === 2 ? docx.AlignmentType.CENTER : docx.AlignmentType.LEFT })],
              shading: i % 2 === 0 ? { fill: "F5F5F5", type: docx.ShadingType.SOLID } : undefined,
              verticalAlign: docx.VerticalAlign.CENTER,
            })
          ),
        })
      ...
      const doc = new docx.Document({
        creator: "AC3 - AceofCloud Cybersecurity Command Center",
        title: `FedRAMP POA&M Report - ${input.systemName}`,
        description: "Plan of Action and Milestones (POA&M) generated by AC3",
        sections: [{
          properties: { page: { size: { orientation: docx.PageOrientation.LANDSCAPE } } },
          children: [
            // Title
            new docx.Paragraph({ children: [new docx.TextRun({ text: "Plan of Action and Milestones (POA&M)", bold: true, size: 36, font: "Calibri", color: "0D47A1" })], spacing: { after: 100 }, alignment: docx.AlignmentType.CENTER }),
            new docx.Paragraph({ children: [new docx.TextRun({ text: `System: ${input.systemName}`, size: 24, font: "Calibri" })], alignment: docx.AlignmentType.CENTER }),
            new docx.Paragraph({ children: [new docx.TextRun({ text: `Prepared by: ${input.preparedBy}`, size: 20, font: "Calibri", color: "666666" })], alignment: docx.AlignmentType.CENTER }),
            ...(input.preparedFor ? [new docx.Paragraph({ children: [new docx.TextRun({ text: `Prepared for: ${input.preparedFor}`, size: 20, font: "Calibri", color: "666666" })], alignment: docx.AlignmentType.CENTER })] : []),
            new docx.Paragraph({ children: [new docx.TextRun({ text: `Date: ${dateStr}`, size: 20, font: "Calibri", color: "666666" })], alignment: docx.AlignmentType.CENTER, spacing: { after: 300 } }),

            // Summary
            new docx.Paragraph({ children: [new docx.TextRun({ text: "Executive Summary", bold: true, size: 28, font: "Calibri", color: "0D47A1" })], spacing: { before: 200, after: 100 } }),
            new docx.Paragraph({ children: [new docx.TextRun({ text: `This POA&M contains ${entries.length} items: ${sevCounts["critical"] || 0} Critical, ${sevCounts["high"] || 0} High, ${sevCounts["medium"] || 0} Medium, ${sevCounts["low"] || 0} Low, ${sevCounts["info"] || 0} Informational.`, size: 20, font: "Calibri" })], spacing: { after: 100 } }),
            new docx.Paragraph({ children: [new docx.TextRun({ text: `Open items: ${entries.filter(e => e.status === "open").length} | In Progress: ${entries.filter(e => e.status === "in_progress").length} | Closed: ${entries.filter(e => e.status === "closed").length} | Risk Accepted: ${entries.filter(e => e.riskDecision === "accept").length}`, size: 20, font: "Calibri" })], spacing: { after: 200 } }),

            // ConMon SLA Reference
            new docx.Paragraph({ children: [new docx.TextRun({ text: "FedRAMP ConMon SLA Reference", bold: true, size: 24, font: "Calibri", color: "0D47A1" })], spacing: { before: 100, after: 100 } }),
            new docx.Paragraph({ children: [new docx.TextRun({ text: "Critical/High: 30 days | Moderate: 90 days | Low: 180 days | Informational: 365 days (recommended)", size: 18, font: "Calibri", italics: true, color: "666666" })], spacing: { after: 200 } }),

            // POA&M Table
            new docx.Paragraph({ children: [new docx.TextRun({ text: "POA&M Items", bold: true, size: 28, font: "Calibri", color: "0D47A1" })], spacing: { before: 200, after: 100 } }),
            new docx.Table({
              rows: [
            ...
            // Footer
            new docx.Paragraph({ children: [new docx.TextRun({ text: `\nGenerated by AC3 - AceofCloud Cybersecurity Command Center on ${dateStr}`, size: 16, font: "Calibri", italics: true, color: "999999" })], spacing: { before: 400 } }),
          ],
        }],
      });

      const buffer = await docx.Packer.toBuffer(doc);
      const fileName = `risk-register/poam-${Date.now()}.docx`;
      ...
      return { url, fileName, totalItems: entries.length };
    }),
});
