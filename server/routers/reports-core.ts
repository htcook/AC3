      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (input.reportType === 'pentest_assessment' || input.reportType === 'full_engagement') {
          try {
            ...
            // ─── Extract no-exploit evidence and partial-exploit non-targeted evidence from ops log ───
            let noExploitEvidence: any = undefined;
            let partialExploitNonTargetedEvidence: any[] | undefined = undefined;
            if (opsState?.log) {
              const logEntries = opsState.log as any[];
              // Find the zero-exploit evidence record
              const noExploitLog = logEntries.find(
                (entry: any) => entry.type === 'evidence' && entry.data?.determination === 'no_exploitable_vulnerabilities'
              );
              if (noExploitLog?.data) {
                const rawEvidenceLog = logEntries.find(
                  (entry: any) => entry.type === 'evidence' && entry.data?.rawEvidenceItems
                );
                noExploitEvidence = {
                  determination: noExploitLog.data.determination,
                  approvalGateSkipped: noExploitLog.data.approvalGateSkipped,
                  reasons: noExploitLog.data.reasons || [],
                  scanCoverage: noExploitLog.data.scanCoverage || { assetsScanned: 0, totalPorts: 0, totalVulns: 0, totalPendingVulns: 0 },
                  perAssetEvidence: noExploitLog.data.perAssetEvidence || [],
                  llmDecision: noExploitLog.data.llmDecision || { reasoning: '', decision: '', rawActions: null },
                  rawEvidenceItems: rawEvidenceLog?.data?.rawEvidenceItems || [],
                  timestamp: noExploitLog.timestamp ? new Date(noExploitLog.timestamp).toISOString() : new Date().toISOString(),
                };
                console.log(`[Report] Extracted no-exploit evidence (${noExploitEvidence.perAssetEvidence.length} assets, ${noExploitEvidence.rawEvidenceItems.length} raw items)`);
              }
              // Find partial-exploit non-targeted evidence
              const partialLog = logEntries.find(
                (entry: any) => entry.type === 'evidence' && entry.data?.partialExploitNonTargeted
              );
              if (partialLog?.data?.partialExploitNonTargeted) {
                partialExploitNonTargetedEvidence = partialLog.data.partialExploitNonTargeted;
                console.log(`[Report] Extracted partial-exploit non-targeted evidence (${partialExploitNonTargetedEvidence!.length} assets)`);
              }
            }
            ...
            const pipelineResult = await runPentestReportPipeline({
              ...
              noExploitEvidence,
              partialExploitNonTargetedEvidence,
            ...
            try {
              ...
              // ── Evidence Archive: Generate ZIP bundle with all raw scan outputs ──
              let evidenceArchiveUrl: string | null = null;
              try {
                const { generateEvidenceArchive } = await import('../lib/evidence-archive-generator');
                const archiveResult = await generateEvidenceArchive({
                  engagementId: String(input.engagementId),
                  engagementName: engagement.name || 'Unknown Engagement',
                  customerName: engagement.customerName || 'Unknown Client',
                  engagementType: engagement.engagementType || 'Penetration Test',
                  reportId: String(reportId),
                  assets: pipelineInput.assets,
                  noExploitEvidence: pipelineInput.noExploitEvidence,
                  partialExploitNonTargetedEvidence: pipelineInput.partialExploitNonTargetedEvidence,
                  generatedAt: new Date().toISOString(),
                });
                evidenceArchiveUrl = archiveResult.url;
                console.log(`[Evidence Archive] Generated ZIP: ${archiveResult.fileCount} files, ${(archiveResult.sizeBytes / 1024).toFixed(1)}KB, manifest checksum: ${archiveResult.manifestChecksum.slice(0, 16)}...`);
              } catch (archiveErr: any) {
                // Non-fatal: don't block report delivery if archive generation fails
                console.error(`[Evidence Archive] Generation failed for engagement ${input.engagementId}:`, archiveErr.message);
              }

              return { id: reportId, url, content: pipelineResult.markdown, lintResult, evidenceArchiveUrl };
            } catch (storageErr) {
              await db.updateReport(reportId, {
                status: 'completed',
                generatedAt: new Date().toISOString().slice(0, 19).replace('T', ' '),
...
<div class="page">
  ...
  <div class="footer">
    ...
    generateEvidenceArchive: protectedProcedure
      .input(z.object({
        engagementId: z.number(),
      ...
      .mutation(async ({ input, ctx }) => {
        const { generateEvidenceArchive } = await import('../lib/evidence-archive-generator');
        const engagement = await db.getEngagement(input.engagementId);
        if (!engagement) throw new TRPCError({ code: 'NOT_FOUND', message: 'Engagement not found' });

        // Get ops state for evidence data
        const opsState = await db.getOpsState(input.engagementId);
        const assets = opsState?.assets || [];

        // Extract noExploitEvidence from ops log
        const noExploitLog = (opsState?.log || []).find(
          (l: any) => l.type === 'evidence' && l.message?.includes('No Exploitable Vulnerabilities')
        );
        const noExploitEvidence = noExploitLog?.metadata || undefined;

        // Extract partial exploit non-targeted evidence
        const partialLogs = (opsState?.log || []).filter(
          (l: any) => l.type === 'evidence' && l.message?.includes('Non-Targeted Asset Evidence')
        );
        const partialExploitNonTargetedEvidence = partialLogs.map((l: any) => l.metadata).filter(Boolean);

        const result = await generateEvidenceArchive({
          engagementId: String(input.engagementId),
          engagementName: engagement.name || 'Unknown Engagement',
          customerName: engagement.customerName || 'Unknown Client',
          engagementType: engagement.engagementType || 'Penetration Test',
