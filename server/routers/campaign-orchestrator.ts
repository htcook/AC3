...
export const campaignOrchestratorRouter = router({
  ...
  /** Create a campaign pre-populated from an attack sequence template */
  createFromTemplate: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const db = await getDbSafe();
      // Fetch the template
      const [template] = await db.select().from(attackSequenceTemplates)
        .where(eq(attackSequenceTemplates.id, input.templateId));
      if (!template) throw new TRPCError({ code: "NOT_FOUND", message: "Template not found" });

      // Parse template data
      const phases = (() => { try { return JSON.parse(template.phases as string || "[]"); } catch { return []; } })();
      const adversary = (() => { try { return JSON.parse(template.calderaAdversaryProfile as string || "{}"); } catch { return {}; } })();

      const now = Date.now();
      // Create campaign
      const [result] = await db.insert(redteamCampaigns).values({
        name: `${template.name}${input.customerName ? ` — ${input.customerName}` : ""}`,
        description: template.description,
        customerName: input.customerName,
        objective: `Execute ${template.attackType} attack sequence using ${adversary.name || "custom"} adversary profile`,
        safetyLevel: "medium" as any,
        maxDurationHours: 72,
        autoAdvance: 1,
        notifyOnStageComplete: 1,
        notifyOnCampaignComplete: 1,
        createdBy: ctx.user.id,
        status: "draft" as any,
      ...
      for (let i = 0; i < phases.length; i++) {
        ...
        const stageType = stageTypeMap[phase.name] || stageTypeMap[phase.tactic] || "custom";
        await db.insert(redteamCampaignStages).values({
          campaignId,
          name: phase.name || `Phase ${i + 1}`,
          description: `Techniques: ${(phase.techniques || []).join(", ")}`,
          stageOrder: i + 1,
          stageType: stageType as any,
          config: {
            techniques: phase.techniques || [],
            adversaryProfile: adversary.name || null,
            calderaAbilities: phase.abilities || [],
            templatePhase: phase.phase || i + 1,
          },
          entryConditions: [],
          exitConditions: [],
          onSuccess: i < phases.length - 1 ? "next" as any : "complete" as any,
          onFailure: "pause" as any,
          maxRetries: 1,
          timeoutMinutes: 60,
          status: "pending" as any,
        });
  ...
  /** Create a campaign pre-populated from a campaign archetype (engagement bundle) */
  createFromArchetype: protectedProcedure
    .input(z.object({
    ...
    .mutation(async ({ input, ctx }) => {
      const db = await getDbSafe();
      const [archetype] = await db.select().from(campaignArchetypes)
        .where(eq(campaignArchetypes.id, input.archetypeId));
      if (!archetype) throw new TRPCError({ code: "NOT_FOUND", message: "Archetype not found" });

      const killChainPhases = (() => { try { return JSON.parse(archetype.killChainPhases as string || "[]"); } catch { return []; } })();
      const defaultTechniques = (() => { try { return JSON.parse(archetype.defaultTechniques as string || "[]"); } catch { return []; } })();
      const defaultAbilities = (() => { try { return JSON.parse(archetype.defaultAbilities as string || "[]"); } catch { return []; } })();

      const now = Date.now();
      const [result] = await db.insert(redteamCampaigns).values({
        name: `${archetype.name}${input.customerName ? ` — ${input.customerName}` : ""}`,
        description: archetype.description,
        customerName: input.customerName,
        objective: `Execute ${archetype.archetypeCategory} engagement using ${archetype.name} archetype`,
        safetyLevel: "medium" as any,
        maxDurationHours: 72,
        autoAdvance: 1,
        notifyOnStageComplete: 1,
        ...
        createdBy: ctx.user.id,
        status: "draft" as any,
        totalStages: killChainPhases.length,
        completedStages: 0,
        failedStages: 0,
        skippedStages: 0,
        createdAt: now,
      ...
      const stageTypeMap: Record<string, string> = {
        ...
        "execution": "exploitation",
        "persistence": "post_exploit",
        "privilege_escalation": "post_exploit",
        "defense_evasion": "post_exploit",
        "credential_access": "post_exploit",
        "discovery": "recon",
        "lateral_movement": "lateral_move",
        "collection": "exfiltration",
        "exfiltration": "exfiltration",
        "impact": "exploitation",
        "command_and_control": "c2_deploy",
      };

      for (let i = 0; i < killChainPhases.length; i++) {
        const phase = killChainPhases[i];
        const phaseName = typeof phase === "string" ? phase : phase.name || `Phase ${i + 1}`;
        const stageType = stageTypeMap[phaseName] || "custom";
        // Find techniques for this phase
        const phaseTechniques = defaultTechniques.filter((t: any) => {
          if (typeof t === "string") return true;
          return t.tactic === phaseName;
        });
        await db.insert(redteamCampaignStages).values({
          campaignId,
          name: phaseName.replace(/_/g, " ").replace(/\b\w/g, (c: string) => c.toUpperCase()),
          description: `Kill chain phase: ${phaseName}. Techniques: ${phaseTechniques.map((t: any) => typeof t === "string" ? t : t.id).join(", ")}`,
          stageOrder: i + 1,
          stageType: stageType as any,
          config: {
            techniques: phaseTechniques,
            abilities: defaultAbilities.filter((a: any) => a.phase === i + 1 || a.tactic === phaseName),
            archetype: archetype.slug,
          },
          entryConditions: [],
          exitConditions: [],
          onSuccess: i < killChainPhases.length - 1 ? "next" as any : "complete" as any,
          onFailure: "pause" as any,
          maxRetries: 1,
          timeoutMinutes: 60,
          status: "pending" as any,
      ...
      return {
        campaignId,
        name: `${archetype.name}${input.customerName ? ` — ${input.customerName}` : ""}`,
        stagesCreated: killChainPhases.length,
        archetypeName: archetype.name,
        category: archetype.archetypeCategory,
      };
    }),
});
