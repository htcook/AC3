...
export const typosquatRouter = router({
    ...
    // ─── Automated Domain Purchase (Namecheap + GoDaddy) ─────────────────────

    // Get configured registrar status
    getRegistrarStatus: protectedProcedure.query(async () => {
      const { getRegistrarStatus } = await import('../lib/domain-registrar');
      return getRegistrarStatus();
    }),

    // Check availability + compare pricing across registrars
    checkAvailabilityMulti: protectedProcedure
      .input(z.object({
        domains: z.array(z.string().min(3)).max(20),
      }))
      .mutation(async ({ input }) => {
        const { checkAvailability, comparePricing } = await import('../lib/domain-registrar');
        const availability = await checkAvailability(input.domains);
        const pricing = await comparePricing(input.domains);
        return { availability, pricing };
      }),

    // One-click purchase from selected registrar with auto-phishing config
    purchaseDomain: protectedProcedure
      .input(z.object({
        domain: z.string().min(3),
        typosquatId: z.number(),
        registrar: z.enum(['namecheap', 'godaddy']),
        contact: z.object({
          firstName: z.string(),
          lastName: z.string(),
        ...
        autoConfigurePhishing: z.boolean().default(true),
        mailServerIp: z.string().optional(),
        engagementId: z.number().optional(),
      }))
      .mutation(async ({ input, ctx }) => {
        const { purchaseDomain: doPurchase } = await import('../lib/domain-registrar');
        ...
        const result = await doPurchase({
          ...
          autoConfigurePhishing: input.autoConfigurePhishing,
          mailServerIp: input.mailServerIp || ENV.MAIL_SERVER_IP,
        });

        if (result.success) {
          // Update typosquat record with purchase details
          await db.updateTyposquatDomain(input.typosquatId, {
            status: result.dnsConfigured ? 'configured' : 'purchased',
            registrar: input.registrar,
            annualCost: result.chargedAmount?.toString(),
            purchaseDate: new Date(),
            notes: [
              `Purchased via ${input.registrar} (Order: ${result.orderId || 'N/A'})`,
              result.dnsConfigured ? 'DNS auto-configured for phishing (MX, SPF, DMARC, DKIM)' : '',
              result.dkim ? `DKIM selector: ${result.dkim.selector}` : '',
            ].filter(Boolean).join('\n'),
          } as any);

          await db.logActivity({
            userId: ctx.user.id,
            action: 'typosquat_auto_purchased',
            details: `Purchased ${input.domain} via ${input.registrar} ($${result.chargedAmount || '?'}). DNS: ${result.dnsConfigured ? 'configured' : 'pending'}`,
          });
        }
    ...
    // Purchase from cheapest registrar automatically
    purchaseCheapest: protectedProcedure
      .input(z.object({
        domain: z.string().min(3),
        typosquatId: z.number(),
        contact: z.object({
      ...
      .mutation(async ({ input, ctx }) => {
        const { purchaseCheapest } = await import('../lib/domain-registrar');
        const { ENV } = await import('../_core/env');

        const result = await purchaseCheapest(input.domain, input.contact, {
          autoConfigurePhishing: input.autoConfigurePhishing,
          mailServerIp: input.mailServerIp || ENV.MAIL_SERVER_IP,
        });
        ...
        if (result.success) {
          await db.updateTyposquatDomain(input.typosquatId, {
            ...
            annualCost: result.chargedAmount?.toString(),
            purchaseDate: new Date(),
            notes: `Auto-purchased from cheapest registrar (${result.registrar}). Order: ${result.orderId || 'N/A'}`,
          } as any);

          await db.logActivity({
            userId: ctx.user.id,
            action: 'typosquat_auto_purchased_cheapest',
            details: `Purchased ${input.domain} from cheapest (${result.registrar}, $${result.chargedAmount || '?'})`,
          });
        }
    ...
    generateDkim: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        const { generateDkimKeypair } = await import('../lib/dkim-generator');
        const keypair = generateDkimKeypair(input.domain, input.selector);

        await db.logActivity({
          userId: ctx.user.id,
          action: 'dkim_generated',
          details: `Generated DKIM keypair for ${input.domain} (selector: ${input.selector})`,
        });
        ...
        return {
          ...
          opendkimConfig: keypair.opendkimConfig,
          // Private key returned for operator to deploy to mail server
          privateKey: keypair.privateKey,
        };
      }),

    // Verify DKIM DNS record is published
    verifyDkim: protectedProcedure
      .input(z.object({
        domain: z.string().min(3),
        selector: z.string().default('default'),
      }))
      .mutation(async ({ input }) => {
        const { verifyDkimRecord } = await import('../lib/dkim-generator');
        return verifyDkimRecord(input.domain, input.selector);
      }),
