...
export const engagementsRouter = router({
    ...
    create: protectedProcedure
      .input(z.object({
        ...
        quickScanMode: z.boolean().default(false),
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        // ── Quick Scan Mode: Auto-Discovery + Stack Profiling + Threat Matching ──
        let quickScanResult: any = null;
        if (input.quickScanMode && (input.targetDomain || input.targetIpRange)) {
          try {
            const { runQuickScan } = await import('../lib/quick-scan-engine');
            quickScanResult = await runQuickScan({
              target: input.targetDomain || input.targetIpRange || '',
              engagementId: id,
              industry: input.industry,
            });
            console.log(`[QuickScan] Completed for engagement #${id}: ${quickScanResult.stackProfile?.primaryStack || 'unknown'} stack, ${quickScanResult.matchedThreats?.length || 0} threat actors matched`);
            // Update engagement notes with Quick Scan results
            const { getDb: getDbQs } = await import('../db');
            const { engagements: engQs } = await import('../../drizzle/schema');
            const { eq: eqQs } = await import('drizzle-orm');
            const dbQs = await getDbQs();
            if (dbQs && quickScanResult.stackProfile) {
              await dbQs.update(engQs)
                .set({
                  notes: `${notes}\n\n[QuickScan] Stack: ${quickScanResult.stackProfile.primaryStack} | Threats: ${quickScanResult.matchedThreats?.map((t: any) => t.name).join(', ') || 'none'} | Tools: ${quickScanResult.recommendedTools?.join(', ') || 'default'}`,
                })
                .where(eqQs(engQs.id, id));
            }
          } catch (qsErr: any) {
            console.warn(`[QuickScan] Non-fatal error for engagement #${id}: ${qsErr.message}`);
          }
        }
        ...
        await db.logActivity({
          ...
          details: `Created engagement: ${input.name} for ${input.customerName}${roeDocId ? ` (RoE #${roeDocId} auto-created)` : ''}${calderaOpId ? ` (Caldera op: ${calderaOpId})` : ''}${input.quickScanMode ? ' [QuickScan]' : ''}`,
        });
        return {
          id,
