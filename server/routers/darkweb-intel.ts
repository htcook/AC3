...
export const darkwebIntelRouter = router({
  ...
  escalationAlerts: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      // Filter out kinetic/military conflict events — this feed is cyber-only
      const kineticKeywords = ['deaths', 'killed', 'airstrike', 'missile strike', 'bombing', 'military attack', 'troops deployed', 'ground offensive', 'naval blockade', 'artillery', 'drone strike'];
      const kineticExclusions = kineticKeywords.map(kw =>
        sql`LOWER(${threatGroupEvents.tgeTitle}) NOT LIKE ${`%${kw}%`}`
      );
      conditions.push(...kineticExclusions);
      // Also exclude events where actorType is explicitly kinetic/military
      conditions.push(sql`(${threatActors.actorType} IS NULL OR ${threatActors.actorType} NOT IN ('state_military', 'kinetic', 'military'))`);
      const where = conditions.length > 0 ? and(...conditions) : undefined;
      const events = await db
        .select({
          id: threatGroupEvents.id,
