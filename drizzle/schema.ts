...
export const defenseSuppliers = mysqlTable("defense_suppliers", {
id: int().autoincrement().notNull(),
supplierId: varchar("supplier_id", { length: 64 }).notNull(),
name: varchar({ length: 255 }).notNull(),
cageCode: varchar("cage_code", { length: 16 }),
dunsNumber: varchar("duns_number", { length: 16 }),
naicsCode: varchar("naics_code", { length: 16 }),
sector: varchar({ length: 128 }),
tier: varchar({ length: 16 }).default('unknown').notNull(),
parentSupplierId: varchar("parent_supplier_id", { length: 64 }),
headquarters: varchar({ length: 255 }),
country: varchar({ length: 3 }),
employeeCount: int("employee_count"),
annualRevenue: varchar("annual_revenue", { length: 32 }),
primaryDomain: varchar("primary_domain", { length: 255 }),
additionalDomains: json("additional_domains"),
ipRanges: json("ip_ranges"),
contractVehicles: json("contract_vehicles"),
criticalProducts: json("critical_products"),
complianceCerts: json("compliance_certs"),
...
index("ds_supplier_id_unique").on(table.supplierId),
index("ds_cage_code_idx").on(table.cageCode),
index("ds_tier_idx").on(table.tier),
index("ds_sector_idx").on(table.sector),
index("ds_risk_band_idx").on(table.riskBand),
index("ds_primary_domain_idx").on(table.primaryDomain),
index("ds_status_idx").on(table.status),
]);

export const supplierTechStacks = mysqlTable("supplier_tech_stacks", {
id: int().autoincrement().notNull(),
supplierId: varchar("supplier_id", { length: 64 }).notNull(),
...
version: varchar({ length: 64 }),
category: varchar({ length: 64 }).notNull(),
deploymentType: varchar("deployment_type", { length: 32 }),
criticality: varchar({ length: 16 }).default('medium').notNull(),
confidence: float().default(0.5).notNull(),
source: varchar({ length: 64 }).notNull(),
evidenceUrl: varchar("evidence_url", { length: 1024 }),
lastVerifiedAt: timestamp("last_verified_at", { mode: 'string' }),
createdAt: timestamp("created_at", { mode: 'string' }).default('CURRENT_TIMESTAMP').notNull(),
}, (table) => [
index("sts_supplier_id_idx").on(table.supplierId),
index("sts_product_idx").on(table.product),
index("sts_vendor_idx").on(table.vendor),
index("sts_category_idx").on(table.category),
index("sts_criticality_idx").on(table.criticality),
]);

export const supplyChainRelationships = mysqlTable("supply_chain_relationships", {
id: int().autoincrement().notNull(),
relationshipId: varchar("relationship_id", { length: 64 }).notNull(),
primeContractorId: varchar("prime_contractor_id", { length: 64 }).notNull(),
subcontractorId: varchar("subcontractor_id", { length: 64 }).notNull(),
relationshipType: varchar("relationship_type", { length: 32 }).notNull(),
programName: varchar("program_name", { length: 255 }),
contractNumber: varchar("contract_number", { length: 128 }),
materialsProvided: json("materials_provided"),
criticalityToProgram: varchar("criticality_to_program", { length: 16 }).default('medium'),
singleSourceRisk: tinyint("single_source_risk").default(0),
status: varchar({ length: 16 }).default('active').notNull(),
validFrom: timestamp("valid_from", { mode: 'string' }),
validTo: timestamp("valid_to", { mode: 'string' }),
createdAt: timestamp("created_at", { mode: 'string' }).default('CURRENT_TIMESTAMP').notNull(),
}, (table) => [
index("scr_relationship_id_unique").on(table.relationshipId),
index("scr_prime_idx").on(table.primeContractorId),
index("scr_sub_idx").on(table.subcontractorId),
index("scr_program_idx").on(table.programName),
index("scr_criticality_idx").on(table.criticalityToProgram),
]);

export const supplierThreatAlerts = mysqlTable("supplier_threat_alerts", {
id: int().autoincrement().notNull(),
alertId: varchar("alert_id", { length: 64 }).notNull(),
supplierId: varchar("supplier_id", { length: 64 }).notNull(),
...
description: text(),
cveId: varchar("cve_id", { length: 32 }),
advisoryId: varchar("advisory_id", { length: 64 }),
threatActorName: varchar("threat_actor_name", { length: 128 }),
affectedProduct: varchar("affected_product", { length: 255 }),
affectedVersion: varchar("affected_version", { length: 64 }),
exploitAvailable: tinyint("exploit_available").default(0),
activeExploitation: tinyint("active_exploitation").default(0),
impactedPrograms: json("impacted_programs"),
cascadeRisk: varchar("cascade_risk", { length: 16 }),
mitigationStatus: varchar("mitigation_status", { length: 32 }).default('unmitigated'),
mitigationNotes: text("mitigation_notes"),
sourceUrl: varchar("source_url", { length: 1024 }),
incidentReportId: int("incident_report_id"),
...
index("sta_alert_id_unique").on(table.alertId),
index("sta_supplier_id_idx").on(table.supplierId),
index("sta_alert_type_idx").on(table.alertType),
index("sta_severity_idx").on(table.severity),
index("sta_cve_id_idx").on(table.cveId),
index("sta_advisory_id_idx").on(table.advisoryId),
index("sta_threat_actor_idx").on(table.threatActorName),
index("sta_mitigation_status_idx").on(table.mitigationStatus),
index("sta_detected_at_idx").on(table.detectedAt),
]);

export const supplierAssessmentHistory = mysqlTable("supplier_assessment_history", {
id: int().autoincrement().notNull(),
supplierId: varchar("supplier_id", { length: 64 }).notNull(),
assessmentType: varchar("assessment_type", { length: 32 }).notNull(),
riskScoreBefore: int("risk_score_before"),
riskScoreAfter: int("risk_score_after"),
findingsCount: int("findings_count").default(0),
criticalFindings: int("critical_findings").default(0),
summary: text(),
assessedBy: varchar("assessed_by", { length: 64 }),
diScanId: int("di_scan_id"),
createdAt: timestamp("created_at", { mode: 'string' }).default('CURRENT_TIMESTAMP').notNull(),
}, (table) => [
index("sah_supplier_id_idx").on(table.supplierId),
index("sah_assessment_type_idx").on(table.assessmentType),
index("sah_created_at_idx").on(table.createdAt),
]);
