...
CREATE TABLE `di_incident_training_data` (
	...
	`actors_discovered` int NOT NULL DEFAULT 0,
	`ttps_discovered` int NOT NULL DEFAULT 0,
	`analyst_rating` enum('not_reviewed','accurate','partially_accurate','inaccurate') NOT NULL DEFAULT 'not_reviewed',
	`analyst_reviewed_by` int,
	`analyst_notes` text,
...
CREATE TABLE `engagements` (
	...
	`description` text,
	`engagementType` enum('red_team','phishing','pentest','purple_team','tabletop','bug_bounty') NOT NULL DEFAULT 'red_team',
...
--> statement-breakpoint
CREATE TABLE `webhook_endpoints_legacy` (
	`id` int AUTO_INCREMENT NOT NULL,
	`webhookId` varchar(64) NOT NULL,
	`name` varchar(255) NOT NULL,
	`url` text NOT NULL,
...
CREATE INDEX `webhookId` ON `webhook_endpoints_legacy` (`webhookId`);--> statement-breakpoint
