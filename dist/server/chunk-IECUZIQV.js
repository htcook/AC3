...
function computeHybridScoreWithProfile(input, profile = PENTEST_PROFILE) {
  const technicalSeverity = computeTechnicalSeverity(input);
  const exposureModifier = computeExposureModifier(input.exposure);
  const missionImpactScore = computeMissionImpact(input);
  const { modifier: attackPathModifier, description: attackPathValue } = computeAttackPathModifier(input.attackPathCategories);
...
function batchScore(findings, profile) {
  const scoreFn = profile ? (i) => computeHybridScoreWithProfile(i, profile) : computeHybridScore;
  return findings.map((f) => ({ id: f.id, result: scoreFn(f.input) })).sort((a, b) => b.result.hybridPriorityScore - a.result.hybridPriorityScore);
}
...
var EXPOSURE_MODIFIERS, EPSS_BANDS, ATTACK_PATH_WEIGHTS, STATE_MULTIPLIERS, HIGH_MISSION_IMPACT_KEYWORDS, PENTEST_PROFILE;
