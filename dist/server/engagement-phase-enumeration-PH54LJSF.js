...
${state.scanPlan?.discoveryStrategy || "Comprehensive port discovery to enrich passive recon data"}`
  ...
  try {
    const { getScanServerConfigForScanForge } = await import("./scan-server-executor-XZZDHFIF.js");
    const { autoSelectTool } = await import("./scanforge-discovery-N2JTOPIU.js");
...
Evasion: ${assetPlan?.evasionTechniques?.join(", ") || "fragmentation, decoys, normal timing"}`
  ...
  try {
    const { beforeDiscoveryScan } = await import("./pcap-auto-capture-3EYXWVMP.js");
  ...
  try {
    ...
    if (discoveryResult.stdout) {
      try {
        const discovery = await import("./scanforge-discovery-N2JTOPIU.js");
...
Ports: ${discoveredPorts.map((p) => `${p.port}/${p.service}${p.product ? ` (${p.product})` : ""}`).join(", ")}`,
  ...
  if (autoCaptureSessionId) {
    try {
      const { afterDiscoveryScan } = await import("./pcap-auto-capture-3EYXWVMP.js");
...
async function runCloudAssetDetection(state, helpers) {
  try {
    const { detectCloudAsset, executeCloudStorageScan, getCloudDetectionPromptContext } = await import("./cloud-storage-scanner-LLXJ33LW.js");
    helpers.addLog({
      phase: "enumeration",
      type: "info",
...
async function executeTargetedToolDeployment(state, helpers) {
  ...
  const { suggestToolCommands } = await import("./scan-server-executor-XZZDHFIF.js");
  for (const asset of state.assets) {
    ...
    const { autoSelectTool: autoSelectToolB } = await import("./scanforge-discovery-N2JTOPIU.js");
