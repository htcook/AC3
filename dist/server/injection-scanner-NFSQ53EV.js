...
async function executeInjectionScanning(ctx) {
  ...
  for (const webApp of webApps) {
    ...
    try {
      ...
      if (approved) {
        const { batchSqlmapScan, analyzeSqlmapFindings, runBlindSqliPass, ingestSqlmapToWebAppFindings } = await import("./sqlmap-scanner-KOEYDJSJ.js");
    ...
    try {
      ...
      if (approved) {
        const { batchXssScan, analyzeXssFindings, ingestXssToWebAppFindings } = await import("./xsstrike-scanner-2NUZVYJE.js");
  ...
  for (const webApp of webApps) {
    ...
    try {
      ...
      if (approved) {
        const { batchCommixScan, ingestCommixToWebAppFindings } = await import("./commix-scanner-OIQCWK6G.js");
        const isLab = state.trainingLabMode === true;
  ...
  for (const webApp of webApps) {
    ...
    try {
      ...
      if (approved) {
        const { batchTplmapScan, ingestTplmapToWebAppFindings } = await import("./tplmap-scanner-QASTGE7F.js");
