...
// server/lib/threat-intel-rss.ts
init_db();
init_schema();
import { eq, sql } from "drizzle-orm";
var THREAT_INTEL_FEEDS = [
...
function parseRSSXml(xml) {
  ...
  if (isAtom) {
    const channelTitle2 = getTag(xml, "title");
    const items2 = [];
    ...
    return { title: channelTitle2, lastBuildDate: "", items: items2 };
  }
...
function classifyForUnderground(item, feed) {
...
function classifyIncidentType(item) {
  const text = (item.title + " " + item.description).toLowerCase();
  if (text.includes("ransomware") || text.includes("ransom")) return "ransomware";
...
function assessSeverity(item) {
  const text = (item.title + " " + item.description).toLowerCase();
  if (text.includes("government") || text.includes("military") || text.includes("critical infrastructure") || text.includes("healthcare") || text.includes("hospital") || text.includes("federal") || text.includes("zero-day") || text.includes("actively exploited")) {
    return "critical";
  }
  if (text.includes("ransomware") || text.includes("data breach") || text.includes("exfiltrat") || text.includes("million") || text.includes("database") || text.includes("cve-")) {
    return "high";
  }
...
function makeSourceId(feedId, item) {
...
async function fetchAndIngestFeed(feed) {
  const result = {
    feedId: feed.id,
    feedName: feed.name,
    tier: feed.tier,
  ...
  try {
    ...
    for (const item of parsed.items) {
      ...
      if (feed.targets.includes("threat_group_events") && actorName) {
        ...
        try {
          const [existing] = await db.select({ id: threatGroupEvents.id }).from(threatGroupEvents).where(sql`${threatGroupEvents.tgeSourceUrl} = ${item.link} AND ${threatGroupEvents.tgeActorId} = ${actorId}`).limit(1);
          if (!existing) {
            const [existingActor] = await db.select().from(threatActors).where(sql`LOWER(${threatActors.name}) = ${actorName.toLowerCase()} OR ${threatActors.actorId} = ${actorId}`).limit(1);
            ...
            await db.insert(threatGroupEvents).values({
              ...
              tgeDescription: cleanDesc.substring(0, 2e3),
      ...
      if (feed.targets.includes("ransomware_events") && classifyForUnderground(item, feed) === "ransomware") {
        try {
          const [existing] = await db.select({ id: ransomwareEvents.id }).from(ransomwareEvents).where(sql`${ransomwareEvents.victimUrl} = ${item.link}`).limit(1);
          if (!existing) {
            ...
            if (actorName) {
              try {
                const [rwGroup] = await db.select().from(ransomwareGroups).where(sql`LOWER(${ransomwareGroups.groupName}) = ${actorName.toLowerCase()}`).limit(1);
                if (rwGroup) {
      ...
      if (feed.targets.includes("underground_intel")) {
        try {
          const [existing] = await db.select({ id: undergroundIntelEvents.id }).from(undergroundIntelEvents).where(sql`${undergroundIntelEvents.uieSourceUrl} = ${item.link} AND ${undergroundIntelEvents.uieSource} = ${feed.id}`).limit(1);
          if (!existing) {
            ...
            await db.insert(undergroundIntelEvents).values({
              ...
              uieDescription: cleanDesc.substring(0, 2e3),
              uieActorName: actorName || null,
              uieSeverity: severity,
      ...
      if (feed.targets.includes("incident_reports")) {
        try {
          const [existing] = await db.select({ id: incidentReports.id }).from(incidentReports).where(sql`${incidentReports.sourceId} = ${sourceId}`).limit(1);
          if (!existing) {
            await db.insert(incidentReports).values({
              sourceId,
              source: feed.id,
              title: item.title.substring(0, 1e3),
              url: item.link,
              publishedAt: item.pubDate || pubDate.toISOString(),
              summary: cleanDesc.substring(0, 3e3),
              actorsIdentified: actorName ? [actorName] : [],
              cvesMentioned: cves,
              incidentType: classifyIncidentType(item),
              irSeverity: severity,
              irStatus: "raw",
              fullContent: (item.contentEncoded || item.description || "").substring(0, 1e4)
            });
...
async function updateFeedRegistry(feed, status, errorMsg, itemCount) {
  try {
    const db = await requireDb();
    const [existing] = await db.select().from(darkwebFeedRegistry).where(eq(darkwebFeedRegistry.dfrFeedName, feed.id)).limit(1);
    if (existing) {
      await db.update(darkwebFeedRegistry).set({
        dfrStatus: status,
        dfrLastSyncAt: (/* @__PURE__ */ new Date()).toISOString(),
        dfrLastError: errorMsg,
        ...itemCount !== void 0 ? { dfrTotalRecordsFetched: itemCount } : {}
      }).where(eq(darkwebFeedRegistry.id, existing.id));
    } else {
      await db.insert(darkwebFeedRegistry).values({
        dfrFeedName: feed.id,
        dfrFeedUrl: feed.url,
        dfrFeedType: feed.category === "ransomware" ? "ransomware" : feed.category === "breach" ? "credential" : feed.category === "zero_day" ? "vulnerability" : "other",
...
async function syncAllThreatIntelFeeds(options = {}) {
