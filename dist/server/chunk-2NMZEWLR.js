...
// server/lib/knowledge/fp-suppression-rules.ts
function findingMatchesRule(finding, agentClass, rule) {
  if (rule.severityFilter.length > 0) {
    const sev = (finding.severity || "").toLowerCase();
    if (!rule.severityFilter.includes(sev)) return false;
  }
...
function applySuppressionRules(findings, profileName = "balanced", customRules) {
...
function getSuppressionRuleSummary() {
...
function getSuppressionProfiles() {
...
function buildFPSuppressionContext() {
...
var FP_SUPPRESSION_RULES, SUPPRESSION_PROFILES;
var init_fp_suppression_rules = __esm({
  "server/lib/knowledge/fp-suppression-rules.ts"() {
    FP_SUPPRESSION_RULES = [
      ...
      // ── New rules based on FP/FN analysis (March 2026) ──
      {
        ...
        name: "ZAP Informational-Only Alerts",
        description: "Suppress ZAP passive scan alerts that are purely informational (CSP, timestamps, etc.).",
        category: "informational_header",
        enabledByDefault: true,
        severityFilter: ["info"],
        sourcePatterns: ["zap"],
        titlePatterns: [
          ...
          /^Sec-Fetch-Dest Header is Missing/i,
          /^Sec-Fetch-Mode Header is Missing/i,
          /^Sec-Fetch-Site Header is Missing/i,
          /^Sec-Fetch-User Header is Missing/i
        ],
        agentClassPatterns: [],
        estimatedSuppression: 8,
        tpRisk: "low",
        rationale: "ZAP passive scan informational alerts are not vulnerabilities \u2014 they are observations about application behavior."
      },
      {
        id: "zap-csp-report",
        name: "ZAP CSP Header Report",
        description: "Suppress ZAP CSP-related findings that are configuration observations, not exploitable vulns.",
        category: "informational_header",
        enabledByDefault: true,
        severityFilter: ["info", "low"],
        sourcePatterns: ["zap"],
        titlePatterns: [
          /^Content Security Policy \(CSP\) Header Not Set$/i,
          /^CSP: Wildcard Directive$/i,
          /^CSP: style-src unsafe-inline$/i,
          /^CSP: script-src unsafe-inline$/i,
          /^CSP$/i
        ],
        agentClassPatterns: [],
        estimatedSuppression: 3,
        tpRisk: "low",
        rationale: "CSP header absence/misconfiguration is a hardening recommendation, not a directly exploitable vulnerability."
      },
      {
        id: "zap-missing-anti-csrf",
        name: "ZAP Missing Anti-CSRF Tokens (Generic)",
        description: "Suppress ZAP generic 'Absence of Anti-CSRF Tokens' when it fires on every form.",
        category: "config_observation",
        enabledByDefault: true,
        severityFilter: ["info", "low"],
        sourcePatterns: ["zap"],
        ...
        estimatedSuppression: 5,
        tpRisk: "medium",
        rationale: "ZAP flags every form without CSRF tokens. On training labs this generates many FPs since not all forms are state-changing. Real CSRF vulns should be detected by active scanning, not passive token absence."
      },
      {
        id: "nuclei-info-detection",
        name: "Nuclei Technology Detection Templates",
        description: "Suppress Nuclei templates that only detect technology presence (not vulnerabilities).",
        category: "config_observation",
        enabledByDefault: true,
        severityFilter: ["info"],
        sourcePatterns: ["nuclei"],
        titlePatterns: [
          /^tech-detect/i,
          /^waf-detect/i,
          /^fingerprint-/i,
        ...
        agentClassPatterns: [],
        estimatedSuppression: 6,
        tpRisk: "low",
        rationale: "Nuclei technology detection templates identify what software is running, not vulnerabilities."
      },
      {
        id: "duplicate-xss-variants",
        name: "Duplicate XSS Variant Findings",
        description: "Suppress duplicate XSS findings that differ only in payload or endpoint path variant.",
        category: "duplicate_finding",
        enabledByDefault: true,
        severityFilter: [],
        sourcePatterns: ["zap"],
        titlePatterns: [
          /^Cross Site Scripting \(Reflected\)$/i,
          /^Cross Site Scripting \(Persistent\)$/i
        ],
        agentClassPatterns: [],
        estimatedSuppression: 3,
        tpRisk: "medium",
        rationale: "ZAP reports XSS per-endpoint. Multiple XSS findings on the same parameter/page should be consolidated into one."
      }
    ];
...
export {
  ...
  getSuppressionRuleSummary,
  getSuppressionProfiles,
