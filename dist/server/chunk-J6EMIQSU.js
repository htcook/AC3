...
// server/lib/owasp-knowledge.ts
function getOwaspScanPlanContext(detectedTech) {
  const sections = [];
...
|----------|-------------|--------------|-------------|
${OWASP_CATEGORIES.map(
    (c) => `| ${c.id} ${c.name} | ${c.primaryTools.slice(0, 2).join(", ")} | ${c.detectionTemplates.slice(0, 3).join(", ") || "N/A"} | ${c.nucleiTags.slice(0, 4).join(", ")} |`
  ).join("\n")}`);
...
function getOwaspVulnCorrelationContext() {
  const sections = [];
...
|---------------|-----------------|-----------------|
${OWASP_CATEGORIES.map(
    (c) => `| ${c.id} ${c.name} | ${c.mitreTechniques.join(", ")} | ${c.attackPatterns[0]} |`
  ).join("\n")}`);
...
function getOwaspAssetClassificationContext() {
...
function getFullOwaspContext(detectedTech) {
  return [
...
function getOwaspPrioritiesForTech(technology) {
...
function getTestingCommandsForCategory(categoryId) {
  const cat = OWASP_CATEGORIES.find((c) => c.id === categoryId);
  return cat?.testingCommands ?? [];
}
function getAllOwaspCategories() {
  return [...OWASP_CATEGORIES];
}
function classifyFindingToOwasp(findingTitle, findingDescription) {
...
var OWASP_CATEGORIES;
var init_owasp_knowledge = __esm({
  "server/lib/owasp-knowledge.ts"() {
    OWASP_CATEGORIES = [
      {
        id: "A01:2025",
