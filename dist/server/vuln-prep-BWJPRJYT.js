...
} from "./chunk-VCSBL66M.js";
import "./chunk-UPHKBNCQ.js";
...
Total combined: ${combinedTechs.size} unique technologies`
  ...
  try {
    const { onBurpScanComplete } = await import("./burp-auto-scan-FEULVJKY.js");
    onBurpScanComplete(async (burpConfig, burpState) => {
      ...
      if (normalizedFindings && normalizedFindings.length > 0) {
        try {
          const { saveEngagementFindings } = await import("./db-PHFZ5GDL.js");
  ...
  try {
    const { getEngagementCredentials } = await import("./credential-harvester-DAGBX3UB.js");
  ...
  try {
    const { onEngagementVulnDetectionPhase, extractScopeUrls } = await import("./burp-auto-scan-FEULVJKY.js");
  ...
  try {
    const { runZapToBurpPipeline } = await import("./zap-burp-pipeline-66HWGVIK.js");
  ...
  try {
    const { runSeverityEscalation } = await import("./zap-burp-pipeline-66HWGVIK.js");
