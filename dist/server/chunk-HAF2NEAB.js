// server/lib/llm-specialists/evidence-package.ts
...
function isGenericCertificateIssuer(subjectO) {
...
function isPrivacyProxy(registrant) {
...
function isHostingProvider(name) {
  const lower = name.toLowerCase();
...
function buildEvidencePackage(assetIdentifier, discoveryResult, whoisData, httpFingerprint, businessIntelData) {
  ...
  if (discoveryResult) {
    if (discoveryResult.hosts) {
      pkg.observedIPs = discoveryResult.hosts.map((h) => h.ip || h.ip_str).filter(Boolean);
    }
...
function extractCertificateEvidence(data) {
  ...
  return {
    ...
    isExpired: validity.end ? new Date(validity.end) < /* @__PURE__ */ new Date() : void 0,
    isSelfSigned: subject.O === issuer.O && subject.CN === issuer.CN,
    isWildcard: (subject.CN || "").startsWith("*.")
  };
}
function extractDNSEvidence(data) {
  const dns = data.dns || data.dnsRecords || {};
  if (!dns && !data.hostnames) return void 0;
  return {
...
function extractBGPEvidence(data) {
  const bgp = data.bgp || data.asn_info || {};
  const asn = data.asn || bgp.asn;
  if (!asn && !bgp.asHolder) return void 0;
  return {
    asn: typeof asn === "number" ? asn : parseInt(asn, 10) || void 0,
...
function extractWHOISEvidence(whois) {
  return {
...
function extractHTTPEvidence(data) {
  const http = data.http || data.httpFingerprint || {};
  if (!http.server && !http.title && !http.technologies) return void 0;
  return {
    serverHeader: http.server || http.serverHeader,
    poweredBy: http.poweredBy || http["x-powered-by"],
    technologies: http.technologies || http.components ? Object.keys(http.components || {}) : void 0,
...
function extractBusinessIntelEvidence(data) {
  return {
...
function computeCrossReferenceConvergence(pkg) {
  const sources = [];
  const orgNames = [];
  ...
  const divergences = uniqueNormalized.length > 1 ? orgNames : void 0;
  return {
    sourcesChecked: sources,
    convergingOn: uniqueNormalized.length === 1 ? orgNames[0] : void 0,
    divergences
  };
}
function normalizeOrgName(name) {
  return name.toLowerCase().replace(/[,.\-_]/g, " ").replace(/\b(inc|llc|ltd|corp|co|na|n\.a\.|plc|gmbh|ag|sa)\b/gi, "").replace(/\s+/g, " ").trim();
}
function computeNegativeEvidence(pkg) {
  const checkedButNotFound = [];
...
function generateAssetId(identifier) {
