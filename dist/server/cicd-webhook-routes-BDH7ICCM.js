  ...
  app.post("/api/cicd/webhook/:pipelineId", async (req, res) => {
    ...
    try {
      ...
      if (secret) {
        ...
        if (signature) {
          const { verifyGitHubWebhook } = await import("./aws-cicd-connector-3KQGFQG4.js");
      ...
      if (targetUrl) {
        import("./aws-cicd-connector-3KQGFQG4.js").then(async ({ executeCicdScan }) => {
          try {
            ...
            if (pipeline.cicdProvider === "github_actions" && commitSha && repository) {
              try {
                ...
                if (githubToken) {
                  ...
                  const { callbackGitHubActions } = await import("./aws-cicd-connector-3KQGFQG4.js");
