...
async function persistObservations(observations) {
  ...
  try {
    ...
    const { scanObservations } = await import("./schema-5274GLXD.js");
...
async function persistSignals(signals) {
  ...
  try {
    const { getDb } = await import("./db-UCRYETLI.js");
    const { scanSignals } = await import("./schema-5274GLXD.js");
    const db = await getDb();
    if (!db) return 0;
    let inserted = 0;
...
async function persistRiskCards(cards) {
  ...
  try {
    const { getDb } = await import("./db-UCRYETLI.js");
    const { scanRiskCards } = await import("./schema-5274GLXD.js");
    const db = await getDb();
    if (!db) return 0;
    let inserted = 0;
