...
async function enrichFromThreatIntel(analyses) {
  try {
    const { enrichThreatIntelligence } = await import("./bug-bounty-intelligence-AONSLGNQ.js");
...
async function enrichFromOpSec(analyses, passiveRecon) {
  try {
    const { enrichOpSec } = await import("./bug-bounty-intelligence-AONSLGNQ.js");
...
async function enrichFromDiscoveryDeepDive(analyses, domain, passiveRecon) {
  try {
    const { securityTrailsDNSHistory, censysCertSearch } = await import("./discovery-engine-4RKAL5DC.js");
