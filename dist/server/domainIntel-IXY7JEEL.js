...
Risk signals detected (${passiveRecon.riskSignals.length}):`);
  ...
  try {
    const { applyMissionBaselines } = await import("./scoring-engine-7QJO2KKK.js");
    let missionIdx = 0;
    for (const a of analyses) {
      missionIdx++;
  ...
  let oemCredentials = [];
  try {
    const { matchCredentialsForAssets, persistMatchedCredentials } = await import("./oem-default-creds-RK6ZKYRP.js");
  ...
  if (oemCredentials.length > 0 && scanMode === "active") {
    try {
      const { runCredentialTests, getCredentialsForService } = await import("./credential-tester-SJBXAC2S.js");
  ...
  try {
    const { runPipelineCrawlStage, enrichOrgWithBusinessIntel, applyBusinessIntelCarverBoosts } = await import("./pipeline-crawl-stage-OINPIGWS.js");
  ...
  try {
    ...
    const { createCarverRiskCard } = await import("./db-LCEQKGBV.js");
    const assetSignals = [];
    if (passiveRecon) {
  ...
  const [threatMatchRetry, incidentSearchRetry, affiliatedDomainsRetry] = await retryMod.parallelWithRetry([
    ...
    },
    {
      ...
      fn: async () => {
        const { runIncidentSearchEnrichment } = await import("./incident-search-enrichment-7BQWBZDM.js");
        ...
        let ingestResult;
        try {
          const { ingestIncidentSearchResults } = await import("./incident-search-ingest-VILXMNSS.js");
    ...
    },
    {
      ...
      fn: async () => {
        const { runAffiliatedDomainDiscovery } = await import("./affiliated-domain-discovery-CCA7YPG2.js");
  ...
  try {
    const { collectTrainingData } = await import("./incident-training-collector-OITO3CRN.js");
