...
${postHeredoc}`;
  let dbRecordId;
  try {
    const { insertExploitationAttempt } = await import("./db-PHFZ5GDL.js");
  ...
  try {
    ...
    if (dbRecordId) {
      try {
        const { updateExploitationAttempt } = await import("./db-PHFZ5GDL.js");
...
async function getExecutionHistory(engagementId) {
  ...
  try {
    const { getExploitationAttempts } = await import("./db-PHFZ5GDL.js");
