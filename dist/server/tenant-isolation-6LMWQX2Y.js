...
async function resolveUserTenant(userId, requestedTenantId) {
  ...
  const { tenantMemberships, tenants } = await import("./schema-OF2ORZ4R.js");
...
async function autoProvisionTenant(userId, userName) {
  const { getDb } = await import("./db-F33RXQPM.js");
  const { tenants, tenantMemberships } = await import("./schema-OF2ORZ4R.js");
  const db = await getDb();
...
async function logTenantAction(userId, tenantId, action, details) {
  try {
    const { getDb } = await import("./db-F33RXQPM.js");
    const { activityLogs } = await import("./schema-OF2ORZ4R.js");
    const db = await getDb();
    if (!db) return;
...
async function getTenantStats(tenantId) {
  const { getDb } = await import("./db-F33RXQPM.js");
  const { tenantMemberships, engagements } = await import("./schema-OF2ORZ4R.js");
  const db = await getDb();
  if (!db) return null;
