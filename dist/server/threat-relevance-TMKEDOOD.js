...
// server/lib/llm-specialists/threat-relevance/specialist.ts
init_llm_json_parser();
...
function computeThreatRelevanceBaseline(pkg, customerIndustry) {
  ...
  for (const profile of THREAT_PROFILES) {
    ...
    const sectorMatch = customerIndustry && profile.sectorPreference?.some(
    ...
    if (pkg.http?.technologies) {
      ...
      if (profile.actorType === "ransomware_group") {
        const vulnTechs = techs.filter(
          (t) => ["exchange", "citrix", "pulse", "fortinet", "sonicwall", "f5"].some((v) => t.includes(v))
        );
  ...
  const overallThreatScore = actorRelevance.length > 0 ? Math.min(95, Math.round(actorRelevance.reduce((sum, a) => sum + a.relevanceScore, 0) / actorRelevance.length + sectorExposure.length * 10)) : 15;
  return { actorRelevance, sectorExposure, overallThreatScore };
}
var THREAT_RELEVANCE_SYSTEM_PROMPT = `You are the Threat Relevance Specialist for the AC3 platform. Analyze structured discovery evidence and assess the threat relevance of a digital asset.
...
async function invokeThreatRelevanceSpecialist(input, llmInvoke) {
...
Customer Industry: ${input.customerIndustry}` : "");
      ...
      actorRelevance = (parsed.actorRelevance || baseline.actorRelevance).map((ar) => {
      ...
      const allEvidence = actorRelevance.flatMap((ar) => ar.supportingEvidence || []);
      validationResult = validateGenericSpecialistOutput(
        allEvidence,
        actorRelevance.map((ar) => ar.reasoning).join(" "),
...
export {
  ...
  invokeThreatRelevanceSpecialist
};
