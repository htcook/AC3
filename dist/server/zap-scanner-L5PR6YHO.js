...
async function executeZapScanning(ctx) {
  ...
  for (const webApp of webApps) {
    ...
    for (const { targetUrl, dedupKey } of scanTargets) {
      ...
      try {
        ...
        const { generateLLMScanConfig, startScan, configureZapAuthentication } = await import("./zap-scanner-QAOIMTMD.js");
        ...
        if (wafVendor) {
          try {
            const { applyWafEvasionConfig } = await import("./zap-scanner-QAOIMTMD.js");
        ...
        if (!zapStarted) {
          ...
          try {
            const { onEngagementVulnDetectionPhase } = await import("./burp-auto-scan-GM4OFGUT.js");
        ...
        if (zapScanId) {
          ...
          const { pollScanProgress } = await import("./zap-scanner-QAOIMTMD.js");
            ...
            try {
              ...
              if (p.status === "completed" || p.status === "error") {
                ...
                try {
                  ...
                  if (db) {
                    const { webAppFindings } = await import("./schema-NFOE7JII.js");
  ...
  try {
    ...
    if (ir && (ir.urlSource === "scope_fallback" || ir.zapUrlsDiscovered === 0)) {
      ...
      const db = await getDb();
      if (db) {
        const { webAppScans } = await import("./schema-NFOE7JII.js");
        ...
        if (scans.length > 0) {
          const { deferredZapBurpRefeed } = await import("./zap-burp-pipeline-FGJFSATQ.js");
