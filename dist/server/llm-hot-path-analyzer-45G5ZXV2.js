...
// server/lib/llm-hot-path-analyzer.ts
function estimateCallCost(tokensIn, tokensOut, model = "gpt-4o") {
  const pricing = MODEL_PRICING[model] || MODEL_PRICING["gpt-4o"];
  return (tokensIn * pricing.inputPer1M + tokensOut * pricing.outputPer1M) / 1e6;
}
function analyzeOutputStability(records) {
  if (records.length < 5) return { stability: 0.5, determinism: 0.5 };
  ...
  const latencyCV = latencyMean > 0 ? Math.sqrt(latencies.reduce((s, v) => s + Math.pow(v - latencyMean, 2), 0) / latencies.length) / latencyMean : 1;
  const determinism = Math.max(0, Math.min(1, 1 - latencyCV * 0.5));
  return { stability, determinism };
}
function computeGraduationScore(caller, stability, determinism, callVolume, errorRate) {
...
function detectRedundancyClusters(callerStats) {
  ...
  for (let i = 0; i < callers.length; i++) {
    ...
    if (cluster.length > 1) {
      ...
      const deduplicatedCalls = Math.floor(totalCalls * 0.4);
      clusters.push({
        ...
        estimatedSavings: {
          ...
          tokensReduced: deduplicatedCalls * 2e3,
          // Avg tokens per call estimate
...
function computeCallerSimilarity(name1, stats1, name2, stats2) {
...
function analyzeHotPaths(telemetry, options = {}) {
  const { engagementId, topN = 20, minCallsForAnalysis = 5 } = options;
  const records = engagementId ? telemetry.filter((r) => r.engagementId === engagementId) : telemetry;
  if (records.length === 0) {
    return {
      engagementId,
      analyzedAt: Date.now(),
  ...
  for (const [caller, callerRecords] of callerGroups) {
    ...
    hotPaths.push({
      caller,
      totalCalls: calls,
      callsPerEngagement: calls,
      // Will be divided if multi-engagement
...
function generateRecommendations(hotPaths, clusters, totalCalls, totalCost) {
  ...
  for (const hp of hotPaths) {
    if (hp.graduationRecommendation === "graduate_now") {
      recommendations.push({
        ...
        estimatedImpact: {
          callsReduced: Math.floor(hp.totalCalls * 0.9),
          // 90% can be graduated
  ...
  for (const cluster of clusters) {
    recommendations.push({
      ...
      estimatedImpact: {
        ...
        latencyReduced: cluster.estimatedSavings.callsReduced * 1500
        // Avg 1.5s per call
      },
      implementationComplexity: "moderate",
    ...
    if (hp.graduationRecommendation === "batch_optimize" && hp.totalCalls > 20) {
      recommendations.push({
        ...
        estimatedImpact: {
          ...
          costReduced: hp.estimatedCost * 0.2,
          // Batching reduces calls but not tokens much
          latencyReduced: hp.totalLatencyMs * 0.5
        },
        implementationComplexity: "moderate",
        confidence: 0.6
...
function projectSavings(recommendations, totalCalls, totalCost, totalLatency) {
  let callsReduced = 0;
  let costReduced = 0;
...
function formatHotPathSummary(analysis) {
  const lines = [
...
var MODEL_PRICING, NON_GRADUATABLE_CALLERS, GRADUATION_CANDIDATE_PATTERNS;
var init_llm_hot_path_analyzer = __esm({
  "server/lib/llm-hot-path-analyzer.ts"() {
    MODEL_PRICING = {
    ...
    NON_GRADUATABLE_CALLERS = /* @__PURE__ */ new Set([
...
init_llm_hot_path_analyzer();
export {
  analyzeHotPaths,
  formatHotPathSummary
};
