...
// server/lib/passive/container-discovery.ts
import { createHash } from "crypto";
function makeAssetId(domain, name, source) {
  return createHash("sha256").update(`${domain}|${name}|${source}`).digest("hex").slice(0, 20);
}
async function probeEndpoint(host, port, path, method, timeout) {
...
function checkSignatures(probe, response) {
  const matched = [];
...
async function runProbe(host, probe, timeout) {
  const hits = [];
...
function generateContainerSubdomains(domain) {
  const prefixes = [
    "registry",
    "docker",
    "containers",
...
async function analyzeContainerExposure(domain, additionalHosts, timeout = 3e3, globalTimeoutMs = 6e4) {
...
var CONTAINER_PROBES, containerDiscoveryConnector;
var init_container_discovery = __esm({
  "server/lib/passive/container-discovery.ts"() {
    CONTAINER_PROBES = [
      // ── Docker Registry v2 ──
      {
        ...
        category: "registry",
        defaultPorts: [5e3, 5001, 443, 8080],
        paths: [
      ...
      // ── Kubernetes Dashboard ──
      {
        ...
        category: "dashboard",
        defaultPorts: [8443, 443, 3e4, 31e3, 32e3],
        paths: [
    ...
    containerDiscoveryConnector = {
      ...
      async collect(domain, config) {
        const timeout = Math.min(config?.timeout ?? 3e3, 2e3);
        const globalTimeout = Math.min(config?.timeout ? config.timeout * 8 : 25e3, 25e3);
        const startTime = Date.now();
        const observations = [];
        const errors = [];
        ...
        for (let i = 0; i < rawCandidates.length; i += DNS_CHECK_CONCURRENCY) {
          if (Date.now() - startTime > globalTimeout * 0.3) break;
