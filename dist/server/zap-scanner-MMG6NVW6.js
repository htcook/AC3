  ...
  for (const webApp of webApps) {
    ...
    for (const { targetUrl, dedupKey } of scanTargets) {
      ...
      try {
        ...
        const { generateLLMScanConfig, startScan, configureZapAuthentication } = await import("./zap-scanner-6BDS4BKL.js");
        ...
        if (wafVendor) {
          try {
            const { applyWafEvasionConfig } = await import("./zap-scanner-6BDS4BKL.js");
        ...
        if (!zapStarted) {
          ...
          try {
            const { onEngagementVulnDetectionPhase } = await import("./burp-auto-scan-Z44C6PGF.js");
        ...
        if (zapScanId) {
          ...
          const { pollScanProgress } = await import("./zap-scanner-6BDS4BKL.js");
            ...
            try {
              ...
              if (p.status === "completed" || p.status === "error") {
                ...
                try {
                  ...
                  if (db) {
                    const { webAppFindings } = await import("./schema-XOTPZHKC.js");
  ...
  try {
    ...
    if (ir && (ir.urlSource === "scope_fallback" || ir.zapUrlsDiscovered === 0)) {
      ...
      if (db) {
        ...
        if (scans.length > 0) {
          const { deferredZapBurpRefeed } = await import("./zap-burp-pipeline-KNZ2J6QE.js");
