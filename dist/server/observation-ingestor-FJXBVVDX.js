...
async function persistObservations(observations) {
  ...
  try {
    ...
    const { scanObservations } = await import("./schema-XIZFPXHE.js");
...
async function persistSignals(signals) {
  ...
  try {
    const { getDb } = await import("./db-FQGKASI3.js");
    const { scanSignals } = await import("./schema-XIZFPXHE.js");
    const db = await getDb();
    if (!db) return 0;
    let inserted = 0;
...
async function persistRiskCards(cards) {
  ...
  try {
    const { getDb } = await import("./db-FQGKASI3.js");
    const { scanRiskCards } = await import("./schema-XIZFPXHE.js");
    const db = await getDb();
    if (!db) return 0;
    let inserted = 0;
