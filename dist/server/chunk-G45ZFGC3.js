...
// server/lib/dns-security-validator.ts
...
async function httpProbe(url, timeoutMs = 8e3) {
  try {
...
async function dnsGoogleResolve(domain, type) {
  try {
...
async function runDnsSecurityAssessment(domain, context = "di_scan") {
  const validator = new DnsSecurityValidator(domain, context);
  return validator.runFullAssessment();
}
var TAKEOVER_FINGERPRINTS, DNSSEC_ALGORITHMS, DnsSecurityValidator;
var init_dns_security_validator = __esm({
  "server/lib/dns-security-validator.ts"() {
    ...
    DnsSecurityValidator = class {
      constructor(domain, context = "di_scan") {
        this.findings = [];
        this.records = [];
      ...
      async collectRecords() {
        ...
        if (caaRecs) caaRecs.forEach((r) => this.records.push({ type: "CAA", name: this.domain, value: `${r.critical ? "critical" : "0"} ${r.issue || r.issuewild || r.iodef || JSON.stringify(r)}`, additional: r }));
      ...
      async checkDanglingSecurity() {
        ...
        for (const aRec of aRecords) {
          const probe = await httpProbe(`http://${aRec.value}`, 5e3);
          if (!probe) {
            this.findings.push({
          ...
          for (const key of dnskeyRecords) {
            ...
            if (algoInfo && (algoInfo.strength === "deprecated" || algoInfo.strength === "weak")) {
              this.findings.push({
                ...
                cvssScore: algoInfo.strength === "deprecated" ? 7 : 5.3,
                cwe: "CWE-327"
              });
              this.failedChecks++;
            } else {
      ...
      async buildDnssecStatus() {
        ...
        const parsedDnskey = dnskeyRecords.map((r) => {
          const parts = (r.data || "").split(" ");
        ...
        return {
          ...
          dsRecords: parsedDs.length > 0 ? parsedDs : void 0,
          dnskeyRecords: parsedDnskey.length > 0 ? parsedDnskey : void 0,
      ...
      async checkOpenResolver() {
        ...
        for (const aRec of aRecords) {
          try {
            ...
            const result = await dnsQuery(() => resolver.resolve4("example.com"), 3e3);
      ...
      async checkWildcard() {
        ...
        const wildcardResult = await dnsQuery(() => resolve4(randomSub), 3e3);
      ...
      async checkTunnelingIndicators() {
      ...
      async checkVersionDisclosure() {
        ...
        for (const ns of nsRecords.slice(0, 2)) {
          try {
            ...
            const versionResult = await dnsQuery(() => resolver.resolveTxt("version.bind"), 3e3);
      ...
      async checkRateLimiting() {
        ...
        const nsRecords = this.records.filter((r) => r.type === "NS");
        const isCloudDns = nsRecords.some(
          (r) => r.value.includes("cloudflare") || r.value.includes("awsdns") || r.value.includes("azure-dns") || r.value.includes("google")
        );
        if (isCloudDns) {
          this.findings.push({
