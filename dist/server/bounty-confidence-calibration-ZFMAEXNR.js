...
// server/lib/bounty-confidence-calibration.ts
var CONFIDENCE_FACTORS, ConfidenceCalibrationEngine, confidenceCalibrationEngine;
var init_bounty_confidence_calibration = __esm({
  "server/lib/bounty-confidence-calibration.ts"() {
    CONFIDENCE_FACTORS = [
      {
        ...
        evaluate: (ctx) => {
          if (ctx.manuallyVerified && ctx.evidenceCount >= 3) {
            return { score: 1, evidence: "Manually verified with multiple evidence artifacts", direction: "positive" };
          }
      ...
      },
      {
        ...
        evaluate: (ctx) => {
          if (ctx.isReproducible) {
            return { score: 1, evidence: "Finding is reliably reproducible", direction: "positive" };
          }
      ...
      },
      {
        ...
        weight: 0.15,
        evaluate: (ctx) => {
          if (ctx.scannerConfidence === void 0) {
      ...
      },
      {
        ...
        evaluate: (ctx) => {
          ...
          const expectedSeverities = Object.entries(severityExpectation).filter(([, classes]) => classes.includes(ctx.vulnClass)).map(([sev]) => sev);
    ...
    ConfidenceCalibrationEngine = class {
      constructor() {
        this.records = [];
        this.curves = /* @__PURE__ */ new Map();
        this.programAdjustments = /* @__PURE__ */ new Map();
      }
      // programHandle → bias adjustment
      /**
       * Assess confidence with full reasoning chain
       */
      assessConfidence(ctx) {
        const factors = CONFIDENCE_FACTORS.map((factorDef) => {
        ...
        const reasoning = {
          ...
          adjustmentExplanation: totalAdjustment !== 0 ? `Score adjusted by ${totalAdjustment > 0 ? "+" : ""}${(totalAdjustment * 100).toFixed(1)}% based on historical calibration (vuln class: ${(calibrationAdj * 100).toFixed(1)}%, program: ${(programAdj * 100).toFixed(1)}%)` : "No calibration adjustment applied (insufficient historical data)"
        };
        ...
        return {
          ...
          driftWarning: driftWarning || void 0
        };
      }
      /**
      ...
      detectDrift() {
        ...
        const vulnClassBiases = /* @__PURE__ */ new Map();
        for (const record of this.records) {
        ...
        const worstVulnClasses = Array.from(vulnClassBiases.entries()).map(([vulnClass, data]) => ({
          vulnClass,
          bias: data.total / data.count,
          sampleSize: data.count
        })).filter((v) => Math.abs(v.bias) > 0.1 && v.sampleSize >= 3).sort((a, b) => Math.abs(b.bias) - Math.abs(a.bias)).slice(0, 5);
        ...
        const direction = overallBias > 0.05 ? "overconfident" : overallBias < -0.05 ? "underconfident" : "well_calibrated";
        let recommendation;
        if (severity === "none") {
        ...
        return {
          ...
          severity,
          direction,
          overallBias: Math.round(overallBias * 1e3) / 1e3,
          worstVulnClasses,
          recommendation,
          lastChecked: Date.now()
        };
      ...
      // ─── Private Helpers ─────────────────────────────────────────────────────
      getCalibrationAdjustment(vulnClass) {
        const curve = this.curves.get(vulnClass);
        if (!curve || curve.sampleSize < 5) return 0;
        return -curve.overallBias * 0.5;
      }
      checkDriftForVulnClass(vulnClass) {
        const curve = this.curves.get(vulnClass);
      ...
      updateCurve(vulnClass) {
        ...
        const buckets = [
          ...
          { predictedRange: { min: 0.8, max: 1 }, actualAcceptanceRate: 0, sampleCount: 0, isCalibrated: false }
        ];
        for (const record of relevant) {
          const bucket = buckets.find(
            (b) => record.predictedConfidence >= b.predictedRange.min && record.predictedConfidence < b.predictedRange.max
      ...
      updateProgramAdjustment(programHandle) {
        ...
        this.programAdjustments.set(programHandle, -avgBias * 0.3);
      }
      scoreToLevel(score) {
      ...
      buildReasoningSummary(factors, score) {
    ...
    confidenceCalibrationEngine = new ConfidenceCalibrationEngine();
  }
});
init_bounty_confidence_calibration();
export {
