...
${postHeredoc}`;
  let dbRecordId;
  try {
    const { insertExploitationAttempt } = await import("./db-TULZR6OH.js");
  ...
  try {
    ...
    if (dbRecordId) {
      try {
        const { updateExploitationAttempt } = await import("./db-TULZR6OH.js");
...
async function getExecutionHistory(engagementId) {
  ...
  try {
    const { getExploitationAttempts } = await import("./db-TULZR6OH.js");
