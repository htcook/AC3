...
// server/lib/report-section-blueprints.ts
var DOCUMENT_CONTROL = {
  id: "document_control",
...
var RISK_MATRIX = {
  ...
  title: "Risk Matrix",
  required: true,
  promptGuidance: "Present a likelihood \xD7 impact risk matrix mapping each finding to its risk rating. Include CVSS base scores, environmental adjustments, and business context modifiers.",
  dataSources: ["findings", "risk_ratings"]
};
var REMEDIATION_ROADMAP = {
...
function getReportBlueprint(assessmentType) {
  const normalized = assessmentType.toLowerCase().replace(/[\s-]/g, "_");
  const typeMap = {
    red_team: "red_team",
...
function getReportSectionTitles(assessmentType) {
...
function getSectionPromptGuidance(assessmentType, sectionId) {
  const blueprint = getReportBlueprint(assessmentType);
  const section = blueprint.sections.find((s) => s.id === sectionId);
  return section?.promptGuidance ?? null;
}
function buildSectionOutline(assessmentType) {
  const blueprint = getReportBlueprint(assessmentType);
