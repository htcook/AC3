...
async function persistObservations(observations) {
  ...
  try {
    ...
    const { scanObservations } = await import("./schema-RDUWS2ES.js");
...
async function persistSignals(signals) {
  ...
  try {
    const { getDb } = await import("./db-LSUZDHGJ.js");
    const { scanSignals } = await import("./schema-RDUWS2ES.js");
    const db = await getDb();
    if (!db) return 0;
    let inserted = 0;
...
async function persistRiskCards(cards) {
  ...
  try {
    const { getDb } = await import("./db-LSUZDHGJ.js");
    const { scanRiskCards } = await import("./schema-RDUWS2ES.js");
    const db = await getDb();
    if (!db) return 0;
    let inserted = 0;
