...
function resolveCalderaPassword() {
...
var init_env = __esm({
  "server/_core/env.ts"() {
    ENV = {
      ...
      AWS_MSF_AMI_ID: process.env.AWS_MSF_AMI_ID || "",
      // Custom AMI with MSF pre-installed (optional)
      ...
      // KMS Key ARN
      S3_BUCKET_KEY_ENABLED: process.env.S3_BUCKET_KEY_ENABLED ?? "false",
      // Reduce KMS API calls
      S3_PRIVATE_MODE: process.env.S3_PRIVATE_MODE ?? "false",
      // Force presigned URLs (auto-enabled with SSE)
      S3_USE_FIPS: process.env.S3_USE_FIPS ?? "",
      // "true" to force FIPS endpoints (auto for us-gov-* regions)
      // Client-Side Encryption (CSE) — envelope encryption for highest-sensitivity artifacts
      S3_CSE_KEY_ARN: process.env.S3_CSE_KEY_ARN ?? "",
      // KMS CMK ARN for data key encryption (or local key ID)
      S3_CSE_ENABLED: process.env.S3_CSE_ENABLED ?? "false",
      // "true" to enable CSE for doStoragePutEncrypted/GetDecrypted
      ...
      SLIVER_OPERATOR_CONFIG: process.env.SLIVER_OPERATOR_CONFIG ?? "",
      // Base64-encoded operator config JSON
