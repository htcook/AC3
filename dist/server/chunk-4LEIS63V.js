...
async function indexMetasploitModules() {
  let count = 0;
  try {
    const { ingestMetasploitModules } = await import("./ttp-ingest-ZEXSEWJR.js");
...
async function indexAc3History() {
  let count = 0;
  try {
    const { recordFeedback, getModulePerformance } = await import("./exploit-feedback-loop-DOJAQUCA.js");
