...
} from "./chunk-WQ4FJHOM.js";
import {
  classifyVulnClass,
  init_exploit_learning_engine
} from "./chunk-JEOGLS4V.js";
import {
...
async function clearOpsState(engagementId) {
  ...
  try {
    const { deleteOpsSnapshot } = await import("./db-UCRYETLI.js");
...
async function getOpsStateWithRecovery(engagementId) {
  ...
  try {
    const { loadOpsSnapshot } = await import("./db-UCRYETLI.js");
...
function persistOpsStateDebounced(engagementId, delayMs = 2e3) {
  ...
  persistTimers.set(engagementId, setTimeout(async () => {
    ...
    if (!state) return;
    try {
      const { saveOpsSnapshot } = await import("./db-UCRYETLI.js");
...
async function flushAllPendingState() {
  ...
  try {
    const { releaseAllClaims } = await import("./engagement-claim-lock-NAKMYPWU.js");
...
async function persistTimelineEvent(engagementId, logEntry) {
  try {
    ...
    const { engagementTimelineEvents } = await import("./schema-5274GLXD.js");
...
async function persistScanResult(opts) {
  try {
    const { insertScanResult, saveEngagementFindings } = await import("./db-UCRYETLI.js");
...
async function auditLog(params) {
  try {
    const { getDb } = await import("./db-UCRYETLI.js");
    const { offensiveAuditLog } = await import("./schema-5274GLXD.js");
    const db = await getDb();
    if (db) {
...
Use alternative tools instead.`;
  ...
  try {
    ...
    const { planAttack } = await import("./attack-planner-3ZGVW6PH.js");
...
async function llmDecide(context) {
  ...
  if (!skipSpecialist) try {
    const { decideNextOp } = await import("./ops-decider-3WPNG75Q.js");
...
${Object.entries(state.dastConfig.customHeaders).map(([k, v]) => `  ${k}: ${v}`).join("\n")}`
  ...
  for (const domain of domains) {
    try {
      ...
      const { isLabDomain } = await import("./passive-P7WX33WH.js");
      ...
      try {
        ...
        if (domainAssets.length > 0) {
          const { analyzeScan } = await import("./scan-analyst-J36L5IMH.js");
...
${(analysis.recommendations || []).slice(0, 3).map((r) => `\u2022 [${r.priority}] ${r.action}`).join("\n")}`,
      ...
      try {
        const domainAssets = state.assets.filter((a) => a.hostname === domain || a.hostname.endsWith("." + domain));
        if (domainAssets.length > 0) {
          const { scoreFullHybrid, buildEngagementContext } = await import("./hybrid-scorer-NX6UIRZ6.js");
...
async function executeEnumeration(state, engagement, operatorCtx) {
  const { executeEnumeration: runEnumerationPhase } = await import("./engagement-phase-enumeration-QXIL4HYJ.js");
...
async function executeVulnDetection(state, engagement, operatorCtx) {
  ...
  const { executeVulnPrep } = await import("./vuln-prep-SVILU36T.js");
  const vulnPrepCtx = {
    state,
    engagement,
    operatorCtx,
  ...
  const { executeZapScanning } = await import("./zap-scanner-JP6O2ISE.js");
  ...
  const { executeCredentialTesting } = await import("./credential-tester-PT3PCN74.js");
  ...
  const { executeVulnCorrelation } = await import("./vuln-correlation-2EDSCZUF.js");
...
async function executeExploitation(state, engagement, operatorCtx) {
  const { executeExploitation: runExploitPhase } = await import("./engagement-phase-exploitation-2EJABQWZ.js");
  return runExploitPhase(state, engagement, operatorCtx);
}
async function executePostExploit(state, engagement, operatorCtx) {
  const { executePostExploit: runPostExploitPhase } = await import("./engagement-phase-post-exploit-VABL3OK5.js");
...
async function executeEngagement(engagementId, operatorCtx, options) {
  ...
  try {
    const { claimEngagement } = await import("./engagement-claim-lock-NAKMYPWU.js");
...
Preserved: ${state.assets.length} assets, ${state.stats.vulnsFound} vulns, ${state.stats.portsFound} ports, ${state.log.length} log entries`
  ...
  let engagement;
  try {
    const db = await import("./db-UCRYETLI.js");
...
Max blast radius: ${engagementSafetyLevel === "passive_only" ? 5 : engagementSafetyLevel === "low_impact" ? 30 : engagementSafetyLevel === "standard" ? 60 : 100}`
  ...
  try {
    const { buildEngagementContext } = await import("./hybrid-scorer-NX6UIRZ6.js");
  ...
  try {
    const { setActiveUser } = await import("./bug-bounty-intelligence-DDCCF7NW.js");
  ...
  try {
    if (startPhase === "recon") {
      ...
      if (!reconGate.allowed) {
        ...
        await breathe();
        try {
          const { executeCustomerIntegrationsForStage, mergeIntegrationResultsIntoObservations } = await import("./pipeline-bridge-NKP47M3U.js");
    ...
    if (["recon", "passive_discovery"].includes(startPhase)) {
      try {
        ...
        await breathe();
        try {
          const { executeCustomerIntegrationsForStage } = await import("./pipeline-bridge-NKP47M3U.js");
        ...
        if (!vulnGate.allowed) {
          ...
          if (state.stats.vulnsFound > 0) {
            try {
              ...
              const { batchAnalyzeFindings, generateAnalysisSummary, classifyVulnerability } = await import("./vuln-analysis-agents-HMHLDH4D.js");
...
PoC: ${cf.analysis.poc || "N/A"}`,
        ...
        if (state.stats.vulnsFound > 0 || state.assets.some((a) => a.cloudProviders?.length > 0)) {
          try {
            ...
            const { runFeedbackLoop, getFeedbackLoopSummary } = await import("./llm-scan-feedback-P3BJSLOK.js");
...
${feedbackState.finalAnalysis?.slice(0, 500) || ""}`,
        ...
        if (state.stats.vulnsFound > 0) {
          try {
            ...
            const { generateEngagementAttackChains } = await import("./cloud-attack-chain-designer-XM4XURGV.js");
...
Promoted: ${promotionSummary.promotedVulns.map((p) => `${p.vulnTitle.slice(0, 50)} [${p.scanner}/${p.category}]`).join("; ")}`,
        ...
        {
          const { executeSocialEngineering } = await import("./engagement-phase-social-engineering-BROA3VW2.js");
      ...
      try {
        const { retryDeferredScans, getDeferredScans, clearDeferredScans } = await import("./job-queue-bridge-WGNVN72C.js");
      ...
      try {
        const { selectFindingsForScreenshot, captureScreenshotBatch } = await import("./screenshot-capture-K5WO4DXU.js");
      ...
      try {
        const { generateAttackNarratives, generateExecutiveSummary } = await import("./attack-narrative-generator-PJF2POUW.js");
      ...
      try {
        const { runAccuracyComparison } = await import("./accuracy-feedback-loop-RA4MQOJX.js");
        ...
        if (targetPreset) {
          ...
          let engNotes = null;
          try {
            const { engagements: engTable } = await import("./schema-5274GLXD.js");
            const { getDbRequired: getDbReq } = await import("./db-UCRYETLI.js");
...
View details on the Knowledge Base \u2192 Accuracy Feedback tab.`
          ...
          try {
            const { runLocalAccuracyComparison } = await import("./accuracy-feedback-loop-RA4MQOJX.js");
    ...
    try {
      const { generateAutoReport } = await import("./engagement-auto-report-6G5JF7V4.js");
    ...
    try {
      ...
      broadcastOpsUpdate(state.engagementId, { type: "log_update" });
      const { generateTestPlanAdherence } = await import("./engagement-report-handoff-3DO6U4G4.js");
    ...
    try {
      const { updateEngagement } = await import("./db-UCRYETLI.js");
    ...
    try {
      const { saveEngagementResult, saveEngagementFindings } = await import("./db-UCRYETLI.js");
    ...
    try {
      const { getEngagementLlmTelemetryRaw } = await import("./db-UCRYETLI.js");
    ...
    try {
      const { runPostPipelineGraduation, extractEngagementMetrics } = await import("./post-pipeline-graduation-AVD5IKAA.js");
    ...
    try {
      const { detectGaps, createGapsBatch } = await import("./intelligence-gaps-BZAJQEIJ.js");
    ...
    try {
      const { updateProfileFromEngagement } = await import("./customer-intel-profile-MHCTYJ5L.js");
    ...
    try {
      const { updateEngagement } = await import("./db-UCRYETLI.js");
      const existingEng = await (await import("./db-UCRYETLI.js")).getEngagementById(engagementId);
...
async function recoverInterruptedEngagements() {
  ...
  try {
    const { getDbRequired } = await import("./db-UCRYETLI.js");
    const db = await getDbRequired();
    const { engagementOpsSnapshots } = await import("./schema-5274GLXD.js");
...
Command: ${command}`,
  ...
  try {
    const { executeTool } = await import("./scan-server-executor-JZBPP72B.js");
