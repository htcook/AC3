...
} from "./chunk-TEA3CEJO.js";
import {
  executeToolViaQueue,
  init_job_queue_bridge
} from "./chunk-NKJ4ZN5O.js";
import {
...
} from "./chunk-S4BLXAZZ.js";
import {
  captureDecision,
  init_engagement_training_bridge
} from "./chunk-6XYVDYNL.js";
import {
...
} from "./chunk-XJZ5LAUE.js";
import {
  classifyVulnClass,
  init_exploit_learning_engine
} from "./chunk-VERTPT5D.js";
import {
...
async function clearOpsState(engagementId) {
  ...
  try {
    const { deleteOpsSnapshot } = await import("./db-65DPEQYH.js");
...
async function getOpsStateWithRecovery(engagementId) {
  ...
  try {
    const { loadOpsSnapshot } = await import("./db-65DPEQYH.js");
...
function persistOpsStateDebounced(engagementId, delayMs = 2e3) {
  ...
  persistTimers.set(engagementId, setTimeout(async () => {
    ...
    if (!state) return;
    try {
      const { saveOpsSnapshot } = await import("./db-65DPEQYH.js");
...
async function flushAllPendingState() {
  ...
  try {
    const { releaseAllClaims } = await import("./engagement-claim-lock-FM34MTZ2.js");
...
async function persistTimelineEvent(engagementId, logEntry) {
  try {
    const { getDb } = await import("./db-65DPEQYH.js");
    const { engagementTimelineEvents } = await import("./schema-TGCU627K.js");
...
async function persistScanResult(opts) {
  try {
    const { insertScanResult, saveEngagementFindings } = await import("./db-65DPEQYH.js");
...
async function auditLog(params) {
  try {
    const { getDb } = await import("./db-65DPEQYH.js");
    const { offensiveAuditLog } = await import("./schema-TGCU627K.js");
    const db = await getDb();
    if (db) {
...
Generate two-phase scan plan. Phase A: ScanForge discovery --top-ports 1000. Phase B: tools per asset.`;
  ...
  try {
    ...
    try {
      const { getToolInventory, getInventoryForLLM } = await import("./scan-server-inventory-4V7MBN6G.js");
...
Use alternative tools instead.`;
  ...
  try {
    ...
    const { planAttack } = await import("./attack-planner-EEYPLKXE.js");
...
async function llmDecide(context) {
  ...
  if (!skipSpecialist) try {
    const { decideNextOp } = await import("./ops-decider-BCAMU64K.js");
...
${Object.entries(state.dastConfig.customHeaders).map(([k, v]) => `  ${k}: ${v}`).join("\n")}`
  ...
  for (const domain of domains) {
    try {
      ...
      const { isLabDomain } = await import("./passive-TM43ZSNQ.js");
      ...
      const { runDomainIntelPipeline } = await import("./domainIntel-MCS43YYR.js");
      ...
      try {
        ...
        if (domainAssets.length > 0) {
          const { analyzeScan } = await import("./scan-analyst-FMGOJ7B5.js");
...
${(analysis.recommendations || []).slice(0, 3).map((r) => `\u2022 [${r.priority}] ${r.action}`).join("\n")}`,
      ...
      try {
        const domainAssets = state.assets.filter((a) => a.hostname === domain || a.hostname.endsWith("." + domain));
        if (domainAssets.length > 0) {
          const { scoreFullHybrid, buildEngagementContext } = await import("./hybrid-scorer-UPA2MWAF.js");
...
async function executeEnumeration(state, engagement, operatorCtx) {
  const { executeEnumeration: runEnumerationPhase } = await import("./engagement-phase-enumeration-HZ73YJ4S.js");
...
async function executeVulnDetection(state, engagement, operatorCtx) {
  ...
  const { executeVulnPrep } = await import("./vuln-prep-DS3O5TVR.js");
  const vulnPrepCtx = {
    state,
    engagement,
    operatorCtx,
  ...
  const { executeZapScanning } = await import("./zap-scanner-SV6NI3TG.js");
  ...
  const { executeInjectionScanning } = await import("./injection-scanner-4QV5JDC6.js");
  ...
  const { executeCredentialTesting } = await import("./credential-tester-CX7222EI.js");
  ...
  const { executeVulnCorrelation } = await import("./vuln-correlation-E4JUEQXX.js");
...
async function executeExploitation(state, engagement, operatorCtx) {
  const { executeExploitation: runExploitPhase } = await import("./engagement-phase-exploitation-G3TYMK5F.js");
  return runExploitPhase(state, engagement, operatorCtx);
}
async function executePostExploit(state, engagement, operatorCtx) {
  const { executePostExploit: runPostExploitPhase } = await import("./engagement-phase-post-exploit-OGR6P2XA.js");
...
async function executeEngagement(engagementId, operatorCtx, options) {
  ...
  try {
    const { claimEngagement } = await import("./engagement-claim-lock-FM34MTZ2.js");
...
Preserved: ${state.assets.length} assets, ${state.stats.vulnsFound} vulns, ${state.stats.portsFound} ports, ${state.log.length} log entries`
  ...
  let engagement;
  try {
    const db = await import("./db-65DPEQYH.js");
...
Max blast radius: ${engagementSafetyLevel === "passive_only" ? 5 : engagementSafetyLevel === "low_impact" ? 30 : engagementSafetyLevel === "standard" ? 60 : 100}`
  ...
  try {
    const { buildEngagementContext } = await import("./hybrid-scorer-UPA2MWAF.js");
  ...
  try {
    const { setActiveUser } = await import("./bug-bounty-intelligence-6H5NHX5N.js");
  ...
  try {
    if (startPhase === "recon") {
      ...
      if (!reconGate.allowed) {
        ...
        await breathe();
        try {
          const { executeCustomerIntegrationsForStage, mergeIntegrationResultsIntoObservations } = await import("./pipeline-bridge-HYJDPDVH.js");
    ...
    if (["recon", "passive_discovery"].includes(startPhase)) {
      try {
        ...
        await breathe();
        try {
          const { executeCustomerIntegrationsForStage } = await import("./pipeline-bridge-HYJDPDVH.js");
    ...
    if (engagement.roeStatus === "signed" || engagement.roeStatus === "pending" || state.trainingLabMode === true) {
      if (["recon", "passive_discovery", "scoping", "test_plan", "enumeration"].includes(startPhase)) {
        ...
        if (!enumGate.allowed) {
          ...
          try {
            const { checkScanServerStatus } = await import("./scan-server-executor-NMDOA3SN.js");
        ...
        if (!vulnGate.allowed) {
          ...
          if (state.stats.vulnsFound > 0) {
            try {
              ...
              const { batchAnalyzeFindings, generateAnalysisSummary, classifyVulnerability } = await import("./vuln-analysis-agents-HJKFCBCK.js");
...
PoC: ${cf.analysis.poc || "N/A"}`,
        ...
        if (state.stats.vulnsFound > 0 || state.assets.some((a) => a.cloudProviders?.length > 0)) {
          try {
            ...
            const { runFeedbackLoop, getFeedbackLoopSummary } = await import("./llm-scan-feedback-ULYLZWDV.js");
...
${feedbackState.finalAnalysis?.slice(0, 500) || ""}`,
        ...
        if (state.stats.vulnsFound > 0) {
          try {
            ...
            const { generateEngagementAttackChains } = await import("./cloud-attack-chain-designer-563R2A6P.js");
...
Promoted: ${promotionSummary.promotedVulns.map((p) => `${p.vulnTitle.slice(0, 50)} [${p.scanner}/${p.category}]`).join("; ")}`,
        ...
        {
          const { executeSocialEngineering } = await import("./engagement-phase-social-engineering-KSWYNUCH.js");
      ...
      try {
        const { retryDeferredScans, getDeferredScans, clearDeferredScans } = await import("./job-queue-bridge-FMD5KEVV.js");
      ...
      try {
        const { selectFindingsForScreenshot, captureScreenshotBatch } = await import("./screenshot-capture-IWDDK76N.js");
      ...
      try {
        const { generateAttackNarratives, generateExecutiveSummary } = await import("./attack-narrative-generator-OID7VWEW.js");
      ...
      try {
        const { runAccuracyComparison } = await import("./accuracy-feedback-loop-ZRMGJ55G.js");
        ...
        if (targetPreset) {
          ...
          let engNotes = null;
          try {
            const { engagements: engTable } = await import("./schema-TGCU627K.js");
            const { getDbRequired: getDbReq } = await import("./db-65DPEQYH.js");
...
View details on the Knowledge Base \u2192 Accuracy Feedback tab.`
          ...
          try {
            const { runLocalAccuracyComparison } = await import("./accuracy-feedback-loop-ZRMGJ55G.js");
    ...
    try {
      const { generateAutoReport } = await import("./engagement-auto-report-HU2UYAX2.js");
    ...
    try {
      ...
      broadcastOpsUpdate(state.engagementId, { type: "log_update" });
      const { generateTestPlanAdherence } = await import("./engagement-report-handoff-H7LISCFZ.js");
    ...
    try {
      const { updateEngagement } = await import("./db-65DPEQYH.js");
    ...
    try {
      const { saveEngagementResult, saveEngagementFindings } = await import("./db-65DPEQYH.js");
    ...
    try {
      const { getEngagementLlmTelemetryRaw } = await import("./db-65DPEQYH.js");
    ...
    try {
      const { runPostPipelineGraduation, extractEngagementMetrics } = await import("./post-pipeline-graduation-H6NI5FZI.js");
    ...
    try {
      const { detectGaps, createGapsBatch } = await import("./intelligence-gaps-TKLUCZID.js");
    ...
    try {
      const { updateProfileFromEngagement } = await import("./customer-intel-profile-2RFADFNG.js");
    ...
    try {
      const { updateEngagement } = await import("./db-65DPEQYH.js");
      const existingEng = await (await import("./db-65DPEQYH.js")).getEngagementById(engagementId);
