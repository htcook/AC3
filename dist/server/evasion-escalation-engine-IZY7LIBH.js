...
// server/lib/evasion-escalation-engine.ts
function detectBlockReason(statusCode, responseBody) {
...
function escalateEvasionProfile(targetProfile, reason, context) {
  const existing = targetProfile.evasionEscalation;
...
function analyzeToolOutputForBlocking(toolOutput, exitCode) {
  const indicators = [];
  let confidence = 0;
...
function getActionDescription(reason, level) {
  const actions = {
    waf_block: {
...
function getRecommendations(reason, level, profile) {
  const recs = [];
...
var ESCALATION_CONFIGS, WAF_BLOCK_PATTERNS;
var init_evasion_escalation_engine = __esm({
  "server/lib/evasion-escalation-engine.ts"() {
    ESCALATION_CONFIGS = {
      1: {
        name: "Normal",
...
init_evasion_escalation_engine();
export {
