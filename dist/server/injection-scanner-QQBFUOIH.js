...
async function buildInjectableUrls(webApp, targetUrl, state, engagementId) {
  ...
  try {
    ...
    if (db) {
      const { webAppScans } = await import("./schema-5UXBHZJS.js");
...
async function executeInjectionScanning(ctx) {
  ...
  for (const webApp of webApps) {
    ...
    try {
      ...
      if (approved) {
        const { batchSqlmapScan, analyzeSqlmapFindings, runBlindSqliPass, ingestSqlmapToWebAppFindings } = await import("./sqlmap-scanner-MN7AI5YN.js");
    ...
    try {
      ...
      if (approved) {
        const { batchXssScan, analyzeXssFindings, ingestXssToWebAppFindings } = await import("./xsstrike-scanner-G55WCJXN.js");
  ...
  for (const webApp of webApps) {
    ...
    try {
      ...
      if (approved) {
        const { batchCommixScan, ingestCommixToWebAppFindings } = await import("./commix-scanner-FCEBLLWE.js");
        const isLab = state.trainingLabMode === true;
  ...
  for (const webApp of webApps) {
    ...
    try {
      ...
      if (approved) {
        const { batchTplmapScan, ingestTplmapToWebAppFindings } = await import("./tplmap-scanner-PT4U2FWW.js");
