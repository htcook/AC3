...
async function persistObservations(observations) {
  ...
  try {
    ...
    const { scanObservations } = await import("./schema-NFOE7JII.js");
...
async function persistSignals(signals) {
  ...
  try {
    const { getDb } = await import("./db-QL5AIQ4A.js");
    const { scanSignals } = await import("./schema-NFOE7JII.js");
    const db = await getDb();
    if (!db) return 0;
    let inserted = 0;
...
async function persistRiskCards(cards) {
  ...
  try {
    const { getDb } = await import("./db-QL5AIQ4A.js");
    const { scanRiskCards } = await import("./schema-NFOE7JII.js");
    const db = await getDb();
    if (!db) return 0;
    let inserted = 0;
