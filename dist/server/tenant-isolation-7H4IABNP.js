...
async function resolveUserTenant(userId, requestedTenantId) {
  ...
  const { tenantMemberships, tenants } = await import("./schema-NFOE7JII.js");
...
async function autoProvisionTenant(userId, userName) {
  const { getDb } = await import("./db-QL5AIQ4A.js");
  const { tenants, tenantMemberships } = await import("./schema-NFOE7JII.js");
  const db = await getDb();
...
async function logTenantAction(userId, tenantId, action, details) {
  try {
    const { getDb } = await import("./db-QL5AIQ4A.js");
    const { activityLogs } = await import("./schema-NFOE7JII.js");
    const db = await getDb();
    if (!db) return;
...
async function getTenantStats(tenantId) {
  const { getDb } = await import("./db-QL5AIQ4A.js");
  const { tenantMemberships, engagements } = await import("./schema-NFOE7JII.js");
  const db = await getDb();
  if (!db) return null;
