...
async function runVulnVerification(highCritVulns, state, addLog, broadcastOpsUpdate) {
  ...
  try {
    const { verifyVulnerability } = await import("./vuln-verifier-IDIQZ6WK.js");
    const { evidenceGate, buildProvenance, createIntegrityEnvelope } = await import("./evidence-integrity-J2PV7HVJ.js");
...
async function runScanForgeReasoning(highCritVulns, state, addLog, broadcastOpsUpdate) {
  ...
  let processed = 0;
  try {
    const { batchRunScanForgeReasoning } = await import("./scanforge-reasoning-AB2HH663.js");
...
async function runThreatActorMapping(state, addLog, broadcastOpsUpdate, scoreEngagementThreatAttribution) {
  ...
  let mapped = 0;
  try {
    const { mapThreats } = await import("./threat-mapper-NHWAZIGF.js");
...
async function executeVulnCorrelation(ctx) {
  ...
  if (allVulns.length > 0) {
    ...
    try {
      ...
      } = await import("./knowledge-lazy-AJCXF322.js");
