...
async function executeLiveCapture(config) {
  const { executeTool, executeRawCommand } = await import("./scan-server-executor-L5FGTKBI.js");
...
async function ingestPcapResults(findings, packets) {
  const { ingestRawObservations } = await import("./observation-ingestor-BQQVOEUH.js");
