  ...
  for (const webApp of webApps) {
    ...
    for (const { targetUrl, dedupKey } of scanTargets) {
      ...
      try {
        ...
        const { generateLLMScanConfig, startScan, configureZapAuthentication } = await import("./zap-scanner-K7NEUT2E.js");
        ...
        if (wafVendor) {
          try {
            const { applyWafEvasionConfig } = await import("./zap-scanner-K7NEUT2E.js");
        ...
        if (!zapStarted) {
          ...
          try {
            const { onEngagementVulnDetectionPhase } = await import("./burp-auto-scan-6FGOB67D.js");
        ...
        if (zapScanId) {
          ...
          const { pollScanProgress } = await import("./zap-scanner-K7NEUT2E.js");
  ...
  try {
    ...
    if (ir && (ir.urlSource === "scope_fallback" || ir.zapUrlsDiscovered === 0)) {
      ...
      if (db) {
        ...
        if (scans.length > 0) {
          const { deferredZapBurpRefeed } = await import("./zap-burp-pipeline-C56HUMAL.js");
