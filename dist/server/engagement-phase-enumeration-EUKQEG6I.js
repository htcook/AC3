...
${state.scanPlan?.discoveryStrategy || "Comprehensive port discovery to enrich passive recon data"}`
  ...
  try {
    const { getScanServerConfigForScanForge } = await import("./scan-server-executor-YEYTXLFW.js");
...
Evasion: ${assetPlan?.evasionTechniques?.join(", ") || "fragmentation, decoys, normal timing"}`
  ...
  try {
    const { beforeDiscoveryScan } = await import("./pcap-auto-capture-WCMYDQ4Q.js");
...
Ports: ${discoveredPorts.map((p) => `${p.port}/${p.service}${p.product ? ` (${p.product})` : ""}`).join(", ")}`,
  ...
  if (autoCaptureSessionId) {
    try {
      const { afterDiscoveryScan } = await import("./pcap-auto-capture-WCMYDQ4Q.js");
...
async function runCloudAssetDetection(state, helpers) {
  try {
    const { detectCloudAsset, executeCloudStorageScan, getCloudDetectionPromptContext } = await import("./cloud-storage-scanner-A5EPBQLA.js");
    helpers.addLog({
      phase: "enumeration",
      type: "info",
...
async function executeTargetedToolDeployment(state, helpers) {
  ...
  const { suggestToolCommands } = await import("./scan-server-executor-YEYTXLFW.js");
  for (const asset of state.assets) {
