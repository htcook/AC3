...
async function runLabScan(sessionId, targetUrl, scanProfile) {
  ...
  const { updateTrainingLabSession } = await import("./db-65DPEQYH.js");
  ...
  try {
    ...
    try {
      const { executeRawCommandViaQueue } = await import("./job-queue-bridge-CHPT4UDM.js");
    ...
    try {
      const { executeToolViaQueue } = await import("./job-queue-bridge-CHPT4UDM.js");
      let discoveryFlags;
    ...
    const { executeRawCommandViaQueue: execNucleiCmd } = await import("./job-queue-bridge-CHPT4UDM.js");
    try {
    ...
    try {
      const { resolveNucleiTemplate } = await import("./nuclei-template-auto-selector-F3Z5WO6Q.js");
    ...
    try {
      const { persistNucleiFindings: persistFindings } = await import("./nuclei-findings-persistence-576YUBJ4.js");
...
var trainingLabRouter = router({
  ...
  startSession: protectedProcedure.input(z.object({
    ...
    const { createTrainingLabSession } = await import("./db-65DPEQYH.js");
  ...
  getSession: publicProcedure.input(z.object({ sessionId: z.string() })).query(async ({ input }) => {
    ...
    const { getTrainingLabSession } = await import("./db-65DPEQYH.js");
  ...
  listSessions: publicProcedure.input(z.object({ limit: z.number().min(1).max(100).default(50) }).optional()).query(async ({ input }) => {
    const { listTrainingLabSessions } = await import("./db-65DPEQYH.js");
  ...
  rerunAnalysis: protectedProcedure.input(z.object({ sessionId: z.string() })).mutation(async ({ input }) => {
    const { getTrainingLabSession, updateTrainingLabSession } = await import("./db-65DPEQYH.js");
    const session = await getTrainingLabSession(input.sessionId);
    if (!session) throw new TRPCError({ code: "NOT_FOUND", message: "Session not found" });
...
${t.outputPreview}`
        ...
        const { getTrainingLabFeedbackForSession } = await import("./db-65DPEQYH.js");
...
Respond with a JSON object containing: executiveSummary, riskScore (1-10), riskRating, findings (array with title, severity, category, description, confidence, evidence, remediation, cve, cvss, exploitMethod object with method, reasoning, primaryTool, cliCommands array), attackChains, missedAreas, recommendations.` }
  ...
  submitFeedback: protectedProcedure.input(z.object({
    ...
    const { insertTrainingLabFeedbackEntry, getTrainingLabSession } = await import("./db-65DPEQYH.js");
  ...
  startContinuousTraining: protectedProcedure.input(z.object({
    ...
    const { getActiveLoop, runContinuousTrainingLoop } = await import("./continuous-training-WCZ4MH36.js");
  ...
  cancelContinuousTraining: protectedProcedure.input(z.object({ sessionId: z.string() })).mutation(async ({ input }) => {
    const { cancelLoop, getActiveLoop } = await import("./continuous-training-WCZ4MH36.js");
  ...
  continuousTrainingStatus: publicProcedure.input(z.object({ sessionId: z.string() })).query(async ({ input }) => {
    const { getActiveLoop } = await import("./continuous-training-WCZ4MH36.js");
    const loop = getActiveLoop(input.sessionId);
    if (!loop) {
      const { getTrainingLabSession } = await import("./db-65DPEQYH.js");
      const session = await getTrainingLabSession(input.sessionId);
  ...
  activeContinuousTraining: publicProcedure.query(async () => {
    const { listActiveLoops } = await import("./continuous-training-WCZ4MH36.js");
