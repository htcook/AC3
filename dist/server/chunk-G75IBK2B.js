...
// server/lib/ttp-ingest.ts
async function ingestAttackStix() {
  ...
  const techniques = objects.filter((o) => o.type === "attack-pattern" && !o.revoked && !o.x_mitre_deprecated).map((o) => {
    const extRef = o.external_references?.find((r) => r.source_name === "mitre-attack");
    const tactics = (o.kill_chain_phases || []).filter((p) => p.kill_chain_name === "mitre-attack").map((p) => p.phase_name);
    return {
      id: extRef?.external_id || "",
      name: o.name || "",
      tactic: tactics.join(", ") || "unknown",
      description: (o.description || "").substring(0, 5e3),
      platforms: o.x_mitre_platforms || [],
      dataSources: o.x_mitre_data_sources || [],
      detection: (o.x_mitre_detection || "").substring(0, 3e3),
      isSubtechnique: o.x_mitre_is_subtechnique || false,
      url: extRef?.url || ""
    };
  }).filter((t) => t.id.startsWith("T"));
  const groups = objects.filter((o) => o.type === "intrusion-set" && !o.revoked).map((o) => {
    const extRef = o.external_references?.find((r) => r.source_name === "mitre-attack");
    return {
      id: extRef?.external_id || o.id,
      name: o.name || "",
      aliases: o.aliases || [],
      description: (o.description || "").substring(0, 2e3)
    };
  });
  const software = objects.filter((o) => (o.type === "malware" || o.type === "tool") && !o.revoked).map((o) => {
    const extRef = o.external_references?.find((r) => r.source_name === "mitre-attack");
    return {
      id: extRef?.external_id || o.id,
      name: o.name || "",
...
async function ingestAtomicRedTeamIndex() {
  ...
  const testsByTechnique = /* @__PURE__ */ new Map();
...
async function ingestLolbas() {
  ...
  const lolbasByTechnique = /* @__PURE__ */ new Map();
...
async function ingestMetasploitModules() {
...
function getKaliToolsCatalog() {
  return KALI_TOOLS_CATALOG;
}
function getKaliToolsForTechnique(techniqueId) {
  return KALI_TOOLS_CATALOG.filter((t) => t.techniques.includes(techniqueId));
}
async function runFullIngestion(options) {
  const errors = [];
  let attackData = null;
  let atomicData = null;
  let lolbasData = null;
  let metasploitData = null;
  ...
  let totalIngested = 0;
  if (attackData) {
    const techniques = options?.maxTechniques ? attackData.techniques.slice(0, options.maxTechniques) : attackData.techniques;
    for (const tech of techniques) {
      try {
        const existing = await getTtpKnowledge(tech.id);
        ...
        if (atomicData?.has(tech.id)) {
          ...
          entry.executionMethods = tests.map((t) => ({
            method: t.name,
        ...
        if (lolbasData?.has(tech.id)) {
          ...
          const existingTools = entry.toolsUsed || [];
        ...
        if (metasploitData) {
          const relatedExploits = metasploitData.exploits.filter((e) => {
        ...
        await upsertTtpKnowledge(entry);
        totalIngested++;
      } catch (err) {
...
var GITHUB_RAW, SOURCES, KALI_TOOLS_CATALOG;
var init_ttp_ingest = __esm({
  "server/lib/ttp-ingest.ts"() {
    init_db();
    ...
    SOURCES = {
      ...
      atomicYaml: (techId) => `${GITHUB_RAW}/redcanaryco/atomic-red-team/master/atomics/${techId}/${techId}.yaml`,
