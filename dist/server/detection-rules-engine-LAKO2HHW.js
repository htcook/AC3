...
// server/lib/detection-rules-engine.ts
var SIGMA_TEMPLATES = {
...
${urls.map((u) => {
      try {
        return `      - "${new URL(u, "http://placeholder").pathname}"`;
      } catch {
        return `      - "${u}"`;
      }
    }).join("\n")}
...
var SURICATA_TEMPLATES = {
  sqli: (ctx, iocs) => {
    ...
    return `alert http $EXTERNAL_NET any -> [${targets}] [${portStr}] (msg:"AC3 SQL Injection Attempt - ${escapeQuotes(ctx.title)}"; flow:established,to_server; content:"UNION"; nocase; content:"SELECT"; nocase; distance:0; within:20; classtype:web-application-attack; sid:${sid}; rev:1; metadata:created ${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]};)
...
alert http $EXTERNAL_NET any -> [${targets}] [${portStr}] (msg:"AC3 SQL Injection - Time Blind"; flow:established,to_server; content:"WAITFOR"; nocase; content:"DELAY"; nocase; distance:0; within:15; classtype:web-application-attack; sid:${sid + 2}; rev:1;)`;
  ...
  xss: (ctx, iocs) => {
    const sid = generateSID();
    const targets = iocs.targetIPs.length > 0 ? iocs.targetIPs.join(",") : "any";
    return `alert http $EXTERNAL_NET any -> [${targets}] any (msg:"AC3 XSS Attempt - ${escapeQuotes(ctx.title)}"; flow:established,to_server; content:"<script"; nocase; classtype:web-application-attack; sid:${sid}; rev:1; metadata:created ${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]};)
...
alert http $EXTERNAL_NET any -> [${targets}] any (msg:"AC3 XSS - Event Handler Injection"; flow:established,to_server; pcre:"/on(error|load|mouseover|focus|click)\\s*=/i"; classtype:web-application-attack; sid:${sid + 1}; rev:1;)`;
  ...
  cmdi: (ctx, iocs) => {
    const sid = generateSID();
    const targets = iocs.targetIPs.length > 0 ? iocs.targetIPs.join(",") : "any";
    return `alert http $EXTERNAL_NET any -> [${targets}] any (msg:"AC3 Command Injection - ${escapeQuotes(ctx.title)}"; flow:established,to_server; content:"/etc/passwd"; nocase; classtype:web-application-attack; sid:${sid}; rev:1; metadata:created ${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]};)
...
alert http $EXTERNAL_NET any -> [${targets}] any (msg:"AC3 Command Injection - Pipe/Semicolon"; flow:established,to_server; pcre:"/[;|]s*(cat|ls|id|whoami|wget|curl|ping|nc|bash|sh)\b/i"; classtype:web-application-attack; sid:${sid + 1}; rev:1;)`;
  ...
  lfi: (ctx, iocs) => {
    const sid = generateSID();
    const targets = iocs.targetIPs.length > 0 ? iocs.targetIPs.join(",") : "any";
    return `alert http $EXTERNAL_NET any -> [${targets}] any (msg:"AC3 Path Traversal - ${escapeQuotes(ctx.title)}"; flow:established,to_server; content:"../"; classtype:web-application-attack; sid:${sid}; rev:1; metadata:created ${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]};)
...
alert http $EXTERNAL_NET any -> [${targets}] any (msg:"AC3 LFI - Encoded Traversal"; flow:established,to_server; content:"%2e%2e%2f"; nocase; classtype:web-application-attack; sid:${sid + 1}; rev:1;)`;
  ...
  bruteforce: (ctx, iocs) => {
    ...
    return `alert tcp $EXTERNAL_NET any -> [${targets}] [${sshPortStr}] (msg:"AC3 SSH Brute Force - ${escapeQuotes(ctx.title)}"; flow:to_server; threshold:type both, track by_src, count 5, seconds 60; classtype:attempted-admin; sid:${sid}; rev:1; metadata:created ${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]};)
...
alert http $EXTERNAL_NET any -> [${targets}] any (msg:"AC3 HTTP Login Brute Force"; flow:established,to_server; content:"POST"; http_method; content:"login"; nocase; http_uri; threshold:type both, track by_src, count 10, seconds 120; classtype:attempted-admin; sid:${sid + 1}; rev:1;)`;
  ...
  generic: (ctx, iocs) => {
    const sid = generateSID();
    const targets = iocs.targetIPs.length > 0 ? iocs.targetIPs.join(",") : "any";
    return `alert ip $EXTERNAL_NET any -> [${targets}] any (msg:"AC3 Security Finding - ${escapeQuotes(ctx.title)}"; classtype:misc-attack; sid:${sid}; rev:1; metadata:created ${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]};)`;
  }
};
var YARA_TEMPLATES = {
  webshell: (ctx, iocs) => {
...
function classifyFindingCategory(finding) {
...
function postProcessDetectionRules(findings, engagementIOCs) {
...
function extractIOCsFromEngagement(assets) {
  const iocs = {
    targetIPs: [],
    targetHostnames: [],
    detectedSoftware: [],
  ...
  for (const asset of assets) {
    ...
    for (const tr of asset.toolResults || []) {
      for (const f of tr.findings || []) {
        if (f.attack) iocs.attackPayloads.push(f.attack);
        if (f.param) iocs.vulnerableParams.push(f.param);
        if (f.url) iocs.vulnerableURLs.push(f.url);
      }
    }
...
function isPlaceholderRule(rule) {
  ...
  return placeholderIndicators.some((p) => lower.includes(p)) || rule.trim().length < 50 || rule.includes("$") && !rule.includes("alert") && !rule.includes("rule") && rule.length < 100;
}
function injectIOCsIntoSigma(rule, iocs) {
  let result = rule;
  result = result.replace(
    /\b(10\.0\.0\.1|192\.168\.1\.1|0\.0\.0\.0|xxx\.xxx\.xxx\.xxx|target_ip)\b/gi,
    iocs.targetIPs[0] || "$EXTERNAL_NET"
  );
  result = result.replace(
    /\b(example\.com|target\.com|target_host)\b/gi,
    iocs.targetHostnames[0] || "target"
  );
  return result;
}
function injectIOCsIntoSuricata(rule, iocs) {
  let result = rule;
  result = result.replace(
    /\b(10\.0\.0\.1|192\.168\.1\.1|0\.0\.0\.0|xxx\.xxx\.xxx\.xxx|TARGET_IP)\b/gi,
...
function sanitizeRuleName(title) {
