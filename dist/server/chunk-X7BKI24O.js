...
async function indexMetasploitModules() {
  let count = 0;
  try {
    const { ingestMetasploitModules } = await import("./ttp-ingest-4REOTCLM.js");
...
async function indexAc3History() {
  let count = 0;
  try {
    const { recordFeedback, getModulePerformance } = await import("./exploit-feedback-loop-6FT5BE2X.js");
