...
Generate two-phase scan plan. Phase A: ScanForge discovery --top-ports 1000. Phase B: tools per asset.`;
  ...
  try {
    ...
    try {
      const { getToolInventory, getInventoryForLLM } = await import("./scan-server-inventory-EX4RXVJZ.js");
...
async function executeEnumeration(state, engagement, operatorCtx) {
  const { executeEnumeration: runEnumerationPhase } = await import("./engagement-phase-enumeration-EUKQEG6I.js");
...
async function executeVulnDetection(state, engagement, operatorCtx) {
  ...
  const { executeVulnPrep } = await import("./vuln-prep-KESXHVWM.js");
  const vulnPrepCtx = {
    state,
    engagement,
    operatorCtx,
  ...
  const { executeZapScanning } = await import("./zap-scanner-BAGYQOYH.js");
  ...
  const { executeInjectionScanning } = await import("./injection-scanner-KTEKZ5RO.js");
  ...
  const { executeCredentialTesting } = await import("./credential-tester-LSNISHCY.js");
...
async function executeExploitation(state, engagement, operatorCtx) {
  const { executeExploitation: runExploitPhase } = await import("./engagement-phase-exploitation-ACTGKGB7.js");
  return runExploitPhase(state, engagement, operatorCtx);
}
async function executePostExploit(state, engagement, operatorCtx) {
  const { executePostExploit: runPostExploitPhase } = await import("./engagement-phase-post-exploit-HHGE72SZ.js");
    ...
    if (engagement.roeStatus === "signed" || engagement.roeStatus === "pending" || state.trainingLabMode === true) {
      if (["recon", "passive_discovery", "scoping", "test_plan", "enumeration"].includes(startPhase)) {
        ...
        if (!enumGate.allowed) {
          ...
          try {
            const { checkScanServerStatus } = await import("./scan-server-executor-YEYTXLFW.js");
...
PoC: ${cf.analysis.poc || "N/A"}`,
        ...
        if (state.stats.vulnsFound > 0 || state.assets.some((a) => a.cloudProviders?.length > 0)) {
          try {
            ...
            const { runFeedbackLoop, getFeedbackLoopSummary } = await import("./llm-scan-feedback-2EOG6Y7D.js");
...
Promoted: ${promotionSummary.promotedVulns.map((p) => `${p.vulnTitle.slice(0, 50)} [${p.scanner}/${p.category}]`).join("; ")}`,
      ...
      try {
        const { retryDeferredScans, getDeferredScans, clearDeferredScans } = await import("./job-queue-bridge-CHA7XTTE.js");
      ...
      try {
        const { selectFindingsForScreenshot, captureScreenshotBatch } = await import("./screenshot-capture-X5NRWXT3.js");
