...
async function getActiveSourcesForStage(stage) {
  try {
    const { getActiveCustomerIntegrationsByStage } = await import("./db-MOHZQFM5.js");
...
async function recordExecutionLogs(results, engagementId) {
  const { createExecutionLog } = await import("./db-MOHZQFM5.js");
  for (const result of results) {
...
async function updateIntegrationStats(results) {
  const { updateCustomerIntegration, getCustomerIntegrationByIntegrationId } = await import("./db-MOHZQFM5.js");
  for (const result of results) {
