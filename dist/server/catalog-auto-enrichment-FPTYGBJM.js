...
async function enrichFromDFIR(config, maxItems) {
  ...
  try {
    const { ingestDFIRReport } = await import("./dfir-report-ingestion-AAMTWTML.js");
...
async function enrichFromIOCs(config, maxItems) {
  ...
  const drizzleDb = await getDb();
  try {
    const { reverseEngineerIOC, batchReverseEngineerIOCs } = await import("./ioc-ttp-reverse-engineer-WKS55T7L.js");
...
async function enrichFromHackingArticles(config, maxItems) {
  ...
  let processed = 0, added = 0, failed = 0;
  try {
    const { processArticleBatch } = await import("./hacking-articles-ingestion-IMJMWQH3.js");
    ...
    if (actorsNeedingEnrichment.length === 0) {
      ...
      if (leastEnrichedActors.length > 0) {
        for (const actor of leastEnrichedActors) {
          try {
            processed++;
            const { enrichThreatActorWithLLM } = await import("./threat-actor-crawler-EGALHZKU.js");
  ...
  try {
    ...
    if (engagement.length > 0) {
      ...
      if (discoveredIOCs.length > 0) {
        try {
          const { batchReverseEngineerIOCs } = await import("./ioc-ttp-reverse-engineer-WKS55T7L.js");
      ...
      try {
        const { enrichCalderaFromCatalog } = await import("./catalog-caldera-enrichment-SZYCMDKH.js");
