...
async function runVulnVerification(highCritVulns, state, addLog, broadcastOpsUpdate) {
  ...
  try {
    const { verifyVulnerability } = await import("./vuln-verifier-RXST37GH.js");
    const { evidenceGate, buildProvenance, createIntegrityEnvelope } = await import("./evidence-integrity-2UJPNT3N.js");
...
async function runScanForgeReasoning(highCritVulns, state, addLog, broadcastOpsUpdate) {
  ...
  let processed = 0;
  try {
    const { batchRunScanForgeReasoning } = await import("./scanforge-reasoning-36KFH52V.js");
...
async function runThreatActorMapping(state, addLog, broadcastOpsUpdate, scoreEngagementThreatAttribution) {
  ...
  let mapped = 0;
  try {
    const { mapThreats } = await import("./threat-mapper-F3HINGRZ.js");
