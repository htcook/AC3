...
async function detectInterruptedEngagements() {
  try {
    ...
    const { engagementOpsSnapshots, engagements } = await import("./schema-5UXBHZJS.js");
...
async function executeAutoResume(engagementId) {
  try {
    ...
    try {
      const { dismissAllStaleApprovals } = await import("./engagement-orchestrator-BCOMESLT.js");
    ...
    const { getOpsStateWithRecovery, initOpsState, addLog: addOpsLog, broadcastOpsUpdate, persistOpsStateNow } = await import("./engagement-orchestrator-BCOMESLT.js");
    ...
    try {
      ...
      if (response.ok && responseData?.result?.data?.json?.started) {
        ...
        const { resumeEngagement } = await import("./engagement-orchestrator-BCOMESLT.js");
