...
async function runVulnVerification(highCritVulns, state, addLog, broadcastOpsUpdate) {
  ...
  try {
    const { verifyVulnerability } = await import("./vuln-verifier-4LLGNF4Q.js");
    const { evidenceGate, buildProvenance, createIntegrityEnvelope } = await import("./evidence-integrity-WZ5KTZOQ.js");
...
async function runScanForgeReasoning(highCritVulns, state, addLog, broadcastOpsUpdate) {
  ...
  let processed = 0;
  try {
    const { batchRunScanForgeReasoning } = await import("./scanforge-reasoning-TEDYLCBF.js");
...
async function runThreatActorMapping(state, addLog, broadcastOpsUpdate, scoreEngagementThreatAttribution) {
  ...
  let mapped = 0;
  try {
    const { mapThreats } = await import("./threat-mapper-BBHTBUYQ.js");
