...
async function triggerScheduledScan(monitor) {
  ...
  try {
    ...
    const { detectSubdomainTakeover, crossReferenceTechVulnerabilities, detectSubdomainChanges } = await import("./domain-intel-advanced-TV2LPZ4W.js");
...
function initScanScheduler() {
  ...
  setTimeout(async () => {
    try {
      const { startPeriodicHealthChecks } = await import("./health-monitor-33PP7OBP.js");
...
function stopScanScheduler() {
  ...
  import("./health-monitor-33PP7OBP.js").then(({ stopPeriodicHealthChecks }) => {
