...
// server/lib/briefing-pdf-generator.ts
...
function generateBriefingHtml(input) {
...
<table>
  ...
  ${matchedActors.map((actor, i) => `
    <tr>
      ...
      <td style="font-size:8pt">${actor.matchedSectors?.slice(0, 3).map((s) => escapeHtml(s)).join(", ") || "\u2014"}</td>
    </tr>
  `).join("")}
  </tbody>
</table>
...
${matchedActors.slice(0, 5).map((actor, i) => `
<div class="section">
  ...
  <table>
    ...
    ${actor.attackVectors?.length > 0 ? `<tr><td style="font-weight:600">Attack Vectors</td><td>${actor.attackVectors.map((v) => escapeHtml(v)).join(", ")}</td></tr>` : ""}
    ${actor.topTechniques?.length > 0 ? `<tr><td style="font-weight:600">Key Techniques</td><td>${actor.topTechniques.map((t) => `${escapeHtml(t.id)}: ${escapeHtml(t.name)}`).join(", ")}</td></tr>` : ""}
    ${actor.topTools?.length > 0 ? `<tr><td style="font-weight:600">Tools</td><td>${actor.topTools.map((t) => escapeHtml(t)).join(", ")}</td></tr>` : ""}
  </table>
  ${actor.recommendedActions?.length > 0 ? `
  <h3 style="margin-top:8px">Recommended Actions</h3>
  <ul class="actions-list">
    ${actor.recommendedActions.map((a) => `<li>${escapeHtml(a)}</li>`).join("")}
  </ul>
  ` : ""}
</div>
`).join("")}
...
<div class="section">
  ${[
    ...
    <div class="carver-bar">
      ...
      <div class="track"><div class="fill" style="width:${Math.min(100, carverProfile[d.key] / 10 * 100)}%;background:${d.color}"></div></div>
      <div class="val">${(carverProfile[d.key] || 0).toFixed(1)}</div>
    </div>
  `).join("")}
...
<table>
  ...
  <tbody>
  ${trends.actorActivityTrend.map((a) => `
    <tr>
...
async function generateBriefingPdf(input) {
...
function generateBriefingHtmlPreview(input) {
  return generateBriefingHtml(input);
}
export {
  generateBriefingHtmlPreview,
  generateBriefingPdf
};
