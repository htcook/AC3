...
${postHeredoc}`;
  let dbRecordId;
  try {
    const { insertExploitationAttempt } = await import("./db-QL5AIQ4A.js");
  ...
  try {
    ...
    if (dbRecordId) {
      try {
        const { updateExploitationAttempt } = await import("./db-QL5AIQ4A.js");
...
async function getExecutionHistory(engagementId) {
  ...
  try {
    const { getExploitationAttempts } = await import("./db-QL5AIQ4A.js");
