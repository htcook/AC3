...
async function runLabScan(sessionId, targetUrl, scanProfile) {
  ...
  const { updateTrainingLabSession } = await import("./db-GNA5CL3K.js");
  ...
  try {
    ...
    const isSelfHosted = hostname === scanServerHost || hostname === (process.env.SCANFORGE_HOST || "") || hostname === "scan.aceofcloud.io";
    ...
    try {
      const { executeRawCommandViaQueue } = await import("./job-queue-bridge-O47LVQSV.js");
    ...
    try {
      const { executeToolViaQueue } = await import("./job-queue-bridge-O47LVQSV.js");
      let discoveryFlags;
    ...
    const { executeRawCommandViaQueue: execNucleiCmd } = await import("./job-queue-bridge-O47LVQSV.js");
    try {
    ...
    try {
      const { resolveNucleiTemplate } = await import("./nuclei-template-auto-selector-KBZ3N27U.js");
    ...
    try {
      const { persistNucleiFindings: persistFindings } = await import("./nuclei-findings-persistence-HYQFEHIF.js");
    ...
    try {
      const { invokeLLM } = await import("./llm-VEPMLFRK.js");
...
var trainingLabRouter = router({
  ...
  startSession: protectedProcedure.input(z.object({
    ...
    const { createTrainingLabSession } = await import("./db-GNA5CL3K.js");
  ...
  checkRoE: protectedProcedure.input(z.object({
    targetId: z.string(),
    scanProfile: z.enum(["quick", "standard", "deep"]).default("standard"),
  ...
  getSession: protectedProcedure.input(z.object({ sessionId: z.string() })).query(async ({ input }) => {
    ...
    const { getTrainingLabSession } = await import("./db-GNA5CL3K.js");
  ...
  listSessions: protectedProcedure.input(z.object({ limit: z.number().min(1).max(100).default(50) }).optional()).query(async ({ input }) => {
    const { listTrainingLabSessions } = await import("./db-GNA5CL3K.js");
  ...
  rerunAnalysis: protectedProcedure.input(z.object({ sessionId: z.string() })).mutation(async ({ input }) => {
    const { getTrainingLabSession, updateTrainingLabSession } = await import("./db-GNA5CL3K.js");
    const session = await getTrainingLabSession(input.sessionId);
    if (!session) throw new TRPCError({ code: "NOT_FOUND", message: "Session not found" });
...
${t.outputPreview}`
        ...
        const { getTrainingLabFeedbackForSession } = await import("./db-GNA5CL3K.js");
...
Respond with a JSON object containing: executiveSummary, riskScore (1-10), riskRating, findings (array with title, severity, category, description, confidence, evidence, remediation, cve, cvss, exploitMethod object with method, reasoning, primaryTool, cliCommands array), attackChains, missedAreas, recommendations.` }
  ...
  submitFeedback: protectedProcedure.input(z.object({
    ...
    const { insertTrainingLabFeedbackEntry, getTrainingLabSession } = await import("./db-GNA5CL3K.js");
  ...
  getFeedback: protectedProcedure.input(z.object({ sessionId: z.string() })).query(async ({ input }) => {
    const { getTrainingLabFeedbackForSession } = await import("./db-GNA5CL3K.js");
  ...
  accuracyTrend: protectedProcedure.input(z.object({ targetPreset: z.string().optional(), limit: z.number().default(50) }).optional()).query(async ({ input }) => {
  ...
  groundTruth: protectedProcedure.input(z.object({ targetPreset: z.string() })).query(async ({ input }) => {
  ...
  learningEntries: protectedProcedure.input(z.object({ targetPreset: z.string() })).query(async ({ input }) => {
  ...
  sessionAccuracy: protectedProcedure.input(z.object({ sessionId: z.string() })).query(async ({ input }) => {
  ...
  startContinuousTraining: protectedProcedure.input(z.object({
    ...
    const { getActiveLoop, runContinuousTrainingLoop } = await import("./continuous-training-BCEOIC6S.js");
  ...
  cancelContinuousTraining: protectedProcedure.input(z.object({ sessionId: z.string() })).mutation(async ({ input }) => {
    const { cancelLoop, getActiveLoop } = await import("./continuous-training-BCEOIC6S.js");
  ...
  continuousTrainingStatus: protectedProcedure.input(z.object({ sessionId: z.string() })).query(async ({ input }) => {
    const { getActiveLoop } = await import("./continuous-training-BCEOIC6S.js");
    const loop = getActiveLoop(input.sessionId);
    if (!loop) {
      const { getTrainingLabSession } = await import("./db-GNA5CL3K.js");
      const session = await getTrainingLabSession(input.sessionId);
  ...
  activeContinuousTraining: protectedProcedure.query(async () => {
    const { listActiveLoops } = await import("./continuous-training-BCEOIC6S.js");
