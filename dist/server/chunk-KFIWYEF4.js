...
async function executeToolViaQueue(config, options) {
  ...
  if (!useQueue) {
    if (!options?.forceLocal) {
      try {
        const { executeToolViaHttp } = await import("./do-scan-api-C5YK7YCK.js");
...
async function executeRawCommandViaQueue(command, timeoutSeconds = 300, options) {
  ...
  if (!options?.forceLocal) {
    try {
      ...
      const { executeRawCommandViaHttp } = await import("./do-scan-api-C5YK7YCK.js");
  ...
  const { executeViaChildProcessSSH } = await import("./scan-server-executor-WPL2NRYI.js");
...
async function retryDeferredScans(engagementId, options) {
  ...
  try {
    const { isDedicatedHealthy } = await import("./scan-service-url-5ZA2MUWH.js");
