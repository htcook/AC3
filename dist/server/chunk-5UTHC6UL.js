...
${postHeredoc}`;
  let dbRecordId;
  try {
    const { insertExploitationAttempt } = await import("./db-LSUZDHGJ.js");
  ...
  try {
    ...
    if (dbRecordId) {
      try {
        const { updateExploitationAttempt } = await import("./db-LSUZDHGJ.js");
...
async function getExecutionHistory(engagementId) {
  ...
  try {
    const { getExploitationAttempts } = await import("./db-LSUZDHGJ.js");
