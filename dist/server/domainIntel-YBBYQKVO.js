...
Risk signals detected (${passiveRecon.riskSignals.length}):`);
  ...
  try {
    const { applyMissionBaselines } = await import("./scoring-engine-KH6Y3XUY.js");
    let missionIdx = 0;
    for (const a of analyses) {
      missionIdx++;
  ...
  let oemCredentials = [];
  try {
    const { matchCredentialsForAssets, persistMatchedCredentials } = await import("./oem-default-creds-4C672PJ7.js");
  ...
  if (oemCredentials.length > 0 && scanMode === "active") {
    try {
      const { runCredentialTests, getCredentialsForService } = await import("./credential-tester-EYPMPBLX.js");
  ...
  try {
    const { runPipelineCrawlStage, enrichOrgWithBusinessIntel, applyBusinessIntelCarverBoosts } = await import("./pipeline-crawl-stage-4TX6S5XX.js");
  ...
  try {
    ...
    const { createCarverRiskCard } = await import("./db-PHFZ5GDL.js");
    const assetSignals = [];
    if (passiveRecon) {
  ...
  const [threatMatchRetry, incidentSearchRetry, affiliatedDomainsRetry] = await retryMod.parallelWithRetry([
    ...
    },
    {
      ...
      fn: async () => {
        const { runIncidentSearchEnrichment } = await import("./incident-search-enrichment-NFTV5VXO.js");
        ...
        let ingestResult;
        try {
          const { ingestIncidentSearchResults } = await import("./incident-search-ingest-RRC4654N.js");
    ...
    },
    {
      ...
      fn: async () => {
        const { runAffiliatedDomainDiscovery } = await import("./affiliated-domain-discovery-LTUTMD6C.js");
  ...
  try {
    const { collectTrainingData } = await import("./incident-training-collector-L3MJXWRV.js");
