...
async function runPostPipelineGraduation(metrics) {
  const { recordScenarioResult, recordTrainingData } = await import("./graduation-lab-bridge-NHUVEVBC.js");
  let methodologyBonus = 0;
  let methodologyRationale = "";
  try {
    const { computeMethodologyGraduationBonus } = await import("./methodology-db-persistence-RXGCVASP.js");
