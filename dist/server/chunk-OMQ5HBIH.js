...
async function runTargetedResearch(engagementId, targets, targetType) {
  ...
  for (const target of targets.slice(0, 10)) {
    ...
    if (isIP) {
      try {
        const { shodanHostLookup } = await import("./darkweb-osint-service-456HN2AH.js");
    ...
    if (isIP) {
      try {
        const { censysHostSearch } = await import("./darkweb-osint-service-456HN2AH.js");
    ...
    if (domain) {
      try {
        const { securityTrailsDomainInfo } = await import("./darkweb-osint-service-456HN2AH.js");
    ...
    if (domain) {
      try {
        const { enrichDomainIntel } = await import("./bug-bounty-intelligence-PLCA7TIS.js");
    ...
    if (isIP) {
      try {
        const { queryAbuseIpdb } = await import("./darkweb-osint-service-456HN2AH.js");
    ...
    if (domain) {
      try {
        const { queryDehashed } = await import("./darkweb-osint-service-456HN2AH.js");
    ...
    if (domain || target.startsWith("http")) {
      try {
        const { queryUrlscan } = await import("./darkweb-osint-service-456HN2AH.js");
