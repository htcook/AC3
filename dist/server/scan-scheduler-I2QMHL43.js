...
async function triggerScheduledScan(monitor) {
  ...
  try {
    ...
    const { detectSubdomainTakeover, crossReferenceTechVulnerabilities, detectSubdomainChanges } = await import("./domain-intel-advanced-S7IJDBMU.js");
...
function initScanScheduler() {
  ...
  setTimeout(async () => {
    try {
      const { startPeriodicHealthChecks } = await import("./health-monitor-CYBYMXCR.js");
...
function stopScanScheduler() {
  ...
  import("./health-monitor-CYBYMXCR.js").then(({ stopPeriodicHealthChecks }) => {
