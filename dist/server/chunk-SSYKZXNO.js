...
// server/lib/knowledge/offensive-tools-knowledge.ts
function getToolsForLab(labId) {
...
function buildToolRecommendationContext(params) {
  const sections = [];
  const phaseToolMap = {
...
function buildAttackPlannerToolContext(targetLab) {
  const sections = [];
...
function searchTools(query) {
  ...
  return OFFENSIVE_TOOLS.filter(
    (t) => t.name.toLowerCase().includes(q) || t.description.toLowerCase().includes(q) || t.useCase.toLowerCase().includes(q) || t.category.includes(q)
  );
}
...
var OFFENSIVE_TOOLS;
var init_offensive_tools_knowledge = __esm({
  "server/lib/knowledge/offensive-tools-knowledge.ts"() {
    OFFENSIVE_TOOLS = [
      // ── Reconnaissance ──
