...
// server/lib/llm-self-learning.ts
async function storeLearningEntry(entry) {
...
async function getLearningEntries(targetPreset) {
  const mysql = await import("mysql2/promise");
  const conn = await mysql.createConnection(process.env.DATABASE_URL);
  try {
...
async function getAllLearningEntries(limit = 200) {
  const mysql = await import("mysql2/promise");
  const conn = await mysql.createConnection(process.env.DATABASE_URL);
  try {
...
function buildCorrectionHistoryPrompt(targetPreset, targetLearnings, globalLearnings) {
...
CONFIRMED CORRECT (keep reporting these):`);
  ...
  const globalIncorrect = globalLearnings.filter(
    (l) => l.feedbackType === "incorrect" || l.feedbackType === "false_positive"
  ).filter((l) => l.targetPreset !== targetPreset);
  const globalMissed = globalLearnings.filter(
    (l) => l.feedbackType === "missed_finding"
  ).filter((l) => l.targetPreset !== targetPreset);
  if (globalIncorrect.length > 0 || globalMissed.length > 0) {
    sections.push(`
\u2550\u2550\u2550 CROSS-TARGET LEARNING PATTERNS \u2550\u2550\u2550`);
    if (globalIncorrect.length > 0) {
      const fpCounts = /* @__PURE__ */ new Map();
...
RECURRING FALSE POSITIVE PATTERNS:`);
    ...
    if (globalMissed.length > 0) {
      const missedCounts = /* @__PURE__ */ new Map();
...
function normalizeTitle(title) {
  return (title || "").replace(/^\[\w+(?:\s*\w+)*\]\s*/i, "").replace(/^\(\w+\)\s*/i, "").replace(/\s+/g, " ").trim();
}
...
function computeMatchScore(gt, f) {
  let matchScore = 0;
  ...
  if (fTitle.includes(gtTitle) || gtTitle.includes(fTitle)) {
    ...
    if (gtSynKey && fSynKey && gtSynKey === fSynKey) {
      ...
      const stopwords = /* @__PURE__ */ new Set(["the", "and", "for", "via", "with", "from", "that", "this", "based"]);
  ...
  if (gt.cwe && f.cwe) {
    const gtCwe = (gt.cwe || "").toLowerCase();
    const fCwe = (f.cwe || "").toLowerCase();
    if (gtCwe && fCwe && gtCwe === fCwe) matchScore += 2;
  }
...
function getDeduplicationSubtypeKey(title) {
  const lower = title.toLowerCase();
  const subtypes = SUBTYPE_KEYWORDS.filter((kw) => lower.includes(kw));
  return subtypes.sort().join("+") || "_default";
}
function deduplicateFindings(findings) {
  const severityOrder = { critical: 4, high: 3, medium: 2, low: 1, info: 0, informational: 0 };
  const synGroups = /* @__PURE__ */ new Map();
  const noSynFindings = [];
...
function scoreAgainstGroundTruth(targetPreset, llmFindings, options) {
  ...
  const groundTruth = autoOnly ? fullGroundTruth.filter((gt) => gt.autoDetectable !== false) : fullGroundTruth;
  ...
  const matchDetails = [];
  const matchedLlmIndices = /* @__PURE__ */ new Set();
  const matchedGtIndices = /* @__PURE__ */ new Set();
  const allPairs = [];
  ...
  return {
    ...
    f1Score: Math.round(f1Score * 1e4) / 1e4,
    severityAccuracy: Math.round(severityAccuracy * 1e4) / 1e4,
    overallScore: Math.round(overallScore * 1e4) / 1e4,
    matchDetails,
    unmatchedLlmFindings
  };
}
async function saveAccuracyScore(sessionId, targetPreset, score) {
  const mysql = await import("mysql2/promise");
  const conn = await mysql.createConnection(process.env.DATABASE_URL);
  try {
    await conn.execute(
...
async function getAccuracyTrend(targetPreset, limit = 50) {
  const mysql = await import("mysql2/promise");
  const conn = await mysql.createConnection(process.env.DATABASE_URL);
  try {
    const query = targetPreset ? `SELECT * FROM llm_accuracy_scores WHERE target_preset = ? ORDER BY scored_at DESC LIMIT ?` : `SELECT * FROM llm_accuracy_scores ORDER BY scored_at DESC LIMIT ?`;
...
async function getAccuracyStats() {
  ...
  try {
    ...
    for (const r of rows) {
      ...
      const scores = trendRows.map((t) => Number(t.f1_score));
      ...
      results.push({
        ...
        avgF1: Math.round(Number(r.avg_f1) * 1e4) / 1e4,
        avgPrecision: Math.round(Number(r.avg_precision) * 1e4) / 1e4,
        avgRecall: Math.round(Number(r.avg_recall) * 1e4) / 1e4,
        avgOverall: Math.round(Number(r.avg_overall) * 1e4) / 1e4,
        latestF1: scores[0] || 0,
        trend
      });
    }
...
async function buildLearningContext(targetPreset) {
  try {
    ...
    if (groundTruth && groundTruth.length > 0) {
      const detectionHints = groundTruth.filter((g) => g.detectionHint).map((g) => `  \u2022 ${g.title} [${g.severity}] (${g.category}): ${g.detectionHint}`).join("\n");
      groundTruthHint = `
...
`;
    ...
    let precisionHint = "";
    if (precisionConfig) {
      const negExamples = precisionConfig.negativeExamples.map((n) => `  \u2717 Do NOT report findings containing "${n.pattern}" \u2014 ${n.reason}`).join("\n");
      precisionHint = `
...
var TARGET_PRECISION_CONFIG, GROUND_TRUTH_LIBRARY, VULN_SUBTYPE_SYNONYMS, VULN_SYNONYMS, INFORMATIONAL_FP_PATTERNS, MEDIUM_SEVERITY_INFORMATIONAL_PATTERNS, SUBTYPE_KEYWORDS;
var init_llm_self_learning = __esm({
  "server/lib/llm-self-learning.ts"() {
    TARGET_PRECISION_CONFIG = {
      "zero-bank": {
        maxFindings: 6,
    ...
    GROUND_TRUTH_LIBRARY = {
      ...
      "dvwa": [
        ...
        { title: "SQL Injection", category: "Injection", owaspCategory: "A03:2025", severity: "critical", cwe: "CWE-89", description: "DVWA SQL Injection exercise: The 'id' parameter on the SQL Injection page (/vulnerabilities/sqli/) is directly concatenated into a MySQL query without sanitization. Payload ' OR 1=1 -- bypasses the query. UNION-based extraction possible. This is one of DVWA's most important exercises and MUST be reported.", detectionHint: "DVWA /vulnerabilities/sqli/ page \u2014 test ID parameter with ' OR 1=1 -- or ' UNION SELECT user,password FROM users--", autoDetectable: true },
        { title: "SQL Injection - Blind", category: "Injection", owaspCategory: "A03:2025", severity: "critical", cwe: "CWE-89", description: "DVWA Blind SQL Injection exercise: The 'id' parameter on the SQL Injection (Blind) page (/vulnerabilities/sqli_blind/) is vulnerable to boolean-based and time-based blind SQL injection. The application returns different responses for true/false conditions, allowing data extraction without direct output. This is a separate DVWA exercise from regular SQL Injection.", detectionHint: "DVWA /vulnerabilities/sqli_blind/ page \u2014 test with 1' AND 1=1# vs 1' AND 1=2# for boolean-based, or 1' AND SLEEP(5)# for time-based", autoDetectable: true },
        { title: "XSS - Reflected", category: "Cross-Site Scripting", owaspCategory: "A03:2025", severity: "high", cwe: "CWE-79", description: "DVWA Reflected XSS exercise: The 'name' parameter on the XSS (Reflected) page (/vulnerabilities/xss_r/) is echoed back without encoding. Payload <script>alert(1)</script> executes immediately. This is a core DVWA exercise and MUST be reported.", detectionHint: "DVWA /vulnerabilities/xss_r/ page \u2014 inject <script>alert(1)</script> in name field", autoDetectable: true },
        { title: "XSS - Stored", category: "Cross-Site Scripting", owaspCategory: "A03:2025", severity: "high", cwe: "CWE-79", description: "DVWA Stored XSS exercise: The guestbook on the XSS (Stored) page (/vulnerabilities/xss_s/) stores user input without sanitization. Submitted XSS payloads persist and execute for all visitors. This is a core DVWA exercise and MUST be reported.", detectionHint: "DVWA /vulnerabilities/xss_s/ page \u2014 submit <script>alert(1)</script> in guestbook name or message field", autoDetectable: true },
        { title: "Command Injection", category: "Injection", owaspCategory: "A03:2025", severity: "critical", cwe: "CWE-78", description: "DVWA Command Injection exercise: The IP address input on the Command Injection page (/vulnerabilities/exec/) is passed directly to shell_exec() with ping. Operators ; | && allow command chaining (e.g., 127.0.0.1; cat /etc/passwd). This is a core DVWA exercise and MUST be reported.", detectionHint: "DVWA /vulnerabilities/exec/ page \u2014 test with 127.0.0.1; id or 127.0.0.1 | cat /etc/passwd", autoDetectable: true },
        { title: "CSRF", category: "Cross-Site Request Forgery", owaspCategory: "A01:2025", severity: "medium", cwe: "CWE-352", description: "DVWA CSRF exercise: The password change form on the CSRF page (/vulnerabilities/csrf/) has no anti-CSRF token. An attacker can craft an external HTML page that auto-submits a password change request. This is a core DVWA exercise and MUST be reported.", detectionHint: "DVWA /vulnerabilities/csrf/ page \u2014 craft external form that auto-submits password change via GET parameters", autoDetectable: true },
        // ── Other DVWA exercises ──
        { title: "XSS - DOM Based", category: "Cross-Site Scripting", owaspCategory: "A03:2025", severity: "high", cwe: "CWE-79", description: "DVWA DOM XSS exercise: The URL parameter on the XSS (DOM) page (/vulnerabilities/xss_d/) is processed client-side without sanitization.", detectionHint: "DVWA /vulnerabilities/xss_d/ \u2014 manipulate URL fragment with XSS payload", autoDetectable: true },
        { title: "File Inclusion - Local", category: "Injection", owaspCategory: "A03:2025", severity: "critical", cwe: "CWE-98", description: "Local file inclusion via page parameter.", detectionHint: "Test with ../../etc/passwd", autoDetectable: true },
        { title: "File Inclusion - Remote", category: "Injection", owaspCategory: "A03:2025", severity: "critical", cwe: "CWE-98", description: "Remote file inclusion allows loading external PHP files.", detectionHint: "Include remote PHP shell via URL", autoDetectable: true },
        { title: "File Upload Vulnerability", category: "Injection", owaspCategory: "A04:2025", severity: "critical", cwe: "CWE-434", description: "Unrestricted file upload allows PHP webshell upload.", detectionHint: "Upload .php file and execute via web", autoDetectable: false },
        { title: "Brute Force", category: "Broken Authentication", owaspCategory: "A02:2025", severity: "high", cwe: "CWE-307", description: "Login form has no rate limiting or account lockout.", detectionHint: "Attempt multiple login attempts with common passwords", autoDetectable: true },
        { title: "Insecure CAPTCHA", category: "Broken Authentication", owaspCategory: "A02:2025", severity: "medium", cwe: "CWE-863", description: "CAPTCHA implementation can be bypassed by manipulating step parameter.", detectionHint: "Skip CAPTCHA step by modifying POST parameters", autoDetectable: false },
        { title: "Weak Session IDs", category: "Broken Authentication", owaspCategory: "A02:2025", severity: "medium", cwe: "CWE-330", description: "Session IDs are predictable and sequential.", detectionHint: "Collect multiple session IDs and analyze pattern", autoDetectable: false },
        { title: "Open HTTP Redirect", category: "Security Misconfiguration", owaspCategory: "A05:2025", severity: "medium", cwe: "CWE-601", description: "Redirect parameter allows redirection to arbitrary URLs.", detectionHint: "Test redirect parameter with external URL", autoDetectable: true },
    ...
    MEDIUM_SEVERITY_INFORMATIONAL_PATTERNS = [
      /^x-frame-options header not set$/i,
      /^x-content-type-options header not set$/i,
