...
// server/lib/owasp-coverage-tracker.ts
function generateCoverageNarrative(score, tested, gaps, partial, assetCount, criticalGaps) {
...
`;
    ...
    if (critCount > 0) {
      ...
      narrative += criticalGaps.filter((g) => g.priority === "critical").map((g) => g.categoryName).join(", ");
...
`;
    ...
    if (highCount > 0) {
      ...
      narrative += criticalGaps.filter((g) => g.priority === "high").map((g) => g.categoryName).join(", ");
      narrative += `.
...
function renderOwaspCoverageHTML(coverage) {
  const statusColors = {
    tested: "#22c55e",
    partial: "#f59e0b",
    not_tested: "#ef4444",
...
function generateOwaspReportSection(coverage) {
  return {
    title: "OWASP Top 10:2025 Coverage Analysis",
    content: coverage.summaryNarrative
  };
}
function getOwaspTracker() {
  if (!_tracker) _tracker = new OwaspCoverageTracker();
  return _tracker;
}
function resetOwaspTracker() {
  _tracker = new OwaspCoverageTracker();
  return _tracker;
}
var OWASP_CATEGORY_NAMES, TOOL_TO_OWASP_COVERAGE, OwaspCoverageTracker, _tracker;
var init_owasp_coverage_tracker = __esm({
  "server/lib/owasp-coverage-tracker.ts"() {
    init_owasp_knowledge();
    ...
    OwaspCoverageTracker = class {
      constructor() {
        this.findings = [];
        this.toolRuns = [];
        this.assetTech = /* @__PURE__ */ new Map();
      }
      /**
       * Register detected technologies for an asset
       */
      registerAssetTech(hostname, technologies) {
        const existing = this.assetTech.get(hostname) || [];
        this.assetTech.set(hostname, [.../* @__PURE__ */ new Set([...existing, ...technologies])]);
      }
      /**
      ...
      getAssetCoverage(hostname) {
        ...
        const coverageScore = applicableCount > 0 ? Math.round((testedCount + partialCount * 0.5) / applicableCount * 100) : 100;
        return {
          hostname,
          ip: void 0,
          detectedTech: tech,
       ...
       */
      getEngagementCoverage(engagementId) {
        ...
        const overallScore = assets.length > 0 ? Math.round(assets.reduce((s, a) => s + a.coverageScore, 0) / assets.length) : 0;
