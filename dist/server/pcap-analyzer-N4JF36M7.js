...
async function executeLiveCapture(config) {
  const { executeTool, executeRawCommand } = await import("./scan-server-executor-MSPK4KCH.js");
...
async function ingestPcapResults(findings, packets) {
  const { ingestRawObservations } = await import("./observation-ingestor-LAFIJXKU.js");
