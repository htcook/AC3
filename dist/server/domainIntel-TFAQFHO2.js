...
Risk signals detected (${passiveRecon.riskSignals.length}):`);
  ...
  try {
    const { applyMissionBaselines } = await import("./scoring-engine-R2WZCQZW.js");
    let missionIdx = 0;
    for (const a of analyses) {
      missionIdx++;
  ...
  let oemCredentials = [];
  try {
    const { matchCredentialsForAssets, persistMatchedCredentials } = await import("./oem-default-creds-BKSQ2KWT.js");
  ...
  if (oemCredentials.length > 0 && scanMode === "active") {
    try {
      const { runCredentialTests, getCredentialsForService } = await import("./credential-tester-UXBQXGMX.js");
  ...
  try {
    const { runPipelineCrawlStage, enrichOrgWithBusinessIntel, applyBusinessIntelCarverBoosts } = await import("./pipeline-crawl-stage-KJ3RWSNS.js");
  ...
  try {
    ...
    const { createCarverRiskCard } = await import("./db-F33RXQPM.js");
    const assetSignals = [];
    if (passiveRecon) {
  ...
  const [threatMatchRetry, incidentSearchRetry, affiliatedDomainsRetry] = await retryMod.parallelWithRetry([
    ...
    },
    {
      ...
      fn: async () => {
        const { runIncidentSearchEnrichment } = await import("./incident-search-enrichment-SNJT5Y3K.js");
        ...
        let ingestResult;
        try {
          const { ingestIncidentSearchResults } = await import("./incident-search-ingest-ISBI2TXJ.js");
    ...
    },
    {
      ...
      fn: async () => {
        const { runAffiliatedDomainDiscovery } = await import("./affiliated-domain-discovery-X2EJYD6P.js");
  ...
  try {
    const { collectTrainingData } = await import("./incident-training-collector-Q7DZBTJP.js");
