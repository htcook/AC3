...
async function buildInjectableUrls(webApp, targetUrl, state, engagementId) {
  ...
  try {
    ...
    if (db) {
      const { webAppScans } = await import("./schema-RL5B6OMI.js");
...
async function executeInjectionScanning(ctx) {
  ...
  for (const webApp of webApps) {
    ...
    try {
      ...
      if (approved) {
        const { batchSqlmapScan, analyzeSqlmapFindings, runBlindSqliPass, ingestSqlmapToWebAppFindings } = await import("./sqlmap-scanner-PFM65ZQR.js");
    ...
    try {
      ...
      if (approved) {
        const { batchXssScan, analyzeXssFindings, ingestXssToWebAppFindings } = await import("./xsstrike-scanner-JGYFRH3Z.js");
  ...
  for (const webApp of webApps) {
    ...
    try {
      ...
      if (approved) {
        const { batchCommixScan, ingestCommixToWebAppFindings } = await import("./commix-scanner-QVKAOESO.js");
        const isLab = state.trainingLabMode === true;
  ...
  for (const webApp of webApps) {
    ...
    try {
      ...
      if (approved) {
        const { batchTplmapScan, ingestTplmapToWebAppFindings } = await import("./tplmap-scanner-5ZL2HMGV.js");
