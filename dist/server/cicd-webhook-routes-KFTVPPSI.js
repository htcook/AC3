...
function registerCicdWebhookRoutes(app) {
  app.post("/api/cicd/webhook/:pipelineId", async (req, res) => {
    ...
    try {
      ...
      const { cicdPipelines, cicdRuns } = await import("./schema-RL5B6OMI.js");
      ...
      if (secret) {
        ...
        if (signature) {
          const { verifyGitHubWebhook } = await import("./aws-cicd-connector-QSYOVL2M.js");
      ...
      if (targetUrl) {
        import("./aws-cicd-connector-QSYOVL2M.js").then(async ({ executeCicdScan }) => {
          try {
            ...
            if (pipeline.cicdProvider === "github_actions" && commitSha && repository) {
              try {
                ...
                if (githubToken) {
                  ...
                  const { callbackGitHubActions } = await import("./aws-cicd-connector-QSYOVL2M.js");
...
Trigger: Webhook (${event})`
  ...
  app.get("/api/cicd/run/:runId/status", async (req, res) => {
    ...
    try {
      const { getDb } = await import("./db-UO2A3C4N.js");
      const { cicdRuns, cicdPipelines } = await import("./schema-RL5B6OMI.js");
      const { eq } = await import("drizzle-orm");
      const db = await getDb();
      if (!db) {
        return res.status(503).json({ error: "Database unavailable" });
