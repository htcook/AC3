...
async function persistGraduationToDB(domain, scores, opts) {
  try {
    const { insertGraduationScore } = await import("./db-JLHOBMS4.js");
...
async function persistConnectorPerfToDB(entries, sector) {
  try {
    const { bulkInsertConnectorPerformance } = await import("./db-JLHOBMS4.js");
...
async function hydrateFromDB(domain) {
  ...
  try {
    const { getGraduationScoresForDomain, getConnectorPerformanceForDomain } = await import("./db-JLHOBMS4.js");
...
async function loadSectorInsights(sector) {
  ...
  try {
    const { getAvgGraduationScoresBySector, getConnectorAvgsBySector } = await import("./db-JLHOBMS4.js");
