...
// server/lib/compliance-evidence-mapper.ts
function makeEvidenceId() {
  return `EVD-${Date.now()}-${++evidenceCounter}`;
}
function matchFindingToRules(finding) {
  const matched = [];
...
function determineStatus(statusLogic, hasFinding, severity) {
  switch (statusLogic) {
...
function mapEngagementToCompliance(state) {
  const evidence = [];
  const now = Date.now();
  ...
  for (const [framework, controls] of Object.entries(FRAMEWORK_CONTROLS)) {
    const controlPostures = controls.map((ctrl) => {
...
var MAPPING_RULES, FRAMEWORK_CONTROLS, evidenceCounter;
var init_compliance_evidence_mapper = __esm({
  "server/lib/compliance-evidence-mapper.ts"() {
    MAPPING_RULES = [
...
export {
  ...
  getMappingRules,
  init_compliance_evidence_mapper
};
