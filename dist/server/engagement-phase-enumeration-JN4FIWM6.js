...
${state.scanPlan?.discoveryStrategy || "Comprehensive port discovery to enrich passive recon data"}`
  ...
  try {
    const { getScanServerConfigForScanForge } = await import("./scan-server-executor-L5FGTKBI.js");
    const { autoSelectTool } = await import("./scanforge-discovery-LDDT7B72.js");
...
Evasion: ${assetPlan?.evasionTechniques?.join(", ") || "fragmentation, decoys, normal timing"}`
  ...
  try {
    const { beforeDiscoveryScan } = await import("./pcap-auto-capture-XFW3WWS6.js");
  ...
  try {
    ...
    if (discoveryResult.stdout) {
      try {
        const discovery = await import("./scanforge-discovery-LDDT7B72.js");
...
Ports: ${discoveredPorts.map((p) => `${p.port}/${p.service}${p.product ? ` (${p.product})` : ""}`).join(", ")}`,
  ...
  if (autoCaptureSessionId) {
    try {
      const { afterDiscoveryScan } = await import("./pcap-auto-capture-XFW3WWS6.js");
...
async function runCloudAssetDetection(state, helpers) {
  try {
    const { detectCloudAsset, executeCloudStorageScan, getCloudDetectionPromptContext } = await import("./cloud-storage-scanner-XJOSD5EO.js");
    helpers.addLog({
      phase: "enumeration",
      type: "info",
...
async function executeTargetedToolDeployment(state, helpers) {
  ...
  const { suggestToolCommands } = await import("./scan-server-executor-L5FGTKBI.js");
  for (const asset of state.assets) {
    ...
    const { autoSelectTool: autoSelectToolB } = await import("./scanforge-discovery-LDDT7B72.js");
