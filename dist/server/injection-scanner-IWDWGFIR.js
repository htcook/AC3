...
async function executeInjectionScanning(ctx) {
  ...
  for (const webApp of webApps) {
    ...
    try {
      ...
      if (approved) {
        const { batchSqlmapScan, analyzeSqlmapFindings, runBlindSqliPass, ingestSqlmapToWebAppFindings } = await import("./sqlmap-scanner-VULZN4FI.js");
    ...
    try {
      ...
      if (approved) {
        const { batchXssScan, analyzeXssFindings, ingestXssToWebAppFindings } = await import("./xsstrike-scanner-SM4I5GFW.js");
  ...
  for (const webApp of webApps) {
    ...
    try {
      ...
      if (approved) {
        const { batchCommixScan, ingestCommixToWebAppFindings } = await import("./commix-scanner-ITAY73ZK.js");
        const isLab = state.trainingLabMode === true;
  ...
  for (const webApp of webApps) {
    ...
    try {
      ...
      if (approved) {
        const { batchTplmapScan, ingestTplmapToWebAppFindings } = await import("./tplmap-scanner-QMVQ63A5.js");
