...
async function resolveUserTenant(userId, requestedTenantId) {
  ...
  const { tenantMemberships, tenants } = await import("./schema-KTWCH6W6.js");
...
async function autoProvisionTenant(userId, userName) {
  const { getDb } = await import("./db-TULZR6OH.js");
  const { tenants, tenantMemberships } = await import("./schema-KTWCH6W6.js");
  const db = await getDb();
...
async function logTenantAction(userId, tenantId, action, details) {
  try {
    const { getDb } = await import("./db-TULZR6OH.js");
    const { activityLogs } = await import("./schema-KTWCH6W6.js");
    const db = await getDb();
    if (!db) return;
...
async function getTenantStats(tenantId) {
  const { getDb } = await import("./db-TULZR6OH.js");
  const { tenantMemberships, engagements } = await import("./schema-KTWCH6W6.js");
  const db = await getDb();
  if (!db) return null;
