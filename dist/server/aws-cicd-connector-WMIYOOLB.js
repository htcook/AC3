...
async function scanServerPreFlight() {
  try {
    const { checkDoScanServiceHealth } = await import("./do-scan-api-O2NBKZFV.js");
...
async function executeCicdScan(request) {
  ...
  try {
    const { correlateCicdFindings } = await import("./cicd-threat-correlator-DD25GIZY.js");
...
async function runZapCicd(targetUrl) {
  const findings = [];
  try {
    const { getActiveZapUrl } = await import("./scan-service-url-JYPT75HE.js");
...
async function generateSbom(imageRef) {
  const { executeRawCommandViaHttp } = await import("./do-scan-api-O2NBKZFV.js");
  const { doStoragePut } = await import("./do-storage-NFBEJBOT.js");
  const result = await executeRawCommandViaHttp(
