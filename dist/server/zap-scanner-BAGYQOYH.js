  ...
  for (const webApp of webApps) {
    ...
    for (const { targetUrl, dedupKey } of scanTargets) {
      ...
      try {
        ...
        const { generateLLMScanConfig, startScan, configureZapAuthentication } = await import("./zap-scanner-CLS2NFJQ.js");
        ...
        if (wafVendor) {
          try {
            const { applyWafEvasionConfig } = await import("./zap-scanner-CLS2NFJQ.js");
        ...
        if (!zapStarted) {
          ...
          try {
            const { onEngagementVulnDetectionPhase } = await import("./burp-auto-scan-PJCWACEA.js");
        ...
        if (zapScanId) {
          ...
          const { pollScanProgress } = await import("./zap-scanner-CLS2NFJQ.js");
  ...
  try {
    ...
    if (ir && (ir.urlSource === "scope_fallback" || ir.zapUrlsDiscovered === 0)) {
      ...
      if (db) {
        ...
        if (scans.length > 0) {
          const { deferredZapBurpRefeed } = await import("./zap-burp-pipeline-FWMSQ7IB.js");
