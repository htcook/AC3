...
Risk signals detected (${passiveRecon.riskSignals.length}):`);
  ...
  try {
    const { applyMissionBaselines } = await import("./scoring-engine-VYFFEN5A.js");
    let missionIdx = 0;
    for (const a of analyses) {
      missionIdx++;
  ...
  let oemCredentials = [];
  try {
    const { matchCredentialsForAssets, persistMatchedCredentials } = await import("./oem-default-creds-TA2NHQEO.js");
  ...
  if (oemCredentials.length > 0 && scanMode === "active") {
    try {
      const { runCredentialTests, getCredentialsForService } = await import("./credential-tester-DU2MDD4C.js");
  ...
  try {
    const { runPipelineCrawlStage, enrichOrgWithBusinessIntel, applyBusinessIntelCarverBoosts } = await import("./pipeline-crawl-stage-MTVEZBW3.js");
  ...
  try {
    ...
    const { createCarverRiskCard } = await import("./db-GNA5CL3K.js");
    const assetSignals = [];
    if (passiveRecon) {
  ...
  const [threatMatchRetry, incidentSearchRetry, affiliatedDomainsRetry] = await retryMod.parallelWithRetry([
    ...
    },
    {
      ...
      fn: async () => {
        const { runIncidentSearchEnrichment } = await import("./incident-search-enrichment-5RGDJURU.js");
        ...
        let ingestResult;
        try {
          const { ingestIncidentSearchResults } = await import("./incident-search-ingest-HC3LQL6Y.js");
    ...
    },
    {
      ...
      fn: async () => {
        const { runAffiliatedDomainDiscovery } = await import("./affiliated-domain-discovery-FUELMWP5.js");
  ...
  try {
    const { collectTrainingData } = await import("./incident-training-collector-2X4QQ5F6.js");
