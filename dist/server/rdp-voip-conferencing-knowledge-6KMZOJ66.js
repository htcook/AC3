...
// server/lib/knowledge/rdp-voip-conferencing-knowledge.ts
function buildSipOptionsProbe(target, port, fromIp) {
...
function generateRdpScanCommands(target, port = 3389) {
  return [
...
function generateSipScanCommands(target, port = 5060) {
  return [
...
function generateConferencingScanCommands(target) {
  return [
...
function getExploitKnowledgeForService(service) {
...
function getLlmPromptContextForService(service, cve) {
...
function getAttackTechniquesForService(service) {
  const svc = service.toLowerCase();
  return RDP_VOIP_ATTACK_TECHNIQUES.filter((t) => t.services.includes(svc));
}
function getScanCommandsForService(service, target, port) {
  const svc = service.toLowerCase();
...
function isRdpVoipConferencingPort(port) {
...
function buildExploitContextForLlm(params) {
...
`;
    context += `NLA Status: ${hasNla === void 0 ? "Unknown" : hasNla ? "Enabled (limits pre-auth attacks)" : "Disabled (BlueKeep/MITM possible)"}
`;
    context += `
...
var RDP_VOIP_PORTS, RDP_VOIP_PORT_LIST, RDP_EXPLOITS, VOIP_SIP_EXPLOITS, CONFERENCING_EXPLOITS, RDP_VOIP_ATTACK_TECHNIQUES;
var init_rdp_voip_conferencing_knowledge = __esm({
  "server/lib/knowledge/rdp-voip-conferencing-knowledge.ts"() {
    RDP_VOIP_PORTS = [
      // ── RDP ──
      ...
      { port: 2e3, protocol: "tcp", service: "sccp", description: "Skinny Client Control Protocol (Cisco phones)", fingerprint: ["SCCP header"], commonVendors: ["Cisco"], attackSurface: ["Phone registration hijacking", "VLAN hopping via CDP", "Firmware extraction", "Configuration download"] },
    ...
    RDP_VOIP_PORT_LIST = RDP_VOIP_PORTS.filter((p) => p.port > 0).map((p) => p.port);
    RDP_EXPLOITS = [
      {
        id: "rdp-bluekeep",
        service: "rdp",
        ...
        commands: [
          "nerva -t TARGET -p 3389 --service-detect",
          // replaces nmap NSE rdp scripts
    ...
    VOIP_SIP_EXPLOITS = [
      {
        ...
        commands: [
          ...
          "nerva -t TARGET -p 5060 --service-detect --udp",
          // replaces nmap sip-enum-users
    ...
    CONFERENCING_EXPLOITS = [
      {
        ...
        commands: [
          ...
          // replaces nmap http-default-accounts
        ],
...
Generate a Python script that tests these credentials and extracts configuration.`
      },
      {
        ...
        commands: [
          ...
          "ssh-audit TARGET && nerva -t TARGET -p 22,443 --service-detect",
          // replaces nmap ssh/http scripts
...
- Information disclosure via unauthenticated API endpoints`
      },
      {
        ...
        tools: ["curl", "httpx"],
        commands: [
          "httpx -u TARGET -ports 9090,443,80 -title -status-code",
          // replaces nmap http-title
...
5. Reports findings as evidence`
      },
      {
        ...
        commands: [
          "telnet TARGET 41795",
          "nerva -t TARGET -p 22,41795,443 --service-detect",
          // replaces nmap telnet-ntlm-info
...
init_rdp_voip_conferencing_knowledge();
export {
  CONFERENCING_EXPLOITS,
  RDP_EXPLOITS,
  RDP_VOIP_ATTACK_TECHNIQUES,
  ...
  buildSipOptionsProbe,
  generateConferencingScanCommands,
  generateRdpScanCommands,
  generateSipScanCommands,
  getAttackTechniquesForService,
  getExploitKnowledgeForService,
  getLlmPromptContextForService,
