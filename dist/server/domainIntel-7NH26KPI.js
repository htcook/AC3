...
Risk signals detected (${passiveRecon.riskSignals.length}):`);
  ...
  try {
    const { applyMissionBaselines } = await import("./scoring-engine-FRB2LKGI.js");
    let missionIdx = 0;
    for (const a of analyses) {
      missionIdx++;
  ...
  let oemCredentials = [];
  try {
    const { matchCredentialsForAssets, persistMatchedCredentials } = await import("./oem-default-creds-BQNHP52P.js");
  ...
  if (oemCredentials.length > 0 && scanMode === "active") {
    try {
      const { runCredentialTests, getCredentialsForService } = await import("./credential-tester-GQBF6CKY.js");
