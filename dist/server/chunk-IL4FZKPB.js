...
function buildDiscoveryScanForgeFlags(profile, target) {
  const flags = [
    profile.discovery.discoveryPorts,
    profile.discovery.timing,
    "-sV",
    "--version-intensity",
    String(profile.discovery.versionIntensity)
  ];
...
function buildNucleiFlags(profile, targetUrl) {
  const flags = [
    "-u",
    targetUrl,
    "-severity",
    profile.nuclei.severityFilter,
    "-rl",
    String(profile.nuclei.rateLimit),
    "-timeout",
    String(profile.nuclei.templateTimeout),
    "-retries",
    "1",
...
function buildGobusterCommand(profile, targetUrl, options = {}) {
  const cfg = profile.gobuster;
  const flags = ["gobuster", "dir", "-u", targetUrl];
