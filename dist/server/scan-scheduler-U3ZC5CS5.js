...
async function triggerScheduledScan(monitor) {
  ...
  try {
    ...
    const { detectSubdomainTakeover, crossReferenceTechVulnerabilities, detectSubdomainChanges } = await import("./domain-intel-advanced-UOZ73MSD.js");
...
function initScanScheduler() {
  ...
  setTimeout(async () => {
    try {
      const { startPeriodicHealthChecks } = await import("./health-monitor-FGGSUFAL.js");
...
function stopScanScheduler() {
  ...
  import("./health-monitor-FGGSUFAL.js").then(({ stopPeriodicHealthChecks }) => {
