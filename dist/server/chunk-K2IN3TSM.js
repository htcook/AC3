...
async function executeToolViaQueue(config, options) {
  ...
  if (!useQueue) {
    if (!options?.forceLocal) {
      try {
        const { executeToolViaHttp } = await import("./do-scan-api-KV3Z3F2D.js");
...
async function executeRawCommandViaQueue(command, timeoutSeconds = 300, options) {
  ...
  const { executeViaChildProcessSSH } = await import("./scan-server-executor-LFT6VWBD.js");
