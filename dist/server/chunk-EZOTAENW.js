...
Generate two-phase scan plan. Phase A: ScanForge discovery --top-ports 1000. Phase B: tools per asset.`;
  ...
  try {
    ...
    try {
      const { getToolInventory, getInventoryForLLM } = await import("./scan-server-inventory-DJU3YK74.js");
...
${Object.entries(state.dastConfig.customHeaders).map(([k, v]) => `  ${k}: ${v}`).join("\n")}`
  ...
  for (const domain of domains) {
    try {
      ...
      const { runDomainIntelPipeline } = await import("./domainIntel-34D3Q6RJ.js");
...
async function executeEnumeration(state, engagement, operatorCtx) {
  const { executeEnumeration: runEnumerationPhase } = await import("./engagement-phase-enumeration-DNQJZF7H.js");
...
async function executeVulnDetection(state, engagement, operatorCtx) {
  ...
  const { executeVulnPrep } = await import("./vuln-prep-PS7K4VM7.js");
  const vulnPrepCtx = {
    state,
    engagement,
    operatorCtx,
  ...
  const { executeZapScanning } = await import("./zap-scanner-6RMQ7Z7B.js");
  ...
  const { executeInjectionScanning } = await import("./injection-scanner-SD5C6TJO.js");
  ...
  const { executeCredentialTesting } = await import("./credential-tester-7UXTQLE2.js");
  ...
  const { executeVulnCorrelation } = await import("./vuln-correlation-FK5IIVQ6.js");
...
async function executeExploitation(state, engagement, operatorCtx) {
  const { executeExploitation: runExploitPhase } = await import("./engagement-phase-exploitation-E3BZVBLX.js");
  return runExploitPhase(state, engagement, operatorCtx);
}
async function executePostExploit(state, engagement, operatorCtx) {
  const { executePostExploit: runPostExploitPhase } = await import("./engagement-phase-post-exploit-IGOXEZDK.js");
    ...
    if (engagement.roeStatus === "signed" || engagement.roeStatus === "pending" || state.trainingLabMode === true) {
      if (["recon", "passive_discovery", "scoping", "test_plan", "enumeration"].includes(startPhase)) {
        ...
        if (!enumGate.allowed) {
          ...
          try {
            const { checkScanServerStatus } = await import("./scan-server-executor-OQA5G5FJ.js");
...
PoC: ${cf.analysis.poc || "N/A"}`,
        ...
        if (state.stats.vulnsFound > 0 || state.assets.some((a) => a.cloudProviders?.length > 0)) {
          try {
            ...
            const { runFeedbackLoop, getFeedbackLoopSummary } = await import("./llm-scan-feedback-ATP7YMEN.js");
...
Promoted: ${promotionSummary.promotedVulns.map((p) => `${p.vulnTitle.slice(0, 50)} [${p.scanner}/${p.category}]`).join("; ")}`,
      ...
      try {
        const { retryDeferredScans, getDeferredScans, clearDeferredScans } = await import("./job-queue-bridge-I2QGZ5TO.js");
      ...
      try {
        const { selectFindingsForScreenshot, captureScreenshotBatch } = await import("./screenshot-capture-6IWK6NQS.js");
      ...
      try {
        const { runAccuracyComparison } = await import("./accuracy-feedback-loop-UTZOHNH4.js");
...
View details on the Knowledge Base \u2192 Accuracy Feedback tab.`
          ...
          try {
            const { runLocalAccuracyComparison } = await import("./accuracy-feedback-loop-UTZOHNH4.js");
...
Command: ${command}`,
  ...
  try {
    const { executeTool } = await import("./scan-server-executor-OQA5G5FJ.js");
