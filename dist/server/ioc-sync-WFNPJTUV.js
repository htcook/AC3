...
async function cleanupStuckSyncs() {
  try {
    ...
    const { iocSyncLogs } = await import("./schema-TGCU627K.js");
...
async function runIocSync(syncType = "scheduled") {
  ...
  try {
    ...
    try {
      const { postSyncDarkwebEnrichment } = await import("./darkweb-ioc-enrichment-ZXYRBKDX.js");
      const newIocs = [];
    ...
    try {
      const { runDarkwebFeedSync, isDarkwebSyncRunning } = await import("./darkweb-osint-service-LYUN6SZN.js");
