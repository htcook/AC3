...
async function triggerScheduledScan(monitor) {
  ...
  try {
    ...
    const { detectSubdomainTakeover, crossReferenceTechVulnerabilities, detectSubdomainChanges } = await import("./domain-intel-advanced-MI3SHCOO.js");
...
function initScanScheduler() {
  ...
  setTimeout(async () => {
    try {
      const { startPeriodicHealthChecks } = await import("./health-monitor-KADDZN4F.js");
...
function stopScanScheduler() {
  ...
  import("./health-monitor-KADDZN4F.js").then(({ stopPeriodicHealthChecks }) => {
