...
async function persistObservations(observations) {
  ...
  try {
    ...
    const { scanObservations } = await import("./schema-TGCU627K.js");
...
async function persistSignals(signals) {
  ...
  try {
    const { getDb } = await import("./db-65DPEQYH.js");
    const { scanSignals } = await import("./schema-TGCU627K.js");
    const db = await getDb();
    if (!db) return 0;
    let inserted = 0;
...
async function persistRiskCards(cards) {
  ...
  try {
    const { getDb } = await import("./db-65DPEQYH.js");
    const { scanRiskCards } = await import("./schema-TGCU627K.js");
    const db = await getDb();
    if (!db) return 0;
    let inserted = 0;
