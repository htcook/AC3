...
async function refreshActorLLMContext(actorId) {
  ...
  try {
    try {
      const { buildDfirContextForActor } = await import("./dfir-report-ingestion-YSHHYCIS.js");
    ...
    try {
      const { buildIocDerivedTtpContext } = await import("./ioc-ttp-reverse-engineer-JZGKAZAC.js");
    ...
    try {
      const { buildActorLearningProfile } = await import("./c2-actor-feedback-loop-MZTINTQ3.js");
    ...
    try {
      const { recordContextContribution } = await import("./context-engine-tracker-IVDKEIRL.js");
