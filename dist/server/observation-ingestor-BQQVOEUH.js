...
async function persistObservations(observations) {
  ...
  try {
    ...
    const { scanObservations } = await import("./schema-OF2ORZ4R.js");
...
async function persistSignals(signals) {
  ...
  try {
    const { getDb } = await import("./db-F33RXQPM.js");
    const { scanSignals } = await import("./schema-OF2ORZ4R.js");
    const db = await getDb();
    if (!db) return 0;
    let inserted = 0;
...
async function persistRiskCards(cards) {
  ...
  try {
    const { getDb } = await import("./db-F33RXQPM.js");
    const { scanRiskCards } = await import("./schema-OF2ORZ4R.js");
    const db = await getDb();
    if (!db) return 0;
    let inserted = 0;
