...
async function getActiveSourcesForStage(stage) {
  try {
    const { getActiveCustomerIntegrationsByStage } = await import("./db-65DPEQYH.js");
...
async function recordExecutionLogs(results, engagementId) {
  const { createExecutionLog } = await import("./db-65DPEQYH.js");
  for (const result of results) {
...
async function updateIntegrationStats(results) {
  const { updateCustomerIntegration, getCustomerIntegrationByIntegrationId } = await import("./db-65DPEQYH.js");
  for (const result of results) {
