...
var BRAND_COLORS = {
  ...
  accent: "#333333",
  // Dark gray accent (minimal, no color)
  cream: "#ffffff",
  // White background
  secondary: "#d9d9d9",
...
function generateAttackPlanReport(plan) {
  ...
  if (phases.length > 0) {
    const chainHtml = phases.map(
      (p) => `<span class="kill-chain-step">${escapeHtml(p.name || "Phase")}</span>`
    ).join("");
    sections.push({
  ...
  for (let i = 0; i < phases.length; i++) {
    ...
    const stepsHtml = (phase.steps || []).map(
      (step) => `<tr>
  ...
  if (plan.detectionOpportunities?.length) {
    sections.push({
      ...
      content: `<ul>${plan.detectionOpportunities.map((d) => `<li>${escapeHtml(d)}</li>`).join("")}</ul>`
    });
  }
  if (plan.recommendations?.length) {
    sections.push({
      title: "Defensive Recommendations",
      content: `<ul>${plan.recommendations.map((r) => `<li>${escapeHtml(r)}</li>`).join("")}</ul>`
    });
  }
...
function generateRemediationReport(stats, items, overdueItems) {
  const sections = [];
  sections.push({
    title: "Executive Summary",
  ...
  if (overdueItems.length > 0) {
    const overdueRows = overdueItems.map(
      (item) => `<tr>
...
function generateCorroborationReport(report) {
  const sections = [];
  sections.push({
    title: "Executive Summary",
    content: `<div class="stats-grid">
...
export {
  generateAttackPlanReport,
  generateCorroborationReport,
