...
Return ONLY a JSON array of objects with these fields.`;
      ...
      getCveDatabase: protectedProcedure.query(async () => {
        const { getFullCveDatabase } = await import("./nvd-cve-refresh-YGTMNEXI.js");
      ...
      cveRefreshStats: protectedProcedure.query(async () => {
        const { getCveRefreshStats } = await import("./nvd-cve-refresh-YGTMNEXI.js");
      ...
      triggerCveRefresh: protectedProcedure.input(z.object({ technologies: z.array(z.string()).optional() }).optional()).mutation(async ({ input }) => {
        const { refreshCveDatabase } = await import("./nvd-cve-refresh-YGTMNEXI.js");
      ...
      diffWithScan: protectedProcedure.input(z.object({
        ...
        const profile = profiles[0];
        const { discoveredAssets } = await import("./schema-5274GLXD.js");
      ...
      createFromScan: protectedProcedure.input(z.object({
        ...
        const { getDb: getDbCore } = await import("./db-UCRYETLI.js");
        const dbConn = await getDbCore();
        if (!dbConn) throw new Error("Database not available");
        const { getDomainIntelScanById, getDiscoveredAssetsByScan } = await import("./db-UCRYETLI.js");
