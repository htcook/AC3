...
async function persistObservations(observations) {
  ...
  try {
    ...
    const { scanObservations } = await import("./schema-KTWCH6W6.js");
...
async function persistSignals(signals) {
  ...
  try {
    const { getDb } = await import("./db-TULZR6OH.js");
    const { scanSignals } = await import("./schema-KTWCH6W6.js");
    const db = await getDb();
    if (!db) return 0;
    let inserted = 0;
...
async function persistRiskCards(cards) {
  ...
  try {
    const { getDb } = await import("./db-TULZR6OH.js");
    const { scanRiskCards } = await import("./schema-KTWCH6W6.js");
    const db = await getDb();
    if (!db) return 0;
    let inserted = 0;
