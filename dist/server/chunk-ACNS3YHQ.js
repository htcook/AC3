...
async function executeToolViaQueue(config, options) {
  ...
  if (!useQueue) {
    if (!options?.forceLocal) {
      try {
        const { executeToolViaHttp } = await import("./do-scan-api-JOPP2QH3.js");
...
async function executeRawCommandViaQueue(command, timeoutSeconds = 300, options) {
  ...
  const { executeViaChildProcessSSH } = await import("./scan-server-executor-XZZDHFIF.js");
