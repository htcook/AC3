...
async function triggerScheduledScan(monitor) {
  ...
  try {
    ...
    const { detectSubdomainTakeover, crossReferenceTechVulnerabilities, detectSubdomainChanges } = await import("./domain-intel-advanced-5YZIQG34.js");
...
function initScanScheduler() {
  ...
  setTimeout(async () => {
    try {
      const { startPeriodicHealthChecks } = await import("./health-monitor-4EHJSQNP.js");
...
function stopScanScheduler() {
  ...
  import("./health-monitor-4EHJSQNP.js").then(({ stopPeriodicHealthChecks }) => {
