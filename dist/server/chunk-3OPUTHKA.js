...
async function getEngagements(scopeUser) {
  const db = await getDb();
  if (!db) return [];
  const { scopeEngagementWhere } = await import("./engagement-access-guard-HEPUEHAV.js");
...
async function getEngagementById(id, scopeUser) {
  const db = await getDb();
  if (!db) return void 0;
  const { scopedAnd } = await import("./engagement-access-guard-HEPUEHAV.js");
...
async function createBugBountyFinding(params) {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
  const { bugBountyFindings } = await import("./schema-5UXBHZJS.js");
