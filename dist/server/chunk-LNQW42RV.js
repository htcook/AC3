...
async function executeToolViaQueue(config, options) {
  ...
  if (!useQueue) {
    if (!options?.forceLocal) {
      try {
        const { executeToolViaHttp } = await import("./do-scan-api-OC4ASMH2.js");
...
async function executeRawCommandViaQueue(command, timeoutSeconds = 300, options) {
  ...
  if (!options?.forceLocal) {
    try {
      ...
      const { executeRawCommandViaHttp } = await import("./do-scan-api-OC4ASMH2.js");
  ...
  const { executeViaChildProcessSSH } = await import("./scan-server-executor-L5FGTKBI.js");
