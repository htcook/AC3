...
async function executeRemoteCommand(scanServerHost, targetIp, command, credentials) {
  ...
  const res = await fetch(`http://${scanServerHost}:3001/exec`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ command: relayCmd, timeout: SSH_TIMEOUT_MS }),
    signal: AbortSignal.timeout(SSH_TIMEOUT_MS + 5e3)
  }).catch(() => null);
  if (!res || !res.ok) {
    throw new Error(`SSH relay failed: ${res?.statusText || "connection refused"}`);
  }
