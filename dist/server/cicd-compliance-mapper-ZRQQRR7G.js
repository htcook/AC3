...
// server/lib/cicd-compliance-mapper.ts
var SOC2_CONTROLS = [
...
function assessControl(control, findings) {
  ...
  for (const finding of findings) {
    const findingTags = (finding.tags || finding.info?.tags || []).map((t) => t.toLowerCase());
    ...
    const findingCwes = (finding.cwe || finding.info?.cwe || []).map((c) => typeof c === "number" ? c : parseInt(String(c).replace(/\D/g, "")));
    ...
    for (const rule of applicableRules) {
      if (rule.tags.some((t) => findingTags.some((ft) => ft.includes(t)))) {
        matched = true;
        break;
      }
    ...
    if (matched) {
      matchedFindings.push({
        ...
        cve: finding.cve?.id || finding.cveId || void 0
      });
    }
  }
  let status;
...
function generateComplianceReport(params) {
  ...
  const categories = Array.from(categoryMap.entries()).map(([name, controls2]) => {
    const passed2 = controls2.filter((c) => c.status === "pass").length;
    const total = controls2.filter((c) => c.status !== "not_tested").length;
    return {
      name,
      controls: controls2,
      categoryScore: total > 0 ? Math.round(passed2 / total * 100) : 0
    };
  });
  ...
  const topGaps = results.filter((r) => r.status === "fail" || r.status === "partial").sort((a, b) => b.remediationPriority - a.remediationPriority).slice(0, 10);
...
function generateRecommendations(gaps, framework) {
  const recs = [];
...
function generateCrossFrameworkSummary(reports) {
  ...
  const findingFrameworkMap = /* @__PURE__ */ new Map();
  ...
  const sharedGaps = Array.from(findingFrameworkMap.entries()).filter(([_, v]) => v.frameworks.size > 1).map(([name, v]) => ({
    findingName: name,
    severity: v.severity,
