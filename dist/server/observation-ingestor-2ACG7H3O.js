...
async function persistObservations(observations) {
  ...
  try {
    ...
    const { scanObservations } = await import("./schema-R6EY37IN.js");
...
async function persistSignals(signals) {
  ...
  try {
    const { getDb } = await import("./db-OF4HQS7N.js");
    const { scanSignals } = await import("./schema-R6EY37IN.js");
    const db = await getDb();
    if (!db) return 0;
    let inserted = 0;
...
async function persistRiskCards(cards) {
  ...
  try {
    const { getDb } = await import("./db-OF4HQS7N.js");
    const { scanRiskCards } = await import("./schema-R6EY37IN.js");
    const db = await getDb();
    if (!db) return 0;
    let inserted = 0;
