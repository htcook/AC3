...
} from "./chunk-NYDLFO63.js";
import {
  capLLMContext,
  init_memory_manager
} from "./chunk-4I5JHMN4.js";
import {
...
import {
  ...
  recordCustodyEvent
} from "./chunk-D7O53KWC.js";
import {
  classifyVulnClass,
  init_exploit_learning_engine
} from "./chunk-SDB7RKKY.js";
import {
...
var PROOF_PAYLOADS, ProofEngine;
...
async function clearOpsState(engagementId) {
  ...
  try {
    const { deleteOpsSnapshot } = await import("./db-JLHOBMS4.js");
...
async function getOpsStateWithRecovery(engagementId) {
  ...
  try {
    const { loadOpsSnapshot } = await import("./db-JLHOBMS4.js");
...
function persistOpsStateDebounced(engagementId, delayMs = 2e3) {
  ...
  persistTimers.set(engagementId, setTimeout(async () => {
    ...
    if (!state) return;
    try {
      const { saveOpsSnapshot } = await import("./db-JLHOBMS4.js");
...
async function flushAllPendingState() {
  ...
  try {
    const { releaseAllClaims } = await import("./engagement-claim-lock-QXXGWANE.js");
...
async function persistTimelineEvent(engagementId, logEntry) {
  try {
    ...
    const { engagementTimelineEvents } = await import("./schema-TZDA52RN.js");
...
async function persistScanResult(opts) {
  try {
    const { insertScanResult, saveEngagementFindings } = await import("./db-JLHOBMS4.js");
...
async function auditLog(params) {
  try {
    const { getDb } = await import("./db-JLHOBMS4.js");
    const { offensiveAuditLog } = await import("./schema-TZDA52RN.js");
    const db = await getDb();
    if (db) {
...
Use alternative tools instead.`;
    ...
    const { capLLMContext: capLLMContext2 } = await import("./memory-manager-VARXZ63M.js");
    ...
    enrichmentCtx = capLLMContext2(_scanPlanContextBlocks);
    try {
  ...
  try {
    ...
    const { planAttack } = await import("./attack-planner-CO7G7FCT.js");
...
async function llmDecide(context) {
  ...
  if (!skipSpecialist) try {
    const { decideNextOp } = await import("./ops-decider-7HAAEEME.js");
...
${Object.entries(state.dastConfig.customHeaders).map(([k, v]) => `  ${k}: ${v}`).join("\n")}`
  ...
  for (const domain of domains) {
    try {
      ...
      const { isLabDomain } = await import("./passive-EOM7JD2Z.js");
      ...
      const { runDomainIntelPipeline } = await import("./domainIntel-6VRXAO7C.js");
      ...
      try {
        ...
        if (domainAssets.length > 0) {
          const { analyzeScan } = await import("./scan-analyst-JNTAYZ64.js");
...
${(analysis.recommendations || []).slice(0, 3).map((r) => `\u2022 [${r.priority}] ${r.action}`).join("\n")}`,
      ...
      try {
        const domainAssets = state.assets.filter((a) => a.hostname === domain || a.hostname.endsWith("." + domain));
        if (domainAssets.length > 0) {
          const { scoreFullHybrid, buildEngagementContext } = await import("./hybrid-scorer-TM6RRIZN.js");
...
async function executeEnumeration(state, engagement, operatorCtx) {
  const { executeEnumeration: runEnumerationPhase } = await import("./engagement-phase-enumeration-2JFUKXM6.js");
...
async function executeVulnDetection(state, engagement, operatorCtx) {
  ...
  if (promotedCount > 0) {
    addLog(state, {
      phase: "vuln_detection",
      type: "info",
      title: `\u{1F4CB} Promoted ${promotedCount} passive recon findings to confirmed vulns`,
  ...
  try {
    ...
    if (taxonomyHypothesized > 0) {
      addLog(state, {
        phase: "vuln_detection",
        type: "info",
        title: `\u{1F9EC} Taxonomy enrichment: ${taxonomyHypothesized} hypothesized vulnerabilities`,
...
Total combined: ${combinedTechs.size} unique technologies`
  ...
  try {
    const { onBurpScanComplete } = await import("./burp-auto-scan-6DTJXQFW.js");
    onBurpScanComplete(async (burpConfig, burpState) => {
      ...
      if (normalizedFindings && normalizedFindings.length > 0) {
        ...
        for (const finding of normalizedFindings) {
          ...
          if (matchingAsset) {
            ...
            const cweStr = finding.cweId || void 0;
      ...
      if (normalizedFindings && normalizedFindings.length > 0) {
        try {
          const { saveEngagementFindings } = await import("./db-JLHOBMS4.js");
  ...
  try {
    const { getEngagementCredentials } = await import("./credential-harvester-VXB23PAJ.js");
  ...
  try {
    const { onEngagementVulnDetectionPhase, extractScopeUrls } = await import("./burp-auto-scan-6DTJXQFW.js");
  ...
  let initialPipelineResult;
  try {
    const { runZapToBurpPipeline } = await import("./zap-burp-pipeline-4SL4QSYJ.js");
  ...
  try {
    const { runSeverityEscalation } = await import("./zap-burp-pipeline-4SL4QSYJ.js");
  ...
  const vulnsBeforePhase3 = state.stats.vulnsFound;
  const existingVulnKeys = /* @__PURE__ */ new Map();
  for (const asset of state.assets) {
  ...
  if (nucleiAssets.length > 0) {
    ...
    const executeTool = (config2) => executeToolViaQueue(config2, { engagementId: state.engagementId, roeScope: roeScope_N, engagementAbortSignal: engagementAbortSig_N });
    async function executeNucleiWithRetry(nucleiArgs, target, maxRetries = 2) {
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        const result = await executeTool({
          ...
          target,
          timeoutSeconds: 300,
          // Reduced from 600s — nuclei CVE scans were hanging ScanForge
          engagementId: state.engagementId
        });
    ...
    for (const asset of nucleiAssets) {
      ...
      const NUCLEI_INFRA_PORTS = /* @__PURE__ */ new Set([1337, 31337, 8834, 9392, 5432, 3306, 27017, 6379]);
      ...
      if (techLower.some((t) => t.includes("joomla"))) techTags.push("joomla");
      if (techLower.some((t) => t.includes("drupal"))) techTags.push("drupal");
      if (techLower.some((t) => t.includes("nginx"))) techTags.push("nginx");
      if (techLower.some((t) => t.includes("apache"))) techTags.push("apache");
      if (techLower.some((t) => t.includes("iis"))) techTags.push("iis");
      if (techLower.some((t) => t.includes("php"))) techTags.push("php");
      if (techLower.some((t) => t.includes("laravel"))) techTags.push("laravel");
      if (techLower.some((t) => t.includes("spring"))) techTags.push("springboot");
      if (techLower.some((t) => t.includes("tomcat"))) techTags.push("tomcat");
      if (techLower.some((t) => t.includes("jenkins"))) techTags.push("jenkins");
      if (techLower.some((t) => t.includes("grafana"))) techTags.push("grafana");
      if (techLower.some((t) => t.includes("gitlab"))) techTags.push("gitlab");
      if (techLower.some((t) => t.includes("cloudfront") || t.includes("aws"))) techTags.push("aws");
      if (techLower.some((t) => t.includes("react") || t.includes("next.js") || t.includes("node"))) techTags.push("nodejs");
      try {
        const { getServiceBasedTags, getTemplateMappingSummary } = await import("./service-template-mapper-PFXGSFMH.js");
        ...
        if (fpResults && fpResults.length > 0) {
          ...
          if (serviceTags.length > 0) {
            ...
            addLog(state, {
              phase: "vuln_detection",
              type: "info",
              title: `\u{1F3AF} Service-Based Template Targeting: ${asset.hostname}`,
...
Services: ${serviceRationale.join(", ")}` + (summary.versionMatchedCves.length > 0 ? `
Version-matched CVEs: ${summary.versionMatchedCves.join(", ")}` : ""),
      ...
      for (const url of nucleiTargetUrls) {
        ...
        if (assetCreds.length > 0 && assetCreds[0].sessionCookie) {
          ...
          authHeaderArg = ` -H "Cookie: ${asset.trainingLabCreds.sessionCookie}"`;
        }
        let nucleiRateLimit2 = state.engagementType === "red_team" ? 50 : 150;
        let nucleiEvasionHeaders = "";
        if (state.targetProfiles) {
          const tp = state.targetProfiles[asset.hostname];
          if (tp) {
            const esc = tp.evasionEscalation;
            if (esc && esc.currentLevel > 1) {
              const ep = tp.recommendedStrategy?.evasionProfile;
              if (ep) {
                nucleiRateLimit2 = Math.min(nucleiRateLimit2, ep.rateLimit);
        ...
        const nucleiArgs = `-u ${url} ${severityArg} ${tagArgs} -jsonl -nc -duc -ni ${timeoutArg} -retries 1 -rate-limit ${nucleiRateLimit2}${authHeaderArg}${nucleiEvasionHeaders}`;
      ...
      try {
        ...
        for (const f of findings) {
          ...
          if (!assetVulnKeys.has(key)) {
            asset.vulns.push({ id: genId(), severity: f.severity, title: f.title, cve: f.cve, description: f.description, cvss: f.cvss, cwe: f.cwe, source: "nuclei", corroborationTier: "confirmed", evidenceDetail: `Confirmed by nuclei scan`, rawEvidence: f.evidence ? JSON.stringify(f.evidence).slice(0, 4e3) : void 0 });
    ...
    for (let i = 0; i < nucleiScanTasks.length; i += NUCLEI_BATCH_SIZE) {
      ...
      try {
        ...
        if (cleaned.freedEstimateBytes > 5e4) {
      ...
      const heapLimit = global.__heapLimitMB || 768;
    ...
    try {
      ...
      if (networkScanTasks.length > 0) {
        const allFps = nucleiAssets.flatMap((a) => (a.fingerprintResults || []).filter((fp) => !fp.error));
        const summary = getTemplateMappingSummary(allFps);
        addLog(state, {
          phase: "vuln_detection",
          type: "info",
          title: `\u{1F50C} Network-Level Nuclei: ${networkScanTasks.length} service-specific scans`,
          detail: `Targeting non-HTTP services: ${summary.serviceBreakdown.map((s) => `${s.protocol}:${s.port}`).slice(0, 8).join(", ")}${summary.serviceBreakdown.length > 8 ? ` (+${summary.serviceBreakdown.length - 8} more)` : ""}
...
Version-matched CVEs: ${summary.versionMatchedCves.join(", ")}` : ""}`
    ...
    if (state.trainingLabMode === true) {
      ...
      if (broadScanTasks.length > 0) {
        addLog(state, {
          phase: "vuln_detection",
          type: "info",
          title: `\u{1F3AF} Training Lab: Broad Nuclei Scan (no tag filter)`,
  ...
  let scanforgeResult = null;
  try {
    const scanforgeTargets = state.assets.filter((a) => a.status !== "pending").map((a) => {
      const creds = (a.confirmedCredentials || []).map((c) => ({
        username: c.username,
        password: c.password,
        service: c.service || "http",
        source: c.source || "hydra",
        loginPath: c.loginPath || a.trainingLabCreds?.loginPath,
        confirmedAt: c.confirmedAt ? new Date(c.confirmedAt).getTime() : Date.now()
      }));
      const trainingLabCreds = a.trainingLabCreds;
      ...
      return {
        ...
        // aceofcloud labs are publicly accessible
        technologies: a.passiveRecon?.technologies || [],
        credentials: creds.length > 0 ? creds : void 0
      };
    });
    ...
    if (targetsWithCreds.length > 0) {
      addLog(state, {
        phase: "vuln_detection",
        type: "info",
        title: `\u{1F511} ScanForge Credential Handoff: ${targetsWithCreds.length} target(s)`,
    ...
    if (scanforgeTargets.length > 0) {
      ...
      scanforgeResult = await executeScanForgePhase(
        ...
        (finding) => {
          ...
          if (asset) {
            ...
            const exists = asset.vulns.some(
              (v) => v.title.toLowerCase() === finding.title.toLowerCase() || v.cve && v.cve === finding.cve
            );
            if (!exists) {
              asset.vulns.push({
                id: vulnId,
      ...
      addLog(state, {
        ...
        detail: `ScanForge: ${scanforgeResult.stats.findingsTotal} findings (${scanforgeResult.stats.findingsVerified} verified) | Templates: ${scanforgeResult.stats.templatesExecuted} | Time: ${(scanforgeResult.stats.executionTimeMs / 1e3).toFixed(1)}s`
      });
      broadcastOpsUpdate(state.engagementId, { type: "stats_update", stats: { ...state.stats } });
      state._scanforgeResult = scanforgeResult;
      if (scanforgeResult.stats.findingsTotal > 0) {
        try {
          const { persistGenericEvidence } = await import("./evidence-persistence-GHQ4FQTF.js");
          const sfEvidence = {
            stats: scanforgeResult.stats,
            findings: scanforgeResult.findings.slice(0, 100).map((f) => ({
              title: f.title,
              severity: f.severity,
              target: f.target,
              port: f.port,
          ...
          addLog(state, {
            phase: "vuln_detection",
            type: "info",
            title: "\u{1F4BE} ScanForge Evidence Persisted",
  ...
  const TRAINING_LAB_ZAP_URL_MAP = {
    ...
    "altoro.lab.aceofcloud.io": {
      ...
      skipPortScan: true
      // Altoro Mutual runs under /altoromutual/ context path on Tomcat
    },
    "brokencrystals.lab.aceofcloud.io": {
      zapBaseUrl: "https://scan.aceofcloud.io/lab/broken-crystals",
      skipPortScan: true
      // Broken Crystals runs behind nginx reverse proxy
    }
  };
  ...
  for (const webApp of webApps) {
    ...
    if (labZapUrl) {
      ...
      if (scannedTargetUrls.has(dedupKey)) {
        ...
        addLog(state, {
          phase: "vuln_detection",
          type: "info",
          title: `\u{1F504} Training Lab URL Rewrite: ${webApp.hostname} \u2192 ${targetUrl}`,
          detail: `ZAP cannot resolve ${webApp.hostname} (Docker DNS limitation). Using reverse proxy URL: ${targetUrl}`
        });
        webApp.resolvedZapUrl = targetUrl;
      }
    }
    const INFRA_PORTS = /* @__PURE__ */ new Set([31337, 8834, 9392, 5432, 3306, 27017, 6379]);
    const COMMON_WEB_PORTS = /* @__PURE__ */ new Set([80, 443, 8080, 8443, 3e3, 3001, 5e3, 5001, 8e3, 8001, 8888, 9e3, 9090, 1337, 4200, 4443]);
    const webPorts = webApp.ports.filter(
    ...
    const httpxLivePorts = webApp.httpxLivePorts || [];
      ...
      try {
        ...
        try {
          ...
          if (wafResult?.detected) {
            ...
            try {
              const wafContent = `WAF detected on ${targetUrl}. Vendor: ${wafVendor}. Target: ${webApp.hostname}. Engagement: ${state.engagementId}. Defense mechanism active \u2014 scan parameters adjusted for evasion. Timestamp: ${(/* @__PURE__ */ new Date()).toISOString()}`;
          ...
          if (wafResult?.activeProbe) {
            ...
            webApp.activeProbe = ap;
          }
        } catch {
        }
        const { generateLLMScanConfig, startScan, configureZapAuthentication } = await import("./zap-scanner-NENSIVAM.js");
        const serviceVersions = webApp.ports.map((p) => p.version).filter(Boolean);
        const httpxTechs = webApp.passiveRecon?.technologies || [];
        const httpxHeaders = webApp.httpxResponseHeaders || {};
        const headerHints = [];
        ...
        const webCreds = (webApp.confirmedCredentials || []).filter(
        ...
        const TRAINING_LAB_DEFAULT_CREDS = {
          "dvwa": { username: "admin", password: "password", loginPath: "/login.php" },
          ...
          for (const [labKey, creds] of Object.entries(TRAINING_LAB_DEFAULT_CREDS)) {
            if (hostname.includes(labKey) || urlLower.includes(labKey)) {
              ...
              addLog(state, {
                phase: "vuln_detection",
                type: "info",
                title: `\u{1F511} Training Lab Default Creds: ${labKey}`,
        ...
        if (wafVendor) {
          try {
            const { applyWafEvasionConfig } = await import("./zap-scanner-NENSIVAM.js");
            ...
            try {
              ...
              if (zapOverrides) {
                ...
                addLog(state, {
                  phase: "vuln_detection",
                  type: "info",
                  title: `\u{1F6E1}\uFE0F ZAP evasion overrides applied: ${webApp.hostname}`,
        ...
        if (hasConfirmedCreds) {
          addLog(state, {
            phase: "vuln_detection",
            type: "info",
            title: `\u{1F511} Credential Handoff: ${webCreds.length} confirmed credential(s) \u2192 ZAP`,
          ...
          addLog(state, {
            phase: "vuln_detection",
            type: "info",
            title: `\u{1F511} Training Lab Credential Handoff: default creds \u2192 ZAP`,
        ...
        if (state.trainingLabMode) {
          ...
          const TRAINING_LAB_SEED_URLS = {
            ...
            "altoro": [
              ...
              "/bank/apply.jsp",
              "/altoromutual/",
              "/altoromutual/login.jsp",
              "/altoromutual/index.jsp",
              "/altoromutual/bank/main.jsp",
              "/altoromutual/bank/transaction.jsp",
              "/altoromutual/search.jsp",
              "/altoromutual/feedback.jsp",
              "/altoromutual/bank/customize.jsp",
              "/altoromutual/bank/queryxpath.jsp"
            ],
            "testphp": [
              "/",
              "/login.php",
            ...
            "brokencrystals": [
              ...
              "/api/auth/login",
              "/api/testimonials",
              "/api/testimonials/count?query=test",
              "/api/metadata",
              "/api/file?path=test",
              "/api/render",
              "/api/users",
              ...
              "/api/spawn",
              "/api/goto?url=https://example.com",
              "/api/subscriptions",
              "/api/userinfo"
            ]
          };
        ...
        for (let attempt = 0; attempt < ZAP_RETRY_DELAYS.length; attempt++) {
          if (attempt > 0) {
            const delaySec = ZAP_RETRY_DELAYS[attempt] / 1e3;
        ...
        if (!zapStarted) {
          ...
          try {
            const { onEngagementVulnDetectionPhase } = await import("./burp-auto-scan-6DTJXQFW.js");
            const burpFallbackResults = await onEngagementVulnDetectionPhase(
              ...
              hasConfirmedCreds ? { username: webCreds[0].username, password: webCreds[0].password } : void 0,
              techHints
            );
        ...
        if (hasConfirmedCreds && zapScanResult?.scanId) {
          try {
            const authResult = await configureZapAuthentication(
              ...
              webCreds,
              { techHints }
              // Pass tech hints for tech-specific login path discovery
            );
            if (authResult.configured) {
              let authValidated = false;
              try {
                const { executeTool } = await import("./scan-server-executor-RYJD5OAQ.js");
          ...
          try {
            ...
            const authResult = await configureZapAuthentication(
              ...
              targetUrl,
              syntheticCreds,
              { techHints, loginPath: trainingLabCreds.loginPath }
            );
            if (authResult.configured) {
              addLog(state, {
                phase: "vuln_detection",
              ...
              try {
                const { preAuthenticateTarget } = await import("./zap-scanner-NENSIVAM.js");
        ...
        if (zapScanId) {
          const { pollScanProgress } = await import("./zap-scanner-NENSIVAM.js");
          let zapDone = false;
          ...
          const zapTimeout = Date.now() + zapTimeoutMinutes * 60 * 1e3;
            ...
            try {
              ...
              if (progress.status === "completed" || progress.status === "error") {
                ...
                try {
                  ...
                  const db = await getDb();
                  if (db) {
                    const { webAppFindings } = await import("./schema-TZDA52RN.js");
                    ...
                    for (const f of detailedFindings) {
                      ...
                      webApp.zapFindings.push({ alert: alertTitle, risk: sev, url: f.url || targetUrl, cweId: f.cweId || void 0 });
                      const cweStr = f.cweId ? `CWE-${f.cweId}` : void 0;
                      const zapRawEvidence = [f.method && f.url ? `${f.method} ${f.url}` : "", f.param ? `Param: ${f.param}` : "", f.attack ? `Attack: ${f.attack.substring(0, 2e3)}` : "", f.evidence ? `Evidence: ${f.evidence.substring(0, 2e3)}` : ""].filter(Boolean).join("\n");
                      webApp.vulns.push({
                        id: genId(),
                        severity: sev,
                        ...
                        cwe: cweStr,
                        evidence: f.evidence || void 0,
                        evidenceDetail: [f.method && f.url ? `${f.method} ${f.url}` : "", f.param ? `Param: ${f.param}` : "", f.attack ? `Attack: ${f.attack.substring(0, 500)}` : "", f.evidence ? `Evidence: ${f.evidence.substring(0, 500)}` : ""].filter(Boolean).join(" | ") || void 0,
                        corroborationTier: "confirmed",
                        rawEvidence: zapRawEvidence || void 0,
                        attack: f.attack || void 0,
                        method: f.method || void 0,
                        param: f.param || void 0,
                        url: f.url || void 0,
                        source: "zap",
                        solution: f.solution || void 0
                      });
                      zapVulnCount++;
                    }
                  ...
                  if (totalAlerts > 0) {
                    if (counts.high > 0) {
                      ...
                      webApp.vulns.push({ id: genId(), severity: "high", title: `[ZAP] ${counts.high} high-risk findings`, source: "zap", corroborationTier: "confirmed", evidenceDetail: `ZAP active scan confirmed ${counts.high} high-risk findings` });
                      state.stats.vulnsFound += counts.high;
                    }
                    if (counts.medium > 0) {
                      webApp.zapFindings.push({ alert: "Medium-risk web vulnerability", risk: "medium", url: targetUrl });
                      webApp.vulns.push({ id: genId(), severity: "medium", title: `[ZAP] ${counts.medium} medium-risk findings`, source: "zap", corroborationTier: "confirmed", evidenceDetail: `ZAP active scan confirmed ${counts.medium} medium-risk findings` });
                ...
                if (!webApp._lastZapProgressKey) webApp._lastZapProgressKey = "";
                if (!webApp._zapStallCount) webApp._zapStallCount = 0;
                if (progressKey === webApp._lastZapProgressKey) {
                  webApp._zapStallCount++;
                } else {
                  webApp._zapStallCount = 0;
                  webApp._lastZapProgressKey = progressKey;
                }
                const hasWafEvasion = state.assets?.some((a) => a.wafDetected && a.wafVendor);
                const MAX_STALL_POLLS = hasWafEvasion ? state.trainingLabMode ? 40 : 24 : state.trainingLabMode ? 12 : 8;
                if (webApp._zapStallCount >= MAX_STALL_POLLS) {
                  addLog(state, { phase: "vuln_detection", type: "warning", title: `ZAP Stalled: ${targetUrl}`, detail: `No progress after ${webApp._zapStallCount} polls (Spider: ${progress.spiderProgress}%, Active: ${progress.activeScanProgress}%, URLs: ${progress.urlsFound}). Aborting scan.` });
                  zapDone = true;
                  try {
                    const { getDb: getDb2 } = await import("./db-JLHOBMS4.js");
                    const db2 = await getDb2();
                    if (db2) {
                      const { webAppScans: webAppScans2 } = await import("./schema-TZDA52RN.js");
                      const { eq: eq2 } = await import("drizzle-orm");
                      await db2.update(webAppScans2).set({
                        status: "error",
                        errorMessage: `ZAP scan stalled \u2014 no progress after ${webApp._zapStallCount * 15}s`,
          ...
          if (!zapDone) {
            ...
            try {
              ...
              const db = await getDb();
              if (db) {
                const { webAppScans } = await import("./schema-TZDA52RN.js");
                const { eq } = await import("drizzle-orm");
  ...
  try {
    const initialResult = state._initialZapBurpPipelineResult;
    if (initialResult && (initialResult.urlSource === "scope_fallback" || initialResult.zapUrlsDiscovered === 0)) {
      ...
      if (db) {
        ...
        const completedScans = await db.select().from(webAppScans).where(and(
          eq(webAppScans.engagementId, state.engagementId),
          eq(webAppScans.status, "completed")
        )).orderBy(desc(webAppScans.completedAt)).limit(1);
        if (completedScans.length > 0) {
          const { deferredZapBurpRefeed } = await import("./zap-burp-pipeline-4SL4QSYJ.js");
          ...
          if (refeedResult) {
            addLog(state, {
              phase: "vuln_detection",
              type: "info",
              title: `\u{1F504} ZAP \u2192 Burp Deferred Re-Feed: ${refeedResult.urlsFedToBurp} URLs`,
              detail: `ZAP scan #${completedScans[0].id} completed with ${refeedResult.zapUrlsDiscovered} discovered URLs. Re-fed ${refeedResult.urlsFedToBurp} to Burp (initial run used ${initialResult.urlsFedToBurp} scope URLs). Tech: ${refeedResult.fingerprint.technologies.slice(0, 3).join(", ") || "none"}.`
            });
          }
        }
      }
  ...
  addLog(state, { phase: "vuln_detection", type: "info", title: "\u{1F489} Supplementary Injection Scanning", detail: "Running SQLMap (SQL injection) and XSStrike (XSS) on discovered web app parameters" });
  for (const webApp of webApps) {
    ...
    try {
      ...
      if (db) {
        ...
        const latestScan = await db.select().from(webAppScans).where(eq(webAppScans.engagementId, state.engagementId)).orderBy(desc(webAppScans.id)).limit(1);
    ...
    if (state.trainingLabMode) {
      const hostname = webApp.hostname.toLowerCase();
      const TRAINING_LAB_INJECTABLE_ENDPOINTS = {
        "juiceshop": [
      ...
      try {
        if (hostname.includes("juiceshop") || hostname.includes("juice-shop")) {
          ...
          addLog(state, {
            phase: "vuln_detection",
            type: "info",
            title: `\u{1F511} Auth Handoff: Acquiring Juice Shop JWT for SQLMap/XSStrike`,
    ...
    try {
      ...
      if (approved) {
        const { batchSqlmapScan, analyzeSqlmapFindings, runBlindSqliPass, ingestSqlmapToWebAppFindings } = await import("./sqlmap-scanner-SJFXH3A3.js");
        ...
        addLog(state, { phase: "vuln_detection", type: "info", title: `\u{1F50D} SQLMap: ${webApp.hostname}`, detail: `Testing ${injectableUrls.length} URLs for SQL injection${isTrainingLabSqlmap ? " (training lab: risk=3, level=5, deep enum)" : ""}
...
Cookie: ${cookieStr ? cookieStr.substring(0, 40) + "..." : "(none)"}` });
        ...
        const sqlmapResults = await batchSqlmapScan(injectableUrls, {
          ...
          techniques: isTrainingLabSqlmap ? "BEUSTQ" : void 0
        });
        ...
        webApp.toolResults.push({
          ...
          exitCode: 0,
          durationMs: sqlmapResults.reduce((sum, r) => sum + r.stats.durationSeconds * 1e3, 0),
          timedOut: false,
        ...
        if (isTrainingLabSqlmap) {
          try {
            let zapScanIdForHandoff;
            try {
              const { getDb: getDbForHandoff } = await import("./db-JLHOBMS4.js");
              const dbForHandoff = await getDbForHandoff();
              if (dbForHandoff) {
                const { webAppScans: wasTable } = await import("./schema-TZDA52RN.js");
                const { desc: descOrder, like: likeOp } = await import("drizzle-orm");
                const recentScans = await dbForHandoff.select({ id: wasTable.id }).from(wasTable).where(likeOp(wasTable.scanName, `%EngOps-${state.engagementId}%`)).orderBy(descOrder(wasTable.id)).limit(1);
            ...
            addLog(state, {
              phase: "vuln_detection",
              type: "info",
              title: `\u{1F52C} Blind SQLi Pass: ${webApp.hostname}`,
            ...
            webApp.toolResults.push({
              ...
              exitCode: 0,
              durationMs: blindResult.results.reduce((sum, r) => sum + r.stats.durationSeconds * 1e3, 0),
              timedOut: false,
...
Stack: ${sqlmapErr.stack?.substring(0, 200) || "N/A"}` });
    ...
    try {
      ...
      if (approved) {
        const { batchXssScan, analyzeXssFindings, ingestXssToWebAppFindings } = await import("./xsstrike-scanner-QAXKP6DT.js");
        addLog(state, { phase: "vuln_detection", type: "info", title: `\u{1F50D} XSS Scan: ${webApp.hostname}`, detail: `Testing ${injectableUrls.length} URLs for XSS vulnerabilities` });
        ...
        webApp.toolResults.push({
          ...
          exitCode: 0,
          durationMs: xssResults.reduce((sum, r) => sum + r.stats.durationSeconds * 1e3, 0),
          timedOut: false,
          findingCount: allFindings.length,
          findings: allFindings.map((f) => ({ severity: f.severity, title: f.title })),
          outputPreview: JSON.stringify(allFindings.slice(0, 5), null, 2).slice(0, 1024),
...
Stack: ${xssErr.stack?.substring(0, 200) || "N/A"}` });
  ...
  for (const webApp of webApps) {
    ...
    try {
      ...
      if (approved) {
        const { batchCommixScan, ingestCommixToWebAppFindings } = await import("./commix-scanner-BNB4QKD6.js");
        const isTrainingLab = state.trainingLabMode === true;
        addLog(state, { phase: "vuln_detection", type: "info", title: `\u{1F489} Commix: ${webApp.hostname}`, detail: `Testing ${injectableUrlsForCommix.length} URLs for OS command injection${isTrainingLab ? " (training lab: level=3)" : ""}` });
        const commixResults = await batchCommixScan(injectableUrlsForCommix, {
          engagementId: state.engagementId,
          cookie: webApp.sessionCookie || void 0,
        ...
        webApp.toolResults.push({
          ...
          exitCode: 0,
          durationMs: commixResults.reduce((sum, r) => sum + r.stats.durationSeconds * 1e3, 0),
          timedOut: false,
          findingCount: allFindings.length,
          findings: allFindings.map((f) => ({ severity: f.severity, title: f.title })),
  ...
  for (const webApp of webApps) {
    ...
    try {
      ...
      if (approved) {
        const { batchTplmapScan, ingestTplmapToWebAppFindings } = await import("./tplmap-scanner-3GVZE77A.js");
        addLog(state, { phase: "vuln_detection", type: "info", title: `\u{1F9EA} tplmap SSTI: ${webApp.hostname}`, detail: `Testing ${injectableUrlsForTplmap.length} URLs for server-side template injection across 15+ engines` });
        ...
        webApp.toolResults.push({
          ...
          exitCode: 0,
          durationMs: tplmapResults.reduce((sum, r) => sum + r.stats.durationSeconds * 1e3, 0),
          timedOut: false,
          findingCount: allFindings.length,
          findings: allFindings.map((f) => ({ severity: f.severity, title: f.title })),
          outputPreview: JSON.stringify(allFindings.slice(0, 3), null, 2).slice(0, 1024),
  ...
  try {
    const { suggestToolCommands: suggestCred } = await import("./scan-server-executor-RYJD5OAQ.js");
    const roeScope_C = [...state.roeScopeGuard?.authorizedDomains || [], ...state.roeScopeGuard?.authorizedIps || []];
    const execToolCred = (config2) => executeToolViaQueue(config2, { engagementId: state.engagementId, roeScope: roeScope_C });
    const hydraAlreadyDone = state.completedScans.hydraCompleted.size;
    if (hydraAlreadyDone > 0) {
      addLog(state, { phase: "vuln_detection", type: "info", title: "\u{1F504} Resume: Hydra Checkpoint", detail: `Skipping ${hydraAlreadyDone} already-completed credential test(s) from previous run` });
    }
    for (const asset of state.assets) {
      ...
      for (const p of asset.ports) {
        if (p.version) {
          techList.push({ name: `${p.service} ${p.version}`, vendor: p.version.split(/[\s\/]+/)[0], version: p.version, port: p.port, protocol: p.service });
        }
      }
      const credCmds = (await suggestCred({
        ...
        type: asset.type,
        ports: asset.ports,
        technologies: techList.length > 0 ? techList : void 0
      ...
      for (const cmd of credCmds) {
        ...
        try {
          ...
          for (const f of findings) {
            if (pushVulnDeduped(asset, { id: genId(), severity: f.severity, title: f.title, cve: f.cve, description: f.description, corroborationTier: "confirmed", evidenceDetail: `Confirmed by ${cmd.tool} credential test`, rawEvidence: f.evidence ? JSON.stringify(f.evidence).slice(0, 4e3) : void 0, source: cmd.tool })) {
              state.stats.vulnsFound++;
            }
          }
          asset.toolResults.push({
          ...
          if (cmd.tool === "hydra" && result.exitCode === 255) {
            ...
            if (cmd.purpose.includes("[OEM Default]")) {
              ...
              if (oemUserMatch && oemPassMatch) {
                ...
                if (!Array.isArray(asset.confirmedCredentials)) asset.confirmedCredentials = [];
                const exists = asset.confirmedCredentials.some(
                  (c) => c.username === oemUser && c.password === oemPass
                );
                if (!exists) {
                  asset.confirmedCredentials.push({
                    username: oemUser,
                  ...
                  addLog(state, {
                    phase: "vuln_detection",
                    type: "info",
                    title: `\u{1F511} OEM Cred Fallback: ${oemUser}:*** stored for auth scanning`,
...
${kevBoost.ransomwareExposure ? "\n\u{1F534} RANSOMWARE EXPOSURE: Some KEV entries are linked to active ransomware campaigns. Prioritize these for immediate exploitation testing." : ""}
You MUST prioritize KEV-listed vulnerabilities in your exploitation strategy. These represent confirmed real-world attack vectors with the highest likelihood of success.`;
      ...
      try {
        const { verifyCvesWithShodanData, createShodanPostureFindings, enrichAssetsWithShodanData, extractShodanVersionEvidence } = await import("./shodan-verifier-KUN4FU3T.js");
        ...
        if (shodanObservations.length > 0) {
          ...
          if (shodanConfirmedKev > 0 || shodanUnconfirmedKev > 0) {
            const confirmRate = shodanConfirmedKev + shodanUnconfirmedKev > 0 ? Math.round(shodanConfirmedKev / (shodanConfirmedKev + shodanUnconfirmedKev) * 100) : 0;
            kevContext += `

\u{1F50D} SHODAN KEV CROSS-VALIDATION:
...
Confirmation rate: ${confirmRate}%. Prioritize CONFIRMED matches for exploitation.`;
            addLog(state, {
              phase: "vuln_detection",
              type: "finding",
              title: `\u{1F50D} Shodan KEV Cross-Validation: ${shodanConfirmedKev} confirmed, ${shodanUnconfirmedKev} unconfirmed`,
          ...
          addLog(state, {
            phase: "vuln_detection",
            type: "info",
            title: `\u{1F6F0}\uFE0F Shodan Enrichment: ${versionEvidence.length} service banners analyzed`,
      ...
      let epssContext = "";
      try {
        const { fetchEpssScores, batchPrioritizeCves, buildEpssContextForLlm } = await import("./epss-service-7HFFOUBB.js");
        const kevCveSet = new Set(kevMatches?.map((m) => m.cveID) || []);
        ...
        if (prioritized.length > 0) {
          ...
          addLog(state, {
            phase: "vuln_detection",
            type: "finding",
            title: `\u{1F4CA} EPSS Scoring: ${criticalCount} critical, ${highCount} high priority CVEs`,
...
${(() => {
        ...
        const triageCtx = getTriageCorpusContext(void 0, 3);
        ...
        const zapVulnCtx = buildZAPKnowledgeContext({
          ...
          authType: state.assets.some((a) => (a.confirmedCredentials || []).length > 0) ? "form" : void 0,
        ...
        const _cappedVulnContext = capLLMContext(_vulnContextBlocks);
        try {
          const { buildContributionFromBlocks } = (init_context_engine_tracker(), __toCommonJS(context_engine_tracker_exports));
          buildContributionFromBlocks(
            state.engagementId,
            state.assets.map((a) => a.hostname).join(", "),
            state.assets.flatMap((a) => a.vulns.filter((v) => v.cve).map((v) => v.cve)).join(", "),
            _vulnContextBlocks,
            _cappedVulnContext,
            "exploit_deferred"
          );
        } catch (e) {
          console.warn("[ContextTracker] Failed to record vuln contribution:", e);
        }
  ...
  state.progress = 55;
  const highCritVulns = state.assets.flatMap(
    (a) => a.vulns.filter((v) => v.severity === "critical" || v.severity === "high").map((v) => ({ ...v, hostname: a.hostname, assetType: a.type }))
  );
  if (highCritVulns.length > 0) {
    addLog(state, {
      phase: "vuln_detection",
      type: "info",
      title: "\u{1F9D0} Vulnerability Verification (AI Specialist)",
      detail: `Verifying ${highCritVulns.length} high/critical findings with Vulnerability Verifier...`
    });
    broadcastOpsUpdate(state.engagementId, { type: "log_update" });
    try {
      const { verifyVulnerability } = await import("./vuln-verifier-PPSOYKOR.js");
      for (const v of highCritVulns.slice(0, 10)) {
        try {
          ...
          const integrityTag = vulnVerifGate.passed ? "\u{1F512}" : "\u26A0\uFE0F";
          addLog(state, {
            phase: "vuln_detection",
            type: "llm_decision",
...
Integrity: ${vulnVerifGate.passed ? "PASSED" : "FAILED"} (hash=${vulnVerifGate.contentHash.slice(0, 12)}...)`,
  ...
  if (highCritVulns.length > 0) {
    try {
      const { batchRunScanForgeReasoning } = await import("./scanforge-reasoning-66IXT7R6.js");
      ...
      addLog(state, {
        phase: "vuln_detection",
        type: "info",
        title: "\u{1F52C} ScanForge Reasoning Pipeline",
  ...
  if (activeFindings.length > 0) {
    try {
      ...
      addLog(state, {
        phase: "vuln_detection",
        type: "info",
        title: "\u{1F3AF} Hybrid Risk Scoring (Active Findings)",
      ...
      for (const asset of assetsWithFindings) {
        ...
        try {
          ...
          addLog(state, {
            phase: "vuln_detection",
            type: "llm_decision",
            title: `\u{1F3AF} Active Scan Risk: ${asset.hostname} \u2014 ${hybridResult.finalScore}/10`,
...
${adjustmentSummary.length > 0 ? "LLM Adjustments:\n" + adjustmentSummary.map((a) => "\u2022 " + a).join("\n") : "No LLM adjustments applied"}`,
  ...
  if (state.assets.some((a) => a.vulns.length > 0 || a.zapFindings.length > 0)) {
    try {
      const { mapThreats } = await import("./threat-mapper-SO24OM52.js");
      addLog(state, {
        phase: "vuln_detection",
        type: "info",
        title: "\u{1F310} Threat Actor Mapping (AI Specialist)",
      ...
      if (threatResult.threat_actors.length > 0) {
        addLog(state, {
          phase: "vuln_detection",
          type: "llm_decision",
          title: `\u{1F3AF} ${threatResult.threat_actors.length} Threat Actor(s) Mapped`,
          detail: threatResult.threat_actors.map(
            (ta) => `${ta.actor_name} (${ta.confidence}) \u2014 ${ta.relevance_rationale}
...
Sector risk: ${threatResult.sector_risk_assessment}`,
        ...
        try {
          const ttps = threatResult.threat_actors.flatMap(
            (ta) => (ta.associated_ttps || []).map((ttpStr) => {
              const match = ttpStr.match(/^(T\d+(?:\.\d+)?)\s*[:\-]?\s*(.*)/);
              return {
                techniqueId: match ? match[1] : void 0,
                techniqueName: match ? match[2].trim() : ttpStr,
                tactic: ta.primary_tactic || void 0
              };
            })
          );
          const cves = state.assets.flatMap(
            (a) => a.vulns.filter((v) => v.cve).map((v) => v.cve)
          );
          ...
          if (ttps.length > 0 || uniqueCves.length > 0) {
            ...
            if (attrResult) {
              addLog(state, {
                phase: "vuln_detection",
                type: "info",
                title: "\u{1F4CA} Threat Attribution Scored (Learning Engine)",
                detail: `Scored ${ttps.length} TTPs and ${uniqueCves.length} CVEs against threat catalog. Top match: ${attrResult.summary?.topGroup || "N/A"} (${attrResult.summary?.confidence || "N/A"}% confidence)`,
  ...
  state.stats.portsFound = state.assets.reduce((sum, a) => sum + a.ports.length, 0);
  try {
    const { runEngagementDedup, runEngagementCoverageAnalysis } = await import("./dedup-coverage-bridge-O5TM4O6X.js");
    addLog(state, {
      phase: "vuln_detection",
      type: "info",
      title: "\u{1F504} Running finding deduplication & normalization",
    ...
    addLog(state, {
      phase: "vuln_detection",
      type: "info",
      title: "\u{1F4CA} Running coverage gap analysis",
    ...
    const coverageEmoji = coverageReport.overallScore >= 80 ? "\u{1F7E2}" : coverageReport.overallScore >= 60 ? "\u{1F7E1}" : "\u{1F534}";
    addLog(state, {
      phase: "vuln_detection",
      type: "info",
...
async function executeExploitation(state, engagement, operatorCtx) {
  const { executeExploitation: runExploitPhase } = await import("./engagement-phase-exploitation-LRTMQKU4.js");
  return runExploitPhase(state, engagement, operatorCtx);
}
async function executePostExploit(state, engagement, operatorCtx) {
  const { executePostExploit: runPostExploitPhase } = await import("./engagement-phase-post-exploit-GG3XCVHD.js");
...
async function executeEngagement(engagementId, operatorCtx, options) {
  ...
  try {
    const { claimEngagement } = await import("./engagement-claim-lock-QXXGWANE.js");
...
Preserved: ${state.assets.length} assets, ${state.stats.vulnsFound} vulns, ${state.stats.portsFound} ports, ${state.log.length} log entries`
  ...
  let engagement;
  try {
    const db = await import("./db-JLHOBMS4.js");
...
Max blast radius: ${engagementSafetyLevel === "passive_only" ? 5 : engagementSafetyLevel === "low_impact" ? 30 : engagementSafetyLevel === "standard" ? 60 : 100}`
  ...
  try {
    const { buildEngagementContext } = await import("./hybrid-scorer-TM6RRIZN.js");
  ...
  try {
    const { setActiveUser } = await import("./bug-bounty-intelligence-PRFR3JAZ.js");
  ...
  try {
    if (startPhase === "recon") {
      ...
      if (!reconGate.allowed) {
        ...
        await breathe();
        try {
          const { executeCustomerIntegrationsForStage, mergeIntegrationResultsIntoObservations } = await import("./pipeline-bridge-T6EAZVQF.js");
    ...
    if (["recon", "passive_discovery"].includes(startPhase)) {
      try {
        ...
        await breathe();
        try {
          const { executeCustomerIntegrationsForStage } = await import("./pipeline-bridge-T6EAZVQF.js");
        ...
        if (!vulnGate.allowed) {
          ...
          if (state.stats.vulnsFound > 0) {
            try {
              ...
              const { batchAnalyzeFindings, generateAnalysisSummary, classifyVulnerability } = await import("./vuln-analysis-agents-VVPI3ST7.js");
...
PoC: ${cf.analysis.poc || "N/A"}`,
        ...
        if (state.stats.vulnsFound > 0 || state.assets.some((a) => a.cloudProviders?.length > 0)) {
          try {
            ...
            const { runFeedbackLoop, getFeedbackLoopSummary } = await import("./llm-scan-feedback-F2ACAEKK.js");
...
${feedbackState.finalAnalysis?.slice(0, 500) || ""}`,
        ...
        if (state.stats.vulnsFound > 0) {
          try {
            ...
            const { generateEngagementAttackChains } = await import("./cloud-attack-chain-designer-PQNMVAKF.js");
...
Promoted: ${promotionSummary.promotedVulns.map((p) => `${p.vulnTitle.slice(0, 50)} [${p.scanner}/${p.category}]`).join("; ")}`,
        ...
        {
          const { executeSocialEngineering } = await import("./engagement-phase-social-engineering-ZOUTU3XW.js");
      ...
      try {
        const { retryDeferredScans, getDeferredScans, clearDeferredScans } = await import("./job-queue-bridge-JUQPD4VN.js");
      ...
      try {
        const { selectFindingsForScreenshot, captureScreenshotBatch } = await import("./screenshot-capture-2W42CUYX.js");
      ...
      try {
        const { generateAttackNarratives, generateExecutiveSummary } = await import("./attack-narrative-generator-WDE3NS3R.js");
      ...
      try {
        const { runAccuracyComparison } = await import("./accuracy-feedback-loop-BD36X3XT.js");
        ...
        if (targetPreset) {
          ...
          let engNotes = null;
          try {
            const { engagements: engTable } = await import("./schema-TZDA52RN.js");
            const { getDbRequired: getDbReq } = await import("./db-JLHOBMS4.js");
...
View details on the Knowledge Base \u2192 Accuracy Feedback tab.`
          ...
          try {
            const { runLocalAccuracyComparison } = await import("./accuracy-feedback-loop-BD36X3XT.js");
    ...
    try {
      const { generateAutoReport } = await import("./engagement-auto-report-DIW7FHPS.js");
    ...
    try {
      ...
      broadcastOpsUpdate(state.engagementId, { type: "log_update" });
      const { generateTestPlanAdherence } = await import("./engagement-report-handoff-EYIDEPQT.js");
    ...
    try {
      const { updateEngagement } = await import("./db-JLHOBMS4.js");
    ...
    try {
      const { saveEngagementResult, saveEngagementFindings } = await import("./db-JLHOBMS4.js");
    ...
    try {
      const { getEngagementLlmTelemetryRaw } = await import("./db-JLHOBMS4.js");
    ...
    try {
      const { runPostPipelineGraduation, extractEngagementMetrics } = await import("./post-pipeline-graduation-DAHF27SZ.js");
    ...
    try {
      const { detectGaps, createGapsBatch } = await import("./intelligence-gaps-DTKSHN5I.js");
    ...
    try {
      const { updateProfileFromEngagement } = await import("./customer-intel-profile-QX35GWII.js");
    ...
    try {
      const { updateEngagement } = await import("./db-JLHOBMS4.js");
      const existingEng = await (await import("./db-JLHOBMS4.js")).getEngagementById(engagementId);
...
async function recoverInterruptedEngagements() {
  ...
  try {
    const { getDbRequired } = await import("./db-JLHOBMS4.js");
    const db = await getDbRequired();
    const { engagementOpsSnapshots } = await import("./schema-TZDA52RN.js");
    const { eq } = await import("drizzle-orm");
...
var fetchKevCatalog, breathe, matchCvesAgainstKev, calculateKevRiskBoost, _serverInstanceId, KNOWN_INFRA_IPS, opsStates, MAX_CONCURRENT_ENGAGEMENTS, approvalResolvers, idCounter, persistTimers, periodicPersistTimers, engagementAbortControllers, memoryWatchdogInterval, OPS_TO_TIMELINE_TYPE, OPS_TO_SEVERITY, PROFILE_ESCALATION_ORDER;
var init_engagement_orchestrator = __esm({
  "server/lib/engagement-orchestrator.ts"() {
    ...
    fetchKevCatalog = lazyFetchKevCatalog;
    breathe = () => new Promise((resolve) => setImmediate(resolve));
    matchCvesAgainstKev = lazyMatchCvesAgainstKev;
    calculateKevRiskBoost = lazyCalculateKevRiskBoost;
