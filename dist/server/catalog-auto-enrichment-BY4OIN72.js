...
async function enrichFromDFIR(config, maxItems) {
  ...
  try {
    const { ingestDFIRReport } = await import("./dfir-report-ingestion-XSBVRJAM.js");
...
async function enrichFromIOCs(config, maxItems) {
  ...
  const drizzleDb = await getDb();
  try {
    const { reverseEngineerIOC, batchReverseEngineerIOCs } = await import("./ioc-ttp-reverse-engineer-JGYUGNCH.js");
...
async function enrichFromHackingArticles(config, maxItems) {
  ...
  let processed = 0, added = 0, failed = 0;
  try {
    const { processArticleBatch } = await import("./hacking-articles-ingestion-ND3QSM2Y.js");
    ...
    if (actorsNeedingEnrichment.length === 0) {
      ...
      if (leastEnrichedActors.length > 0) {
        for (const actor of leastEnrichedActors) {
          try {
            processed++;
            const { enrichThreatActorWithLLM } = await import("./threat-actor-crawler-ONJMO2CI.js");
  ...
  try {
    ...
    if (engagement.length > 0) {
      ...
      if (discoveredIOCs.length > 0) {
        try {
          const { batchReverseEngineerIOCs } = await import("./ioc-ttp-reverse-engineer-JGYUGNCH.js");
      ...
      try {
        const { enrichCalderaFromCatalog } = await import("./catalog-caldera-enrichment-GPUIWHSW.js");
