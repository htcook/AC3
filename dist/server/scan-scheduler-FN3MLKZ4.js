...
async function triggerScheduledScan(monitor) {
  ...
  try {
    ...
    const { detectSubdomainTakeover, crossReferenceTechVulnerabilities, detectSubdomainChanges } = await import("./domain-intel-advanced-PGIG3QAD.js");
...
function initScanScheduler() {
  ...
  setTimeout(async () => {
    try {
      const { startPeriodicHealthChecks } = await import("./health-monitor-NR4X26D2.js");
...
function stopScanScheduler() {
  ...
  import("./health-monitor-NR4X26D2.js").then(({ stopPeriodicHealthChecks }) => {
