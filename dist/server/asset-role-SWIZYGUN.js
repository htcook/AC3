...
// server/lib/llm-specialists/asset-role/specialist.ts
init_llm_json_parser();
...
var CUSTOMER_FACING_INDICATORS = [
  "www.",
  "app.",
  "api.",
  "portal.",
...
function computeRoleBaseline(pkg) {
  ...
  if (DEV_INDICATORS.some((ind) => identifier.includes(ind))) {
    environment = identifier.includes("staging") || identifier.includes("stage") || identifier.includes("preprod") ? "staging" : identifier.includes("test") || identifier.includes("qa") || identifier.includes("uat") ? "testing" : "development";
    envScore = 70;
    evidence.push({
      source: "asset_identifier",
      evidenceType: "naming_convention",
  ...
  } else if (exposure === "customer_facing" || pkg.certificate && !pkg.certificate.isSelfSigned && !pkg.certificate.isExpired) {
...
var ROLE_SPECIALIST_SYSTEM_PROMPT = `You are the Asset Role Specialist for the AC3 platform. Your role is to analyze structured discovery evidence and determine the operational role of a digital asset.
...
async function invokeRoleSpecialist(input, llmInvoke) {
