...
async function runVulnVerification(highCritVulns, state, addLog, broadcastOpsUpdate) {
  ...
  try {
    const { verifyVulnerability } = await import("./vuln-verifier-K5YE5F3J.js");
    const { evidenceGate, buildProvenance, createIntegrityEnvelope } = await import("./evidence-integrity-66YYPLS3.js");
...
async function runScanForgeReasoning(highCritVulns, state, addLog, broadcastOpsUpdate) {
  ...
  let processed = 0;
  try {
    const { batchRunScanForgeReasoning } = await import("./scanforge-reasoning-XDJYHC4G.js");
...
async function runThreatActorMapping(state, addLog, broadcastOpsUpdate, scoreEngagementThreatAttribution) {
  ...
  let mapped = 0;
  try {
    const { mapThreats } = await import("./threat-mapper-5XBUZNNF.js");
