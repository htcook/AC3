...
// server/lib/nextcloud-attack-playbook.ts
init_nextcloud_test_lab();
var PHASE_1_AUTH = {
  id: "phase-1-auth",
...
function generateAttackPlaybook(targetHost = "localhost:8443") {
...
function generatePlaybookForEngagement(engagement, labConfig = DEFAULT_LAB_CONFIG) {
  ...
  playbook.phases = playbook.phases.filter((phase) => {
    ...
    if (phase.id === "phase-3-e2e") return true;
...
function getAutomatedTestCases() {
  return ALL_PHASES.flatMap((p) => p.testCases.filter((tc) => tc.automated));
}
function getTestCasesForBurpProfile(profileName) {
  return ALL_PHASES.flatMap(
    (p) => p.testCases.filter((tc) => tc.burpProfile === profileName)
  );
}
function getAllAttackTechniques() {
...
export {
  ...
  generatePlaybookForEngagement,
  getAllAttackTechniques,
  getAllTargetCwes,
  getAutomatedTestCases,
  getPhase,
  getPlaybookStats,
  getTestCasesForBurpProfile
};
