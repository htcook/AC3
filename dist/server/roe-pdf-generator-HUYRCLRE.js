...
// server/lib/roe-pdf-generator.ts
function esc(str) {
  if (!str) return "";
...
function generateRoePdfHtml(data) {
  const doc = data.document;
  const personnel = data.personnel || [];
  const signatures = data.signatures || [];
  const testingTypes = doc.testingTypes || [];
  const attackVectors = doc.attackVectors || [];
  const reportDeliverables = doc.reportDeliverables || [];
  const inScopeAssets = doc.inScopeAssets || [];
  const outOfScopeAssets = doc.outOfScopeAssets || [];
  const inScopeIpRanges = doc.inScopeIpRanges || [];
  const outOfScopeIpRanges = doc.outOfScopeIpRanges || [];
  const inScopeDomains = doc.inScopeDomains || [];
  const outOfScopeDomains = doc.outOfScopeDomains || [];
  const inScopeApplications = doc.inScopeApplications || [];
  const cloudEnvironments = doc.cloudEnvironments || [];
  const testingDays = doc.testingDays || [];
  const complianceFrameworks = doc.complianceFrameworks || [];
  const thirdPartyAgreements = doc.thirdPartyAgreements || [];
  const enabledTypes = testingTypes.filter((t) => t.enabled);
  const enabledVectors = attackVectors.filter((v) => v.enabled);
  const requiredDeliverables = reportDeliverables.filter((d) => d.required);
  const optionalDeliverables = reportDeliverables.filter((d) => !d.required);
  const fedrampBadge = doc.fedrampCompliant ? `<span class="fedramp-badge">FedRAMP ${esc(doc.fedrampImpactLevel?.toUpperCase())} | ${esc(doc.serviceModel?.toUpperCase())}</span>` : "";
...
<div class="doc-control">
  ...
  <table class="control-table">
    ...
    <tbody>
      ${personnel.map((p) => `<tr><td>${esc(p.name)}</td><td>${esc(ROLE_LABELS[p.role] || p.role)}</td><td>${esc(p.organization)}</td><td>${esc(p.email)}</td></tr>`).join("")}
...
<div class="section page-break">
  ...
  <div class="subsection">
    ...
    <table class="data-table">
      ...
      <tbody>${inScopeIpRanges.map((r) => `<tr><td class="mono">${esc(r.cidr)}</td><td>${esc(r.description)}</td><td>${esc(r.vlan)}</td><td>${esc(r.location)}</td></tr>`).join("")}</tbody>
    </table>` : ""}
    ...
    <table class="data-table">
      ...
      <tbody>${inScopeDomains.map((d) => `<tr><td class="mono">${esc(d.domain)}</td><td>${boolLabel(d.includeSubdomains)}</td><td>${esc(d.description)}</td></tr>`).join("")}</tbody>
    </table>` : ""}
    ...
    <table class="data-table">
      ...
      <tbody>${inScopeApplications.map((a) => `<tr><td>${esc(a.name)}</td><td class="mono">${esc(a.url)}</td><td>${esc(a.type)}</td><td>${boolLabel(a.authRequired)}</td><td>${esc(a.description)}</td></tr>`).join("")}</tbody>
    </table>` : ""}
    ...
    <table class="data-table">
      ...
      <tbody>${inScopeAssets.map((a) => `<tr><td>${esc(a.name)}</td><td>${esc(a.type)}</td><td class="mono">${esc(a.ipAddress)}</td><td class="mono">${esc(a.hostname)}</td><td>${esc(a.os)}</td><td><span class="badge badge-${a.criticality || "low"}">${esc(a.criticality?.toUpperCase())}</span></td></tr>`).join("")}</tbody>
    </table>` : ""}
  </div>
  ...
  <div class="subsection">
    ...
    <table class="data-table">
      ...
      <tbody>${outOfScopeIpRanges.map((r) => `<tr><td class="mono">${esc(r.cidr)}</td><td>${esc(r.description)}</td><td>${esc(r.location)}</td></tr>`).join("")}</tbody>
    </table>` : ""}

    ${outOfScopeDomains.length > 0 ? `
    <table class="data-table">
      <thead><tr><th>Domain</th><th>Description</th></tr></thead>
      <tbody>${outOfScopeDomains.map((d) => `<tr><td class="mono">${esc(d.domain)}</td><td>${esc(d.description)}</td></tr>`).join("")}</tbody>
    </table>` : ""}

    ${outOfScopeAssets.length > 0 ? `
    <table class="data-table">
      <thead><tr><th>Name</th><th>Type</th><th>IP</th><th>Reason</th></tr></thead>
      <tbody>${outOfScopeAssets.map((a) => `<tr><td>${esc(a.name)}</td><td>${esc(a.type)}</td><td class="mono">${esc(a.ipAddress)}</td><td>${esc(a.description)}</td></tr>`).join("")}</tbody>
    </table>` : ""}
  ...
  <div class="subsection">
    ...
    <table class="data-table">
      ...
      <tbody>${cloudEnvironments.map((c) => `<tr><td>${esc(c.provider?.toUpperCase())}</td><td class="mono">${esc(c.accountId)}</td><td>${esc(c.region)}</td><td>${(c.services || []).map((s) => esc(s)).join(", ")}</td><td>${esc(c.description)}</td></tr>`).join("")}</tbody>
    </table>
  </div>` : ""}
</div>
...
<div class="section page-break">
  ...
  <div class="subsection">
    ...
    <table class="data-table">
      ...
      <tbody>
        ${enabledTypes.map((t) => `<tr><td><strong>${esc(t.name)}</strong></td><td><span class="badge badge-enabled">${esc(t.category?.replace("_", " ").toUpperCase())}</span></td><td>${esc(t.description)}</td></tr>`).join("")}
  ...
  <div class="subsection">
    ...
    <table class="data-table">
      ...
      <tbody>
        ${enabledVectors.map((v) => `<tr><td><strong>${esc(v.name)}</strong></td><td>${esc(v.description)}</td><td>${v.fedrampRequired ? '<span class="badge badge-fedramp">FedRAMP Required</span>' : '<span class="badge badge-enabled">Enabled</span>'}</td></tr>`).join("")}
...
<div class="section page-break">
  ...
  <div class="subsection mt-2">
    ...
    <p>${testingDays.map((d) => d.charAt(0).toUpperCase() + d.slice(1)).join(", ") || "TBD"}</p>
  </div>
...
<div class="section page-break">
  ...
  <div class="grid-2">
    <div class="info-box">
      ...
      <p>${esc(doc.communicationFrequency?.replace("-", " ").replace(/\b\w/g, (c) => c.toUpperCase()))}</p>
    </div>
    <div class="info-box">
      <div class="box-title">Primary Method</div>
      <p>${esc(doc.communicationMethod?.replace("_", " ").replace(/\b\w/g, (c) => c.toUpperCase()))}</p>
    </div>
  </div>
  ...
  <div class="subsection">
    ...
    <table class="data-table">
      ...
      <tbody>
        ...
        <tr><td>Destruction Method</td><td>${esc(doc.evidenceDestructionMethod?.replace("_", " ").replace(/\b\w/g, (c) => c.toUpperCase()))}</td></tr>
      </tbody>
    </table>
  </div>
</div>
...
<div class="section page-break">
  ...
  <div class="subsection">
    ...
    <div style="display: flex; flex-wrap: wrap; gap: 6px;">
      ${complianceFrameworks.map((f) => `<span class="badge badge-fedramp">${esc(f)}</span>`).join("")}
    </div>
  </div>` : ""}
  ...
  <div class="subsection">
    ...
    <table class="data-table">
      ...
      <tbody>${thirdPartyAgreements.map((a) => `<tr><td>${esc(a.name)}</td><td>${esc(a.description)}</td></tr>`).join("")}</tbody>
    </table>
  </div>` : ""}
</div>
...
<div class="section page-break">
  ...
  <div class="subsection">
    ...
    <table class="data-table">
      ...
      <tbody>
        ${requiredDeliverables.map((d) => `<tr><td><strong>${esc(d.name)}</strong> <span class="badge badge-required">Required</span></td><td>${esc(d.description)}</td></tr>`).join("")}
      </tbody>
    </table>
  </div>
  ...
  <div class="subsection">
    ...
    <table class="data-table">
      <thead><tr><th>Deliverable</th><th>Description</th></tr></thead>
      <tbody>
        ${optionalDeliverables.map((d) => `<tr><td>${esc(d.name)} <span class="badge badge-optional">Optional</span></td><td>${esc(d.description)}</td></tr>`).join("")}
      </tbody>
    </table>
  </div>` : ""}
  ...
  <div class="subsection">
    ...
    <p>Report frequency: <strong>${esc(doc.reportFrequency?.replace("_", " ").replace(/\b\w/g, (c) => c.toUpperCase()))}</strong></p>
  </div>
</div>
...
<div class="section page-break">
  ...
  <table class="data-table">
    ...
    <tbody>
      ${personnel.map((p) => `<tr><td><strong>${esc(p.name)}</strong></td><td>${esc(ROLE_LABELS[p.role] || p.role)}</td><td>${esc(p.organization)}</td><td>${esc(p.email)}</td><td>${esc(p.phone)}</td><td>${p.isPrimary ? '<span class="badge badge-enabled">Primary</span>' : ""}</td></tr>`).join("")}
