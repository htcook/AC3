...
async function triggerScheduledScan(monitor) {
  ...
  try {
    ...
    const { detectSubdomainTakeover, crossReferenceTechVulnerabilities, detectSubdomainChanges } = await import("./domain-intel-advanced-Y3BJ3NFT.js");
...
function initScanScheduler() {
  ...
  setTimeout(async () => {
    try {
      const { startPeriodicHealthChecks } = await import("./health-monitor-DPJ64MVI.js");
...
function stopScanScheduler() {
  ...
  import("./health-monitor-DPJ64MVI.js").then(({ stopPeriodicHealthChecks }) => {
