...
async function runVulnVerification(highCritVulns, state, addLog, broadcastOpsUpdate) {
  ...
  try {
    const { verifyVulnerability } = await import("./vuln-verifier-RT36JYTY.js");
    const { evidenceGate, buildProvenance, createIntegrityEnvelope } = await import("./evidence-integrity-E4ASVYFK.js");
...
async function runScanForgeReasoning(highCritVulns, state, addLog, broadcastOpsUpdate) {
  ...
  let processed = 0;
  try {
    const { batchRunScanForgeReasoning } = await import("./scanforge-reasoning-3NFLMOFC.js");
...
async function runThreatActorMapping(state, addLog, broadcastOpsUpdate, scoreEngagementThreatAttribution) {
  ...
  let mapped = 0;
  try {
    const { mapThreats } = await import("./threat-mapper-6GLRIPLM.js");
...
async function executeVulnCorrelation(ctx) {
  ...
  if (allVulns.length > 0) {
    ...
    try {
      ...
      } = await import("./knowledge-lazy-VLWLHFSC.js");
