...
async function resolveUserTenant(userId, requestedTenantId) {
  ...
  const { tenantMemberships, tenants } = await import("./schema-RLVX4V4P.js");
...
async function autoProvisionTenant(userId, userName) {
  const { getDb } = await import("./db-GNA5CL3K.js");
  const { tenants, tenantMemberships } = await import("./schema-RLVX4V4P.js");
  const db = await getDb();
...
async function logTenantAction(userId, tenantId, action, details) {
  try {
    const { getDb } = await import("./db-GNA5CL3K.js");
    const { activityLogs } = await import("./schema-RLVX4V4P.js");
    const db = await getDb();
    if (!db) return;
...
async function getTenantStats(tenantId) {
  const { getDb } = await import("./db-GNA5CL3K.js");
  const { tenantMemberships, engagements } = await import("./schema-RLVX4V4P.js");
  const db = await getDb();
  if (!db) return null;
