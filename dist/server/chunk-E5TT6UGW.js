...
// server/lib/nextcloud-test-lab.ts
function getNextcloudMaxReward(severity) {
  const s = severity.toLowerCase();
  const tier = NEXTCLOUD_BOUNTY_POLICY.rewards.find((r) => r.impact === s);
  return tier?.maxReward ?? 0;
}
function validateNextcloudSubmission(finding) {
  const issues = [];
...
function generateDockerCompose(config = DEFAULT_LAB_CONFIG) {
  const services = {};
  services.db = {
    image: "mariadb:11.4",
...
function generateAppInstallScript(config = DEFAULT_LAB_CONFIG) {
...
function generateUserProvisioningScript(config = DEFAULT_LAB_CONFIG) {
  const lines = [
    "#!/bin/bash",
...
function generateLdapSeedScript(config = DEFAULT_LAB_CONFIG) {
  return `#!/bin/bash
...
function generateConfigScript(config = DEFAULT_LAB_CONFIG) {
  const lines = [
    "#!/bin/bash",
...
function generateFullDeployScript(config = DEFAULT_LAB_CONFIG) {
  return `#!/bin/bash
...
function generateStatusScript(config = DEFAULT_LAB_CONFIG) {
  return `#!/bin/bash
...
function generateTeardownScript(config = DEFAULT_LAB_CONFIG) {
  return `#!/bin/bash
...
function getTestLabInfo(config = DEFAULT_LAB_CONFIG) {
...
`;
        ...
        const v = typeof val === "string" && (val.includes(":") || val.includes("#") || val.includes("{")) ? `"${val.replace(/"/g, '\\"')}"` : val;
        out += `${pad}${key}: ${v}
`;
      }
    }
...
var NEXTCLOUD_VERSIONS, BOUNTY_ELIGIBLE_APPS, NEXTCLOUD_BOUNTY_POLICY, DEFAULT_LAB_CONFIG;
var init_nextcloud_test_lab = __esm({
  "server/lib/nextcloud-test-lab.ts"() {
    NEXTCLOUD_VERSIONS = {
      latest: "30.0.6",
    ...
    NEXTCLOUD_BOUNTY_POLICY = {
      ...
      rewards: [
        { impact: "critical", definition: "Gaining remote code execution on the server as a non-admin user (RCE)", maxReward: 1e4 },
        { impact: "high", definition: "Gaining access to complete user data of any other user (Auth Bypass)", maxReward: 4e3 },
...
export {
  ...
  BOUNTY_ELIGIBLE_APPS,
  NEXTCLOUD_BOUNTY_POLICY,
  getNextcloudMaxReward,
  validateNextcloudSubmission,
  DEFAULT_LAB_CONFIG,
