...
async function triggerScheduledScan(monitor) {
  ...
  try {
    ...
    const { detectSubdomainTakeover, crossReferenceTechVulnerabilities, detectSubdomainChanges } = await import("./domain-intel-advanced-UIRSCZ76.js");
...
function initScanScheduler() {
  ...
  setTimeout(async () => {
    try {
      const { startPeriodicHealthChecks } = await import("./health-monitor-Q7XPH64D.js");
...
function stopScanScheduler() {
  ...
  import("./health-monitor-Q7XPH64D.js").then(({ stopPeriodicHealthChecks }) => {
