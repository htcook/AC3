...
async function persistObservations(observations) {
  ...
  try {
    ...
    const { scanObservations } = await import("./schema-AEHUE7AH.js");
...
async function persistSignals(signals) {
  ...
  try {
    const { getDb } = await import("./db-EEYUM2OC.js");
    const { scanSignals } = await import("./schema-AEHUE7AH.js");
    const db = await getDb();
    if (!db) return 0;
    let inserted = 0;
...
async function persistRiskCards(cards) {
  ...
  try {
    const { getDb } = await import("./db-EEYUM2OC.js");
    const { scanRiskCards } = await import("./schema-AEHUE7AH.js");
    const db = await getDb();
    if (!db) return 0;
    let inserted = 0;
