...
async function triggerScheduledScan(monitor) {
  ...
  try {
    ...
    const { detectSubdomainTakeover, crossReferenceTechVulnerabilities, detectSubdomainChanges } = await import("./domain-intel-advanced-AHE4BPFB.js");
...
function initScanScheduler() {
  ...
  setTimeout(async () => {
    try {
      const { startPeriodicHealthChecks } = await import("./health-monitor-OYEVWZPT.js");
...
function stopScanScheduler() {
  ...
  import("./health-monitor-OYEVWZPT.js").then(({ stopPeriodicHealthChecks }) => {
