...
function registerCicdWebhookRoutes(app) {
  app.post("/api/cicd/webhook/:pipelineId", async (req, res) => {
    ...
    try {
      ...
      const { cicdPipelines, cicdRuns } = await import("./schema-TGCU627K.js");
      ...
      if (secret) {
        ...
        if (signature) {
          const { verifyGitHubWebhook } = await import("./aws-cicd-connector-G5I27VFF.js");
      ...
      if (targetUrl) {
        import("./aws-cicd-connector-G5I27VFF.js").then(async ({ executeCicdScan }) => {
          try {
            ...
            if (pipeline.cicdProvider === "github_actions" && commitSha && repository) {
              try {
                ...
                if (githubToken) {
                  ...
                  const { callbackGitHubActions } = await import("./aws-cicd-connector-G5I27VFF.js");
