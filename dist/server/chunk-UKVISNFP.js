...
Generate two-phase scan plan. Phase A: ScanForge discovery --top-ports 1000. Phase B: tools per asset.`;
  ...
  try {
    ...
    try {
      const { getToolInventory, getInventoryForLLM } = await import("./scan-server-inventory-PMPNRICG.js");
...
${Object.entries(state.dastConfig.customHeaders).map(([k, v]) => `  ${k}: ${v}`).join("\n")}`
  ...
  for (const domain of domains) {
    try {
      ...
      const { runDomainIntelPipeline } = await import("./domainIntel-HFLQ6BPR.js");
...
async function executeEnumeration(state, engagement, operatorCtx) {
  const { executeEnumeration: runEnumerationPhase } = await import("./engagement-phase-enumeration-DDBLQ2IG.js");
...
async function executeVulnDetection(state, engagement, operatorCtx) {
  ...
  const { executeVulnPrep } = await import("./vuln-prep-I56UOG3H.js");
  const vulnPrepCtx = {
    state,
    engagement,
    operatorCtx,
  ...
  const { executeZapScanning } = await import("./zap-scanner-4JNZEBJ6.js");
  ...
  const { executeInjectionScanning } = await import("./injection-scanner-W7ZNG3CD.js");
  ...
  const { executeCredentialTesting } = await import("./credential-tester-M2UOBF6Q.js");
  ...
  const { executeVulnCorrelation } = await import("./vuln-correlation-CNXMLT3N.js");
...
async function executeExploitation(state, engagement, operatorCtx) {
  const { executeExploitation: runExploitPhase } = await import("./engagement-phase-exploitation-V4JN4GQW.js");
  return runExploitPhase(state, engagement, operatorCtx);
}
async function executePostExploit(state, engagement, operatorCtx) {
  const { executePostExploit: runPostExploitPhase } = await import("./engagement-phase-post-exploit-7VKC7QWY.js");
    ...
    if (engagement.roeStatus === "signed" || engagement.roeStatus === "pending" || state.trainingLabMode === true) {
      if (["recon", "passive_discovery", "scoping", "test_plan", "enumeration"].includes(startPhase)) {
        ...
        if (!enumGate.allowed) {
          ...
          try {
            const { checkScanServerStatus } = await import("./scan-server-executor-6TKRZBDI.js");
...
PoC: ${cf.analysis.poc || "N/A"}`,
        ...
        if (state.stats.vulnsFound > 0 || state.assets.some((a) => a.cloudProviders?.length > 0)) {
          try {
            ...
            const { runFeedbackLoop, getFeedbackLoopSummary } = await import("./llm-scan-feedback-BUF2GO3C.js");
...
Promoted: ${promotionSummary.promotedVulns.map((p) => `${p.vulnTitle.slice(0, 50)} [${p.scanner}/${p.category}]`).join("; ")}`,
      ...
      try {
        const { retryDeferredScans, getDeferredScans, clearDeferredScans } = await import("./job-queue-bridge-7VJDUFV6.js");
      ...
      try {
        const { selectFindingsForScreenshot, captureScreenshotBatch } = await import("./screenshot-capture-6XZBUO2Z.js");
      ...
      try {
        const { runAccuracyComparison } = await import("./accuracy-feedback-loop-PKEKLGLO.js");
...
View details on the Knowledge Base \u2192 Accuracy Feedback tab.`
          ...
          try {
            const { runLocalAccuracyComparison } = await import("./accuracy-feedback-loop-PKEKLGLO.js");
...
Command: ${command}`,
  ...
  try {
    const { executeTool } = await import("./scan-server-executor-6TKRZBDI.js");
