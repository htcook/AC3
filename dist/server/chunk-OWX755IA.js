...
} from "./chunk-5RWXW2NC.js";
import {
  classifyVulnClass,
  init_exploit_learning_engine
} from "./chunk-YHYDUOLL.js";
import {
...
async function clearOpsState(engagementId) {
  ...
  try {
    const { deleteOpsSnapshot } = await import("./db-FQGKASI3.js");
...
async function getOpsStateWithRecovery(engagementId) {
  ...
  try {
    const { loadOpsSnapshot } = await import("./db-FQGKASI3.js");
...
function persistOpsStateDebounced(engagementId, delayMs = 2e3) {
  ...
  persistTimers.set(engagementId, setTimeout(async () => {
    ...
    if (!state) return;
    try {
      const { saveOpsSnapshot } = await import("./db-FQGKASI3.js");
...
async function flushAllPendingState() {
  ...
  try {
    const { releaseAllClaims } = await import("./engagement-claim-lock-EDOA2NM2.js");
...
async function persistTimelineEvent(engagementId, logEntry) {
  try {
    ...
    const { engagementTimelineEvents } = await import("./schema-XIZFPXHE.js");
...
async function persistScanResult(opts) {
  try {
    const { insertScanResult, saveEngagementFindings } = await import("./db-FQGKASI3.js");
...
async function auditLog(params) {
  try {
    const { getDb } = await import("./db-FQGKASI3.js");
    const { offensiveAuditLog } = await import("./schema-XIZFPXHE.js");
    const db = await getDb();
    if (db) {
...
Generate two-phase scan plan. Phase A: ScanForge discovery --top-ports 1000. Phase B: tools per asset.`;
  ...
  try {
    ...
    try {
      const { getToolInventory, getInventoryForLLM } = await import("./scan-server-inventory-ASEYND2Q.js");
...
Use alternative tools instead.`;
  ...
  try {
    ...
    const { planAttack } = await import("./attack-planner-CTX56OPZ.js");
...
async function llmDecide(context) {
  ...
  if (!skipSpecialist) try {
    const { decideNextOp } = await import("./ops-decider-XMLI5DWK.js");
...
${Object.entries(state.dastConfig.customHeaders).map(([k, v]) => `  ${k}: ${v}`).join("\n")}`
  ...
  for (const domain of domains) {
    try {
      ...
      const { isLabDomain } = await import("./passive-ZOCRKBEE.js");
      ...
      const { runDomainIntelPipeline } = await import("./domainIntel-6LMEYIXC.js");
      ...
      try {
        ...
        if (domainAssets.length > 0) {
          const { analyzeScan } = await import("./scan-analyst-4VHZM6MH.js");
...
${(analysis.recommendations || []).slice(0, 3).map((r) => `\u2022 [${r.priority}] ${r.action}`).join("\n")}`,
      ...
      try {
        const domainAssets = state.assets.filter((a) => a.hostname === domain || a.hostname.endsWith("." + domain));
        if (domainAssets.length > 0) {
          const { scoreFullHybrid, buildEngagementContext } = await import("./hybrid-scorer-SQ5MW3DJ.js");
...
async function executeEnumeration(state, engagement, operatorCtx) {
  const { executeEnumeration: runEnumerationPhase } = await import("./engagement-phase-enumeration-LIGLD3Z5.js");
...
async function executeVulnDetection(state, engagement, operatorCtx) {
  ...
  const { executeVulnPrep } = await import("./vuln-prep-VWZWEWXU.js");
  const vulnPrepCtx = {
    state,
    engagement,
    operatorCtx,
  ...
  const { executeZapScanning } = await import("./zap-scanner-2HAF3JBY.js");
  ...
  const { executeInjectionScanning } = await import("./injection-scanner-LI3LQFVA.js");
  ...
  const { executeCredentialTesting } = await import("./credential-tester-ZOQTPWEA.js");
  ...
  const { executeVulnCorrelation } = await import("./vuln-correlation-NYTAYWIQ.js");
...
async function executeExploitation(state, engagement, operatorCtx) {
  const { executeExploitation: runExploitPhase } = await import("./engagement-phase-exploitation-XDECCWC7.js");
  return runExploitPhase(state, engagement, operatorCtx);
}
async function executePostExploit(state, engagement, operatorCtx) {
  const { executePostExploit: runPostExploitPhase } = await import("./engagement-phase-post-exploit-UZQV26O3.js");
...
async function executeEngagement(engagementId, operatorCtx, options) {
  ...
  try {
    const { claimEngagement } = await import("./engagement-claim-lock-EDOA2NM2.js");
...
Preserved: ${state.assets.length} assets, ${state.stats.vulnsFound} vulns, ${state.stats.portsFound} ports, ${state.log.length} log entries`
  ...
  let engagement;
  try {
    const db = await import("./db-FQGKASI3.js");
...
Max blast radius: ${engagementSafetyLevel === "passive_only" ? 5 : engagementSafetyLevel === "low_impact" ? 30 : engagementSafetyLevel === "standard" ? 60 : 100}`
  ...
  try {
    const { buildEngagementContext } = await import("./hybrid-scorer-SQ5MW3DJ.js");
  ...
  try {
    const { setActiveUser } = await import("./bug-bounty-intelligence-THPSB567.js");
  ...
  try {
    if (startPhase === "recon") {
      ...
      if (!reconGate.allowed) {
        ...
        await breathe();
        try {
          const { executeCustomerIntegrationsForStage, mergeIntegrationResultsIntoObservations } = await import("./pipeline-bridge-MB55OYH4.js");
    ...
    if (["recon", "passive_discovery"].includes(startPhase)) {
      try {
        ...
        await breathe();
        try {
          const { executeCustomerIntegrationsForStage } = await import("./pipeline-bridge-MB55OYH4.js");
    ...
    if (engagement.roeStatus === "signed" || engagement.roeStatus === "pending" || state.trainingLabMode === true) {
      if (["recon", "passive_discovery", "scoping", "test_plan", "enumeration"].includes(startPhase)) {
        ...
        if (!enumGate.allowed) {
          ...
          try {
            const { checkScanServerStatus } = await import("./scan-server-executor-F73PWIB5.js");
        ...
        if (!vulnGate.allowed) {
          ...
          if (state.stats.vulnsFound > 0) {
            try {
              ...
              const { batchAnalyzeFindings, generateAnalysisSummary, classifyVulnerability } = await import("./vuln-analysis-agents-O4W2AJOI.js");
...
PoC: ${cf.analysis.poc || "N/A"}`,
        ...
        if (state.stats.vulnsFound > 0 || state.assets.some((a) => a.cloudProviders?.length > 0)) {
          try {
            ...
            const { runFeedbackLoop, getFeedbackLoopSummary } = await import("./llm-scan-feedback-XQVNSC5I.js");
...
${feedbackState.finalAnalysis?.slice(0, 500) || ""}`,
        ...
        if (state.stats.vulnsFound > 0) {
          try {
            ...
            const { generateEngagementAttackChains } = await import("./cloud-attack-chain-designer-GD573WJN.js");
...
Promoted: ${promotionSummary.promotedVulns.map((p) => `${p.vulnTitle.slice(0, 50)} [${p.scanner}/${p.category}]`).join("; ")}`,
        ...
        {
          const { executeSocialEngineering } = await import("./engagement-phase-social-engineering-3VPWAMUT.js");
      ...
      try {
        const { retryDeferredScans, getDeferredScans, clearDeferredScans } = await import("./job-queue-bridge-D5A5ERCQ.js");
      ...
      try {
        const { selectFindingsForScreenshot, captureScreenshotBatch } = await import("./screenshot-capture-GCLBGX2O.js");
      ...
      try {
        const { generateAttackNarratives, generateExecutiveSummary } = await import("./attack-narrative-generator-4HOKIZBW.js");
      ...
      try {
        const { runAccuracyComparison } = await import("./accuracy-feedback-loop-E6H2FJXY.js");
        ...
        if (targetPreset) {
          ...
          let engNotes = null;
          try {
            const { engagements: engTable } = await import("./schema-XIZFPXHE.js");
            const { getDbRequired: getDbReq } = await import("./db-FQGKASI3.js");
...
View details on the Knowledge Base \u2192 Accuracy Feedback tab.`
          ...
          try {
            const { runLocalAccuracyComparison } = await import("./accuracy-feedback-loop-E6H2FJXY.js");
    ...
    try {
      const { generateAutoReport } = await import("./engagement-auto-report-QURY2BG2.js");
    ...
    try {
      ...
      broadcastOpsUpdate(state.engagementId, { type: "log_update" });
      const { generateTestPlanAdherence } = await import("./engagement-report-handoff-TRBD7L52.js");
    ...
    try {
      const { updateEngagement } = await import("./db-FQGKASI3.js");
    ...
    try {
      const { saveEngagementResult, saveEngagementFindings } = await import("./db-FQGKASI3.js");
    ...
    try {
      const { getEngagementLlmTelemetryRaw } = await import("./db-FQGKASI3.js");
    ...
    try {
      const { runPostPipelineGraduation, extractEngagementMetrics } = await import("./post-pipeline-graduation-45KRUOL5.js");
    ...
    try {
      const { detectGaps, createGapsBatch } = await import("./intelligence-gaps-SKIQTU3U.js");
    ...
    try {
      const { updateProfileFromEngagement } = await import("./customer-intel-profile-JYZFNQK3.js");
    ...
    try {
      const { updateEngagement } = await import("./db-FQGKASI3.js");
      const existingEng = await (await import("./db-FQGKASI3.js")).getEngagementById(engagementId);
...
async function recoverInterruptedEngagements() {
  ...
  try {
    const { getDbRequired } = await import("./db-FQGKASI3.js");
    const db = await getDbRequired();
    const { engagementOpsSnapshots } = await import("./schema-XIZFPXHE.js");
