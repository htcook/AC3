...
async function resolveUserTenant(userId, requestedTenantId) {
  ...
  const { tenantMemberships, tenants } = await import("./schema-5274GLXD.js");
...
async function autoProvisionTenant(userId, userName) {
  const { getDb } = await import("./db-UCRYETLI.js");
  const { tenants, tenantMemberships } = await import("./schema-5274GLXD.js");
  const db = await getDb();
...
async function logTenantAction(userId, tenantId, action, details) {
  try {
    const { getDb } = await import("./db-UCRYETLI.js");
    const { activityLogs } = await import("./schema-5274GLXD.js");
    const db = await getDb();
    if (!db) return;
...
async function getTenantStats(tenantId) {
  const { getDb } = await import("./db-UCRYETLI.js");
  const { tenantMemberships, engagements } = await import("./schema-5274GLXD.js");
  const db = await getDb();
  if (!db) return null;
