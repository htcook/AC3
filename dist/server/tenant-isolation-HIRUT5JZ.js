...
async function resolveUserTenant(userId, requestedTenantId) {
  ...
  const { tenantMemberships, tenants } = await import("./schema-TGCU627K.js");
...
async function autoProvisionTenant(userId, userName) {
  const { getDb } = await import("./db-65DPEQYH.js");
  const { tenants, tenantMemberships } = await import("./schema-TGCU627K.js");
  const db = await getDb();
...
async function logTenantAction(userId, tenantId, action, details) {
  try {
    const { getDb } = await import("./db-65DPEQYH.js");
    const { activityLogs } = await import("./schema-TGCU627K.js");
    const db = await getDb();
    if (!db) return;
...
async function getTenantStats(tenantId) {
  const { getDb } = await import("./db-65DPEQYH.js");
  const { tenantMemberships, engagements } = await import("./schema-TGCU627K.js");
  const db = await getDb();
  if (!db) return null;
