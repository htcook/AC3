...
} from "./chunk-Q5VYZDN7.js";
import "./chunk-VWQPFA5H.js";
...
Total combined: ${combinedTechs.size} unique technologies`
  ...
  try {
    const { onBurpScanComplete } = await import("./burp-auto-scan-UORP2N5V.js");
    onBurpScanComplete(async (burpConfig, burpState) => {
      ...
      if (normalizedFindings && normalizedFindings.length > 0) {
        try {
          const { saveEngagementFindings } = await import("./db-LSUZDHGJ.js");
  ...
  try {
    const { getEngagementCredentials } = await import("./credential-harvester-BYZG5PO3.js");
  ...
  try {
    const { onEngagementVulnDetectionPhase, extractScopeUrls } = await import("./burp-auto-scan-UORP2N5V.js");
  ...
  try {
    const { runZapToBurpPipeline } = await import("./zap-burp-pipeline-RW3K6QYY.js");
  ...
  try {
    const { runSeverityEscalation } = await import("./zap-burp-pipeline-RW3K6QYY.js");
