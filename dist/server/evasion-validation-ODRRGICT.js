...
async function runEvasionAwareTakeoverValidation(candidates, evasionConfig) {
  const { validateTakeoverCandidates } = await import("./domain-intel-advanced-ZRGUB4WJ.js");
  ...
  for (let i = 0; i < normalResult.results.length; i++) {
    ...
    if (finding.finalResult === "bypassed" && finding.attempts.length > 0) {
      ...
      const domIntelMod = await import("./domain-intel-advanced-ZRGUB4WJ.js");
      const TAKEOVER_FINGERPRINTS = domIntelMod.TAKEOVER_FINGERPRINTS;
      const fingerprint = (await import("./domain-intel-advanced-ZRGUB4WJ.js")).TAKEOVER_FINGERPRINTS.find((f) => f.service === candidate.service);
