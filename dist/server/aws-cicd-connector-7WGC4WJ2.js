...
async function scanServerPreFlight() {
  try {
    const { checkDoScanServiceHealth } = await import("./do-scan-api-JOPP2QH3.js");
...
async function executeCicdScan(request) {
  ...
  try {
    const { correlateCicdFindings } = await import("./cicd-threat-correlator-TLDKH72G.js");
...
async function runNucleiCicd(targetUrl) {
  const { executeRawCommandViaHttp } = await import("./do-scan-api-JOPP2QH3.js");
  const findings = [];
  try {
...
async function runZapCicd(targetUrl) {
  const findings = [];
  try {
    const { getActiveZapUrl } = await import("./scan-service-url-CBKAYLTJ.js");
...
async function generateSbom(imageRef) {
  const { executeRawCommandViaHttp } = await import("./do-scan-api-JOPP2QH3.js");
  const { doStoragePut } = await import("./do-storage-55FTSNMH.js");
  const result = await executeRawCommandViaHttp(
