...
var init_dedup_coverage = __esm({
  "server/scanforge/intelligence/dedup-coverage.ts"() {
    ...
    DeduplicationEngine = class {
        ...
        switch (findingType) {
          case "web":
            parts = [
              ...
              endpoint || "*",
              // Critical: prevents over-merging web vulns
       ...
       */
      classifyFindingType(finding) {
        const text = `${finding.title} ${finding.source} ${(finding.cwes || []).join(" ")}`.toLowerCase();
        if (text.includes("credential") || text.includes("default password") || text.includes("weak password") || text.includes("brute force") || finding.evidence?.data?.username || finding.evidence?.data?.credential) {
          return "credential";
        }
        const webIndicators = [
          "xss",
        ...
        if (finding.evidence?.data?.url || finding.evidence?.data?.endpoint || finding.evidence?.data?.path || finding.evidence?.data?.matched_at) {
          return "web";
        }
        if (finding.target.startsWith("http://") || finding.target.startsWith("https://")) {
          return "web";
        }
        if (finding.port && [80, 443, 8080, 8443, 3e3, 8e3, 8888].includes(finding.port)) {
          return "web";
        }
        return "network";
      }
       ...
       */
      extractEndpoint(finding) {
        const endpoint = finding.evidence?.data?.endpoint || finding.evidence?.data?.path || finding.evidence?.data?.url || finding.evidence?.data?.matched_at;
        if (endpoint) {
          try {
