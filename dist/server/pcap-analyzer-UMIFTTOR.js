...
async function executeLiveCapture(config) {
  const { executeTool, executeRawCommand } = await import("./scan-server-executor-JZBPP72B.js");
...
async function ingestPcapResults(findings, packets) {
  const { ingestRawObservations } = await import("./observation-ingestor-QEOTLX3C.js");
