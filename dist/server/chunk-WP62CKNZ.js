...
// server/lib/knowledge/bugbounty-methodology-knowledge.ts
function getAllBugBountyTools() {
  return BUG_BOUNTY_TOOLS;
}
function getToolsByPhase(phase) {
...
function searchTools(query) {
  ...
  return BUG_BOUNTY_TOOLS.filter(
    (t) => t.name.toLowerCase().includes(q) || t.description.toLowerCase().includes(q) || t.category.toLowerCase().includes(q) || t.use_case.toLowerCase().includes(q)
  );
}
...
function getMethodologiesForLab(labId) {
  return ATTACK_METHODOLOGIES.filter((m) => m.applicable_labs.includes(labId));
}
function getMethodologiesByCategory(category) {
  return ATTACK_METHODOLOGIES.filter((m) => m.category === category);
}
function getWorkflow() {
  return BUG_BOUNTY_WORKFLOW;
}
function getWorkflowByPhase(phase) {
  return BUG_BOUNTY_WORKFLOW.filter((s) => s.phase === phase);
}
function buildMethodologyContext(targetPreset) {
  const methodologies = targetPreset ? getMethodologiesForLab(targetPreset) : ATTACK_METHODOLOGIES;
...
function buildPhaseToolContext(phase) {
...
function buildVulnTestingContext(vulnType) {
  const typeMap = {
...
function buildScanPlanningContext(targetPreset, phase) {
  const parts = [];
...
var BUG_BOUNTY_TOOLS, ATTACK_METHODOLOGIES, BUG_BOUNTY_WORKFLOW;
var init_bugbounty_methodology_knowledge = __esm({
  "server/lib/knowledge/bugbounty-methodology-knowledge.ts"() {
    BUG_BOUNTY_TOOLS = [
