...
async function triggerScheduledScan(monitor) {
  ...
  try {
    ...
    const { detectSubdomainTakeover, crossReferenceTechVulnerabilities, detectSubdomainChanges } = await import("./domain-intel-advanced-YZQHCVOY.js");
...
function initScanScheduler() {
  ...
  setTimeout(async () => {
    try {
      const { startPeriodicHealthChecks } = await import("./health-monitor-MBW63EHR.js");
...
function stopScanScheduler() {
  ...
  import("./health-monitor-MBW63EHR.js").then(({ stopPeriodicHealthChecks }) => {
