...
async function buildInjectableUrls(webApp, targetUrl, state, engagementId) {
  ...
  try {
    ...
    if (db) {
      const { webAppScans } = await import("./schema-RDUWS2ES.js");
...
async function executeInjectionScanning(ctx) {
  ...
  for (const webApp of webApps) {
    ...
    try {
      ...
      if (approved) {
        const { batchSqlmapScan, analyzeSqlmapFindings, runBlindSqliPass, ingestSqlmapToWebAppFindings } = await import("./sqlmap-scanner-EFW33HOX.js");
    ...
    try {
      ...
      if (approved) {
        const { batchXssScan, analyzeXssFindings, ingestXssToWebAppFindings } = await import("./xsstrike-scanner-M7HIPMHY.js");
  ...
  for (const webApp of webApps) {
    ...
    try {
      ...
      if (approved) {
        const { batchCommixScan, ingestCommixToWebAppFindings } = await import("./commix-scanner-55ZVX57O.js");
        const isLab = state.trainingLabMode === true;
  ...
  for (const webApp of webApps) {
    ...
    try {
      ...
      if (approved) {
        const { batchTplmapScan, ingestTplmapToWebAppFindings } = await import("./tplmap-scanner-AOAZNDQV.js");
