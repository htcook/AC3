...
async function resolveUserTenant(userId, requestedTenantId) {
  ...
  const { tenantMemberships, tenants } = await import("./schema-AEHUE7AH.js");
...
async function autoProvisionTenant(userId, userName) {
  const { getDb } = await import("./db-EEYUM2OC.js");
  const { tenants, tenantMemberships } = await import("./schema-AEHUE7AH.js");
  const db = await getDb();
...
async function logTenantAction(userId, tenantId, action, details) {
  try {
    const { getDb } = await import("./db-EEYUM2OC.js");
    const { activityLogs } = await import("./schema-AEHUE7AH.js");
    const db = await getDb();
    if (!db) return;
...
async function getTenantStats(tenantId) {
  const { getDb } = await import("./db-EEYUM2OC.js");
  const { tenantMemberships, engagements } = await import("./schema-AEHUE7AH.js");
  const db = await getDb();
  if (!db) return null;
