...
async function executeToolViaQueue(config, options) {
  ...
  if (!useQueue) {
    if (!options?.forceLocal) {
      try {
        const { executeToolViaHttp } = await import("./do-scan-api-4KSMQ3XA.js");
...
async function executeRawCommandViaQueue(command, timeoutSeconds = 300, options) {
  ...
  if (!options?.forceLocal) {
    try {
      ...
      const { executeRawCommandViaHttp } = await import("./do-scan-api-4KSMQ3XA.js");
  ...
  const { executeViaChildProcessSSH } = await import("./scan-server-executor-F3P7YWRR.js");
...
async function retryDeferredScans(engagementId, options) {
  ...
  try {
    const { isDedicatedHealthy } = await import("./scan-service-url-JYPT75HE.js");
