...
// server/lib/engagement-builder.ts
init_llm();
init_db();
init_credential_service();
init_schema();
import { eq, sql, desc } from "drizzle-orm";
var BUILDABLE_ASSET_TYPES = /* @__PURE__ */ new Set([
  "SOURCE_CODE",
  "DOWNLOADABLE_EXECUTABLES",
  "HARDWARE",
  "SMART_CONTRACT"
]);
var SCANNABLE_ASSET_TYPES = /* @__PURE__ */ new Set([
  "URL",
  "DOMAIN",
  "WILDCARD",
  "IP_ADDRESS",
...
async function fetchProgramData(programId) {
  ...
  const [program] = await db.select().from(bugBountyPrograms).where(eq(bugBountyPrograms.id, programId)).limit(1);
  if (!program) throw new Error(`Program not found: ${programId}`);
  const scopes = await db.select().from(bugBountyProgramScopes).where(eq(bugBountyProgramScopes.programId, programId));
  const weaknesses = await db.select().from(bugBountyProgramWeaknesses).where(eq(bugBountyProgramWeaknesses.programId, programId));
  const findings = await db.select().from(bugBountyFindings).where(eq(bugBountyFindings.programHandle, program.handle)).orderBy(desc(bugBountyFindings.disclosedAt)).limit(50);
...
async function fetchProgramPage(url) {
  try {
    ...
    return text.substring(0, 15e3);
  } catch (err) {
    return `[Failed to fetch: ${err.message}]`;
  }
}
async function buildEngagementPreview(input) {
  ...
  if (programData && (!programData.scopes || programData.scopes.length === 0) && platform === "hackerone" && programHandle) {
    try {
      ...
      const h1Creds = await getH1CredentialsForUser();
      if (h1Creds) {
        ...
        if (scopeRes.ok) {
          ...
          if (scopeData?.data?.length) {
            programData.scopes = scopeData.data.map((item) => {
...
${programData.weaknesses.slice(0, 30).map((w) => `- ${w.cweId || ""} ${w.name}`).join("\n")}` : "";
  const findingsContext = programData?.findings?.length ? `

RECENT DISCLOSED FINDINGS (${programData.findings.length}):
${programData.findings.slice(0, 20).map(
    (f) => `- [${f.severityRating}] ${f.title} (CWE: ${f.cweId || "N/A"}, CVE: ${f.cveIds?.join(",") || "N/A"}, bounty: $${f.awardedAmount || 0})`
  ).join("\n")}` : "";
...
Always include toolRequirements based on the target tech stack \u2014 analyze what specialized security tools are needed beyond standard web scanners.`;
  ...
  const scopeAssets = (plan.scope?.assets || []).map((a) => ({
    ...a,
...
async function createEngagementFromPreview(preview, userId, options) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const engagementName = options?.customName || `${preview.programName} Bug Bounty Engagement`;
  ...
  for (const phase of preview.phases) {
    const ts = Date.now() + parseInt(phase.week || "1") * 7 * 24 * 60 * 60 * 1e3;
  ...
  if (preview.scope.assets.length > 0) {
    ...
    if (existing?.cnt === 0 || existing?.[0]?.cnt === 0) {
      for (const asset of preview.scope.assets) {
        try {
...
function categorizeAssetsByTier(assets) {
  const result = {
    tier1_critical: [],
    tier2_high: [],
    tier3_medium: [],
...
function extractPrimaryDomain(preview) {
