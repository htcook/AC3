...
async function persistObservations(observations) {
  ...
  try {
    ...
    const { scanObservations } = await import("./schema-J7P4MLKR.js");
...
async function persistSignals(signals) {
  ...
  try {
    const { getDb } = await import("./db-2N7OEZBU.js");
    const { scanSignals } = await import("./schema-J7P4MLKR.js");
    const db = await getDb();
    if (!db) return 0;
    let inserted = 0;
...
async function persistRiskCards(cards) {
  ...
  try {
    const { getDb } = await import("./db-2N7OEZBU.js");
    const { scanRiskCards } = await import("./schema-J7P4MLKR.js");
    const db = await getDb();
    if (!db) return 0;
    let inserted = 0;
