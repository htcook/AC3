...
async function persistObservations(observations) {
  ...
  try {
    ...
    const { scanObservations } = await import("./schema-RLVX4V4P.js");
...
async function persistSignals(signals) {
  ...
  try {
    const { getDb } = await import("./db-GNA5CL3K.js");
    const { scanSignals } = await import("./schema-RLVX4V4P.js");
    const db = await getDb();
    if (!db) return 0;
    let inserted = 0;
...
async function persistRiskCards(cards) {
  ...
  try {
    const { getDb } = await import("./db-GNA5CL3K.js");
    const { scanRiskCards } = await import("./schema-RLVX4V4P.js");
    const db = await getDb();
    if (!db) return 0;
    let inserted = 0;
