  ...
  for (const webApp of webApps) {
    ...
    for (const { targetUrl, dedupKey } of scanTargets) {
      ...
      try {
        ...
        const { generateLLMScanConfig, startScan, configureZapAuthentication } = await import("./zap-scanner-OTE2ZIKO.js");
        ...
        if (wafVendor) {
          try {
            const { applyWafEvasionConfig } = await import("./zap-scanner-OTE2ZIKO.js");
        ...
        if (!zapStarted) {
          ...
          try {
            const { onEngagementVulnDetectionPhase } = await import("./burp-auto-scan-3A3K2NAU.js");
        ...
        if (zapScanId) {
          ...
          const { pollScanProgress } = await import("./zap-scanner-OTE2ZIKO.js");
            ...
            try {
              ...
              if (p.status === "completed" || p.status === "error") {
                ...
                try {
                  ...
                  if (db) {
                    const { webAppFindings } = await import("./schema-5UXBHZJS.js");
  ...
  try {
    ...
    if (ir && (ir.urlSource === "scope_fallback" || ir.zapUrlsDiscovered === 0)) {
      ...
      if (db) {
        ...
        if (scans.length > 0) {
          const { deferredZapBurpRefeed } = await import("./zap-burp-pipeline-OGPGITSR.js");
