...
async function persistObservations(observations) {
  ...
  try {
    ...
    const { scanObservations } = await import("./schema-TZDA52RN.js");
...
async function persistSignals(signals) {
  ...
  try {
    const { getDb } = await import("./db-JLHOBMS4.js");
    const { scanSignals } = await import("./schema-TZDA52RN.js");
    const db = await getDb();
    if (!db) return 0;
    let inserted = 0;
...
async function persistRiskCards(cards) {
  ...
  try {
    const { getDb } = await import("./db-JLHOBMS4.js");
    const { scanRiskCards } = await import("./schema-TZDA52RN.js");
    const db = await getDb();
    if (!db) return 0;
    let inserted = 0;
