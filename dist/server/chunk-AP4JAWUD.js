...
async function indexMetasploitModules() {
  let count = 0;
  try {
    const { ingestMetasploitModules } = await import("./ttp-ingest-7AP5FV6T.js");
...
async function indexAc3History() {
  let count = 0;
  try {
    const { recordFeedback, getModulePerformance } = await import("./exploit-feedback-loop-KHMQLT7N.js");
