...
async function triggerScheduledScan(monitor) {
  ...
  try {
    ...
    const { detectSubdomainTakeover, crossReferenceTechVulnerabilities, detectSubdomainChanges } = await import("./domain-intel-advanced-R5MG4HLF.js");
...
function initScanScheduler() {
  ...
  setTimeout(async () => {
    try {
      const { startPeriodicHealthChecks } = await import("./health-monitor-EOAPLDN6.js");
...
function stopScanScheduler() {
  ...
  import("./health-monitor-EOAPLDN6.js").then(({ stopPeriodicHealthChecks }) => {
