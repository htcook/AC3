...
async function resolveUserTenant(userId, requestedTenantId) {
  ...
  const { tenantMemberships, tenants } = await import("./schema-R6EY37IN.js");
...
async function autoProvisionTenant(userId, userName) {
  const { getDb } = await import("./db-OF4HQS7N.js");
  const { tenants, tenantMemberships } = await import("./schema-R6EY37IN.js");
  const db = await getDb();
...
async function logTenantAction(userId, tenantId, action, details) {
  try {
    const { getDb } = await import("./db-OF4HQS7N.js");
    const { activityLogs } = await import("./schema-R6EY37IN.js");
    const db = await getDb();
    if (!db) return;
...
async function getTenantStats(tenantId) {
  const { getDb } = await import("./db-OF4HQS7N.js");
  const { tenantMemberships, engagements } = await import("./schema-R6EY37IN.js");
  const db = await getDb();
  if (!db) return null;
