...
async function triggerScheduledScan(monitor) {
  ...
  try {
    ...
    const { detectSubdomainTakeover, crossReferenceTechVulnerabilities, detectSubdomainChanges } = await import("./domain-intel-advanced-QEV24PT7.js");
...
function initScanScheduler() {
  ...
  setTimeout(async () => {
    try {
      const { startPeriodicHealthChecks } = await import("./health-monitor-ZZGD2JSA.js");
...
function stopScanScheduler() {
  ...
  import("./health-monitor-ZZGD2JSA.js").then(({ stopPeriodicHealthChecks }) => {
