...
async function executeEnumeration(state, engagement, operatorCtx) {
  const { executeEnumeration: runEnumerationPhase } = await import("./engagement-phase-enumeration-KG5KTWV2.js");
...
Total combined: ${combinedTechs.size} unique technologies`
  ...
  try {
    const { onBurpScanComplete } = await import("./burp-auto-scan-S2YM35KT.js");
  ...
  try {
    const { onEngagementVulnDetectionPhase, extractScopeUrls } = await import("./burp-auto-scan-S2YM35KT.js");
  ...
  try {
    const { runZapToBurpPipeline } = await import("./zap-burp-pipeline-T2WRRTDF.js");
  ...
  try {
    const { runSeverityEscalation } = await import("./zap-burp-pipeline-T2WRRTDF.js");
    ...
    for (const { targetUrl, dedupKey } of scanTargets) {
      ...
      try {
        ...
        if (!zapStarted) {
          ...
          try {
            const { onEngagementVulnDetectionPhase } = await import("./burp-auto-scan-S2YM35KT.js");
    ...
    if (initialResult && (initialResult.urlSource === "scope_fallback" || initialResult.zapUrlsDiscovered === 0)) {
      ...
      if (db) {
        ...
        if (completedScans.length > 0) {
          const { deferredZapBurpRefeed } = await import("./zap-burp-pipeline-T2WRRTDF.js");
...
Integrity: ${vulnVerifGate.passed ? "PASSED" : "FAILED"} (hash=${vulnVerifGate.contentHash.slice(0, 12)}...)`,
  ...
  if (highCritVulns.length > 0) {
    try {
      const { batchRunScanForgeReasoning } = await import("./scanforge-reasoning-QC7ZVQ4I.js");
...
async function executeExploitation(state, engagement, operatorCtx) {
  const { executeExploitation: runExploitPhase } = await import("./engagement-phase-exploitation-PSB6CCMW.js");
  return runExploitPhase(state, engagement, operatorCtx);
}
async function executePostExploit(state, engagement, operatorCtx) {
  const { executePostExploit: runPostExploitPhase } = await import("./engagement-phase-post-exploit-DNGQNM4P.js");
