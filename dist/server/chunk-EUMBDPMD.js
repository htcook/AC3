...
async function getActiveSourcesForStage(stage) {
  try {
    const { getActiveCustomerIntegrationsByStage } = await import("./db-PHFZ5GDL.js");
...
async function recordExecutionLogs(results, engagementId) {
  const { createExecutionLog } = await import("./db-PHFZ5GDL.js");
  for (const result of results) {
...
async function updateIntegrationStats(results) {
  const { updateCustomerIntegration, getCustomerIntegrationByIntegrationId } = await import("./db-PHFZ5GDL.js");
  for (const result of results) {
