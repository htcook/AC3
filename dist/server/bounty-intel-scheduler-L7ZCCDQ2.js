...
async function runBountyIntelPipeline(trigger = "manual") {
  ...
  try {
    ...
    await runStage(stages, "llm_extract_h1", async () => {
      const { extractFromHackerOneFindings } = await import("./bounty-training-engine-XSPGXUWE.js");
    ...
    await runStage(stages, "llm_extract_engagements", async () => {
      const { extractFromEngagementFindings } = await import("./bounty-training-engine-XSPGXUWE.js");
    ...
    await runStage(stages, "llm_enrich", async () => {
      const { enrichTrainingSamples } = await import("./bounty-training-engine-XSPGXUWE.js");
    ...
    await runStage(stages, "scanforge_templates", async () => {
      const { generateScanForgeTemplatesFromFindings } = await import("./bounty-training-engine-XSPGXUWE.js");
