...
async function triggerScheduledScan(monitor) {
  ...
  try {
    ...
    const { detectSubdomainTakeover, crossReferenceTechVulnerabilities, detectSubdomainChanges } = await import("./domain-intel-advanced-VVJRMLCE.js");
...
function initScanScheduler() {
  ...
  setTimeout(async () => {
    try {
      const { startPeriodicHealthChecks } = await import("./health-monitor-QYOQEC3F.js");
...
function stopScanScheduler() {
  ...
  import("./health-monitor-QYOQEC3F.js").then(({ stopPeriodicHealthChecks }) => {
