...
async function indexMetasploitModules() {
  let count = 0;
  try {
    const { ingestMetasploitModules } = await import("./ttp-ingest-VQS5BUOV.js");
...
async function indexAc3History() {
  let count = 0;
  try {
    const { recordFeedback, getModulePerformance } = await import("./exploit-feedback-loop-F5IFHXOP.js");
