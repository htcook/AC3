...
async function generateAutoReport(state, engagement, callbacks) {
  ...
  const { ac3Reports: reportsTable, ac3ReportFindings: findingsTable } = await import("./schema-TZDA52RN.js");
...
Top findings:
${allRptFindings.slice(0, 10).map((f) => `- [${f.rfSeverity}] ${f.rfTitle}`).join("\n")}
...
5. Executive narrative (2-3 paragraphs)`
  ...
  let gapsSummary = "";
  try {
    const { listGaps, formatGapsForReport } = await import("./intelligence-gaps-DTKSHN5I.js");
