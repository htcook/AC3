...
} from "./chunk-HAMUZ4DZ.js";
import {
  classifyVulnClass,
  init_exploit_learning_engine
} from "./chunk-LIU5PNIY.js";
import {
...
async function clearOpsState(engagementId) {
  ...
  try {
    const { deleteOpsSnapshot } = await import("./db-EEYUM2OC.js");
...
async function getOpsStateWithRecovery(engagementId) {
  ...
  try {
    const { loadOpsSnapshot } = await import("./db-EEYUM2OC.js");
...
function persistOpsStateDebounced(engagementId, delayMs = 2e3) {
  ...
  persistTimers.set(engagementId, setTimeout(async () => {
    ...
    if (!state) return;
    try {
      const { saveOpsSnapshot } = await import("./db-EEYUM2OC.js");
...
async function flushAllPendingState() {
  ...
  try {
    const { releaseAllClaims } = await import("./engagement-claim-lock-JNAJI2QL.js");
...
async function persistTimelineEvent(engagementId, logEntry) {
  try {
    ...
    const { engagementTimelineEvents } = await import("./schema-AEHUE7AH.js");
...
async function persistScanResult(opts) {
  try {
    const { insertScanResult, saveEngagementFindings } = await import("./db-EEYUM2OC.js");
...
async function auditLog(params) {
  try {
    const { getDb } = await import("./db-EEYUM2OC.js");
    const { offensiveAuditLog } = await import("./schema-AEHUE7AH.js");
    const db = await getDb();
    if (db) {
...
Generate two-phase scan plan. Phase A: ScanForge discovery --top-ports 1000. Phase B: tools per asset.`;
  ...
  try {
    ...
    try {
      const { getToolInventory, getInventoryForLLM } = await import("./scan-server-inventory-6FVYIU7J.js");
...
Use alternative tools instead.`;
  ...
  try {
    ...
    const { planAttack } = await import("./attack-planner-P55KAGKT.js");
...
async function llmDecide(context) {
  ...
  if (!skipSpecialist) try {
    const { decideNextOp } = await import("./ops-decider-NVCRIGIT.js");
...
${Object.entries(state.dastConfig.customHeaders).map(([k, v]) => `  ${k}: ${v}`).join("\n")}`
  ...
  for (const domain of domains) {
    try {
      ...
      const { isLabDomain } = await import("./passive-2A2AG76S.js");
      ...
      const { runDomainIntelPipeline } = await import("./domainIntel-KUGHE6I2.js");
      ...
      try {
        ...
        if (domainAssets.length > 0) {
          const { analyzeScan } = await import("./scan-analyst-4WPP5IQW.js");
...
async function executeEnumeration(state, engagement, operatorCtx) {
  const { executeEnumeration: runEnumerationPhase } = await import("./engagement-phase-enumeration-UZ3EQMYH.js");
...
async function executeVulnDetection(state, engagement, operatorCtx) {
  ...
  const { executeVulnPrep } = await import("./vuln-prep-JXVF7SAX.js");
  const vulnPrepCtx = {
    state,
    engagement,
    operatorCtx,
  ...
  const { executeZapScanning } = await import("./zap-scanner-TEJIT6OK.js");
  ...
  const { executeInjectionScanning } = await import("./injection-scanner-NFSQ53EV.js");
  ...
  const { executeCredentialTesting } = await import("./credential-tester-JVGU256T.js");
  ...
  const { executeVulnCorrelation } = await import("./vuln-correlation-TRE4TXAX.js");
...
async function executeExploitation(state, engagement, operatorCtx) {
  const { executeExploitation: runExploitPhase } = await import("./engagement-phase-exploitation-JCL5HPN5.js");
  return runExploitPhase(state, engagement, operatorCtx);
}
async function executePostExploit(state, engagement, operatorCtx) {
  const { executePostExploit: runPostExploitPhase } = await import("./engagement-phase-post-exploit-VMC64CFQ.js");
...
async function executeEngagement(engagementId, operatorCtx, options) {
  ...
  try {
    const { claimEngagement } = await import("./engagement-claim-lock-JNAJI2QL.js");
...
Preserved: ${state.assets.length} assets, ${state.stats.vulnsFound} vulns, ${state.stats.portsFound} ports, ${state.log.length} log entries`
  ...
  let engagement;
  try {
    const db = await import("./db-EEYUM2OC.js");
...
Max blast radius: ${engagementSafetyLevel === "passive_only" ? 5 : engagementSafetyLevel === "low_impact" ? 30 : engagementSafetyLevel === "standard" ? 60 : 100}`
  ...
  try {
    const { buildEngagementContext } = await import("./hybrid-scorer-5U6LQ3VG.js");
  ...
  try {
    const { setActiveUser } = await import("./bug-bounty-intelligence-I762ND2V.js");
  ...
  try {
    if (startPhase === "recon") {
      ...
      if (!reconGate.allowed) {
        ...
        await breathe();
        try {
          const { executeCustomerIntegrationsForStage, mergeIntegrationResultsIntoObservations } = await import("./pipeline-bridge-KQ2SE7IG.js");
    ...
    if (["recon", "passive_discovery"].includes(startPhase)) {
      try {
        ...
        await breathe();
        try {
          const { executeCustomerIntegrationsForStage } = await import("./pipeline-bridge-KQ2SE7IG.js");
    ...
    if (engagement.roeStatus === "signed" || engagement.roeStatus === "pending" || state.trainingLabMode === true) {
      if (["recon", "passive_discovery", "scoping", "test_plan", "enumeration"].includes(startPhase)) {
        ...
        if (!enumGate.allowed) {
          ...
          try {
            const { checkScanServerStatus } = await import("./scan-server-executor-YX4MKSRW.js");
        ...
        if (!vulnGate.allowed) {
          ...
          if (state.stats.vulnsFound > 0) {
            try {
              ...
              const { batchAnalyzeFindings, generateAnalysisSummary, classifyVulnerability } = await import("./vuln-analysis-agents-IKLBZAAX.js");
...
PoC: ${cf.analysis.poc || "N/A"}`,
        ...
        if (state.stats.vulnsFound > 0 || state.assets.some((a) => a.cloudProviders?.length > 0)) {
          try {
            ...
            const { runFeedbackLoop, getFeedbackLoopSummary } = await import("./llm-scan-feedback-JN2SK7JQ.js");
...
${feedbackState.finalAnalysis?.slice(0, 500) || ""}`,
        ...
        if (state.stats.vulnsFound > 0) {
          try {
            ...
            const { generateEngagementAttackChains } = await import("./cloud-attack-chain-designer-PN6DTT62.js");
...
Promoted: ${promotionSummary.promotedVulns.map((p) => `${p.vulnTitle.slice(0, 50)} [${p.scanner}/${p.category}]`).join("; ")}`,
        ...
        {
          const { executeSocialEngineering } = await import("./engagement-phase-social-engineering-SBYDLGPP.js");
      ...
      try {
        const { retryDeferredScans, getDeferredScans, clearDeferredScans } = await import("./job-queue-bridge-6VBHY3Q6.js");
      ...
      try {
        const { selectFindingsForScreenshot, captureScreenshotBatch } = await import("./screenshot-capture-ELT65B65.js");
      ...
      try {
        const { generateAttackNarratives, generateExecutiveSummary } = await import("./attack-narrative-generator-OWCSPKYV.js");
      ...
      try {
        const { runAccuracyComparison } = await import("./accuracy-feedback-loop-D5P5GJZI.js");
        ...
        if (targetPreset) {
          ...
          let engNotes = null;
          try {
            const { engagements: engTable } = await import("./schema-AEHUE7AH.js");
...
View details on the Knowledge Base \u2192 Accuracy Feedback tab.`
          ...
          try {
            const { runLocalAccuracyComparison } = await import("./accuracy-feedback-loop-D5P5GJZI.js");
    ...
    try {
      const { generateAutoReport } = await import("./engagement-auto-report-KJ4DLTEU.js");
    ...
    try {
      ...
      broadcastOpsUpdate(state.engagementId, { type: "log_update" });
      const { generateTestPlanAdherence } = await import("./engagement-report-handoff-WTUOB7AM.js");
    ...
    try {
      const { updateEngagement } = await import("./db-EEYUM2OC.js");
    ...
    try {
      const { saveEngagementResult, saveEngagementFindings } = await import("./db-EEYUM2OC.js");
    ...
    try {
      const { getEngagementLlmTelemetryRaw } = await import("./db-EEYUM2OC.js");
    ...
    try {
      const { runPostPipelineGraduation, extractEngagementMetrics } = await import("./post-pipeline-graduation-KSLNPHJQ.js");
    ...
    try {
      const { detectGaps, createGapsBatch } = await import("./intelligence-gaps-TBRUK2YI.js");
    ...
    try {
      const { updateProfileFromEngagement } = await import("./customer-intel-profile-LESQIEEQ.js");
    ...
    try {
      const { updateEngagement } = await import("./db-EEYUM2OC.js");
      const existingEng = await (await import("./db-EEYUM2OC.js")).getEngagementById(engagementId);
...
async function recoverInterruptedEngagements() {
  ...
  try {
    const { getDbRequired } = await import("./db-EEYUM2OC.js");
    const db = await getDbRequired();
    const { engagementOpsSnapshots } = await import("./schema-AEHUE7AH.js");
