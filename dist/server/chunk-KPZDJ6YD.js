...
import {
  init_llm_throttle,
  throttledLLMCall
} from "./chunk-ZLYIEM35.js";
import {
  executeToolViaQueue,
  init_job_queue_bridge
} from "./chunk-BFQHW7GG.js";
import {
...
} from "./chunk-MYN3IICF.js";
import {
  classifyVulnClass,
  init_exploit_learning_engine
} from "./chunk-3DJKSSV7.js";
import {
...
async function clearOpsState(engagementId) {
  ...
  try {
    const { deleteOpsSnapshot } = await import("./db-OF4HQS7N.js");
...
async function getOpsStateWithRecovery(engagementId) {
  ...
  try {
    const { loadOpsSnapshot } = await import("./db-OF4HQS7N.js");
...
function persistOpsStateDebounced(engagementId, delayMs = 2e3) {
  ...
  persistTimers.set(engagementId, setTimeout(async () => {
    ...
    if (!state) return;
    try {
      const { saveOpsSnapshot } = await import("./db-OF4HQS7N.js");
...
async function flushAllPendingState() {
  ...
  try {
    const { releaseAllClaims } = await import("./engagement-claim-lock-GCFVIZSF.js");
...
async function persistTimelineEvent(engagementId, logEntry) {
  try {
    ...
    const { engagementTimelineEvents } = await import("./schema-R6EY37IN.js");
...
async function persistScanResult(opts) {
  try {
    const { insertScanResult, saveEngagementFindings } = await import("./db-OF4HQS7N.js");
...
async function auditLog(params) {
  try {
    const { getDb } = await import("./db-OF4HQS7N.js");
    const { offensiveAuditLog } = await import("./schema-R6EY37IN.js");
    const db = await getDb();
    if (db) {
...
Generate two-phase scan plan. Phase A: ScanForge discovery --top-ports 1000. Phase B: tools per asset.`;
  ...
  try {
    ...
    try {
      const { getToolInventory, getInventoryForLLM } = await import("./scan-server-inventory-CRJCH2TU.js");
...
Use alternative tools instead.`;
  ...
  try {
    ...
    const { planAttack } = await import("./attack-planner-2C5NQ6VX.js");
...
async function llmDecide(context) {
  ...
  if (!skipSpecialist) try {
    const { decideNextOp } = await import("./ops-decider-KWVU6GAL.js");
...
${Object.entries(state.dastConfig.customHeaders).map(([k, v]) => `  ${k}: ${v}`).join("\n")}`
  ...
  for (const domain of domains) {
    try {
      ...
      const { isLabDomain } = await import("./passive-WGTCS46T.js");
      ...
      const { runDomainIntelPipeline } = await import("./domainIntel-X5H3TVWO.js");
      ...
      try {
        ...
        if (domainAssets.length > 0) {
          const { analyzeScan } = await import("./scan-analyst-TFXVGCQM.js");
...
${(analysis.recommendations || []).slice(0, 3).map((r) => `\u2022 [${r.priority}] ${r.action}`).join("\n")}`,
      ...
      try {
        const domainAssets = state.assets.filter((a) => a.hostname === domain || a.hostname.endsWith("." + domain));
        if (domainAssets.length > 0) {
          const { scoreFullHybrid, buildEngagementContext } = await import("./hybrid-scorer-R5POGUVH.js");
...
async function executeEnumeration(state, engagement, operatorCtx) {
  const { executeEnumeration: runEnumerationPhase } = await import("./engagement-phase-enumeration-O56BNR4H.js");
...
async function executeVulnDetection(state, engagement, operatorCtx) {
  ...
  const { executeVulnPrep } = await import("./vuln-prep-HUIJ3DLM.js");
  const vulnPrepCtx = {
    state,
    engagement,
    operatorCtx,
  ...
  const { executeZapScanning } = await import("./zap-scanner-46XBO763.js");
  ...
  const { executeInjectionScanning } = await import("./injection-scanner-DUPYAHSV.js");
  ...
  const { executeCredentialTesting } = await import("./credential-tester-AT5BSKHW.js");
  ...
  const { executeVulnCorrelation } = await import("./vuln-correlation-Y3J64CYR.js");
...
async function executeExploitation(state, engagement, operatorCtx) {
  const { executeExploitation: runExploitPhase } = await import("./engagement-phase-exploitation-RCIG2YSM.js");
  return runExploitPhase(state, engagement, operatorCtx);
}
async function executePostExploit(state, engagement, operatorCtx) {
  const { executePostExploit: runPostExploitPhase } = await import("./engagement-phase-post-exploit-P5UJ2EDC.js");
...
async function executeEngagement(engagementId, operatorCtx, options) {
  ...
  try {
    const { claimEngagement } = await import("./engagement-claim-lock-GCFVIZSF.js");
...
Preserved: ${state.assets.length} assets, ${state.stats.vulnsFound} vulns, ${state.stats.portsFound} ports, ${state.log.length} log entries`
  ...
  let engagement;
  try {
    const db = await import("./db-OF4HQS7N.js");
...
Max blast radius: ${engagementSafetyLevel === "passive_only" ? 5 : engagementSafetyLevel === "low_impact" ? 30 : engagementSafetyLevel === "standard" ? 60 : 100}`
  ...
  try {
    const { buildEngagementContext } = await import("./hybrid-scorer-R5POGUVH.js");
  ...
  try {
    const { setActiveUser } = await import("./bug-bounty-intelligence-NRVE7VYK.js");
  ...
  try {
    if (startPhase === "recon") {
      ...
      if (!reconGate.allowed) {
        ...
        await breathe();
        try {
          const { executeCustomerIntegrationsForStage, mergeIntegrationResultsIntoObservations } = await import("./pipeline-bridge-R67MQQAE.js");
    ...
    if (["recon", "passive_discovery"].includes(startPhase)) {
      try {
        ...
        await breathe();
        try {
          const { executeCustomerIntegrationsForStage } = await import("./pipeline-bridge-R67MQQAE.js");
    ...
    if (engagement.roeStatus === "signed" || engagement.roeStatus === "pending" || state.trainingLabMode === true) {
      if (["recon", "passive_discovery", "scoping", "test_plan", "enumeration"].includes(startPhase)) {
        ...
        if (!enumGate.allowed) {
          ...
          try {
            const { checkScanServerStatus } = await import("./scan-server-executor-IU64YSBG.js");
        ...
        if (!vulnGate.allowed) {
          ...
          if (state.stats.vulnsFound > 0) {
            try {
              ...
              const { batchAnalyzeFindings, generateAnalysisSummary, classifyVulnerability } = await import("./vuln-analysis-agents-LBNQUGUY.js");
...
PoC: ${cf.analysis.poc || "N/A"}`,
        ...
        if (state.stats.vulnsFound > 0 || state.assets.some((a) => a.cloudProviders?.length > 0)) {
          try {
            ...
            const { runFeedbackLoop, getFeedbackLoopSummary } = await import("./llm-scan-feedback-MJA4SRWW.js");
...
${feedbackState.finalAnalysis?.slice(0, 500) || ""}`,
        ...
        if (state.stats.vulnsFound > 0) {
          try {
            ...
            const { generateEngagementAttackChains } = await import("./cloud-attack-chain-designer-36UYQ72T.js");
...
Promoted: ${promotionSummary.promotedVulns.map((p) => `${p.vulnTitle.slice(0, 50)} [${p.scanner}/${p.category}]`).join("; ")}`,
        ...
        {
          const { executeSocialEngineering } = await import("./engagement-phase-social-engineering-MR3TASX7.js");
      ...
      try {
        const { retryDeferredScans, getDeferredScans, clearDeferredScans } = await import("./job-queue-bridge-NDO2Q2BK.js");
      ...
      try {
        const { selectFindingsForScreenshot, captureScreenshotBatch } = await import("./screenshot-capture-OF4FIUBH.js");
      ...
      try {
        const { generateAttackNarratives, generateExecutiveSummary } = await import("./attack-narrative-generator-FXAZXP2P.js");
      ...
      try {
        const { runAccuracyComparison } = await import("./accuracy-feedback-loop-W4DWJ6XJ.js");
        ...
        if (targetPreset) {
          ...
          let engNotes = null;
          try {
            const { engagements: engTable } = await import("./schema-R6EY37IN.js");
            const { getDbRequired: getDbReq } = await import("./db-OF4HQS7N.js");
...
View details on the Knowledge Base \u2192 Accuracy Feedback tab.`
          ...
          try {
            const { runLocalAccuracyComparison } = await import("./accuracy-feedback-loop-W4DWJ6XJ.js");
    ...
    try {
      const { generateAutoReport } = await import("./engagement-auto-report-7XPEMUHG.js");
    ...
    try {
      ...
      broadcastOpsUpdate(state.engagementId, { type: "log_update" });
      const { generateTestPlanAdherence } = await import("./engagement-report-handoff-RLSVA5PL.js");
    ...
    try {
      const { updateEngagement } = await import("./db-OF4HQS7N.js");
    ...
    try {
      const { saveEngagementResult, saveEngagementFindings } = await import("./db-OF4HQS7N.js");
    ...
    try {
      const { getEngagementLlmTelemetryRaw } = await import("./db-OF4HQS7N.js");
    ...
    try {
      const { runPostPipelineGraduation, extractEngagementMetrics } = await import("./post-pipeline-graduation-LMW24JCT.js");
    ...
    try {
      const { detectGaps, createGapsBatch } = await import("./intelligence-gaps-NYJN4LRT.js");
    ...
    try {
      const { updateProfileFromEngagement } = await import("./customer-intel-profile-5MQEBZCQ.js");
    ...
    try {
      const { updateEngagement } = await import("./db-OF4HQS7N.js");
      const existingEng = await (await import("./db-OF4HQS7N.js")).getEngagementById(engagementId);
...
async function recoverInterruptedEngagements() {
  ...
  try {
    const { getDbRequired } = await import("./db-OF4HQS7N.js");
    const db = await getDbRequired();
    const { engagementOpsSnapshots } = await import("./schema-R6EY37IN.js");
