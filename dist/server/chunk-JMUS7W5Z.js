...
async function executeToolViaQueue(config, options) {
  ...
  if (!useQueue) {
    if (!options?.forceLocal) {
      try {
        const { executeToolViaHttp } = await import("./do-scan-api-GSFXOMYW.js");
...
async function executeRawCommandViaQueue(command, timeoutSeconds = 300, options) {
  ...
  if (!options?.forceLocal) {
    try {
      ...
      const { executeRawCommandViaHttp } = await import("./do-scan-api-GSFXOMYW.js");
  ...
  const { executeViaChildProcessSSH } = await import("./scan-server-executor-MSPK4KCH.js");
