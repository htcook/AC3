...
async function persistObservations(observations) {
  ...
  try {
    ...
    const { scanObservations } = await import("./schema-RL5B6OMI.js");
...
async function persistSignals(signals) {
  ...
  try {
    const { getDb } = await import("./db-UO2A3C4N.js");
    const { scanSignals } = await import("./schema-RL5B6OMI.js");
    const db = await getDb();
    if (!db) return 0;
    let inserted = 0;
...
async function persistRiskCards(cards) {
  ...
  try {
    const { getDb } = await import("./db-UO2A3C4N.js");
    const { scanRiskCards } = await import("./schema-RL5B6OMI.js");
    const db = await getDb();
    if (!db) return 0;
    let inserted = 0;
