...
async function executeToolViaQueue(config, options) {
  ...
  if (!useQueue) {
    if (!options?.forceLocal) {
      try {
        const { executeToolViaHttp } = await import("./do-scan-api-7IEEU6VL.js");
...
async function executeRawCommandViaQueue(command, timeoutSeconds = 300, options) {
  ...
  if (!options?.forceLocal) {
    try {
      ...
      const { executeRawCommandViaHttp } = await import("./do-scan-api-7IEEU6VL.js");
  ...
  const { executeViaChildProcessSSH } = await import("./scan-server-executor-EX3LSZL7.js");
...
async function retryDeferredScans(engagementId, options) {
  ...
  try {
    const { isDedicatedHealthy } = await import("./scan-service-url-5PU2PCMP.js");
