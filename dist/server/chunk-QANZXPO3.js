...
async function getActiveSourcesForStage(stage) {
  try {
    const { getActiveCustomerIntegrationsByStage } = await import("./db-OF4HQS7N.js");
...
async function recordExecutionLogs(results, engagementId) {
  const { createExecutionLog } = await import("./db-OF4HQS7N.js");
  for (const result of results) {
...
async function updateIntegrationStats(results) {
  const { updateCustomerIntegration, getCustomerIntegrationByIntegrationId } = await import("./db-OF4HQS7N.js");
  for (const result of results) {
