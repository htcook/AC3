...
async function resolveUserTenant(userId, requestedTenantId) {
  ...
  const { tenantMemberships, tenants } = await import("./schema-XOTPZHKC.js");
...
async function autoProvisionTenant(userId, userName) {
  const { getDb } = await import("./db-MOHZQFM5.js");
  const { tenants, tenantMemberships } = await import("./schema-XOTPZHKC.js");
  const db = await getDb();
...
async function logTenantAction(userId, tenantId, action, details) {
  try {
    const { getDb } = await import("./db-MOHZQFM5.js");
    const { activityLogs } = await import("./schema-XOTPZHKC.js");
    const db = await getDb();
    if (!db) return;
...
async function getTenantStats(tenantId) {
  const { getDb } = await import("./db-MOHZQFM5.js");
  const { tenantMemberships, engagements } = await import("./schema-XOTPZHKC.js");
  const db = await getDb();
  if (!db) return null;
