...
} from "./chunk-JGSIUMJS.js";
import "./chunk-LFPYJDEH.js";
...
Total combined: ${combinedTechs.size} unique technologies`
  ...
  try {
    const { onBurpScanComplete } = await import("./burp-auto-scan-UGIQDRH5.js");
    onBurpScanComplete(async (burpConfig, burpState) => {
      ...
      if (normalizedFindings && normalizedFindings.length > 0) {
        try {
          const { saveEngagementFindings } = await import("./db-UO2A3C4N.js");
  ...
  try {
    const { getEngagementCredentials } = await import("./credential-harvester-YVAGOQUE.js");
  ...
  try {
    const { onEngagementVulnDetectionPhase, extractScopeUrls } = await import("./burp-auto-scan-UGIQDRH5.js");
  ...
  try {
    const { runZapToBurpPipeline } = await import("./zap-burp-pipeline-V2NUXSJ4.js");
  ...
  try {
    const { runSeverityEscalation } = await import("./zap-burp-pipeline-V2NUXSJ4.js");
