...
async function resolveUserTenant(userId, requestedTenantId) {
  ...
  const { tenantMemberships, tenants } = await import("./schema-J7P4MLKR.js");
...
async function autoProvisionTenant(userId, userName) {
  const { getDb } = await import("./db-2N7OEZBU.js");
  const { tenants, tenantMemberships } = await import("./schema-J7P4MLKR.js");
  const db = await getDb();
...
async function logTenantAction(userId, tenantId, action, details) {
  try {
    const { getDb } = await import("./db-2N7OEZBU.js");
    const { activityLogs } = await import("./schema-J7P4MLKR.js");
    const db = await getDb();
    if (!db) return;
...
async function getTenantStats(tenantId) {
  const { getDb } = await import("./db-2N7OEZBU.js");
  const { tenantMemberships, engagements } = await import("./schema-J7P4MLKR.js");
  const db = await getDb();
  if (!db) return null;
