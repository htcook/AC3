...
async function enrichFromThreatIntel(hosts) {
  try {
    const { enrichThreatIntelligence } = await import("./bug-bounty-intelligence-W5MKQB7D.js");
...
async function enrichFromOpSec(hosts) {
  try {
    const { enrichOpSec } = await import("./bug-bounty-intelligence-W5MKQB7D.js");
...
async function runDiscoveryPipeline(targets, config, onProgress) {
  ...
  try {
    ...
    const { createCarverRiskCardsBatch } = await import("./db-F33RXQPM.js");
