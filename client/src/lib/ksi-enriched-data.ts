/**
 * KSI Enriched Data — Comprehensive mapping of all 75 FedRAMP 20x Key Security Indicators
 * to Ace C3 capabilities, with detailed descriptions of how each is met or supported.
 *
 * Usage:
 *   import { getKsiEnriched, getThemeEnriched, ALL_ENRICHED_KSIS, ALL_ENRICHED_THEMES } from "@/lib/ksi-enriched-data";
 *   const ksi = getKsiEnriched("KSI-AFR-ADS");
 *   const theme = getThemeEnriched("AFR");
 */
...
export interface EnrichedKSI {
  ...
  requirement: string;
  /** How Ace C3 meets or supports this KSI — the self-explanatory description */
  howAceC3Delivers: string;
  /** Specific Ace C3 modules involved, with per-module role descriptions */
  aceModules: AceC3Module[];
  /** Types of evidence Ace C3 generates for this KSI */
  evidenceTypes: string[];
  /** Validation approach: machine, human, mixed, or n/a */
  validationMethod: string;
  /** Recommended validation frequency */
  frequency: string;
  /** NIST SP 800-53 control families this KSI maps to */
  nistControls: string[];
}
...
export interface EnrichedTheme {
  ...
  name: string;
  description: string;
  /** Overall narrative of how Ace C3 addresses this theme */
  aceC3Narrative: string;
  ksiIds: string[];
}

// ─── Theme Data ─────────────────────────────────────────────────────

export const ALL_ENRICHED_THEMES: EnrichedTheme[] = [
  {
    code: "AFR",
    ...
    aceC3Narrative: "Ace C3 provides continuous compliance monitoring through the KSI Dashboard with automated validation scheduling, OSCAL-formatted evidence export for machine-readable data sharing, and full penetration testing across all 6 mandatory attack vectors to satisfy periodic vulnerability assessment requirements. The platform generates tamper-resistant evidence chains with SHA-256 integrity hashing, maintains version-controlled rules of engagement, and produces AI-powered assessment reports — covering the technical evidence generation that CSPs need for FedRAMP authorization.",
    ksiIds: ["KSI-AFR-ADS", "KSI-AFR-CCM", "KSI-AFR-FSI", "KSI-AFR-ICP", "KSI-AFR-MAS", "KSI-AFR-PVA", "KSI-AFR-SCG", "KSI-AFR-SCN"],
  },
  {
    code: "CMT",
    ...
    aceC3Narrative: "Ace C3 records all security-relevant changes through its Audit Log with timestamps and user attribution, triggers post-deployment security validation through the Validation Scheduler, and tests detection coverage after changes using Agentless BAS and ATT&CK Validation Tests. The platform supports change management governance through version-controlled RoE documentation and scope enforcement, while Config Baseline Engine detects drift from approved configurations.",
    ksiIds: ["KSI-CMT-LMC", "KSI-CMT-RMV", "KSI-CMT-RVP", "KSI-CMT-VTD", "KSI-CMT-CMG"],
  },
  {
    code: "CNA",
    ...
    aceC3Narrative: "Ace C3 continuously discovers the external attack surface through Domain Intel with Shodan, Censys, and SecurityTrails integrations. DigitalOcean infrastructure auditing validates cloud configurations across droplets, firewalls, load balancers, and databases. ZAP DAST validates TLS/encryption configurations, NGFW Validation tests network segmentation and firewall rules, and Cloud Attack Paths maps permission chains to identify architectural weaknesses.",
    ksiIds: ["KSI-CNA-DFP", "KSI-CNA-EDE", "KSI-CNA-MAS", "KSI-CNA-OFA", "KSI-CNA-RNT", "KSI-CNA-RVP", "KSI-CNA-SBD", "KSI-CNA-ULN", "KSI-CNA-HCI", "KSI-CNA-NSD"],
  },
  {
    code: "CED",
    ...
    aceC3Narrative: "Ace C3 measures security awareness through realistic phishing campaigns with 17 exploit techniques including AiTM, BITB, device code phishing, and HTML smuggling. Purple Team exercises provide hands-on incident response training with measurable detection and response metrics. AD Attack Simulation targets privileged users for high-risk role training validation. DAST findings inform developer security training priorities.",
    ksiIds: ["KSI-CED-RGT", "KSI-CED-RRT", "KSI-CED-RST", "KSI-CED-DET"],
  },
  {
    code: "IAM",
    ...
    aceC3Narrative: "Ace C3 tests MFA resilience through real phishing campaigns with 17 MFA bypass techniques, validates privileged access controls through AD Attack Simulation, identifies excessive permissions through Cloud Attack Paths, and tests password policies through the Credential Attack Engine. AD Domain Connector enumerates accounts, group memberships, and privileged access for comprehensive IAM assessment.",
    ksiIds: ["KSI-IAM-MFA", "KSI-IAM-APM", "KSI-IAM-ELP", "KSI-IAM-PRA", "KSI-IAM-AAM", "KSI-IAM-JIT", "KSI-IAM-SNU", "KSI-IAM-SUS"],
  },
  {
    code: "INR",
    ...
    aceC3Narrative: "Ace C3 generates comprehensive post-engagement reports with MITRE ATT&CK heatmaps and remediation recommendations. Purple Team exercises test IR procedures with measurable detection and response metrics. Real threat intelligence feeds from abuse.ch, Shodan, SecurityTrails, and DeHashed provide continuous threat data. IOC Feed manages indicators with automated enrichment, and Threat Actor Matcher correlates findings with known threat actor TTPs.",
    ksiIds: ["KSI-INR-AAR", "KSI-INR-RIR", "KSI-INR-RPI", "KSI-INR-IRP", "KSI-INR-TIF", "KSI-INR-TIU", "KSI-INR-IOC"],
  },
  {
    code: "MLA",
    ...
    aceC3Narrative: "Ace C3 validates SIEM detection coverage through SIEM Connectors and Feedback Loop integration, auto-generates Sigma/YARA/Suricata detection rules from executed TTPs via the Detection Rule Generator, tests alert quality through the SIEM Mutation Engine, and measures detection coverage against MITRE techniques through the ATT&CK Coverage Matrix. Config Baseline Engine and SCAP Compliance Scanner evaluate security configurations against approved baselines.",
    ksiIds: ["KSI-MLA-OSM", "KSI-MLA-LET", "KSI-MLA-EVC", "KSI-MLA-ALE", "KSI-MLA-ALA", "KSI-MLA-RVL"],
  },
  {
    code: "PIY",
    ...
    aceC3Narrative: "Ace C3 generates real-time asset inventories through Domain Intel, Service Fingerprinting, and Web Crawler. Risk Trending and the Scoring Engine (CARVER+Shock/CVSS) provide quantifiable security investment ROI metrics. Bug Bounty Intelligence integrates HackerOne program data for vulnerability disclosure program oversight. DAST and API Security Testing validate SDLC security outputs.",
    ksiIds: ["KSI-PIY-GIV", "KSI-PIY-RIS", "KSI-PIY-RSD", "KSI-PIY-RVD", "KSI-PIY-RES"],
  },
  {
    code: "RPL",
    name: "Recovery Planning",
    description: "Recovery plan alignment, RTO/RPO objectives, recovery capability testing, and disaster recovery planning.",
    aceC3Narrative: "Ace C3's Config Baseline Engine can track whether recovery configurations align with stated objectives. Dedicated recovery validation testing with automated failover exercises is planned for future development. Recovery planning is primarily an organizational process outside the scope of an offensive security platform, but Ace C3 can verify configuration alignment.",
    ksiIds: ["KSI-RPL-ARP", "KSI-RPL-ABO", "KSI-RPL-RRO", "KSI-RPL-TRC"],
  },
  {
    code: "SVC",
    ...
    aceC3Narrative: "Ace C3 provides comprehensive service configuration security through continuous attack surface management (Domain Intel + Shodan + Censys), real vulnerability scanning (ZAP DAST + Nuclei + scanner imports), risk-based vulnerability prioritization (CARVER+Shock/CVSS scoring with temporal decay), and full vulnerability lifecycle management including remediation verification and risk trending. DigitalOcean Infrastructure Audit validates cloud endpoint configurations.",
    ksiIds: ["KSI-SVC-ASM", "KSI-SVC-VSR", "KSI-SVC-VRI", "KSI-SVC-VCM", "KSI-SVC-VRM", "KSI-SVC-EIS", "KSI-SVC-ACM", "KSI-SVC-PRR", "KSI-SVC-SNT"],
  },
  {
    code: "SCR",
    ...
    aceC3Narrative: "Ace C3 delivers the core FedRAMP 20x penetration testing requirement through its multi-phase Engagement Pipeline with 16,000+ Metasploit modules. Adversary emulation with MITRE ATT&CK technique execution provides APT simulation. Phishing Ops with GoPhish integration delivers security awareness testing. NVD/KEV integration and Container Registry Scanner provide third-party vulnerability monitoring.",
    ksiIds: ["KSI-SCR-PEN", "KSI-SCR-APT", "KSI-SCR-SAT", "KSI-SCR-MON", "KSI-SCR-MIT"],
  },
  {
    code: "SDE",
    name: "Secure Development",
    description: "Secure software testing and secure development practices validation.",
    aceC3Narrative: "Ace C3 performs automated security testing of developed software through ZAP DAST for web application testing, API Security Testing for authentication and injection validation, and Nuclei Scanner for template-based security checks. DAST findings and vulnerability trend analysis inform secure development priorities by identifying common vulnerability classes.",
    ksiIds: ["KSI-SDE-SST", "KSI-SDE-SDP"],
  },
  {
    code: "PPM",
    name: "Policy & Procedure Management",
    description: "Policy and procedure review and implementation verification.",
    aceC3Narrative: "Ace C3 supports policy documentation through version-controlled Rules of Engagement (RoE Builder) and tamper-resistant Evidence Chain storage. Scope Enforcement Middleware and Scan Policy Engine demonstrate policy implementation within the platform's operational scope. The platform supports policy documentation but organizational policy management remains outside its scope.",
    ksiIds: ["KSI-PPM-PPR", "KSI-PPM-PPI"],
  },
];

// ─── KSI Data ───────────────────────────────────────────────────────

export const ALL_ENRICHED_KSIS: EnrichedKSI[] = [
  // ═══ AFR — Authorization by FedRAMP ═══════════════════════════════
  {
    ...
    aceModules: [
      ...
      { name: "Evidence Chain", role: "Maintains tamper-resistant audit trails with SHA-256 integrity hashing for data authenticity verification" },
    ],
    evidenceTypes: ["OSCAL assessment packages", "STIX threat intelligence bundles", "SHA-256 hash-chained evidence records"],
    validationMethod: "machine",
  ...
  },
  {
    ...
    aceModules: [
      ...
      { name: "Auto-Collectors", role: "Gathers evidence from live API integrations (Shodan, Censys, abuse.ch, NVD) for continuous monitoring" },
    ],
    evidenceTypes: ["KSI posture scores", "Validation run results", "Auto-collected evidence records", "Overdue alerts"],
    validationMethod: "machine",
    frequency: "continuous",
  ...
  },
  {
    ...
    evidenceTypes: ["Notification logs"],
  ...
  },
  {
    ...
    evidenceTypes: ["Initial posture assessment", "NIST control mapping documentation", "Evidence chain integrity records"],
    validationMethod: "mixed",
    frequency: "initial + annual",
    nistControls: ["CA-2", "PL-2"],
  },
  {
    ...
    aceModules: [
      ...
      { name: "Scope Guard", role: "Validates every scan target against approved boundaries before execution to prevent out-of-scope testing" },
    ],
    evidenceTypes: ["RoE documentation", "Scope boundary definitions", "Assessment coverage reports", "Scope violation logs"],
    validationMethod: "mixed",
  ...
  },
  {
    ...
    aceModules: [
      ...
      { name: "Vuln Scanner Import", role: "Normalizes Nessus, Qualys, and Burp Suite results into a unified vulnerability format" },
      { name: "Engagement Pipeline", role: "Orchestrates periodic assessment cycles with configurable scheduling and multi-phase execution" },
    ],
    evidenceTypes: ["DAST scan results", "Nuclei scan findings", "Normalized scanner reports", "Assessment timeline records"],
    validationMethod: "machine",
    frequency: "monthly",
    nistControls: ["RA-5", "CA-8"],
  },
  {
    ...
    howAceC3Delivers: "Ace C3's Config Baseline Engine tracks security configurations against approved baselines and detects drift from expected states. The SCAP Compliance Scanner checks CIS benchmarks for infrastructure security posture. These tools inform secure configuration guide development and validate compliance, but do not auto-generate the configuration guides themselves.",
    aceModules: [
    ...
    evidenceTypes: ["Configuration baseline reports", "CIS benchmark compliance scores", "Drift detection alerts"],
    validationMethod: "machine",
    frequency: "weekly",
    nistControls: ["CM-6", "CM-2"],
  },
  {
    ...
    aceModules: [
      ...
      { name: "Evidence Chain", role: "Provides tamper-resistant change records with SHA-256 integrity hashing" },
    ],
    evidenceTypes: ["Audit log entries", "Configuration drift alerts", "Integrity-verified change records"],
    validationMethod: "machine",
    frequency: "continuous",
    nistControls: ["CM-3", "CM-4"],
  },
  ...
  // ═══ CMT — Change Management ══════════════════════════════════════
  {
    ...
    evidenceTypes: ["Audit log entries", "Integrity-verified modification records", "Change timeline reports"],
    validationMethod: "machine",
    frequency: "continuous",
    nistControls: ["CM-3", "AU-2"],
  },
  {
    ...
    evidenceTypes: ["Deployment configuration compliance reports", "Baseline drift history", "Deployment event logs"],
    validationMethod: "machine",
    frequency: "per-deployment",
    nistControls: ["CM-2", "CM-3"],
  },
  {
    ...
    evidenceTypes: ["Procedure documentation", "Version history records", "Procedure review analysis"],
  ...
  },
  {
    ...
    aceModules: [
      ...
      { name: "ATT&CK Validation Tests", role: "Confirms security controls detect known attack techniques after deployment" },
    ],
    evidenceTypes: ["Post-deployment validation results", "BAS detection coverage reports", "ATT&CK technique test results"],
    validationMethod: "machine",
    frequency: "per-deployment",
    nistControls: ["CM-4", "CA-2"],
  },
  {
    ...
    aceModules: [
      ...
      { name: "Scope Guard", role: "Enforces operational boundaries during testing, demonstrating governance enforcement" },
    ],
    evidenceTypes: ["Governance documentation", "Audit trail records", "Scope enforcement logs"],
    validationMethod: "human",
    frequency: "quarterly",
    nistControls: ["CM-1", "CM-9"],
  },
  ...
  // ═══ CNA — Cloud Native Architecture ══════════════════════════════
  {
    ...
    evidenceTypes: ["Permission chain maps", "Privilege escalation test results", "Over-privileged role reports"],
    validationMethod: "machine",
    frequency: "monthly",
  ...
  },
  {
    ...
    evidenceTypes: ["TLS configuration test results", "Cipher suite analysis", "Certificate validity reports", "Email security scores"],
    validationMethod: "machine",
    frequency: "weekly",
    nistControls: ["SC-13", "SC-8"],
  },
  {
    ...
    aceModules: [
      ...
      { name: "Shodan Integration", role: "Provides internet-wide scanning data for exposed services and banners" },
      { name: "Censys Integration", role: "Discovers certificates, hosts, and services through certificate transparency logs" },
    ],
    evidenceTypes: ["Attack surface discovery reports", "Subdomain enumeration", "Service exposure maps", "Technology stack inventories"],
    validationMethod: "machine",
    frequency: "weekly",
    nistControls: ["CM-7", "RA-5"],
  },
  {
    ...
    evidenceTypes: ["Configuration compliance reports"],
    validationMethod: "n/a",
    frequency: "n/a",
    nistControls: ["CP-10", "SC-5"],
  },
  {
    ...
    evidenceTypes: ["Firewall rule audit reports", "Port exposure analysis", "NGFW validation results"],
    validationMethod: "machine",
    frequency: "weekly",
  ...
  },
  {
    ...
    evidenceTypes: ["DoS protection configuration reports"],
    validationMethod: "n/a",
    frequency: "n/a",
    nistControls: ["SC-5"],
  },
  {
    ...
    evidenceTypes: ["Architecture weakness reports", "Permission chain analysis", "CIS benchmark compliance scores"],
    validationMethod: "mixed",
    frequency: "monthly",
    nistControls: ["SA-8", "SC-7"],
  },
  {
    ...
    evidenceTypes: ["Network segmentation test results", "Security group audit reports", "Egress rule validation"],
    validationMethod: "machine",
    frequency: "weekly",
    nistControls: ["SC-7", "AC-4"],
  },
  {
    ...
    evidenceTypes: ["Infrastructure audit reports", "Misconfiguration findings", "Hardening compliance scores", "Resource-level security assessments"],
    validationMethod: "machine",
    frequency: "weekly",
    nistControls: ["CM-6", "SC-7"],
  },
  {
    ...
    howAceC3Delivers: "NGFW Validation tests network segmentation boundaries by attempting to traverse them, proving whether segmentation actually prevents unauthorized access. Service Fingerprinting identifies services that may cross segmentation boundaries, revealing potential segmentation violations. Ace C3 tests segmentation effectiveness but does not implement segmentation controls.",
    aceModules: [
    ...
    evidenceTypes: ["Segmentation boundary test results", "Cross-boundary service discovery", "Lateral movement test results"],
    validationMethod: "machine",
    frequency: "monthly",
    nistControls: ["SC-7", "AC-4"],
  },
  ...
  // ═══ CED — Cybersecurity Education ════════════════════════════════
  {
    ...
    aceModules: [
      ...
      { name: "Template Generator", role: "Creates realistic phishing templates based on current threat landscape and organizational context" },
    ],
    evidenceTypes: ["Phishing campaign results", "Click rate metrics", "Credential capture rates", "Improvement trend analysis"],
    validationMethod: "machine",
    frequency: "quarterly",
    nistControls: ["AT-2", "AT-3"],
  },
  {
    ...
    aceModules: [
      ...
      { name: "SIEM Feedback Loop", role: "Validates whether IR procedures detect real attacks during exercises for objective measurement" },
    ],
    evidenceTypes: ["Purple Team exercise reports", "Detection time metrics", "Containment action records", "SIEM alert coverage during exercises"],
    validationMethod: "mixed",
    frequency: "quarterly",
    nistControls: ["AT-3", "IR-2"],
  },
  {
    ...
    evidenceTypes: ["Privileged account compromise test results", "Targeted phishing campaign results", "Role-specific vulnerability reports"],
    validationMethod: "mixed",
    frequency: "quarterly",
    nistControls: ["AT-3", "AT-4"],
  },
  {
    ...
    evidenceTypes: ["Vulnerability class trend analysis", "DAST findings by vulnerability category"],
    validationMethod: "n/a",
    frequency: "n/a",
    nistControls: ["AT-3", "SA-11"],
  },
  ...
  // ═══ IAM — Identity & Access Management ═══════════════════════════
  {
    ...
    evidenceTypes: ["MFA bypass test results", "Technique success/failure rates", "Session token interception attempts", "Campaign analytics"],
    validationMethod: "machine",
    frequency: "quarterly",
  ...
  },
  {
    ...
    evidenceTypes: ["Password policy test results", "Lockout threshold validation", "Credential attack success rates", "Authentication mechanism analysis"],
    validationMethod: "machine",
    frequency: "quarterly",
    nistControls: ["IA-5", "AC-7"],
  },
  {
    ...
    aceModules: [
      ...
      { name: "Privilege Escalation Engine", role: "Automates discovery of escalation vectors across cloud and on-premises environments" },
    ],
    evidenceTypes: ["Permission chain maps", "Privilege escalation test results", "Over-privileged role reports", "Escalation vector discovery"],
    validationMethod: "machine",
    frequency: "monthly",
    nistControls: ["AC-6", "AC-2"],
  },
  {
    ...
    aceModules: [
      ...
      { name: "Audit Log", role: "Records all privileged access events for comprehensive activity monitoring" },
    ],
    evidenceTypes: ["Domain enumeration reports", "Group membership analysis", "Privileged account inventory", "Access activity logs"],
    validationMethod: "mixed",
    frequency: "monthly",
  ...
  },
  {
    ...
    evidenceTypes: ["Stale account reports", "Orphaned account discovery", "Group membership sprawl analysis", "Provisioning event logs"],
    validationMethod: "machine",
    frequency: "monthly",
    nistControls: ["AC-2"],
  },
  {
    ...
    evidenceTypes: ["Persistent admin access reports", "Standing privilege analysis", "JIT conversion recommendations"],
    validationMethod: "machine",
    frequency: "monthly",
    nistControls: ["AC-2", "AC-6"],
  },
  {
    ...
    evidenceTypes: ["Exposed service account reports", "API authentication test results", "Token handling analysis"],
    validationMethod: "machine",
    frequency: "monthly",
    nistControls: ["IA-5", "IA-9"],
  },
  {
    ...
    evidenceTypes: ["Compromisable account reports", "Suspicious activity detection results", "Red team exercise detection metrics"],
    validationMethod: "mixed",
    frequency: "quarterly",
    nistControls: ["AC-2", "SI-4"],
  },
  ...
  // ═══ INR — Incident Response ══════════════════════════════════════
  {
    ...
    aceModules: [
      ...
      { name: "MITRE Heatmaps", role: "Visualizes ATT&CK technique coverage and detection gaps for lessons learned" },
    ],
    evidenceTypes: ["Post-engagement reports", "MITRE ATT&CK heatmaps", "Evidence-linked findings", "Remediation roadmaps"],
    validationMethod: "mixed",
    frequency: "per-engagement",
  ...
  },
  {
    ...
    aceModules: [
      ...
      { name: "Detection Rule Generator", role: "Creates Sigma/YARA/Suricata rules from executed TTPs to improve future detection" },
    ],
    evidenceTypes: ["Purple Team exercise results", "Detection time metrics", "SIEM alert coverage reports", "Generated detection rules"],
    validationMethod: "mixed",
    frequency: "quarterly",
    nistControls: ["IR-3", "IR-4"],
  },
  {
    ...
    aceModules: [
      ...
      { name: "Ransomware Intel", role: "Tracks ransomware group activity, targets, and techniques for trend analysis" },
    ],
    evidenceTypes: ["Threat intelligence reports", "Threat actor TTP analysis", "Ransomware trend data", "Incident pattern analysis"],
    validationMethod: "mixed",
    frequency: "monthly",
    nistControls: ["IR-4", "SI-5"],
  },
  {
    ...
    aceModules: [
      ...
      { name: "Emulation Playbooks", role: "Defines repeatable attack scenarios for consistent IR exercise execution" },
    ],
    evidenceTypes: ["Red team exercise scope documentation", "IR plan test results", "Emulation playbook execution records"],
    validationMethod: "human",
    frequency: "quarterly",
    nistControls: ["IR-1", "IR-8"],
  },
  {
    ...
    aceModules: [
      ...
      { name: "SecurityTrails", role: "Provides DNS and domain intelligence with historical records" },
      { name: "DeHashed", role: "Provides credential breach data for exposure monitoring" },
    ],
    evidenceTypes: ["Threat feed integration evidence", "IOC ingestion logs", "Feed freshness metrics", "Threat data correlation reports"],
    validationMethod: "machine",
    frequency: "continuous",
    nistControls: ["SI-5", "PM-16"],
  },
  {
    ...
    aceModules: [
      ...
      { name: "IOC Feed", role: "Integrates indicators into operational workflows to drive testing and remediation priorities" },
    ],
    evidenceTypes: ["Threat actor correlation reports", "Enriched IOC records", "Threat-informed testing priorities", "TTP mapping evidence"],
    validationMethod: "machine",
    frequency: "continuous",
  ...
  },
  {
    ...
    aceModules: [
      ...
      { name: "Threat Intel Ingest", role: "Normalizes and stores indicators from multiple formats for operational use" },
    ],
    evidenceTypes: ["IOC database records", "Enrichment logs", "Dark web monitoring reports", "Indicator lifecycle tracking"],
    validationMethod: "machine",
    frequency: "continuous",
    nistControls: ["SI-5", "IR-4"],
  },
  ...
  // ═══ MLA — Monitoring, Logging & Auditing ═════════════════════════
  {
    ...
    evidenceTypes: ["SIEM connectivity test results", "Event forwarding validation", "Alert firing verification during exercises"],
    validationMethod: "machine",
    frequency: "weekly",
  ...
  },
  {
    ...
    evidenceTypes: ["Event type catalog", "Detection rule coverage maps", "Event-to-technique mappings"],
    validationMethod: "machine",
    frequency: "monthly",
  ...
  },
  {
    ...
    evidenceTypes: ["Configuration baseline compliance reports", "CIS benchmark scores", "Drift detection alerts", "Remediation guidance"],
    validationMethod: "machine",
    frequency: "weekly",
    nistControls: ["CM-6", "CM-3"],
  },
  {
    ...
    aceModules: [
      ...
      { name: "Sigma Rule Engine", role: "Validates and optimizes detection rules for performance and accuracy" },
    ],
    evidenceTypes: ["Generated detection rules", "Alert quality test results", "Mutation test coverage", "Rule optimization reports"],
    validationMethod: "machine",
    frequency: "monthly",
  ...
  },
  {
    ...
    evidenceTypes: ["Log integrity verification records", "Access control configuration reports", "Tamper detection alerts"],
    validationMethod: "machine",
    frequency: "continuous",
    nistControls: ["AU-9", "AC-6"],
  },
  {
    ...
    evidenceTypes: ["ATT&CK detection coverage percentages", "SIEM feedback loop results", "Log review effectiveness metrics"],
    validationMethod: "mixed",
    frequency: "monthly",
    nistControls: ["AU-6", "AU-7"],
  },

  // ═══ PIY — Policy & Inventory ═══════════════════════════════════════
  {
    ...
    aceModules: [
      ...
      { name: "Web Crawler", role: "Maps application structure and content for web asset inventory" },
    ],
    evidenceTypes: ["Asset discovery reports", "Subdomain enumeration", "Service inventory", "Technology stack identification"],
    validationMethod: "machine",
    frequency: "weekly",
  ...
  },
  {
    ...
    aceModules: [
      ...
      { name: "Temporal Decay", role: "Models score degradation over time to reveal investment sustainability" },
    ],
    evidenceTypes: ["Risk trend reports", "Security posture scores", "Temporal decay analysis", "ROI metrics"],
    validationMethod: "mixed",
    frequency: "quarterly",
    nistControls: ["PM-4", "PM-11"],
  },
  {
    ...
    aceModules: [
      ...
      { name: "Nuclei Scanner", role: "Checks for known vulnerabilities in deployed software using template-based scanning" },
    ],
    evidenceTypes: ["DAST scan results", "API security test results", "Vulnerability class trend analysis"],
    validationMethod: "machine",
    frequency: "monthly",
    nistControls: ["SA-11", "SA-15"],
  },
  {
    ...
    evidenceTypes: ["VDP activity reports", "Submission trend analysis", "Resolution metrics", "Severity distribution data"],
    validationMethod: "machine",
    frequency: "monthly",
    nistControls: ["SI-2", "PM-15"],
  },
  {
  ...
  // ═══ RPL — Recovery Planning ════════════════════════════════════════
  {
    ...
    evidenceTypes: ["Recovery configuration compliance reports", "Baseline alignment verification"],
    validationMethod: "machine",
    frequency: "monthly",
    nistControls: ["CP-2", "CP-10"],
  },
  {
    ...
    howAceC3Delivers: "Recovery planning is an organizational process outside the scope of an offensive security platform. Ace C3 can validate recovery configurations through the Config Baseline Engine, but recovery plan development and organizational alignment require dedicated business continuity processes.",
    aceModules: [],
    evidenceTypes: [],
    validationMethod: "n/a",
    frequency: "n/a",
  ...
  },
  {
    ...
    howAceC3Delivers: "RTO/RPO measurement requires infrastructure-level testing capabilities (failover testing, backup restoration timing) that are outside the platform's scope. Ace C3 focuses on offensive security testing rather than infrastructure resilience validation.",
    aceModules: [],
    evidenceTypes: [],
    validationMethod: "n/a",
    frequency: "n/a",
  ...
  // ═══ SVC — Service Configuration ════════════════════════════════════
  {
    ...
    aceModules: [
      ...
      { name: "Censys", role: "Discovers certificates, hosts, and services through certificate transparency logs" },
      { name: "Service Fingerprinting", role: "Identifies exposed services, technologies, and versions across the attack surface" },
    ],
    evidenceTypes: ["Attack surface discovery reports", "Subdomain enumeration", "Service exposure maps", "Change-over-time tracking"],
    validationMethod: "machine",
    frequency: "weekly",
    nistControls: ["CM-8", "RA-5"],
  },
  {
    ...
    evidenceTypes: ["DAST scan results", "Nuclei scan findings", "Normalized multi-vendor scan reports", "Vulnerability trend data"],
    validationMethod: "machine",
    frequency: "monthly",
    nistControls: ["RA-5", "SI-2"],
  },
  {
    ...
    aceModules: [
      ...
      { name: "Risk Trending", role: "Tracks historical risk trajectory for trend analysis and remediation measurement" },
    ],
    evidenceTypes: ["Risk-ranked vulnerability reports", "CARVER+Shock scores", "Temporal decay analysis", "Risk trend data"],
    validationMethod: "machine",
    frequency: "continuous",
  ...
  },
  {
    ...
    aceModules: [
      ...
      { name: "Remediation Verification", role: "Tracks and validates fix implementation through re-scanning" },
    ],
    evidenceTypes: ["NVD/KEV integration evidence", "Normalized scan reports", "Remediation tracking records", "Fix verification results"],
    validationMethod: "machine",
    frequency: "continuous",
    nistControls: ["SI-2", "CM-6"],
  },
  {
    ...
    evidenceTypes: ["Remediation tracking reports", "Re-scan validation results", "Risk reduction trend data", "Fix implementation rates"],
    validationMethod: "machine",
    frequency: "monthly",
    nistControls: ["SI-2", "RA-5"],
  },
  {
    ...
    aceModules: [
      ...
      { name: "DO Infrastructure Audit", role: "Validates cloud endpoint configurations across droplets, firewalls, LBs, databases, and K8s" },
    ],
    evidenceTypes: ["Endpoint scan results", "Infrastructure audit reports", "Cloud configuration assessments", "Vulnerability findings"],
    validationMethod: "machine",
    frequency: "weekly",
    nistControls: ["SI-2", "CM-6"],
  },
  {
    ...
    evidenceTypes: ["Configuration baseline compliance reports", "CIS benchmark scores", "Drift detection alerts"],
    validationMethod: "machine",
    frequency: "weekly",
    nistControls: ["CM-2", "CM-6"],
  },
  {
    ...
    evidenceTypes: ["Post-change validation results", "Residual risk identification reports"],
    validationMethod: "machine",
  ...
  },
  {
    ...
    howAceC3Delivers: "Service notification and transparency requires organizational communication processes outside the platform's scope. Ace C3's notification system provides operational alerting but dedicated service status communication is planned for future development.",
    aceModules: [],
    evidenceTypes: [],
    validationMethod: "n/a",
    frequency: "n/a",
    nistControls: ["IR-6", "SI-5"],
  },
  ...
  // ═══ SCR — Supply Chain Risk ═════════════════════════════════════════
  {
    ...
    aceModules: [
      ...
      { name: "ZAP DAST", role: "Performs automated web application vulnerability discovery and testing" },
      { name: "Nuclei Scanner", role: "Executes template-based vulnerability checks across infrastructure" },
    ],
    evidenceTypes: ["Penetration test reports", "Exploitation evidence", "MITRE ATT&CK heatmaps", "Remediation recommendations"],
    validationMethod: "mixed",
    frequency: "annual + per-significant-change",
    nistControls: ["CA-8", "RA-5"],
  },
  {
    ...
    howAceC3Delivers: "Ace C3 Emulation provides full adversary emulation with MITRE ATT&CK technique execution across the kill chain — from initial access through lateral movement to data exfiltration. Emulation Playbooks define repeatable APT scenarios based on real threat actor TTPs. Threat Actor Matcher correlates findings with known threat actor capabilities, showing which real-world adversaries could exploit discovered weaknesses.",
    aceModules: [
      { name: "Ace C3 Emulation", role: "Provides full adversary emulation with MITRE ATT&CK technique execution across the kill chain" },
      { name: "Emulation Playbooks", role: "Defines repeatable APT scenarios based on real threat actor TTPs" },
      { name: "Threat Actor Matcher", role: "Correlates findings with known threat actor capabilities and TTPs" },
    ],
    evidenceTypes: ["APT simulation results", "MITRE ATT&CK technique coverage", "Detection rate metrics", "Threat actor TTP correlation"],
    validationMethod: "mixed",
    frequency: "quarterly",
    nistControls: ["CA-8", "SI-4"],
  },
  {
    ...
    aceModules: [
      ...
      { name: "GoPhish", role: "Provides campaign infrastructure for phishing simulation and tracking" },
    ],
    evidenceTypes: ["Phishing campaign results", "Click rate metrics", "Credential capture rates", "Improvement trend analysis"],
    validationMethod: "machine",
    frequency: "quarterly",
  ...
  },
  {
    ...
    evidenceTypes: ["NVD/KEV monitoring evidence", "Container image scan results", "Dependency vulnerability reports"],
    validationMethod: "machine",
    frequency: "continuous",
    nistControls: ["RA-5", "SI-2"],
  },
  {
    ...
    evidenceTypes: ["Container dependency analysis", "Supply chain vulnerability reports", "Third-party risk identification"],
    validationMethod: "machine",
    frequency: "monthly",
    nistControls: ["SR-3", "SR-6"],
  },
  ...
  // ═══ SDE — Secure Development ═══════════════════════════════════════
  {
    ...
    aceModules: [
      ...
      { name: "Nuclei Scanner", role: "Executes template-based security checks against deployed applications" },
    ],
    evidenceTypes: ["DAST scan results", "API security test findings", "Nuclei scan reports", "Vulnerability evidence artifacts"],
    validationMethod: "machine",
    frequency: "per-release + monthly",
    nistControls: ["SA-11", "SI-2"],
  },
  {
    ...
    evidenceTypes: ["Vulnerability class trend analysis", "Common vulnerability category reports", "Development security metrics"],
    validationMethod: "mixed",
    frequency: "quarterly",
    nistControls: ["SA-11", "SA-15"],
  },
  ...
  // ═══ PPM — Policy & Procedure Management ════════════════════════════
  {
    ...
    evidenceTypes: ["Policy documentation", "Version history records", "Integrity verification records", "Policy review analysis"],
    validationMethod: "human",
    frequency: "annual",
    nistControls: ["PL-1", "PL-2"],
  },
  {
    ...
    evidenceTypes: ["Scope enforcement logs", "Scan policy compliance records", "Operational policy enforcement evidence"],
    validationMethod: "machine",
    frequency: "continuous",
    nistControls: ["PL-2", "PM-1"],
  },
];

// ─── Lookup Helpers ─────────────────────────────────────────────────

const _ksiMap = new Map<string, EnrichedKSI>();
for (const ksi of ALL_ENRICHED_KSIS) _ksiMap.set(ksi.id, ksi);

const _themeMap = new Map<string, EnrichedTheme>();
for (const theme of ALL_ENRICHED_THEMES) _themeMap.set(theme.code, theme);

/** Get enriched data for a single KSI by ID (e.g. "KSI-AFR-ADS") */
export function getKsiEnriched(ksiId: string): EnrichedKSI | undefined {
  return _ksiMap.get(ksiId);
}

/** Get enriched theme data by code (e.g. "AFR") */
export function getThemeEnriched(themeCode: string): EnrichedTheme | undefined {
  return _themeMap.get(themeCode);
}

/** Get all enriched KSIs for a given theme */
export function getKsisByTheme(themeCode: string): EnrichedKSI[] {
  return ALL_ENRICHED_KSIS.filter(k => k.themeCode === themeCode);
}

/** Coverage level label with color class */
export function getCoverageBadgeClass(level: CoverageLevel): string {
  switch (level) {
    case "direct": return "bg-emerald-500/20 text-emerald-400 border-emerald-500/30";
    case "supporting": return "bg-amber-500/20 text-amber-400 border-amber-500/30";
    case "planned": return "bg-slate-500/20 text-slate-400 border-slate-500/30";
  }
}

/** Summary stats */
export const KSI_STATS = {
  total: ALL_ENRICHED_KSIS.length,
  direct: ALL_ENRICHED_KSIS.filter(k => k.coverageLevel === "direct").length,
  supporting: ALL_ENRICHED_KSIS.filter(k => k.coverageLevel === "supporting").length,
  planned: ALL_ENRICHED_KSIS.filter(k => k.coverageLevel === "planned").length,
  get coveragePercent() {
    return Math.round(((this.direct + this.supporting) / this.total) * 100);
  },
};
