...
export default function TyposquatManager({ engagementId }: { engagementId?: number }) {
  ...
  const purchaseDomainMutation = trpc.typosquat.purchaseDomain.useMutation();
  const checkAvailabilityMutation = trpc.typosquat.checkAvailabilityMulti.useMutation();
  const registrarStatusQuery = trpc.typosquat.getRegistrarStatus.useQuery();

  // Purchase mode: "manual" (old flow) or "auto" (Namecheap/GoDaddy API)
  const [purchaseMode, setPurchaseMode] = useState<"manual" | "auto">("auto");
  const [priceComparison, setPriceComparison] = useState<any>(null);
  const [purchaseContact, setPurchaseContact] = useState({
    firstName: "", lastName: "", email: "", phone: "",
    address: "", city: "", state: "", postalCode: "", country: "US", organization: "",
  });
  ...
  const handleAutoIntegrate = async () => {
    ...
    if (purchaseMode === "auto" && (registrar === "namecheap" || registrar === "godaddy")) {
      // Automated purchase via API
      setIntegrationStep("purchasing");
      try {
        const res = await purchaseDomainMutation.mutateAsync({
          domain: selectedDomain.domain,
          typosquatId: lastTyposquatId,
          registrar: registrar as "namecheap" | "godaddy",
          contact: purchaseContact,
          years: 1,
          privacy: true,
          autoConfigurePhishing: true,
          engagementId,
        });

        if (res.success) {
          setIntegrationStep("done");
          toast.success(`${selectedDomain.domain} purchased via ${registrar} and auto-configured!`);
        } else {
          toast.error(`Purchase failed: ${res.error}`);
          setIntegrationStep("confirm");
        }
      } catch (err: any) {
        toast.error(`Purchase failed: ${sanitizeErrorForToast(err)}`);
        setIntegrationStep("confirm");
      }
    } else {
      // Manual purchase flow (old behavior)
      setIntegrationStep("purchasing");
      setTimeout(async () => {
        ...
        try {
          const res = await autoIntegrateMutation.mutateAsync({
            ...
            engagementId,
            fromName,
            mailServerIp: "gophish.ac3-dev.local",
          });
          if (res.success) {
            setIntegrationStep("done");
            toast.success(`${selectedDomain.domain} fully integrated!`);
          } else {
            toast.error("Some integration steps failed.");
            setIntegrationStep("done");
          }
        } catch (err: any) {
  ...
  const handleCheckPricing = async () => {
    if (!selectedDomain) return;
    try {
      const res = await checkAvailabilityMutation.mutateAsync({ domains: [selectedDomain.domain] });
                  ...
                  {integrationStep === "confirm" && (
                    <div className="mt-2 space-y-2">
                      <div className="flex items-center gap-2">
                        <select
                          value={purchaseMode}
                          onChange={(e) => setPurchaseMode(e.target.value as "manual" | "auto")}
                          className="text-xs bg-background border border-border px-2 py-1 font-display tracking-wider"
                        >
                          <option value="auto">AUTO-PURCHASE (API)</option>
                          <option value="manual">MANUAL (buy externally)</option>
                        </select>
                        <select
                          value={registrar}
                          ...
                          className="text-xs bg-background border border-border px-2 py-1 font-display tracking-wider"
                        >
                          <option value="namecheap">Namecheap{registrarStatusQuery.data?.find((r: any) => r.id === 'namecheap')?.configured ? ' ✓' : ' (not configured)'}</option>
                          <option value="godaddy">GoDaddy{registrarStatusQuery.data?.find((r: any) => r.id === 'godaddy')?.configured ? ' ✓' : ' (not configured)'}</option>
                          <option value="porkbun">Porkbun (manual only)</option>
                          <option value="cloudflare">Cloudflare (manual only)</option>
                        </select>
                      </div>
                      <div className="flex items-center gap-2">
                        <Button variant="outline" size="sm" onClick={handleCheckPricing} className="text-xs font-display tracking-wider h-7">
                          <DollarSign className="w-3 h-3 mr-1" /> COMPARE PRICES
                        </Button>
                        {purchaseMode === "manual" && (
                          <a
                      ...
                      {priceComparison?.pricing?.[0] && (
                        <div className="text-xs bg-muted/50 border border-border p-2 space-y-1">
                          <p className="font-display tracking-wider text-muted-foreground">PRICE COMPARISON:</p>
                          {priceComparison.pricing[0].prices.namecheap !== null && (
                            <p className={`flex items-center gap-2 ${priceComparison.pricing[0].cheapest === 'namecheap' ? 'text-emerald-400' : 'text-muted-foreground'}`}>
                              {priceComparison.pricing[0].cheapest === 'namecheap' && <CheckCircle className="w-3 h-3" />}
                              Namecheap: ${priceComparison.pricing[0].prices.namecheap?.toFixed(2)}/yr
                            </p>
                          )}
                          {priceComparison.pricing[0].prices.godaddy !== null && (
                            <p className={`flex items-center gap-2 ${priceComparison.pricing[0].cheapest === 'godaddy' ? 'text-emerald-400' : 'text-muted-foreground'}`}>
                              {priceComparison.pricing[0].cheapest === 'godaddy' && <CheckCircle className="w-3 h-3" />}
                              GoDaddy: ${priceComparison.pricing[0].prices.godaddy?.toFixed(2)}/yr
                            </p>
                          )}
                          {priceComparison.pricing[0].savings > 0 && (
                            <p className="text-emerald-400">Save ${priceComparison.pricing[0].savings.toFixed(2)} with {priceComparison.pricing[0].cheapest}</p>
                          )}
                        </div>
                      )}
                    </div>
            ...
            {integrationStep === "confirm" && (
              <div className="space-y-3 pt-2 border-t border-border">
                ...
                <div className="text-xs text-muted-foreground space-y-1">
                  <p className="flex items-center gap-1"><Settings className="w-3 h-3" /> <strong>Mail Server:</strong> gophish.ac3-dev.local (AC3 mail server)</p>
