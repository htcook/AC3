...
const DEFAULT_SERVER = {
  ...
  ipAddress: "caldera.ac3-dev.local",
  httpsUrl: "https://ac3.aceofcloud.io",
  httpUrl: "/emulation",
...
function DashboardInner() {
  ...
  // Attack template & campaign stats from DB
  const { data: attackTemplateData } = trpc.threatIntelTraining.listTemplates.useQuery(
    { status: 'production', limit: 1, offset: 0 },
    { staleTime: 60_000 }
  );
  const { data: campaignListData } = trpc.campaignOrchestrator.list.useQuery(
    {},
    { staleTime: 60_000 }
  );
  const attackTemplateCount = attackTemplateData?.total ?? 0;
  const campaignCount = campaignListData?.total ?? 0;
      ...
      <div className="p-4 sm:p-6 space-y-6">
        ...
        <section>
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-2">
            ...
            <MiniStat value={attackTemplateCount.toString()} label="ATTACK TEMPLATES" color="text-amber-400" href="/attack-templates" />
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-2 mt-2">
            <MiniStat value={campaignCount.toString()} label="CAMPAIGNS" color="text-emerald-400" href="/campaigns" />
            <MiniStat value={gophish.totalCampaigns.toString()} label="PHISHING OPS" color="text-emerald-400" href="/phishing-ops" />
            <MiniStat value={gophish.totalTemplates.toString()} label="EMAIL TEMPLATES" color="text-emerald-400" href="/templates" />
        ...
        >
          ...
          <div className="grid md:grid-cols-2 gap-3">
            ...
            <a href="/phishing" target="_blank" rel="noopener noreferrer" className="bg-card border border-emerald-500/30 p-4 cursor-pointer hover:border-emerald-500 transition-colors group block">
              <div className="flex items-center gap-3 mb-3">
